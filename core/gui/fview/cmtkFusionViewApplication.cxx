...
cmtk::FusionViewApplication ::FusionViewApplication(int &argc, char *argv[])
  ...
  cl.AddOption(CommandLine::Key("fixed"), &imagePathFix,
               "Fixed image path. If this is not provided, the program "
               "attempts to automatically deduce the fixed image path of the "
               "first transformation in the given sequence.")
      ->SetProperties(cmtk::CommandLine::PROPS_IMAGE);

  std::string imagePathMov = "";
  cl.AddOption(CommandLine::Key("moving"), &imagePathMov,
               "Moving image path. If this is not provided, the program "
               "attempts to automatically deduce the fixed image path of the "
               "first transformation in the given sequence.")
      ->SetProperties(cmtk::CommandLine::PROPS_IMAGE);
  ...
  cl.AddParameterVector(
        &xformList, "XformList",
        "List of concatenated transformations. Insert '--inverse' to use the "
        "inverse of the transformation listed next.")
  ...
  try {
    ...
    StdErr << "WARNING: encountered singular matrix when creating affine-only "
              "transformation. Will substitute (incorrect) identify "
  ...
  if (imagePathFix.empty()) {
    ...
    if (!imagePathFix.empty()) {
      ...
      StdErr << "ERROR: could not deduce fixed image path from "
                "transformation(s).\n";
  ...
  // per-dimension scale factors make sure non-square pixels are displayed
  // square
  ...
  if (imagePathMov.empty()) {
    ...
    if (!imagePathMov.empty()) {
      ...
      StdErr << "ERROR: could not deduce moving image path from "
                "transformation(s).\n";
      throw(ExitException(1));
    }
  }
  ...
  QObject::connect(this->m_MainWindowUI.blackSliderFix,
                   SIGNAL(valueChanged(int)), this,
                   SLOT(fixedBlackWhiteChanged()));
  QObject::connect(this->m_MainWindowUI.whiteSliderFix,
                   SIGNAL(valueChanged(int)), this,
                   SLOT(fixedBlackWhiteChanged()));

  QObject::connect(this->m_MainWindowUI.blackSliderMov,
                   SIGNAL(valueChanged(int)), this,
                   SLOT(movingBlackWhiteChanged()));
  QObject::connect(this->m_MainWindowUI.whiteSliderMov,
                   SIGNAL(valueChanged(int)), this,
                   SLOT(movingBlackWhiteChanged()));
  ...
  QObject::connect(zoomGroup, SIGNAL(triggered(QAction *)), this,
  ...
  QObject::connect(fixedColorGroup, SIGNAL(triggered(QAction *)), this,
                   SLOT(changeFixedColor(QAction *)));
  ...
  QObject::connect(movingColorGroup, SIGNAL(triggered(QAction *)), this,
                   SLOT(changeMovingColor(QAction *)));
  ...
  QObject::connect(sliceGroup, SIGNAL(triggered(QAction *)), this,
                   SLOT(changeSliceDirection(QAction *)));

  QActionGroup *interpGroup = new QActionGroup(this->m_MainWindow);
  interpGroup->setExclusive(true);
  this->m_MainWindowUI.actionInterpLinear->setData(
      QVariant(Interpolators::LINEAR));
  interpGroup->addAction(this->m_MainWindowUI.actionInterpLinear);
  this->m_MainWindowUI.actionInterpCubic->setData(
      QVariant(Interpolators::CUBIC));
  interpGroup->addAction(this->m_MainWindowUI.actionInterpCubic);
  this->m_MainWindowUI.actionInterpSinc->setData(
      QVariant(Interpolators::COSINE_SINC));
  interpGroup->addAction(this->m_MainWindowUI.actionInterpSinc);
  this->m_MainWindowUI.actionInterpNearestNeighbour->setData(
      QVariant(Interpolators::NEAREST_NEIGHBOR));
  interpGroup->addAction(this->m_MainWindowUI.actionInterpNearestNeighbour);
  this->m_MainWindowUI.actionInterpPartialVolume->setData(
      QVariant(Interpolators::PARTIALVOLUME));
  interpGroup->addAction(this->m_MainWindowUI.actionInterpPartialVolume);

  QObject::connect(interpGroup, SIGNAL(triggered(QAction *)), this,
                   SLOT(changeInterpolator(QAction *)));
  ...
  QObject::connect(xformGroup, SIGNAL(triggered(QAction *)), this,
                   SLOT(changeXform(QAction *)));

  this->m_MainWindowUI.alphaSlider->setRange(0, 1000);
  this->m_MainWindowUI.alphaSlider->setValue(this->m_Transparency * 1000);
  QObject::connect(this->m_MainWindowUI.alphaSlider, SIGNAL(valueChanged(int)),
                   this, SLOT(setTransparency(int)));

  this->changeSliceDirection(AXIS_Z);
  QObject::connect(this->m_MainWindowUI.sliceSlider, SIGNAL(valueChanged(int)),
                   this, SLOT(setFixedSlice(int)));

  QObject::connect(this->m_MainWindowUI.actionLinkedCursor,
                   SIGNAL(toggled(bool)), this,
                   SLOT(setLinkedCursorFlag(bool)));

  // synchronize sliders of the two graphics views
  QObject::connect(this->m_MainWindowUI.fixedView->horizontalScrollBar(),
                   SIGNAL(valueChanged(int)),
                   this->m_MainWindowUI.movingView->horizontalScrollBar(),
                   SLOT(setValue(int)));
  QObject::connect(this->m_MainWindowUI.fixedView->verticalScrollBar(),
                   SIGNAL(valueChanged(int)),
                   this->m_MainWindowUI.movingView->verticalScrollBar(),
                   SLOT(setValue(int)));

  QObject::connect(this->m_MainWindowUI.movingView->horizontalScrollBar(),
                   SIGNAL(valueChanged(int)),
                   this->m_MainWindowUI.fixedView->horizontalScrollBar(),
                   SLOT(setValue(int)));
  QObject::connect(this->m_MainWindowUI.movingView->verticalScrollBar(),
                   SIGNAL(valueChanged(int)),
                   this->m_MainWindowUI.fixedView->verticalScrollBar(),
                   SLOT(setValue(int)));

  this->m_MainWindow->show();
}

void cmtk::FusionViewApplication ::InitViewData(Self::Data &data,
                                                QGraphicsView *view) {
  ...
  (data.m_CursorLines[0] = data.m_Scene->addLine(0, 0, 0, 0, redPen))
      ->setZValue(100);
  (data.m_CursorLines[1] = data.m_Scene->addLine(0, 0, 0, 0, redPen))
      ->setZValue(100);
  (data.m_PixmapItem = new QGraphicsPixmapItemEvents)->setZValue(0);
  data.m_Scene->addItem(data.m_PixmapItem);

  QObject::connect(data.m_PixmapItem,
                   SIGNAL(mousePressed(QGraphicsSceneMouseEvent *)), this,
                   SLOT(mousePressed(QGraphicsSceneMouseEvent *)));

  data.m_View = view;
  data.m_View->setScene(data.m_Scene);
}

void cmtk::FusionViewApplication ::setTransparency(int slice) {
  this->m_Transparency = static_cast<float>(slice) / 1000;
  this->UpdateMovingImage();
}

void cmtk::FusionViewApplication ::changeFixedColor(QAction *action) {
  this->m_Fixed.m_ColorMapIndex = action->data().toInt();
  this->UpdateFixedImage();
}

void cmtk::FusionViewApplication ::changeMovingColor(QAction *action) {
  this->m_Moving.m_ColorMapIndex = action->data().toInt();
  this->UpdateMovingImage();
}

void cmtk::FusionViewApplication ::setLinkedCursorFlag(bool flag) {
...
void cmtk::FusionViewApplication ::setFixedSlice(int slice) {
  if (this->m_SliceIndex != slice) {
    this->m_SliceIndex = slice;
    this->m_CursorPosition[this->m_SliceAxis] = this->m_SliceIndex;

    this->m_Fixed.m_Slice = this->m_Fixed.m_Volume->ExtractSlice(
        this->m_SliceAxis, this->m_SliceIndex);
    this->UpdateFixedImage();

    this->UpdateMovingSlice();

    this->m_MainWindowUI.sliceLabel->setText(
        QString("Slice: %1").arg(this->m_SliceIndex));
  }
}

void cmtk::FusionViewApplication ::changeZoom(QAction *action) {
  this->m_ZoomFactor = static_cast<float>(
      action->data().toDouble());  // older Qt doesn't have QVariant::toFloat()
  this->UpdateFixedImage();
  this->UpdateMovingImage();
}

void cmtk::FusionViewApplication ::changeInterpolator(QAction *action) {
  this->m_Interpolator =
      static_cast<Interpolators::InterpolationEnum>(action->data().toInt());
  this->UpdateMovingSlice();
}

void cmtk::FusionViewApplication ::changeXform(QAction *action) {
  this->m_XformModel = action->data().toInt();
  this->UpdateMovingSlice();
}

void cmtk::FusionViewApplication ::fixedBlackWhiteChanged() {
  this->UpdateFixedImage();
}

void cmtk::FusionViewApplication ::movingBlackWhiteChanged() {
  this->UpdateMovingImage();
}

void cmtk::FusionViewApplication ::changeSliceDirection(QAction *action) {
  this->changeSliceDirection(action->data().toInt());
}

void cmtk::FusionViewApplication ::mousePressed(
...
void cmtk::FusionViewApplication ::changeSliceDirection(const int sliceAxis) {
  if (sliceAxis != this->m_SliceAxis) {
    this->m_SliceAxis = sliceAxis;

    this->m_SliceIndex = -1;  // unset previously set slice index to ensure
                              // update of moving slice
    const int newSliceIndex = static_cast<int>(
        this->m_CursorPosition[this->m_SliceAxis]);  // store to safeguard
                                                     // against unwanted updates
    this->m_MainWindowUI.sliceSlider->setRange(
        0, this->m_Fixed.m_Volume->GetDims()[this->m_SliceAxis] - 1);
...
void cmtk::FusionViewApplication ::UpdateMovingSlice() {
  ReformatVolume::Plain plain(TYPE_FLOAT);
  UniformVolumeInterpolatorBase::SmartPtr interpolator(
      ReformatVolume::CreateInterpolator(this->m_Interpolator,
                                         this->m_Moving.m_Volume));

  const XformList noXforms;
  TypedArray::SmartPtr reformatData;
  switch (this->m_XformModel) {
    case 0:
      reformatData = ReformatVolume::ReformatUnmasked(
          this->m_Fixed.m_Slice, noXforms, noXforms, plain,
          this->m_Moving.m_Volume, interpolator);
      break;
    case 1:
      reformatData = ReformatVolume::ReformatUnmasked(
          this->m_Fixed.m_Slice, this->m_XformListAllAffine, noXforms, plain,
          this->m_Moving.m_Volume, interpolator);
      break;
    case 2:
    default:
      reformatData = ReformatVolume::ReformatUnmasked(
          this->m_Fixed.m_Slice, this->m_XformList, noXforms, plain,
          this->m_Moving.m_Volume, interpolator);
      break;
  }
...
void cmtk::FusionViewApplication ::MakeColorTable(Self::Data &data) {
...
void cmtk::FusionViewApplication ::UpdateFixedImage() {
  this->MakeColorTable(this->m_Fixed);

  const float black = this->m_Fixed.m_DataRange.m_LowerBound +
                      this->m_Fixed.m_DataRange.Width() *
                          this->m_MainWindowUI.blackSliderFix->value() / 500;
  const float white = this->m_Fixed.m_DataRange.m_LowerBound +
                      this->m_Fixed.m_DataRange.Width() *
                          this->m_MainWindowUI.whiteSliderFix->value() / 500;

  this->MakeImage(this->m_Fixed.m_Image, *(this->m_Fixed.m_Slice),
                  this->m_Fixed.m_ColorTable, black, white);
  this->UpdateView(this->m_Fixed, this->m_Fixed.m_Image);
}

void cmtk::FusionViewApplication ::UpdateMovingImage() {
  this->MakeColorTable(this->m_Moving);

  const float black = this->m_Moving.m_DataRange.m_LowerBound +
                      this->m_Moving.m_DataRange.Width() *
                          this->m_MainWindowUI.blackSliderMov->value() / 500;
  const float white = this->m_Moving.m_DataRange.m_LowerBound +
                      this->m_Moving.m_DataRange.Width() *
                          this->m_MainWindowUI.whiteSliderMov->value() / 500;

  this->MakeImage(this->m_Moving.m_Image, *(this->m_Moving.m_Slice),
                  this->m_Moving.m_ColorTable, black, white);

  this->m_FusedImage =
      QImage(this->m_Moving.m_Image.width(), this->m_Moving.m_Image.height(),
             QImage::Format_RGB32);
  for (int y = 0; y < this->m_Moving.m_Image.height(); ++y) {
    for (int x = 0; x < this->m_Moving.m_Image.width(); ++x) {
      ...
      rgbMov = QColor(this->m_Transparency * rgbMov.red() +
                          (1.0 - this->m_Transparency) * rgbFix.red(),
                      this->m_Transparency * rgbMov.green() +
                          (1.0 - this->m_Transparency) * rgbFix.green(),
                      this->m_Transparency * rgbMov.blue() +
                          (1.0 - this->m_Transparency) * rgbFix.blue());
...
void cmtk::FusionViewApplication ::MakeImage(QImage &image,
                                             const UniformVolume &slice,
    ...
    for (int x = 0; x < dimX; ++x, ++idx) {
      image.setPixel(
          ...
          static_cast<int>(
              255 *
              std::min<float>(
                  1, std::max<float>(0, (slice.GetDataAt(idx) - blackLevel) *
                                            scaleLevel))));
    }
  }
}

void cmtk::FusionViewApplication ::UpdateView(Self::Data &data, QImage &image) {
  ...
  if (this->m_CursorDisplayed) {
    data.m_CursorLines[0]->setLine(this->m_CursorPosition[idxX], bb.top(),
                                   this->m_CursorPosition[idxX], bb.bottom());
    data.m_CursorLines[1]->setLine(bb.left(), this->m_CursorPosition[idxY],
                                   bb.right(), this->m_CursorPosition[idxY]);
  }

  QTransform zoomTransform =
      QTransform::fromScale(-this->m_ZoomFactor * this->m_ScalePixels[idxX],
                            -this->m_ZoomFactor * this->m_ScalePixels[idxY]);
