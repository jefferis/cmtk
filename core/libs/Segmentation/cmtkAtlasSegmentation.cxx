...
void cmtk::AtlasSegmentation ::RegisterAffine() {
  ...
  try {
    ...
    StdErr << "ERROR: singular matrix in "
              "AffineRegistration::GetTransformation()\n";
    throw ExitException(1);
  }
}

void cmtk::AtlasSegmentation ::RegisterSpline() {
  ...
  const Types::Coordinate minSize = std::min(
      std::min(this->m_TargetImage->m_Size[0], this->m_TargetImage->m_Size[1]),
      this->m_TargetImage->m_Size[2]);
  er.SetGridSpacing(minSize / 2);
  er.SetRefineGrid(std::max<int>(
      0,
      static_cast<int>(
          (log(minSize / this->m_TargetImage->GetMaxDelta()) / log(2.0)) - 3)));
...
void cmtk::AtlasSegmentation ::ReformatLabels() {
