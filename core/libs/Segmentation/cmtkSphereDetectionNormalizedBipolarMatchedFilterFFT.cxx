...
cmtk::SphereDetectionNormalizedBipolarMatchedFilterFFT::
    ...
    : m_NumberOfPixels(image.GetNumberOfPixels()),
      ...
      m_SumFilter(0.0),
      m_SumFilterMask(0.0) {
  this->m_ImageFT = static_cast<fftw_complex *>(
      fftw_malloc(sizeof(fftw_complex) * this->m_NumberOfPixels));
  this->m_ImageSquareFT = static_cast<fftw_complex *>(
      fftw_malloc(sizeof(fftw_complex) * this->m_NumberOfPixels));

  this->m_FilterFT = static_cast<fftw_complex *>(
      fftw_malloc(sizeof(fftw_complex) * this->m_NumberOfPixels));
  this->m_FilterMaskFT = static_cast<fftw_complex *>(
      fftw_malloc(sizeof(fftw_complex) * this->m_NumberOfPixels));
  this->m_FilterMaskFT2 = static_cast<fftw_complex *>(
      fftw_malloc(sizeof(fftw_complex) * this->m_NumberOfPixels));

  this->m_PlanFilter = fftw_plan_dft_3d(
      this->m_ImageDims[2], this->m_ImageDims[1], this->m_ImageDims[0],
      this->m_FilterFT, this->m_FilterFT, FFTW_FORWARD, FFTW_ESTIMATE);
  this->m_PlanFilterMask = fftw_plan_dft_3d(
      this->m_ImageDims[2], this->m_ImageDims[1], this->m_ImageDims[0],
      this->m_FilterMaskFT, this->m_FilterMaskFT, FFTW_FORWARD, FFTW_ESTIMATE);

  this->m_PlanBackward = fftw_plan_dft_3d(
      this->m_ImageDims[2], this->m_ImageDims[1], this->m_ImageDims[0],
      this->m_FilterFT, this->m_FilterFT, FFTW_BACKWARD, FFTW_ESTIMATE);
  this->m_PlanBackwardMask = fftw_plan_dft_3d(
      this->m_ImageDims[2], this->m_ImageDims[1], this->m_ImageDims[0],
      this->m_FilterMaskFT, this->m_FilterMaskFT, FFTW_BACKWARD, FFTW_ESTIMATE);
  this->m_PlanBackwardMask2 =
      fftw_plan_dft_3d(this->m_ImageDims[2], this->m_ImageDims[1],
                       this->m_ImageDims[0], this->m_FilterMaskFT2,
                       this->m_FilterMaskFT2, FFTW_BACKWARD, FFTW_ESTIMATE);

  // initialize image FT
  fftw_plan plan_image = fftw_plan_dft_3d(
      this->m_ImageDims[2], this->m_ImageDims[1], this->m_ImageDims[0],
      this->m_ImageFT, this->m_ImageFT, FFTW_FORWARD, FFTW_ESTIMATE);
  fftw_plan plan_image_square =
      fftw_plan_dft_3d(this->m_ImageDims[2], this->m_ImageDims[1],
                       this->m_ImageDims[0], this->m_ImageSquareFT,
                       this->m_ImageSquareFT, FFTW_FORWARD, FFTW_ESTIMATE);
...
cmtk::SphereDetectionNormalizedBipolarMatchedFilterFFT::
    ~SphereDetectionNormalizedBipolarMatchedFilterFFT() {
...
cmtk::SphereDetectionNormalizedBipolarMatchedFilterFFT::GetFilteredImageData(
    const Types::Coordinate sphereRadius, const int marginWidth) {
  // check if the requested kernel parameters are the same we previously used.
  if ((sphereRadius == this->m_SphereRadius) &&
  ...
  memset(this->m_FilterMaskFT, 0,
         sizeof(fftw_complex) * this->m_NumberOfPixels);

  this->MakeFilter(sphereRadius, marginWidth);

  const Types::DataItem denom2 =
      sqrt(this->m_SumFilterMask -
           (this->m_SumFilter * this->m_SumFilter) / this->m_SumFilterMask);
...
#pragma omp parallel for
  ...
  if (!this->m_FilterResponse)
    this->m_FilterResponse =
        TypedArray::Create(cmtk::TYPE_ITEM, this->m_NumberOfPixels);

#pragma omp parallel for
  for (int n = 0; n < static_cast<int>(this->m_NumberOfPixels); ++n) {
    const Types::DataItem num = FFTW::Magnitude(this->m_FilterFT[n]) -
                                (FFTW::Magnitude(this->m_FilterMaskFT[n]) *
                                 this->m_SumFilter / this->m_SumFilterMask);
    const Types::DataItem denom1 = sqrt(std::max<Types::DataItem>(
        0, FFTW::Magnitude(this->m_FilterMaskFT2[n]) -
               FFTW::SumOfSquares(this->m_FilterMaskFT[n]) /
                   this->m_SumFilterMask));
...
void cmtk::SphereDetectionNormalizedBipolarMatchedFilterFFT::MakeFilter(
    const Types::Coordinate sphereRadius, const int marginWidth) {
  const int nRadius[3] = {
      1 + marginWidth + static_cast<int>(sphereRadius / this->m_PixelSize[0]),
  ...
  const Types::Coordinate outerRadiusSq =
      MathUtil::Square(sphereRadius + marginWidth);
...
#pragma omp parallel for reduction(+ : sumFilter) reduction(+ : sumFilterMask)
  for (int k = 0; k < nRadius[2]; ++k) {
    ...
    for (int j = 0; j < nRadius[1]; ++j) {
      const Types::Coordinate dyzSq =
          dzSq + MathUtil::Square(j * this->m_PixelSize[1]);
      for (int i = 0; i < nRadius[0]; ++i) {
        const Types::Coordinate distanceSq =
            dyzSq + MathUtil::Square(i * this->m_PixelSize[0]);
        if (distanceSq <= outerRadiusSq) {
          ...
          for (int kk = k; kk < this->m_ImageDims[2];
               kk += (this->m_ImageDims[2] - 1 - 2 * k))
            for (int jj = j; jj < this->m_ImageDims[1];
                 jj += (this->m_ImageDims[1] - 1 - 2 * j))
              for (int ii = i; ii < this->m_ImageDims[0];
                   ii += (this->m_ImageDims[0] - 1 - 2 * i)) {
                const size_t ofs = ii + this->m_ImageDims[0] *
                                            (jj + this->m_ImageDims[1] * kk);
