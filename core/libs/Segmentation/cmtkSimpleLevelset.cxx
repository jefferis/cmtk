...
void cmtk::SimpleLevelset ::InitializeCenteredSphere() {
  ...
  painter.DrawSphere(
      ...
      this->m_ScaleInitialSphere *
          ((this->m_Levelset->GetDims()[0] + this->m_Levelset->GetDims()[1] +
            this->m_Levelset->GetDims()[2]) /
           6),
      1.0);
}
...
void cmtk::SimpleLevelset ::Evolve(const int numberOfIterations,
  ...
  for (int it = 0; (it < numberOfIterations) &&
    ...
    this->m_Levelset->SetData(UniformVolumeGaussianFilter(this->m_Levelset)
                                  .GetFiltered3D(this->m_FilterSigma));
#pragma omp parallel for reduction(+:nInside) reduction(+:insideSum) reduction(+:outsideSum)
    ...
    DebugOutput(1) << it << " IN: " << nInside << "  " << mInside
                   << "  OUT: " << nOutside << "  " << mOutside << "\r";
...
#pragma omp parallel for
    for (int n = 0; n < static_cast<int>(numberOfPixels); ++n) {
      ...
      this->m_Levelset->SetDataAt(
          std::min<Types::DataItem>(
              this->m_LevelsetThreshold,
              std::max<Types::DataItem>(-this->m_LevelsetThreshold, newLevel)),
          n);
    }

#ifdef CMTK_BUILD_DEMO
    UniformVolume::SmartConstPtr slice =
        this->m_Levelset->ExtractSlice(AXIS_Z, this->m_Levelset->m_Dims[2] / 2);
...
cmtk::UniformVolume::SmartPtr& cmtk::SimpleLevelset ::GetLevelset(
    const bool binarize, const float threshold) {
  if (binarize) {
    this->m_Levelset->GetData()->Binarize(threshold);
    this->m_Levelset->SetData(
        TypedArray::SmartPtr(this->m_Levelset->GetData()->Convert(TYPE_BYTE)));
