...
cmtk::SimpleLevelsetCommandLineBase::SimpleLevelsetCommandLineBase()
  ...
  this->m_CommandLine.SetProgramInfo(
      ...
      "Levelset-type segmentation of foreground/background using minimum "
      "regional variance energy");
  this->m_CommandLine.SetProgramInfo(CommandLine::PRG_CATEG,
                                     "CMTK.Segmentation");

  typedef CommandLine::Key Key;
  this->m_CommandLine.AddSwitch(
      Key('b', "binarize"), &this->m_Binarize, true,
      "Binarize levelset and write as byte mask, rather than write "
      "floating-point levelset function itself.");

  this->m_CommandLine
      .BeginGroup("Levelset Initialization",
                  "These parameters control the initialization of the levelset "
                  "function")
      ->SetProperties(CommandLine::PROPS_ADVANCED);
  this->m_CommandLine.AddOption(Key("scale-initial-sphere"),
                                &this->m_ScaleInitialSphere,
                                "Scale factor to reduce or increase the size "
                                "of the initial foreground region sphere.");
  ...
  this->m_CommandLine
      ...
      ->SetProperties(CommandLine::PROPS_ADVANCED);
  this->m_CommandLine.AddOption(Key('n', "iterations"),
                                &this->m_NumberOfIterations,
                                "Maximum number of iterations");
  this->m_CommandLine.AddSwitch(Key('f', "force-iterations"),
                                &this->m_ForceIterations, true,
                                "Force given number of iterations, even when "
                                "convergence has been detected");

  this->m_CommandLine.AddOption(
      Key('s', "filter-sigma"), &this->m_FilterSigma,
      "Gaussian filter sigma in world coordinate units (e.g., mm)");
  this->m_CommandLine.AddOption(
      Key('d', "delta"), &this->m_TimeDelta,
      "Time constant for levelset evolution; must be > 0; larger is faster");
  this->m_CommandLine.AddOption(
      Key('t', "levelset-threshold"), &this->m_LevelsetThreshold,
  ...
  this->m_CommandLine
      .AddParameter(&this->m_InFile, "InputImage", "Input image path")
      ->SetProperties(CommandLine::PROPS_IMAGE);
  this->m_CommandLine
      .AddParameter(&this->m_OutFile, "OutputImage", "Output image path")
      ->SetProperties(CommandLine::PROPS_IMAGE | CommandLine::PROPS_LABELS |
                      CommandLine::PROPS_OUTPUT);
}
