...
void cmtk::LabelCombinationLocalVoting::AddAtlas(
...
int cmtk::LabelCombinationLocalVoting::ComputeMaximumLabelValue() const {
  int maxLabel = 0;
  for (size_t n = 0; n < this->m_AtlasLabels.size(); ++n) {
    maxLabel = std::max(
        maxLabel,
        static_cast<int>(
            this->m_AtlasLabels[n]->GetData()->GetRange().m_UpperBound));
  }

  return maxLabel;
}

size_t cmtk::LabelCombinationLocalVoting::ComputeLabelNumberOfPixels(
...
cmtk::TypedArray::SmartPtr cmtk::LabelCombinationLocalVoting::GetResult()
    const {
  ...
  if (this->m_UseGlobalAtlasWeights) {
    for (size_t n = 0; n < nAtlases; ++n) {
      this->m_GlobalAtlasWeights[n] =
          1.0 / TypedArraySimilarity::GetCrossCorrelation(
                    targetImage.GetData(), this->m_AtlasImages[n]->GetData());
    }
  } else {
    std::fill(this->m_GlobalAtlasWeights.begin(),
              this->m_GlobalAtlasWeights.end(), 1.0);
  }
...
void cmtk::LabelCombinationLocalVoting::ComputeResultForRegion(
    const Self::TargetRegionType &region, TypedArray &result) const {
  const UniformVolume &targetImage = *(this->m_TargetImage);
  ...
  for (RegionIndexIterator<TargetRegionType> it(region); it != it.end(); ++it) {
    ...
    if (allTheSame) {
      ...
      const TargetRegionType patchRegion(
          Max(wholeImageRegion.From(), it.Index() - this->m_PatchRadius),
          Min(wholeImageRegion.To(), it.Index() + this->m_PatchRadiusPlusOne));
      ...
      for (size_t n = 0; n < nAtlases; ++n) {
        if (valid[n]) {
          ...
          labelToTotalWeight[labels[n]] +=
              TypedArraySimilarity::GetCrossCorrelation(targetDataPatch,
                                                        atlasDataPatch) *
              this->m_GlobalAtlasWeights[n];
        }
      }
      ...
      for (std::map<unsigned short, Types::DataItem>::const_iterator mapIt =
               labelToTotalWeight.begin();
           mapIt != labelToTotalWeight.end(); ++mapIt) {
