...
void cmtk::LabelCombinationLocalBinaryShapeBasedAveraging::AddAtlas(
    const UniformVolume::SmartConstPtr image,
    const UniformVolume::SmartConstPtr atlas) {
  Superclass::AddAtlasImage(image);

  this->m_AtlasDMaps.push_back(
      UniformDistanceMap<float>(*atlas, UniformDistanceMap<float>::SIGNED |
                                            UniformDistanceMap<float>::SQUARED)
          .Get());
}
...
cmtk::LabelCombinationLocalBinaryShapeBasedAveraging::GetResult() const {
  ...
  cmtk::TypedArray::SmartPtr result(
      TypedArray::Create(TYPE_SHORT, targetImage.GetNumberOfPixels()));
...
void cmtk::LabelCombinationLocalBinaryShapeBasedAveraging::
    ComputeResultForRegion(const Self::TargetRegionType &region,
  ...
  for (RegionIndexIterator<TargetRegionType> it(region); it != it.end(); ++it) {
    ...
    if (this->m_DetectLocalOutliers) {
      ...
      size_t nn = 0;
      for (size_t n = 0; n < nAtlases; ++n) {
        if (valid[n]) distances[nn++] = this->m_AtlasDMaps[n]->GetDataAt(i);
      }
    ...
    if (allTheSame) {
      ...
      for (size_t n = 0; n < nAtlases; ++n) {
        if (valid[n]) {
          totalDistance +=
              (weights[n] - minWeight) / maxWeight *
              this->m_AtlasDMaps[n]->GetDataAt(i + bestPatchOffset[n]);
        }
      }
