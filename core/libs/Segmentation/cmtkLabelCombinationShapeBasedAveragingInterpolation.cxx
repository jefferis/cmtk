...
LabelCombinationShapeBasedAveragingInterpolation::
    LabelCombinationShapeBasedAveragingInterpolation(
        ...
        const std::vector<cmtk::XformUniformVolume::SmartConstPtr>
            &xformsToLabelImages,
        const UniformVolume::SmartConstPtr &targetGrid,
  ...
  if (this->m_LabelImages.size() != this->m_Transformations.size()) {
    StdErr << "ERROR: number of transformations does not match number of input "
              "images\n";
...
LabelCombinationShapeBasedAveragingInterpolation::GetResult() const {
  ...
  for (int label = 0; label < this->m_NumberOfLabels; ++label) {
    ...
    std::fill(labelDistance.begin(), labelDistance.end(),
    ...
    for (size_t k = 0; k < this->m_LabelImages.size(); ++k) {
      cmtk::UniformVolume::SmartPtr signedDistanceMap =
          cmtk::UniformDistanceMap<float>(
              *(this->m_LabelImages[k]),
              cmtk::DistanceMap::VALUE_EXACT + cmtk::DistanceMap::SIGNED, label)
              .Get();
      cmtk::UniformVolumeInterpolator<cmtk::Interpolators::Linear> interpolator(
          *signedDistanceMap);
        ...
        for (int y = 0; y < targetDims[1]; ++y) {
          for (int x = 0; x < targetDims[0]; ++x, ++i) {
            ...
            if (interpolator.GetDataAt(v, dvalue)) {
              labelDistance[i] +=
                  static_cast<Self::DistanceMapRealType>(dvalue);
            }
          }
        }
      }
