...
//@{
void EntropyMinimizationIntensityCorrectionFunctionalBase ::SetInputImage(
    UniformVolume::SmartConstPtr &inputImage) {
  ...
  if (this->m_UseLogIntensities) {
    this->m_EntropyHistogram = HistogramType::SmartPtr(
        new LogHistogramType(this->m_NumberOfHistogramBins));
  } else {
    this->m_EntropyHistogram = HistogramType::SmartPtr(
        new HistogramType(this->m_NumberOfHistogramBins));
  }
  // extend value range to accomodate corrected intensities without overflows
  this->m_EntropyHistogram->SetRange(
      Types::DataItemRange(range.m_LowerBound - this->m_InputImageRange,
                           range.m_UpperBound + this->m_InputImageRange));

  if (this->m_ForegroundMask.size()) this->UpdateCorrectionFactors();
  ...
  this->m_OutputImage =
      UniformVolume::SmartPtr(this->m_InputImage->CloneGrid());
  this->m_OutputImage->CreateDataArray(TYPE_FLOAT);
}

void EntropyMinimizationIntensityCorrectionFunctionalBase ::SetForegroundMask(
  ...
  if (maskPixels != this->m_NumberOfPixels) {
    throw Exception(
        "Number of mask pixels does not match number of input image pixels.");
  }

  this->m_ForegroundMask.resize(maskPixels);
  if ((this->m_SamplingDensity > 0) && (this->m_SamplingDensity < 1)) {
    for (size_t i = 0; i < maskPixels; ++i) {
      this->m_ForegroundMask[i] =
          (foregroundMask.GetDataAt(i) > 0) &&
          (MathUtil::UniformRandom() <= this->m_SamplingDensity);
    }
  } else {
    for (size_t i = 0; i < maskPixels; ++i) {
  ...
  if (this->m_InputImage) this->UpdateCorrectionFactors();
}

UniformVolume::SmartPtr &
EntropyMinimizationIntensityCorrectionFunctionalBase ::GetOutputImage(
    CoordinateVector &v, const bool foregroundOnly) {
...
EntropyMinimizationIntensityCorrectionFunctionalBase ::EvaluateAt(
    CoordinateVector &v) {
  this->SetParamVector(v);
...
#ifdef CMTK_BUILD_DEMO
void EntropyMinimizationIntensityCorrectionFunctionalBase ::SnapshotAt(
    CoordinateVector &v) {
  this->SetParamVector(v);
  this->UpdateBiasFields();
  ...
  UniformVolume::SmartConstPtr slice = this->m_OutputImage->ExtractSlice(
      AXIS_Z, this->m_OutputImage->m_Dims[2] / 2);
...
void EntropyMinimizationIntensityCorrectionFunctionalBase ::UpdateOutputImage(
    const bool foregroundOnly) {
...
void EntropyMinimizationIntensityCorrectionFunctionalBase ::
    UpdateOutputImageThreadFunc(void *args, const size_t taskIdx,
                                const size_t taskCnt, const size_t,
                                const size_t) {
  UpdateOutputImageThreadParameters *threadParameters =
      static_cast<UpdateOutputImageThreadParameters *>(args);
  ...
  const float *biasFieldPtrAdd =
      ThisConst->m_BiasFieldAdd->GetDataPtrTemplate();
  const float *biasFieldPtrMul =
      ThisConst->m_BiasFieldMul->GetDataPtrTemplate();

  Types::DataItem value;
  for (size_t ofs = taskIdx; ofs < numberOfPixels; ofs += taskCnt) {
    if (!threadParameters->m_ForegroundOnly ||
        ThisConst->m_ForegroundMask[ofs]) {
      if (inputImage->GetDataAt(value, ofs)) {
        outputData->Set(value * biasFieldPtrMul[ofs] + biasFieldPtrAdd[ofs],
                        ofs);
      } else {
