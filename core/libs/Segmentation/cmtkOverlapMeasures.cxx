...
OverlapMeasures::OverlapMeasures(
    const std::vector<TypedArray::SmartPtr> &dataArrays) {
  ...
  for (size_t i = 0; i < this->m_DataArrays.size(); ++i) {
    ...
    this->m_MaxLabelValue = std::max<unsigned int>(
        this->m_MaxLabelValue, static_cast<unsigned int>(range.m_UpperBound));
  }
  ...
  for (size_t i = 1; i < this->m_NumberOfImages; ++i) {
    this->m_NumberOfPixels =
        std::min(this->m_NumberOfPixels, this->m_DataArrays[i]->GetDataSize());
  }
}

size_t OverlapMeasures::ComputeGroupwiseOverlap(
    const int firstLabel, const int numberOfLabels,
    double &overlapEqualWeighted, double &overlapVolumeWeighted,
    double &overlapInverseWeighted) const {
  ...
  for (int label = 0; label < numberOfLabels; ++label) {
    if (labelExists[label]) ++numberOfLabelsIncluded;
  }
  if (!numberOfLabelsIncluded) return numberOfLabelsIncluded;

  // run overlap computation
  const size_t progressPixels = 100000;
  Progress::Begin(0, this->m_NumberOfPixels, progressPixels,
                  "Overlap computation");
  ...
  for (int px = 0; px < static_cast<int>(this->m_NumberOfPixels); ++px) {
...
#ifdef _OPENMP
    const size_t labelsPerPixelOfs =
        this->m_NumberOfImages * omp_get_thread_num();
#else
    ...
    for (int label = 0; label < numberOfLabels; ++label) {
      if (labelExists[label]) {
        for (size_t i = 0; i < this->m_NumberOfImages; ++i) {
          const int wi =
              (labelsPerPixel[labelsPerPixelOfs + i] == label) ? 1 : 0;
          for (size_t j = 0; j < i; ++j, ++ofs) {
            const int wj =
                (labelsPerPixel[labelsPerPixelOfs + j] == label) ? 1 : 0;
...
double OverlapMeasures::ComputePairwiseOverlapMinMax(
    double &overlap_min, double &overlap_max, const TypedArray::SmartPtr &data0,
    const TypedArray::SmartPtr &data1, const int label) const {
