...
cmtk::LabelCombinationLocalShapeBasedAveraging::GetResult() const {
  ...
  const int maxLabelValue = (this->m_MaxLabelValue > 0)
                                ? this->m_MaxLabelValue
                                : this->ComputeMaximumLabelValue();
  for (int label = 0; label <= maxLabelValue; ++label) {
    if (this->ComputeLabelNumberOfPixels(label) >
        0)  // skip unused label values
    {
      DebugOutput(2) << "Processing label " << label << "\n";

      DebugOutput(5) << "  Creating distance maps\n";
      for (size_t n = 0; n < nAtlases; ++n) {
        atlasDMaps[n] = (UniformDistanceMap<float>(*(this->m_AtlasLabels[n]),
                                                   DistanceMap::SIGNED |
                                                       DistanceMap::SQUARED |
                                                       DistanceMap::VALUE_EXACT,
                                                   label)
                             .Get());
      }
...
#pragma omp parallel for
      for (int slice = region.From()[2]; slice < region.To()[2]; ++slice) {
        ...
        this->ComputeResultForRegion(*result, resultDistance, label,
                                     threadRegion, atlasDMaps);
      }
#else   // _OPENMP
      this->ComputeResultForRegion(*result, resultDistance, label, region,
                                   atlasDMaps);
#endif  // _OPENMP
    }
  }
...
void cmtk::LabelCombinationLocalShapeBasedAveraging::ComputeResultForRegion(
    TypedArray &result, std::vector<float> &resultDistance, const int label,
    const Self::TargetRegionType &region,
    std::vector<UniformVolume::SmartConstPtr> dmaps) const {
  const UniformVolume &targetImage = *(this->m_TargetImage);
  ...
  for (RegionIndexIterator<TargetRegionType> it(region); it != it.end(); ++it) {
    ...
    // detect local outliers in the distance maps, ie., grossly misregistered
    // atlases
    if (this->m_DetectLocalOutliers) {
      // create vector of distance values
      size_t nn = 0;
      for (size_t n = 0; n < nAtlases; ++n) {
        if (valid[n]) distances[nn++] = dmaps[n]->GetDataAt(i);
      }
      ...
      for (size_t n = 0; n < nAtlases; ++n) {
        if (valid[n]) {
          ...
          if ((d < lThresh) || (d > uThresh)) valid[n] = false;
        }
      }
    }
    ...
    size_t firstValid = 0;
    while ((firstValid < nAtlases) && !valid[firstValid]) ++firstValid;

    // if all input atlases are undefined (padding) for this pixel, set output
    // to padding and skip to next pixel.
    ...
    const TargetRegionType patchSearchRegion(
        Max((-1) * wholeImageRegion.From(), this->m_SearchRegion.From()),
        Min(wholeImageRegion.To() - it.Index(), this->m_SearchRegion.To()));
    for (RegionIndexIterator<TargetRegionType> searchIt(patchSearchRegion);
         searchIt != searchIt.end(); ++searchIt) {
      const TargetRegionType patchRegion(
          Max(wholeImageRegion.From(),
              it.Index() + searchIt.Index() - this->m_PatchRadius),
          Min(wholeImageRegion.To(),
              it.Index() + searchIt.Index() + this->m_PatchRadiusPlusOne));
      TypedArray::SmartConstPtr targetDataPatch(
          targetImage.GetRegionData(patchRegion));

      for (size_t n = 0; n < nAtlases; ++n) {
        if (valid[n]) {
          TypedArray::SmartConstPtr atlasDataPatch(
              this->m_AtlasImages[n]->GetRegionData(patchRegion));
          const Types::DataItem w = TypedArraySimilarity::GetCrossCorrelation(
              targetDataPatch, atlasDataPatch);
          if (w > weights[n]) {
            weights[n] = w;
            bestPatchOffset[n] =
                targetImage.GetOffsetFromIndex(searchIt.Index());
          }
        }
      }
    }
    ...
    for (size_t n = 0; n < nAtlases; ++n) {
      if (valid[n]) {
        totalDistance += (weights[n] - minWeight) / maxWeight *
                         dmaps[n]->GetDataAt(i + bestPatchOffset[n]);
      }
    }
