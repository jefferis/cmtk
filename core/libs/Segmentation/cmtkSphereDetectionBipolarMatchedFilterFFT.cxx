...
cmtk::SphereDetectionBipolarMatchedFilterFFT::
...
cmtk::SphereDetectionBipolarMatchedFilterFFT::
    ~SphereDetectionBipolarMatchedFilterFFT() {
...
cmtk::SphereDetectionBipolarMatchedFilterFFT::GetFilteredImageData(
  ...
  if (cnt) {
    ...
    for (size_t n = 0; n < this->m_NumberOfPixels; ++n) {
      this->m_FilterFT[n][1] *=
          -1;  // correlation is multiplication with complex conjugate of
               // filter, but we can also just conjugate the image
  ...
  for (size_t n = 0; n < this->m_NumberOfPixels; ++n) {
    result->Set(FFTW::Magnitude(this->m_FilterFT[n]) / this->m_NumberOfPixels,
                n);
  }
...
size_t cmtk::SphereDetectionBipolarMatchedFilterFFT::MakeFilter(
    const Types::Coordinate sphereRadius, const int marginWidth) {
  const int nRadius[3] = {
  ...
  for (int k = 0; k < nRadius[2]; ++k)
    for (int j = 0; j < nRadius[1]; ++j)
      for (int i = 0; i < nRadius[0]; ++i) {
        const cmtk::Types::Coordinate distance =
            sqrt(cmtk::MathUtil::Square(k * this->m_PixelSize[2]) +
                 cmtk::MathUtil::Square(j * this->m_PixelSize[1]) +
                 cmtk::MathUtil::Square(i * this->m_PixelSize[0]));
            ...
            for (int kk = k; kk < this->m_ImageDims[2];
              ...
              for (int jj = j; jj < this->m_ImageDims[1];
                ...
                for (int ii = i; ii < this->m_ImageDims[0];
                  ...
                  this->m_FilterFT[ii + this->m_ImageDims[0] *
