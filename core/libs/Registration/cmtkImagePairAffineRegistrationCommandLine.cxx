...
ImagePairAffineRegistrationCommandLine ::ImagePairAffineRegistrationCommandLine(
  ...
  try {
    ...
    cl.SetProgramInfo(CommandLine::PRG_DESCR,
                      "This program performs rigid and affine image "
                      "registration using multi-resolution optimization of "
                      "voxel-based image similarity measures.");
    ...
    cl.AddOption(Key("auto-multi-levels"), &this->m_AutoMultiLevels,
                 "Automatic optimization and resolution parameter generation "
                 "for <n> levels");
    ...
    cl.AddOption(Key("max-stepsize"), &this->m_MaxStepSize,
                 ...
                 "exploration.");
    cl.AddOption(Key("min-stepsize"), &this->m_MinStepSize,
                 "Minimum optimizer step size, which determines precision.");
    cl.AddOption(
        Key("stepfactor"), &this->m_OptimizerStepFactor,
        "Factor for search step size reduction. Must be > 0.0 and < 1.0");
    cl.AddOption(Key("delta-f-threshold"), &this->m_DeltaFThreshold,
    ...
    cl.AddOption(Key('s', "sampling"), &this->m_Sampling,
                 "Image sampling (finest resampled image resolution)");
    cl.AddOption(Key("coarsest"), &this->m_CoarsestResolution,
                 "Upper limit for image sampling in multiresolution hierarchy");

    cl.AddSwitch(
        Key("omit-original-data"), &this->m_UseOriginalData, false,
    ...
    cl.AddVector(Key("dofs"), this->NumberDOFs,
                 "Add number of degrees of freedom. This can be 3 "
                 "(translation), 6 (rigid: translation and rotation), "
                 "7 (rigid plus global scale), 9 (rigid plus anisotropic "
                 "scales), 12 (rigid plus scales plus shears), 3003 "
                 "(translation plus shear), 3033 (translation, shear, and "
                 "scale) or 3303 (rigid plus shears, but no scale). "
                 "This option can be repeated, in which case DOFs are used for "
                 "successive optimization runs in the order that they appear.");
    cl.AddVector(Key("dofs-final"), this->NumberDOFsFinal,
                 "Add number of degrees of freedom for final level only [can "
                 "be repeated]");
    cl.AddSwitch(
        Key("symmetric"), &this->m_SymmetricFwdBwd, true,
        "Use symmetric registration functional to simultaneously estimate "
        "forward and inverse transformation. "
        "This increases ragistration time substantially but produces a result "
        "that is invariant under exchange of fixed and moving image. "
        "It may also be more robust and/or more accurate than forward-only "
        "registration.");

    CommandLine::EnumGroup<int>::SmartPtr inPlaneGroup =
        cl.AddEnum("restrict-in-plane", &this->m_RestrictToInPlane,
                   "Restrict the affine transformation to be in-plane for "
                   "planes perpendicular to a given coordinate axis.");
    inPlaneGroup->AddSwitch(Key("xy"), 2,
                            "Transformation restricted to in-plane for 'xy' "
                            "plane (perpendicular to z coordinate axis).");
    inPlaneGroup->AddSwitch(Key("xz"), 1,
                            "Transformation restricted to in-plane for 'xz' "
                            "plane (perpendicular to y coordinate axis).");
    inPlaneGroup->AddSwitch(Key("yz"), 0,
                            "Transformation restricted to in-plane for 'yz' "
                            "plane (perpendicular to x coordinate axis).");
    inPlaneGroup->AddSwitch(Key("none"), -1,
                            "Full 3D affine transformation is computed.");

    CommandLine::EnumGroup<MakeInitialAffineTransformation::Mode>::SmartPtr
        initGroup =
            cl.AddEnum("init", &this->m_Initializer,
                       "Select initializer for the affine trasnformation.");
    initGroup->AddSwitch(Key("none"), MakeInitialAffineTransformation::NONE,
                         "Use input transformation, or identity transformation "
                         "if none was provided.");
    initGroup->AddSwitch(Key("fov"), MakeInitialAffineTransformation::FOV,
                         "Align centers of field of view (or crop regions) "
                         "using a translation.");
    initGroup->AddSwitch(Key("com"), MakeInitialAffineTransformation::COM,
                         "Align centers of mass using a translation.");
    initGroup->AddSwitch(Key("pax"), MakeInitialAffineTransformation::PAX,
                         "Align images by rotation using principal axes and "
                         "translation using centers of mass.");
    initGroup->AddSwitch(
        Key("physical"), MakeInitialAffineTransformation::PHYS,
        "Align images by rotation using direction vectors stored in input "
        "images and translation using image origins.");

    cl.AddOption(Key("initial"), &InitialStudylist,
                 "Initialize transformation from given path")
        ->SetProperties(CommandLine::PROPS_XFORM);
    cl.AddSwitch(
        Key("initial-is-inverse"), &this->m_InitialXformIsInverse, true,
        "Invert initial transformation before initializing registration");
    cl.EndGroup();

    cl.BeginGroup("Image data", "Image data");
    CommandLine::EnumGroup<int>::SmartPtr metricGroup =
        cl.AddEnum("registration-metric", &this->m_Metric,
                   "Registration metric for motion estimation by "
                   "image-to-image registration.");
    metricGroup->AddSwitch(Key("nmi"), 0,
    ...
    metricGroup->AddSwitch(Key("ncc"), 5,
                           "Normalized Cross Correlation metric");

    cl.BeginGroup("Interpolation", "Floating Image Interpolation Options");
    cmtk::CommandLine::EnumGroup<Interpolators::InterpolationEnum>::SmartPtr
        kernelGroup =
            cl.AddEnum("interpolation", &this->m_FloatingImageInterpolation,
                       "Interpolation method for floating image sampling:");
    kernelGroup->AddSwitch(
        Key("nearest-neighbor"), Interpolators::NEAREST_NEIGHBOR,
        "Nearest neighbor interpolation (for intensity and label data)");
    kernelGroup->AddSwitch(Key("linear"), Interpolators::LINEAR,
                           "Trilinear interpolation");
    kernelGroup->AddSwitch(Key("cubic"), Interpolators::CUBIC,
                           "Tricubic interpolation");
    kernelGroup->AddSwitch(
        Key("cosine-sinc"), Interpolators::COSINE_SINC,
        "Cosine-windowed sinc interpolation (most accurate but slowest)");
    kernelGroup->AddSwitch(Key("partial-volume"), Interpolators::PARTIALVOLUME,
                           "Partial volume interpolation (for label data)");
    kernelGroup->AddSwitch(
        Key("automatic"), Interpolators::DEFAULT,
        "Select interpolation automatically based on data type: linear for "
        "grey-level data, nearest neighbor for label data.");

    cl.AddSwitch(
        Key("match-histograms"), &this->m_MatchFltToRefHistogram, true,
        "Match floating image histogram to reference image histogram.");
    cl.AddOption(Key("force-outside-value"), &forceOutsideValue,
                 "Force values outside field of view to this value rather than "
                 "drop incomplete pixel pairs",
                 &forceOutsideFlag);
    ...
    cl.AddOption(Key('o', "output"), &this->Studylist,
                 "Output path for final transformation");
    cl.AddOption(Key("write-matrix"), &this->OutMatrixName,
                 "Output path for final transformation in matrix format");
    cl.AddOption(
        Key("write-parameters"), &this->OutParametersName,
        "Output path for final transformation in plain parameter list format");
    cl.AddOption(Key("write-protocol"), &this->m_ProtocolFileName,
                 "Optimization protocol output file name");
    cl.AddOption(Key("write-time"), &this->Time,
                 "Computation time statistics output file name");
    cl.EndGroup();

    cl.BeginGroup("SlicerImport", "Import Results into Slicer");
    cl.AddOption(Key("write-itk"), &this->m_OutputPathITK,
    ...
    cl.AddOption(Key("write-reformatted"), &this->m_ReformattedImagePath,
    ...
    cl.AddOption(Key("db"), &this->m_UpdateDB,
  ...
  if ((this->m_OptimizerStepFactor <= 0) ||
      (this->m_OptimizerStepFactor >= 1)) {
    StdErr << "ERROR: step factor value " << this->m_OptimizerStepFactor
           << " is invalid. Must be in range (0..1)\n";
    throw cmtk::ExitException(1);
  }

  // check for supported numbers of degrees of freedom
  const std::set<short> supportedDOFs = AffineXform::GetSupportedDOFs();
  for (std::vector<short>::iterator it = this->NumberDOFs.begin();
       it != this->NumberDOFs.end(); ++it) {
  ...
  // check for supported numbers of degrees of freedom
  for (std::vector<short>::iterator it = this->NumberDOFsFinal.begin();
       it != this->NumberDOFsFinal.end(); ++it) {
    if (*it == 603)  // fix legacy value
      *it = 3303;
  ...
  if (!clArg2.empty()) {
    ...
    if (!InitialStudylist.empty()) {
      StdErr << "WARNING: transformation of input studylist will be overriden "
                "by transformation provided with '--initial'.\n";
    }

    DebugOutput(1) << "Reading input studylist " << inStudylist << ".\n";

    ClassStreamInput typedStream(MountPoints::Translate(inStudylist),
                                 "registration");
    ...
    if (!this->Study2.empty()) {
      ...
      try {
        ...
        StdErr << "ERROR: singular matrix from legacy transformation file "
                  "cannot be inverted in "
                  "ImagePairAffineRegistrationCommandLine constructor.\n";
  ...
  this->SetVolume_1(UniformVolume::SmartPtr(
      this->m_PreprocessorRef.GetProcessedImage(volume)));
  ...
  this->SetVolume_2(UniformVolume::SmartPtr(
      this->m_PreprocessorFlt.GetProcessedImage(volume)));
  ...
  if (!InitialStudylist.empty()) {
    ...
    if (!affine) {
      StdErr << "ERROR: transformation " << InitialStudylist
             << " is not affine.\n";
      throw cmtk::ExitException(1);
    }

    if (affine->GetMetaInfo(META_SPACE) !=
        AnatomicalOrientation::ORIENTATION_STANDARD) {
      try {
        TransformChangeFromSpaceAffine toStandardSpace(
            *affine, *(this->m_Volume_1), *(this->m_Volume_2));
        *affine = toStandardSpace.GetTransformation();
        affine->SetMetaInfo(META_SPACE,
                            AnatomicalOrientation::ORIENTATION_STANDARD);
      } catch (const AffineXform::MatrixType::SingularMatrixException &) {
        StdErr << "ERROR: singular matrix cannot be inverted to change "
                  "transformation to standard space in "
                  "ImagePairAffineRegistrationCommandLine constructor.\n";
        throw ExitException(1);
      }
    }
  ...
  if (this->m_Initializer != MakeInitialAffineTransformation::NONE) {
    if (!(inStudylist.empty() && InitialStudylist.empty())) {
      StdErr << "INFO: initial transformation was provided. Selected "
                "transformation initializer will be ignored.\n";
    }
  }

  if (!this->m_ProtocolFileName.empty()) {
    RegistrationCallback::SmartPtr callback(
        new ProtocolCallback(this->m_ProtocolFileName));
...
CallbackResult ImagePairAffineRegistrationCommandLine::InitRegistration() {
  CallbackResult Result = Superclass::InitRegistration();
  return Result;
}

void ImagePairAffineRegistrationCommandLine::OutputResultMatrix(
...
void ImagePairAffineRegistrationCommandLine::OutputResultParameters(
...
void ImagePairAffineRegistrationCommandLine::OutputResultList(
    const std::string &studyList) const {
  ClassStreamOutput classStream(studyList, "studylist",
                                ClassStreamOutput::MODE_WRITE);
  ...
  classStream.WriteString("reference_study",
                          CompressedStream::GetBaseName(Study1));
  classStream.WriteString("floating_study",
                          CompressedStream::GetBaseName(Study2));
  ...
  classStream.WriteString(
      "initializer",
      MakeInitialAffineTransformation::GetModeName(this->m_Initializer));
...
void ImagePairAffineRegistrationCommandLine::OutputResult(
    const CoordinateVector *v, const CallbackResult irq) {
  ...
  if (!this->m_OutputPathITK.empty()) {
    try {
      TransformChangeToSpaceAffine toNative(
          *(this->GetTransformation()), *(this->m_Volume_1),
          *(this->m_Volume_2), AnatomicalOrientationBase::SPACE_ITK);
      if (irq != CALLBACK_OK)
        AffineXformITKIO::Write(this->m_OutputPathITK + "-partial",
                                toNative.GetTransformation());
      else
      ...
      StdErr << "ERROR: singular matrix cannot be inverted to change "
                "transformation to standard space in "
                "ImagePairAffineRegistrationCommandLine::OutputResult\n";
      throw ExitException(1);
    }
  }
  ...
  if (!this->m_ReformattedImagePath.empty()) {
    if (irq != CALLBACK_OK)
      VolumeIO::Write(*(this->GetReformattedFloatingImage()),
                      this->m_ReformattedImagePath + "-partial");
    else
      VolumeIO::Write(*(this->GetReformattedFloatingImage()),
                      this->m_ReformattedImagePath);
  }
  ...
  if (!this->m_UpdateDB.empty()) {
    try {
      ...
      if (!this->m_ReformattedImagePath.empty()) {
        db.AddImage(this->m_ReformattedImagePath,
                    this->m_ReferenceVolume->GetMetaInfo(META_FS_PATH));
      }

      if (!this->Studylist.empty()) {
        if (!this->m_InitialXformPath.empty()) {
          db.AddRefinedXform(this->Studylist, true /*invertible*/,
                             this->m_InitialXformPath,
                             this->m_InitialXformIsInverse);
        } else {
          db.AddImagePairXform(
              this->Studylist, true /*invertible*/,
              this->m_ReferenceVolume->GetMetaInfo(META_FS_PATH),
              this->m_FloatingVolume->GetMetaInfo(META_FS_PATH));
        }
      }
...
void ImagePairAffineRegistrationCommandLine::EnterResolution(
    CoordinateVector::SmartPtr &v, Functional::SmartPtr &f, const int index,
    const int total) {
  DebugOutput(1).GetStream().printf(
      "\rEntering resolution level %d out of %d...\n", index, total);
  this->Superclass::EnterResolution(v, f, index, total);
}

CallbackResult ImagePairAffineRegistrationCommandLine::Register() {
