...
CallbackResult ImagePairAffineRegistration::InitRegistration() {
  ...
  if (this->m_InitialTransformation) {
    if (this->m_InitialXformIsInverse) {
      affineXform =
          AffineXform::SmartPtr(this->m_InitialTransformation->MakeInverse());
    } else {
      affineXform = AffineXform::SmartPtr(this->m_InitialTransformation);
    }
  } else {
    affineXform = AffineXform::SmartPtr(MakeInitialAffineTransformation::Create(
...
Functional *ImagePairAffineRegistration ::MakeFunctional(
  ...
  AffineXform::SmartPtr affineXform =
      AffineXform::SmartPtr::DynamicCastFrom(this->m_Xform);
  if (!affineXform) {
  ...
  if (levelParameters->m_Resolution > 0) {
    nextRef = UniformVolume::SmartPtr(
        this->m_ReferenceVolume->GetResampled(levelParameters->m_Resolution));
    nextFlt = UniformVolume::SmartPtr(
        this->m_FloatingVolume->GetResampled(levelParameters->m_Resolution));
  } else {
  ...
  if (this->m_SymmetricFwdBwd) {
    ImagePairSymmetricAffineRegistrationFunctional *functional =
        ImagePairSymmetricAffineRegistrationFunctional::Create(
            this->m_Metric, nextRef, nextFlt,
            this->m_FloatingImageInterpolation, affineXform);
    functional->SetForceOutside(this->m_ForceOutsideFlag,
    ...
    ImagePairAffineRegistrationFunctional *functional =
        ImagePairAffineRegistrationFunctional::Create(
            this->m_Metric, nextRef, nextFlt,
            this->m_FloatingImageInterpolation, affineXform);
    functional->SetForceOutside(this->m_ForceOutsideFlag,
                                this->m_ForceOutsideValue);
...
void ImagePairAffineRegistration::EnterResolution(CoordinateVector::SmartPtr &v,
  ...
  if (affineXform) {
    ...
    if (this->m_Callback) {
      char buffer[64];
      snprintf(buffer, sizeof(buffer), "Setting Number DOFs to %d.",
               *NumberDOFsIterator);
...
int ImagePairAffineRegistration::DoneResolution(CoordinateVector::SmartPtr &v,
                                                Functional::SmartPtr &f,
                                                const int level,
                                                const int total) {
...
AffineXform::SmartPtr ImagePairAffineRegistration::GetTransformation() const {
  AffineXform::SmartPtr affineXform =
      AffineXform::SmartPtr::DynamicCastFrom(this->m_Xform);
  return affineXform;
}
...
ImagePairAffineRegistration::GetReformattedFloatingImage(
