...
ImagePairNonrigidRegistrationCommandLine
    *ImagePairNonrigidRegistrationCommandLine::StaticThis = NULL;

ImagePairNonrigidRegistrationCommandLine ::
    ImagePairNonrigidRegistrationCommandLine(const int argc,
                                             const char *argv[]) {
  ...
  try {
    ...
    cl.AddOption(Key("initial"), &this->m_InitialTransformationFile,
                 "Initialize transformation from given path")
        ->SetProperties(CommandLine::PROPS_XFORM);
    cl.AddSwitch(Key("invert-initial"), &this->m_InitialTransformationInverse,
                 true, "Invert given (affine) initial transformation.");
    cl.AddOption(Key("write-itk-xform"), &this->m_OutputPathITK,
                 "Output path for final transformation in ITK format")
        ->SetProperties(CommandLine::PROPS_XFORM | CommandLine::PROPS_OUTPUT)
        ->SetAttribute("type", "bspline")
    ...
    cl.AddOption(Key("grid-spacing"), &this->m_GridSpacing,
                 "Control point grid spacing");
    cl.AddOption(
        Key("grid-refine"), &this->m_RefineGrid,
        "Number of refinements (control point grid resolution levels)");
    cl.AddSwitch(Key("delay-refine"), &this->m_DelayRefineGrid, true,
                 "Delay control point grid refinement; first switch to next "
                 "higher image resolution");
    cl.AddSwitch(Key("exact-spacing"), &this->m_ExactGridSpacing, true,
                 "Use exact control point spacing; do not modify spacing to "
                 "fit reference image bounding box");

    cl.AddOption(Key("ignore-edge"), &this->IgnoreEdge,
                 "Ignore n control point layers along each image face");
    cl.AddOption(Key("restrict"), &this->RestrictToAxes,
                 "Restrict deformation to coordinate dimension(s) [one or more "
                 "of 'x','y','z']");

    cl.AddSwitch(Key("no-adaptive-fix"), &this->m_AdaptiveFixParameters, false,
                 "Disable adaptive fixing of control points; optimize all "
                 "deformation parameters");
    cl.AddOption(
        Key("adaptive-fix-thresh"), &this->m_AdaptiveFixThreshFactor,
        "Threshold factor for entropy criterion to fix local control points");
    cl.AddSwitch(Key("accurate"), &this->m_FastMode, false,
                 "Accurate computation mode: may give slightly better results "
                 "after substantially longer computation");
    cl.AddSwitch(Key("fast"), &this->m_FastMode, true,
    ...
    cl.AddOption(
        Key("jacobian-constraint-weight"), &this->m_JacobianConstraintWeight,
        "Weight for Jacobian-based local volume preservation constraint");
    cl.AddOption(Key("smoothness-constraint-weight"), &this->m_GridEnergyWeight,
                 "Weight for smoothness constraint based on second-order grid "
                 "bending energy.");
    cl.AddOption(Key("inverse-consistency-weight"),
                 &this->m_InverseConsistencyWeight,
                 "Weight for inverse consistency constraint");
    cl.AddOption(Key("constraint-relaxation-factor"), &this->m_RelaxWeight,
                 "Weight relaxation factor for alternating under-constrained "
                 "iterations");
    cl.AddSwitch(Key("relax-to-unfold"), &this->m_RelaxToUnfold, true,
    ...
    cl.AddOption(Key("stepfactor"), &this->m_OptimizerStepFactor,
    ...
    cl.AddSwitch(Key("no-maxnorm"), &this->m_UseMaxNorm, false,
    ...
    cl.AddSwitch(Key("omit-original-data"), &this->m_UseOriginalData, false,
    ...
    cl.AddSwitch(
        Key("repeat-match-histograms"), &this->m_RepeatMatchFltToRefHistogram,
        true,
        "Repeat histogram matching after every level of the registration to "
        "account for volume changes. When registering masked data, it is "
        "advisable to also use the --force-outside-value option to prevent "
    ...
    cl.AddOption(Key('o', "outlist"), &this->Studylist,
                 "Output path for final transformation");
    cl.AddOption(Key('t', "time"), &this->Time,
                 "Computation time statistics output file name");
    cl.AddSwitch(Key("output-intermediate"), &this->m_OutputIntermediate, true,
    ...
    cl.AddParameter(
          &clArg3, "InitialXform",
          "Initial affine transformation from reference to floating image")
        ->SetProperties(CommandLine::PROPS_NOXML | CommandLine::PROPS_XFORM |
                        CommandLine::PROPS_OPTIONAL);
  ...
  if (!clArg2.empty()) {
    ...
    ClassStreamInput classStream(MountPoints::Translate(InputStudylist),
                                 "registration");
    ...
    if (!Study2.empty()) {
      ...
      try {
        ...
        StdErr
            << "ERROR: singular matrix from initialization file in "
               "cmtk::ImagePairNonrigidRegistrationCommandLine constructor\n";
  ...
  if (!this->m_InitialTransformationFile.empty()) {
    Xform::SmartPtr initialXform(
        XformIO::Read(this->m_InitialTransformationFile));

    if (initialXform) {
      AffineXform::SmartPtr affineXform =
          AffineXform::SmartPtr::DynamicCastFrom(initialXform);
      if (affineXform) {
        if (this->m_InitialTransformationInverse) {
          try {
            this->SetInitialTransformation(affineXform->GetInverse());
          } catch (const AffineXform::MatrixType::SingularMatrixException &) {
            StdErr << "ERROR: singular matrix from initialization file in "
                      "cmtk::ImagePairNonrigidRegistrationCommandLine "
                      "constructor\n";
            throw ExitException(1);
          }
        } else {
        ...
        InitialWarpXform =
            SplineWarpXform::SmartPtr::DynamicCastFrom(initialXform);
      }
    } else {
      StdErr << "ERROR: could not read initial transformation from "
             << this->m_InitialTransformationFile << "\n";
      throw ExitException(1);
    }
  }
  ...
  AffineXform::SmartPtr affineXform(
      AffineXform::SmartPtr::DynamicCastFrom(this->m_InitialTransformation));
  if (affineXform) {
    if (affineXform->MetaKeyExists(META_SPACE) &&
        (affineXform->GetMetaInfo(META_SPACE) !=
         AnatomicalOrientation::ORIENTATION_STANDARD)) {
      try {
        TransformChangeFromSpaceAffine toStandardSpace(
            *affineXform, *(this->m_Volume_1), *(this->m_Volume_2),
            affineXform->GetMetaInfo(META_SPACE).c_str());
        *affineXform = toStandardSpace.GetTransformation();
        affineXform->SetMetaInfo(META_SPACE,
                                 AnatomicalOrientation::ORIENTATION_STANDARD);
        this->SetInitialTransformation(affineXform);
      } catch (const AffineXform::MatrixType::SingularMatrixException &) {
        StdErr << "ERROR: singular matrix cannot be inverted to change "
                  "transformation to standard space in "
                  "ElasticRegistrationCommandLine constructor.\n";
        throw ExitException(1);
      }
    }
  }
...
CallbackResult ImagePairNonrigidRegistrationCommandLine ::InitRegistration() {
  CallbackResult result = this->Superclass::InitRegistration();
  if (result != CALLBACK_OK) return result;

  if (this->m_OutputIntermediate) this->OutputIntermediate();
...
void ImagePairNonrigidRegistrationCommandLine ::OutputResult(
  ...
  if (!this->m_OutputPathITK.empty()) {
    if (irq != CALLBACK_OK)
      SplineWarpXformITKIO::Write(
          this->m_OutputPathITK + "-partial", *(this->GetTransformation()),
          *(this->m_ReferenceVolume), *(this->m_FloatingVolume));
    else
      SplineWarpXformITKIO::Write(
          this->m_OutputPathITK, *(this->GetTransformation()),
          *(this->m_ReferenceVolume), *(this->m_FloatingVolume));
  }
      ...
      if (!this->Studylist.empty()) {
        if (!this->InputStudylist.empty()) {
          ...
          if (!this->m_InitialTransformationFile.empty()) {
            db.AddRefinedXform(this->Studylist, true /*invertible*/,
                               this->m_InitialTransformationFile,
                               m_InitialTransformationInverse);
          } else {
...
void ImagePairNonrigidRegistrationCommandLine ::EnterResolution(
...
int ImagePairNonrigidRegistrationCommandLine::DoneResolution(
    CoordinateVector::SmartPtr &v, Functional::SmartPtr &f, const int index,
    const int total) {
  if (this->m_OutputIntermediate) this->OutputIntermediate();
...
void ImagePairNonrigidRegistrationCommandLine::OutputWarp(
    const std::string &path) const {
  ClassStreamOutput classStream(path, "studylist",
  ...
  classStream.WriteBool("adaptive_fix_parameters",
                        this->m_AdaptiveFixParameters);
  classStream.WriteDouble("adaptive_fix_parameters_thresh",
                          this->m_AdaptiveFixThreshFactor);
  ...
  classStream.WriteDouble("thread_time_level",
                          this->GetThreadLevelElapsedTime());
  classStream.WriteDouble("thread_time_total",
                          this->GetThreadTotalElapsedTime());
  ...
  const WarpXform::SmartPtr warp =
      WarpXform::SmartPtr::DynamicCastFrom(this->m_Xform);
  if (warp) {
    ...
    if (classStream.IsValid()) {
      ...
      classStream.WriteString("reference_study",
                              CompressedStream::GetBaseName(this->Study1));
      classStream.WriteString("floating_study",
                              CompressedStream::GetBaseName(this->Study2));
...
void ImagePairNonrigidRegistrationCommandLine::OutputIntermediate(
    const bool incrementCount) {
  char path[PATH_MAX];
  if (!Studylist.empty()) {
    snprintf(path, sizeof(path), "%s%clevel-%02d.list", Studylist.c_str(),
             (int)CMTK_PATH_SEPARATOR, IntermediateResultIndex);
  } else {
    snprintf(path, sizeof(path), "level-%02d.list", IntermediateResultIndex);
  }
  this->OutputWarp(path);

  if (incrementCount) ++IntermediateResultIndex;
}

CallbackResult ImagePairNonrigidRegistrationCommandLine::Register() {
...
void cmtkImagePairNonrigidRegistrationCommandLineDispatchSIGUSR1(int sig) {
  fprintf(stderr,
          "Received USR1 (%d) signal. Writing intermediate result #%d.\nNote "
          "that this result is not final.\n",
          sig,
          cmtk::ImagePairNonrigidRegistrationCommandLine::StaticThis
  ...
  cmtk::ImagePairNonrigidRegistrationCommandLine::StaticThis
      ->OutputIntermediate(true /* Increment count*/);
}
