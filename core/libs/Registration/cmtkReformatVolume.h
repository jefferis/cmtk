 ...
 * from the respective other images. The class provides optional checkerboard
 * filling of areas with no valid original image data present.
 */
class ReformatVolume {
 public:
  ...
  /// Create interpolator object for given volume according to interpolation
  /// mode set in this object.
  UniformVolumeInterpolatorBase::SmartPtr CreateInterpolator(
      const UniformVolume::SmartConstPtr &volume);

  /// Create interpolator object for given volume according to interpolation
  /// mode set in this object.
  static UniformVolumeInterpolatorBase::SmartPtr CreateInterpolator(
      const cmtk::Interpolators::InterpolationEnum interpolation,
      const UniformVolume::SmartConstPtr &volume);
  ...
  static TypedArray::SmartPtr ReformatMasked(
      const UniformVolume *target, const cmtk::XformList &targetToRef,
      const cmtk::XformList &refToFloat, Fct &fct,
      const UniformVolume *floating = NULL,
      TInterpolator &interpolator = TInterpolator::Null);

  /// Complex reformat without mask.
  template <class TInterpolator, class Fct>
  static TypedArray::SmartPtr ReformatUnmasked(
      const UniformVolume *target, const cmtk::XformList &targetToRef,
      const cmtk::XformList &refToFloat, Fct &fct,
      const UniformVolume *floating = NULL,
      TInterpolator &interpolator = TInterpolator::Null);
  ...
  typedef enum { REFORMAT_PLAIN, REFORMAT_JACOBIAN } Mode;
  ...
  class Plain {
   public:
    ...
    Plain(const ScalarDataType dataType = TYPE_NONE)
        : DataType(dataType), PaddingValue(0), UsePaddingValue(false){};
    ...
    bool operator()(Types::DataItem &value, const Vector3D &inRef,
  ...
  class Jacobian :
   ...
   public:
    /** Constructor. */
    Jacobian(const ScalarDataType dataType = TYPE_FLOAT,
 ...
 private:
  ...
  class GetTransformedReferenceTP : public ThreadParameters<ReformatVolume> {
   public:
    ...
    const std::vector<UniformVolumeInterpolatorBase::SmartConstPtr>
        *interpolatorList;
  ...
  static CMTK_THREAD_RETURN_TYPE GetTransformedReferenceGreyAvg(
      void *const arg);
  ...
  static CMTK_THREAD_RETURN_TYPE GetTransformedReferenceJacobianAvgThread(
      void *const arg);

  /// Create uniform volume with correct dimensions for reformatted reference.
  UniformVolume *CreateTransformedReference(
