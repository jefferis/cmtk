...
void ImagePairNonrigidRegistrationFunctional::WeightedDerivative(
    double &lower, double &upper, SplineWarpXform &warp, const int param,
  ...
  if (!finite(upper) || !finite(lower)) {
    ...
    if (this->m_InverseTransformation) {
      ...
      warp.GetDerivativeInverseConsistencyError(
          lowerIC, upperIC, this->m_InverseTransformation,
          this->m_ReferenceGrid, &(this->VolumeOfInfluence[param]), param,
          step);
...
void ImagePairNonrigidRegistrationFunctional::SetWarpXform(
  ...
  if (this->m_Warp) {
    ...
    for (size_t dim = 0; dim < Dim; ++dim, ++VOIptr) {
      ...
      *VOIptr = this->GetReferenceGridRange(
          this->m_Warp->GetVolumeOfInfluence(dim, this->m_ReferenceDomain));
    }
...
ImagePairNonrigidRegistrationFunctional::Create(
  ...
  switch (metric) {
    case 0:
      return new ImagePairNonrigidRegistrationFunctionalTemplate<
          ImagePairSimilarityMeasureNMI>(refVolume, fltVolume, interpolation);
    case 1:
      return new ImagePairNonrigidRegistrationFunctionalTemplate<
          ImagePairSimilarityMeasureMI>(refVolume, fltVolume, interpolation);
    case 2:
      return new ImagePairNonrigidRegistrationFunctionalTemplate<
          ImagePairSimilarityMeasureCR>(refVolume, fltVolume, interpolation);
    case 3:
      return new ImagePairNonrigidRegistrationFunctionalTemplate<
          ImagePairSimilarityMeasureRMS>(refVolume, fltVolume, interpolation);
    case 4:
      return new ImagePairNonrigidRegistrationFunctionalTemplate<
          ImagePairSimilarityMeasureMSD>(refVolume, fltVolume, interpolation);
    case 5:
      return new ImagePairNonrigidRegistrationFunctionalTemplate<
          ImagePairSimilarityMeasureNCC>(refVolume, fltVolume, interpolation);
    default:
      return NULL;
  }
