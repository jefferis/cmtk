...
CallbackResult ElasticRegistration::InitRegistration() {
  this->m_ReferenceVolume = this->m_Volume_1;
  this->m_FloatingVolume = this->m_Volume_2;

  if (this->m_MatchFltToRefHistogram) {
    this->GetVolume_2()->GetData()->ApplyFunctionObject(
        TypedArrayFunctionHistogramMatching(*(this->GetVolume_2()->GetData()),
                                            *(this->GetVolume_1()->GetData())));
  }

  AffineXform::SmartPtr affineXform = this->m_InitialTransformation;
  AffineXform::SmartPtr initialInverse = AffineXform::SmartPtr::DynamicCastFrom(
  ...
  if (InitialWarpXform) {
    ...
    SplineWarpXform::SmartPtr warpXform(
        this->MakeWarpXform(this->m_ReferenceVolume->m_Size, affineXform));

    if (this->m_InverseConsistencyWeight > 0)
      InverseWarpXform = SplineWarpXform::SmartPtr(
          this->MakeWarpXform(this->m_FloatingVolume->m_Size, initialInverse));
  ...
  if (this->m_UseOriginalData) {
    Functional::SmartPtr nextFunctional(
        this->MakeFunctional(this->m_ReferenceVolume, this->m_FloatingVolume,
                             this->m_RigidityConstraintMap));
  ...
  for (; (currSampling <= this->CoarsestResolution); currSampling *= 2) {
    ...
    if (this->m_RigidityConstraintMap) {
      nextRigidityMap = UniformVolume::SmartPtr(
          this->m_RigidityConstraintMap->GetResampled(currSampling));
    }

    Functional::SmartPtr nextFunctional(
        this->MakeFunctional(nextRef, nextFlt, nextRigidityMap));
    FunctionalStack.push(nextFunctional);
  ...
  switch (this->m_Algorithm) {
    case 0:
      this->m_Optimizer =
          Optimizer::SmartPtr(new BestNeighbourOptimizer(OptimizerStepFactor));
      break;
    case 1:
    case 2:
...
const SplineWarpXform::SmartPtr ElasticRegistration::MakeWarpXform(
...
Functional *ElasticRegistration::MakeFunctional(
    UniformVolume::SmartPtr &refVolume, UniformVolume::SmartPtr &fltVolume,
    UniformVolume::SmartPtr &rigidityMap) const {
  if (this->m_InverseConsistencyWeight > 0) {
    SymmetricElasticFunctional *newFunctional =
        CreateSymmetricElasticFunctional(this->m_Metric, refVolume, fltVolume);
    ...
    VoxelMatchingElasticFunctional *newFunctional =
        CreateElasticFunctional(this->m_Metric, refVolume, fltVolume);
    newFunctional->SetAdaptiveFixParameters(this->m_AdaptiveFixParameters);
    newFunctional->SetAdaptiveFixThreshFactor(this->m_AdaptiveFixThreshFactor);
    newFunctional->SetJacobianConstraintWeight(
        this->m_JacobianConstraintWeight);
...
void ElasticRegistration::EnterResolution(CoordinateVector::SmartPtr &v,
  ...
  SmartPointer<VoxelMatchingElasticFunctional> elasticFunctional =
      VoxelMatchingElasticFunctional::SmartPtr::DynamicCastFrom(functional);
  if (elasticFunctional) {
    elasticFunctional->SetWarpXform(warpXform);

    if (this->m_RelaxToUnfold) warpXform->RelaxToUnfold();
    ...
    SmartPointer<SymmetricElasticFunctional> symmetricFunctional =
        SymmetricElasticFunctional::SmartPtr::DynamicCastFrom(functional);
    if (symmetricFunctional) {
      ...
      symmetricFunctional->SetRigidityConstraintWeight(
          effRigidityConstraintWeight);
      ...
      // badly wrong.
      StdErr << "Fatal coding error: Non-elastic functional in "
...
int ElasticRegistration::DoneResolution(CoordinateVector::SmartPtr &v,
                                        Functional::SmartPtr &functional,
                                        const int idx, const int total) {
...
const UniformVolume::SmartPtr ElasticRegistration::GetReformattedFloatingImage(
