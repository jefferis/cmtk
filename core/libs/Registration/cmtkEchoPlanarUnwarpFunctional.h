 ...
 *\see D. Holland, J. M. Kuperman, and A. M. Dale, "Efficient correction of
 *inhomogeneous static magnetic field-induced distortion in Echo Planar
 *Imaging," NeuroImage, vol. 50, no. 1, pp. 175-183, 2010.
 */
class EchoPlanarUnwarpFunctional {
 public:
  ...
  EchoPlanarUnwarpFunctional(
      UniformVolume::SmartConstPtr &imageFwd /*!< "Forward direction" EPI */,
      UniformVolume::SmartConstPtr &imageRev /*!< "Reverse" direction EPI */, const byte phaseEncodeDirection /*!< Phase encoding direction (image coordinate axis) */, const bool initShiftCentersOfMass = false /*!< When set, initialize by shifting the centers of mass of each row to match */);

  /** Set image smoothing kernel width and create smoothed images.
   *\note Smoothing is applied only in phase encoding direction because the
   *deformation field is restricted to this direction, and the optimization can
   *thus not get "caught" in local minima in the other two directions.
   */
  ...
  UniformVolume::SmartPtr GetCorrectedImage(
      const int
          direction /*!< Select forward (+1) vs. reverse encoding (-1) image */)
      const;

  /// Return either first or second Jacobian intensity correction factor map.
  UniformVolume::SmartPtr GetJacobianMap(
      const int
          direction /*!< Select forward (+1) vs. reverse encoding (-1) map */)
      const;
  ...
  void Optimize(const int numberOfIterations,
                const Units::GaussianSigma &smoothMax,
   ...
   * This is based on the log() of the local 1D Jacobians of the deformation
   * field.
   */
 ...
 private:
  ...
  /** Readout direction.
   * This is a bit misnamed - really this is the coordinate dimension, excluding
   * the phase encoding direction, with the maximum number of pixels. The
   * purpose of this is for a maximally efficient partition for SMP computation.
   * So while the name may not always fit the determined value, and there may be
   * cases when this is selected as the slice direction, it does serve its
   * purpose.
   */
  ...
  void MakeGradientImage(
      const ap::real_1d_array &x /*!< Current deformation parameter vector.*/,
      const int direction /*!< +1 = forward image, -1 = reverse image */,
      const UniformVolume &sourceImage /*!< Image to compute gradient for.*/, std::vector<Types::DataItem> &gradientImageData /*!< Reference to data array for computed gradient data.*/);

  /// Compute deformed image.
  void ComputeDeformedImage(const ap::real_1d_array
                                &x /*!< Current deformation parameter vector.*/,
                            int direction /*!< Deformation direction - 1
                                             computes unwarped "forward" image,
                                             -1 computed unwarped "reverse"
                                             image.*/
                            ,
                            const UniformVolume
                                &sourceImage /*!< Undeformed input image.*/,
                            std::vector<Types::DataItem>
                                &targetUnwarpData /*!< Reference to deformed
                                                     output image data.*/
                            ,
                            std::
                                vector<Types::DataItem> &targetCorrectedData /*!< Reference to deformed and intensity-corrected output image data.*/);

  /// 1D sinc interpolation
  Types::DataItem Interpolate1D(
      const UniformVolume &sourceImage /*!< Image to interpolate from */, const FixedVector<3, int> &baseIdx /*!< Grid base index - this is the grid cell where the interpolation kernel is anchored. */, const Types::Coordinate relative /*!< Relative position of interpolation location in grid cell, relative to phase encoding direction. */)
      const;

  /** Get partial image deformation Jacobian.
   *\return The forward image Jacobian can be computed from this as Jfwd =
   *1+Jpartial, the reverse Jacobian as Jrev = 1-Jpartial.
   */
  Types::Coordinate GetPartialJacobian(
      const ap::real_1d_array &x /*!< Current deformation parameter vector.*/, const FixedVector<
                                                                                   3,
                                                                                   int>
                                                                                   &baseIdx /*!< Grid base index - this is the grid cell where the differential operator stencil is anchored. */)
      const;
