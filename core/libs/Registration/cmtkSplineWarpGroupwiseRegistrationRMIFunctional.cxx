...
void SplineWarpGroupwiseRegistrationRMIFunctional ::
  ...
  for (size_t cp = beginCP; cp < endCP; ++cp) {
    ...
    std::vector<DataGrid::RegionType>::const_iterator voi =
        this->m_VolumeOfInfluenceArray.begin() + cp;
    for (size_t img = this->m_ActiveImagesFrom; img < this->m_ActiveImagesTo;
         ++img) {
      ...
      for (Types::GridIndexType z = voi->From()[2]; z < voi->To()[2]; ++z) {
        for (Types::GridIndexType y = voi->From()[1]; y < voi->To()[1]; ++y) {
          ...
          for (Types::GridIndexType x = voi->From()[0]; x < voi->To()[0];
               ++x, ++ofs) {
      ...
      this->m_InformationByControlPoint[cp] = std::max(
          (byte)(voiMax - voiMin), this->m_InformationByControlPoint[cp]);
    }
  }
        ...
        for (int k = z; k < xform0->m_Dims[2]; k += 4) {
          for (int j = y; j < xform0->m_Dims[1]; j += 4) {
            for (int i = x; i < xform0->m_Dims[0]; i += 4, ++ofs) {
              this->m_ControlPointSchedule[ofs] =
                  i + xform0->m_Dims[0] * (j + xform0->m_Dims[1] * k);
            }
          }
        }
      }
...
void SplineWarpGroupwiseRegistrationRMIFunctional::UpdateActiveControlPoints() {
  ...
  if (this->m_DeactivateUninformativeMode) {
    ...
    for (size_t cp = 0; cp < numberOfControlPoints; ++cp, ++voi) {
      if (this->m_ActiveControlPointFlags[cp]) {
        this->m_ActiveControlPointFlags[cp] =
            (this->m_InformationByControlPoint[cp] >
             (this->m_HistogramBins / 4));
      }
