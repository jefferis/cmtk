...
TypedArraySimilarity::ReturnType TypedArraySimilarity::GetMutualInformation(
  ...
  if (memory) {
    histogram = JointHistogram<unsigned int>::SmartPtr(
        memory->CreateHistogram(array0, array1));
  } else {
    size_t numBins = std::max<unsigned>(
        std::min<unsigned>(static_cast<unsigned>(sqrt((float)dataSize)), 128),
        8);

    histogram = JointHistogram<unsigned int>::SmartPtr(
        new JointHistogram<unsigned int>(numBins, numBins));
  ...
  return static_cast<TypedArraySimilarity::ReturnType>(
      histogram->GetMutualInformation(false));
}

TypedArraySimilarity::ReturnType TypedArraySimilarity::GetCorrelationRatio(
    const TypedArray *array0, const TypedArray *array1) {
  ...
  unsigned int numBins = std::max<unsigned>(
      std::min<unsigned>(static_cast<unsigned>(sqrt((float)dataSize)), 128), 8);
  ...
  for (unsigned int j = 0; j < numBins; ++j) {
    ...
    if (histogram[j]) {
      ...
      double sigmaSq =
          (mu * mu * histogram[j] - 2.0 * mu * sumJ[j] + sumSquareJ[j]) /
          histogram[j];
...
TypedArraySimilarity::GetNormalizedMutualInformation(
  ...
  return static_cast<TypedArraySimilarity::ReturnType>(
      histogram->GetMutualInformation(true));
}

TypedArraySimilarity::ReturnType
TypedArraySimilarity::GetMinusMeanSquaredDifference(const TypedArray *array0,
  ...
  if (!countPixels)
    ...
    return static_cast<TypedArraySimilarity::ReturnType>(
        -(sumOfSquares / (float)countPixels));
}

TypedArraySimilarity::ReturnType
TypedArraySimilarity::GetPeakSignalToNoiseRatio(const TypedArray *data,
                                                const TypedArray *signal) {
  return -10.0 *
         log(-GetMinusMeanSquaredDifference(data, signal) /
             signal->GetRange().Width()) /
         log(10.0);
}

TypedArraySimilarity::ReturnType TypedArraySimilarity::GetCrossCorrelation(
    const TypedArray *array0, const TypedArray *array1) {
  if (!CheckArrayDimensions(array0, array1))
    return std::numeric_limits<float>::signaling_NaN();
...
TypedArray::SmartPtr TypedArraySimilarity::GetDifferenceArray(
    const TypedArray *array0, const TypedArray *array1,
    Types::DataItem &scaleFactor) {
  const size_t numberOfPixels = array0->GetDataSize();

  TypedArray::SmartPtr differenceArray =
      TypedArray::Create(GetSignedDataType(array0->GetType()), numberOfPixels);
...
TypedArraySimilarity::ReturnType
TypedArraySimilarity::GetDifferenceArrayEntropy(const TypedArray *array0,
                                                const TypedArray *array1,
                                                Types::DataItem &scaleFactor) {
  TypedArray::SmartPtr differenceArray(
...
bool TypedArraySimilarity::CheckArrayDimensions(const TypedArray *array0,
                                                const TypedArray *array1) {
