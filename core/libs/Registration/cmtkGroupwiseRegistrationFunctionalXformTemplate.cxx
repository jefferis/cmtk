...
GroupwiseRegistrationFunctionalXformTemplate<
    TXform>::GroupwiseRegistrationFunctionalXformTemplate() {
  // allocate storage for four times me parallel tasks than threads in the
  // global thread pool.
  this->m_InterpolateTaskInfo.resize(
      4 * ThreadPool::GetGlobalThreadPool().GetNumberOfThreads());
}

template <class TXform>
void GroupwiseRegistrationFunctionalXformTemplate<TXform>::InterpolateImage(
  ...
  if (this->m_ProbabilisticSamples.size())
    threadPool.Run(InterpolateImageProbabilisticThread,
                   this->m_InterpolateTaskInfo);
  else
  ...
  for (size_t task = 0; task < this->m_InterpolateTaskInfo.size(); ++task) {
    numberOfOutsidePixels +=
        this->m_InterpolateTaskInfo[task].m_NumberOfOutsidePixels;
  }

  // Check whether more than defined proportion threshold was outside
  if (numberOfOutsidePixels >
      static_cast<size_t>(this->m_MaxRelativeNumberOutsidePixels *
                          this->m_TemplateNumberOfSamples)) {
...
void GroupwiseRegistrationFunctionalXformTemplate<
    TXform>::InterpolateImageThread(void *const args, const size_t taskIdx,
...
template <class TXform>
void GroupwiseRegistrationFunctionalXformTemplate<
    TXform>::InterpolateImageProbabilisticThread(void *const args,
