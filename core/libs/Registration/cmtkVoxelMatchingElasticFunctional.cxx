...
VoxelMatchingElasticFunctional::VoxelMatchingElasticFunctional(
  ...
  this->m_ReferenceDomain = UniformVolume::CoordinateRegionType(
      UniformVolume::CoordinateVectorType(0.0), reference->m_Size);
...
template <class W>
void VoxelMatchingElasticFunctional_WarpTemplate<W>::WeightedDerivative(
    double &lower, double &upper, W &warp, const int param,
    const Types::Coordinate step) const {
  if (this->m_JacobianConstraintWeight > 0) {
    double lowerConstraint = 0, upperConstraint = 0;
    warp.GetJacobianConstraintDerivative(lowerConstraint, upperConstraint,
                                         param, VolumeOfInfluence[param], step);
  ...
  if (this->m_RigidityConstraintWeight > 0) {
    ...
    if (this->m_RigidityConstraintMap) {
      warp.GetRigidityConstraintDerivative(lowerConstraint, upperConstraint,
                                           param, VolumeOfInfluence[param],
                                           step, this->m_RigidityConstraintMap);
    } else {
      warp.GetRigidityConstraintDerivative(lowerConstraint, upperConstraint,
                                           param, VolumeOfInfluence[param],
                                           step);
  ...
  if (!finite(upper) || !finite(lower)) {
    ...
    if (this->m_LandmarkPairs) {
      ...
      warp.GetDerivativeLandmarksMSD(lowerMSD, upperMSD,
                                     *(this->m_LandmarkPairs), param, step);
    ...
    if (InverseTransformation) {
      ...
      warp.GetDerivativeInverseConsistencyError(
          lowerIC, upperIC, this->InverseTransformation, this->ReferenceGrid,
          &(this->VolumeOfInfluence[param]), param, step);
...
template <class W>
void VoxelMatchingElasticFunctional_WarpTemplate<W>::SetWarpXform(
  ...
  if (Warp) {
    ...
    if (Dim != Warp->VariableParamVectorDim()) {
      if (VolumeOfInfluence) Memory::ArrayC::Delete(VolumeOfInfluence);
      Dim = Warp->VariableParamVectorDim();
      this->StepScaleVector.resize(Dim);
      this->VolumeOfInfluence =
          Memory::ArrayC::Allocate<DataGrid::RegionType>(Dim);
    }
    ...
    for (size_t dim = 0; dim < Dim; ++dim, ++VOIptr) {
      ...
      *VOIptr = this->GetReferenceGridRange(
          Warp->GetVolumeOfInfluence(dim, this->m_ReferenceDomain));
    }
...
template <class VM>
void VoxelMatchingElasticFunctional_Template<VM>::UpdateWarpFixedParameters() {
...
#pragma omp parallel for reduction(+ : inactive)
    for (int ctrl = 0; ctrl < numCtrlPoints; ++ctrl) {
      ...
      const DataGrid::RegionType voi = this->GetReferenceGridRange(
          this->Warp->GetVolumeOfInfluence(3 * ctrl, this->m_ReferenceDomain,
                                           false /* disable fast mode */));

      Types::GridIndexType r =
          voi.From()[0] +
          this->DimsX * (voi.From()[1] + this->DimsY * voi.From()[2]);

      bool active = false;
      for (Types::GridIndexType pZ = voi.From()[2];
           (pZ < voi.To()[2]) && !active; ++pZ) {
        for (Types::GridIndexType pY = voi.From()[1];
             (pY < voi.To()[1]) && !active; ++pY) {
          for (Types::GridIndexType pX = voi.From()[0]; (pX < voi.To()[0]);
               ++pX, ++r) {
            if ((this->Metric->GetSampleX(r) != 0) ||
                ((this->WarpedVolume[r] != unsetY) &&
                 (this->WarpedVolume[r] != 0))) {
              active = true;
              break;
            }
          }
...
#ifdef _OPENMP
    if (this->m_ThreadConsistencyHistograms.size() <
        static_cast<size_t>(omp_get_max_threads())) {
      this->m_ThreadConsistencyHistograms.resize(omp_get_max_threads());

      const Types::GridIndexType numSamplesX =
          this->Metric->DataX.NumberOfSamples;
      const Types::DataItemRange rangeX = this->Metric->DataX.GetValueRange();
      const size_t numBinsX =
          JointHistogramBase::CalcNumBins(numSamplesX, rangeX);

      const Types::GridIndexType numSamplesY =
          this->Metric->DataY.NumberOfSamples;
      const Types::DataItemRange rangeY = this->Metric->DataY.GetValueRange();
      const size_t numBinsY =
          JointHistogramBase::CalcNumBins(numSamplesY, rangeY);

      for (size_t thread = 0;
           thread < static_cast<size_t>(omp_get_max_threads()); ++thread) {
        if (!this->m_ThreadConsistencyHistograms[thread]) {
          this->m_ThreadConsistencyHistograms[thread] =
              JointHistogram<Types::GridIndexType>::SmartPtr(
                  new JointHistogram<Types::GridIndexType>());

          this->m_ThreadConsistencyHistograms[thread]->Resize(numBinsX,
                                                              numBinsY);
...
#pragma omp parallel for
    for (int ctrl = 0; ctrl < numCtrlPoints; ++ctrl) {
#ifdef _OPENMP
      JointHistogram<Types::GridIndexType> &threadHistogram =
          *(this->m_ThreadConsistencyHistograms[omp_get_thread_num()]);
#else
      JointHistogram<Types::GridIndexType> &threadHistogram =
          *(this->m_ConsistencyHistogram);
#endif
      ...
      const Types::GridIndexType endOfLine =
          (voi.From()[0] + (this->DimsX - voi.To()[0]));
      const Types::GridIndexType endOfPlane =
        ...
        for (Types::GridIndexType pY = voi.From()[1]; pY < voi.To()[1]; ++pY) {
          for (Types::GridIndexType pX = voi.From()[0]; pX < voi.To()[0];
            ...
            if (this->WarpedVolume[r] != unsetY) {
              threadHistogram.Increment(
                  threadHistogram.ValueToBinX(this->Metric->GetSampleX(r)),
                  threadHistogram.ValueToBinY(this->WarpedVolume[r]));
            }
          }
          r += endOfLine;
        }
    ...
    const double refThresh =
        refMin + this->m_AdaptiveFixThreshFactor * (refMax - refMin);
    const double modThresh =
        modMin + this->m_AdaptiveFixThreshFactor * (modMax - modMin);
...
VoxelMatchingElasticFunctional *CreateElasticFunctional(
  ...
  switch (fltVolume->GetData()->GetDataClass()) {
    ...
    case DATACLASS_GREY:
      switch (metric) {
        case 0:
          return new VoxelMatchingElasticFunctional_Template<
...
template class VoxelMatchingElasticFunctional_Template<
