...
void SplineWarpCongealingFunctional ::SetTemplateGrid(
    UniformVolume::SmartPtr &templateGrid,
    const Types::GridIndexType downsample, const bool useTemplateData) {
...
void SplineWarpCongealingFunctional ::InitializeXformsFromAffine(
    const Types::Coordinate gridSpacing,
    std::vector<AffineXform::SmartPtr> initialAffineXformsVector,
    const bool exactSpacing) {
  this->Superclass::InitializeXformsFromAffine(
      gridSpacing, initialAffineXformsVector, exactSpacing);

  // clear thread storage because we need to re-initialize these.
  this->m_StaticThreadStorage.resize(0);
}

void SplineWarpCongealingFunctional ::RefineTransformationGrids() {
  this->Superclass::RefineTransformationGrids();
  // clear thread storage because we need to re-initialize these.
  this->m_StaticThreadStorage.resize(0);
}

void SplineWarpCongealingFunctional ::UpdateStandardDeviationByPixel() {
  this->Superclass::UpdateStandardDeviationByPixel();
  this->UpdateActiveControlPoints();
}

void SplineWarpCongealingFunctional ::UpdateActiveControlPoints() {
  ...
  if (this->m_DeactivateUninformativeMode) {
    ...
    const Vector3D templateTo(this->m_TemplateGrid->m_Offset +
                              this->m_TemplateGrid->m_Size);
    Vector3D fromVOI, toVOI;

    std::vector<DataGrid::RegionType>::const_iterator voi =
        this->m_VolumeOfInfluenceArray.begin();
    for (size_t cp = 0; cp < numberOfControlPoints; ++cp, ++voi) {
      bool active = false;
      if (this->m_ActiveControlPointFlags[cp]) {
        for (Types::GridIndexType z = voi->From()[2];
             (z < voi->To()[2]) && !active; ++z) {
          for (Types::GridIndexType y = voi->From()[1];
               (y < voi->To()[1]) && !active; ++y) {
            size_t ofs =
                this->m_TemplateGrid->GetOffsetFromIndex(voi->From()[0], y, z);
            for (Types::GridIndexType x = voi->From()[0];
                 (x < voi->To()[0]) && !active; ++x, ++ofs) {
    ...
    DebugOutput(2) << "Enabled " << this->m_NumberOfActiveControlPoints << "/"
                   << this->m_ParametersPerXform / 3
                   << " control points as informative.\n";
  }
...
SplineWarpCongealingFunctional ::Evaluate() {
    ...
    if (this->m_JacobianConstraintWeight > 0) {
      for (size_t i = 0; i < this->m_XformVector.size(); ++i) {
        const SplineWarpXform *splineWarp =
            dynamic_cast<const SplineWarpXform *>(
                this->m_XformVector[i].GetPtr());
        if (!splineWarp) {
          StdErr << "ERROR: dynamic_cast to SplineWarpXform failed in "
                    "SplineWarpCongealingFunctional::Evaluate()\n";
...
void SplineWarpCongealingFunctional ::EvaluateThread(void *args,
