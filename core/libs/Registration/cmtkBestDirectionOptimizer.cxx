...
CallbackResult BestDirectionOptimizer::Optimize(
  ...
  for (int stepIdx = 0; (stepIdx < numOfSteps) && (irq == CALLBACK_OK);
    ...
    while (update && (irq == CALLBACK_OK)) {
      ...
      Self::ReturnType current =
          this->EvaluateWithGradient(v, directionVector, step);
      irq = this->CallbackExecuteWithData(v, current);

      const Self::ReturnType previous = current;

      // Daniel Rueckert is supposedly using Euclid's norm here, but we found
      // this to be less efficient AND accurate. Makes no sense anyway.
      const Self::ParameterType vectorLength =
          (this->m_UseMaxNorm) ? directionVector.MaxNorm()
                               : directionVector.EuclidNorm();
      if (vectorLength > 0) {
...
#pragma omp parallel for
          for (int idx = 0; idx < Dim; ++idx)
            if (fabs(directionVector[idx]) >
                (vectorLength * this->m_DirectionThreshold)) {
              directionVector[idx] *= (stepLength * this->GetParamStep(idx));
            } else {
        ...
        while (next > current) {
          if ((irq = this->CallbackExecute()) != CALLBACK_OK) break;
          current = next;
          update = true;
      ...
      if (update) this->m_Functional->SnapshotAt(v);
#endif

      if ((fabs(previous - current) / (fabs(previous) + fabs(current))) <
