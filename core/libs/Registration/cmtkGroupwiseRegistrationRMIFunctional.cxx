...
GroupwiseRegistrationRMIFunctional<TXform>::GroupwiseRegistrationRMIFunctional()
    : m_TotalNumberOfSamples(0) {
  this->SetNumberOfHistogramBins(255);
}

template <class TXform>
void GroupwiseRegistrationRMIFunctional<TXform>::SetTemplateGrid(
...
GroupwiseRegistrationRMIFunctional<TXform>::Evaluate() {
  ...
  std::fill(this->m_SumOfProductsMatrix.begin(),
            this->m_SumOfProductsMatrix.end(), 0);
  ...
  return this->GetMetric(this->m_SumOfProductsMatrix, this->m_SumsVector,
...
template <class TXform>
typename GroupwiseRegistrationRMIFunctional<TXform>::ReturnType
GroupwiseRegistrationRMIFunctional<TXform>::GetMetric(
    ...
    typename GroupwiseRegistrationRMIFunctional<TXform>::CovarianceMatrixType
  ...
  for (size_t j = 0; j < numberOfImages; ++j) {
    for (size_t i = 0; i <= j; ++i, ++midx) {
      covarianceMatrix(i, j) =
          (sumOfProductsMatrix[midx] -
           ((1.0 * sumsVector[i] * sumsVector[j]) / totalNumberOfSamples)) /
          totalNumberOfSamples;
    }
  }

  const std::vector<typename Self::ReturnType> eigenvalues =
      EigenValuesSymmetricMatrix<typename Self::ReturnType>(covarianceMatrix)
          .GetEigenvalues();

  const typename Self::ReturnType EIGENVALUE_THRESHOLD = 1e-6;
  typename Self::ReturnType determinant = 1.0;
  for (size_t i = 0; i < numberOfImages; ++i) {
    if (eigenvalues[i] > EIGENVALUE_THRESHOLD) determinant *= eigenvalues[i];
  }

  if (determinant > 0) {
    const static double alpha = 1.41893853320467;
    typename Self::ReturnType metric =
        numberOfImages * alpha + .5 * log(determinant);
    return -metric;
  } else {
    return -FLT_MAX;
  }
...
template <class TXform>
void GroupwiseRegistrationRMIFunctional<TXform>::EvaluateThread(
  ...
  SumsAndProductsVectorType &sumOfProductsMatrix =
      This->m_ThreadSumOfProductsMatrix[threadIdx];
...
template <class TXform>
void GroupwiseRegistrationRMIFunctional<TXform>::EvaluateProbabilisticThread(
    void *const args, const size_t taskIdx, const size_t taskCnt,
    const size_t threadIdx, const size_t) {
  EvaluateThreadParameters *threadParameters =
      static_cast<EvaluateThreadParameters *>(args);
...
template <class TXform>
typename GroupwiseRegistrationRMIFunctional<TXform>::ReturnType
GroupwiseRegistrationRMIFunctional<TXform>::EvaluateWithGradient(
...
template <class TXform>
bool GroupwiseRegistrationRMIFunctional<TXform>::Wiggle() {
