...
CallbackResult ImagePairNonrigidRegistration::InitRegistration() {
  ...
  Types::Coordinate currSampling = std::max(
      this->m_Sampling, 2 * std::min(this->m_ReferenceVolume->GetMinDelta(),
                                     this->m_FloatingVolume->GetMinDelta()));
  ...
  if (InitialWarpXform) {
    ...
    SplineWarpXform::SmartPtr warpXform(this->MakeWarpXform(
        this->m_ReferenceVolume->m_Size, this->m_InitialTransformation));

    if (this->m_InverseConsistencyWeight > 0)
      InverseWarpXform = SplineWarpXform::SmartPtr(
          this->MakeWarpXform(this->m_FloatingVolume->m_Size,
                              this->m_InitialTransformation->GetInverse()));
  ...
  if (this->m_MaxStepSize <= 0) {
    const SplineWarpXform *warp =
        SplineWarpXform::SmartPtr::DynamicCastFrom(this->m_Xform);
    this->m_MaxStepSize =
        0.25 * std::max(warp->m_Spacing[0],
                        std::max(warp->m_Spacing[1], warp->m_Spacing[2]));
  }
  ...
  if (this->m_UseOriginalData) {
    this->m_ParameterStack.push(
        Self::LevelParameters::SmartPtr(new Self::LevelParameters(-1)));
  }

  for (; (currSampling <= this->m_CoarsestResolution); currSampling *= 2) {
    this->m_ParameterStack.push(Self::LevelParameters::SmartPtr(
        new Self::LevelParameters(currSampling)));
  }

  switch (this->m_Algorithm) {
    case 0:
      this->m_Optimizer = Optimizer::SmartPtr(
          new BestNeighbourOptimizer(this->m_OptimizerStepFactor));
      break;
    case 1:
    case 2:
    ...
    case 3: {
      BestDirectionOptimizer *optimizer =
          new BestDirectionOptimizer(this->m_OptimizerStepFactor);
...
SplineWarpXform::SmartPtr ImagePairNonrigidRegistration::MakeWarpXform(
    const UniformVolume::CoordinateVectorType &size,
    const AffineXform *initialAffine) const {
  SplineWarpXform::SmartPtr warpXform(new SplineWarpXform(
      size, this->m_GridSpacing, initialAffine, this->m_ExactGridSpacing));
...
Functional *ImagePairNonrigidRegistration::MakeFunctional(
    const int level, const Superclass::LevelParameters *parameters) {
  const Self::LevelParameters *levelParameters =
      dynamic_cast<const Self::LevelParameters *>(parameters);
  if (!levelParameters) {
  ...
  if (!level && this->m_MatchFltToRefHistogram) {
    floatingVolume =
        UniformVolume::SmartPtr(floatingVolume->Clone(true /*copyData*/));
    floatingVolume->GetData()->ApplyFunctionObject(
        TypedArrayFunctionHistogramMatching(*(floatingVolume->GetData()),
                                            *(referenceVolume->GetData())));
  } else {
    if (this->m_RepeatMatchFltToRefHistogram) {
      floatingVolume =
          UniformVolume::SmartPtr(floatingVolume->Clone(true /*copyData*/));
      UniformVolume::SmartPtr reformat(
          this->GetReformattedFloatingImage(Interpolators::NEAREST_NEIGHBOR));
      floatingVolume->GetData()->ApplyFunctionObject(
          TypedArrayFunctionHistogramMatching(*(reformat->GetData()),
                                              *(referenceVolume->GetData())));
    }
  }
  ...
  if (levelParameters->m_Resolution > 0) {
    ...
    referenceVolume = UniformVolume::SmartPtr(
        referenceVolume->GetResampled(levelParameters->m_Resolution));
    floatingVolume = UniformVolume::SmartPtr(
        floatingVolume->GetResampled(levelParameters->m_Resolution));
  }

  if (this->m_InverseConsistencyWeight > 0) {
    ImagePairSymmetricNonrigidRegistrationFunctional *newFunctional =
        ImagePairSymmetricNonrigidRegistrationFunctional::Create(
            this->m_Metric, referenceVolume, floatingVolume,
            this->m_FloatingImageInterpolation);
    newFunctional->SetInverseConsistencyWeight(
    ...
    newFunctional->SetJacobianConstraintWeight(
    ...
    newFunctional->SetJacobianConstraintWeight(
        this->m_JacobianConstraintWeight);
    newFunctional->SetForceOutside(this->m_ForceOutsideFlag,
                                   this->m_ForceOutsideValue);
    newFunctional->SetGridEnergyWeight(this->m_GridEnergyWeight);
    return newFunctional;
  }
}
...
void ImagePairNonrigidRegistration::EnterResolution(
  ...
  SplineWarpXform::SmartPtr warpXform =
      SplineWarpXform::SmartPtr::DynamicCastFrom(this->m_Xform);

  // handle simple nonrigid functional
  SmartPointer<ImagePairNonrigidRegistrationFunctional> nonrigidFunctional =
      ImagePairNonrigidRegistrationFunctional::SmartPtr::DynamicCastFrom(
          functional);
  if (nonrigidFunctional) {
    nonrigidFunctional->SetWarpXform(warpXform);

    if (this->m_RelaxToUnfold)  // has to come after functional->SetWarpXform so
                                // reference volume is registered with warp grid
      warpXform->RelaxToUnfold();

    nonrigidFunctional->SetGridEnergyWeight(effGridEnergyWeight);
    nonrigidFunctional->SetJacobianConstraintWeight(
        effJacobianConstraintWeight);
  } else {
    // handle inverse-consistent nonrigid functional
    SmartPointer<ImagePairSymmetricNonrigidRegistrationFunctional>
        symmetricFunctional = ImagePairSymmetricNonrigidRegistrationFunctional::
            SmartPtr::DynamicCastFrom(functional);
    if (symmetricFunctional) {
      symmetricFunctional->SetWarpXform(warpXform, this->InverseWarpXform);

      if (this->m_RelaxToUnfold)  // has to come after functional->SetWarpXform
                                  // so reference volume is registered with warp
                                  // grid
      {
        warpXform->RelaxToUnfold();
      ...
      symmetricFunctional->SetJacobianConstraintWeight(
          effJacobianConstraintWeight);
      symmetricFunctional->SetInverseConsistencyWeight(
          effInverseConsistencyWeight);
    } else {
      // neither simple nor inverse-consistent functional: something went
      // badly wrong.
      StdErr << "Fatal coding error: Non-nonrigid functional in "
...
int ImagePairNonrigidRegistration::DoneResolution(
    CoordinateVector::SmartPtr &v, Functional::SmartPtr &functional,
    const int idx, const int total) {
  ...
  if ((RefinedGridAtLevel != idx) || (idx == total)) {
    if (RefineGridCount < this->m_RefineGrid) {
      if ((!this->m_DelayRefineGrid) || RefineDelayed || (idx == total)) {
        ...
        if (warpXform) {
          warpXform->Refine();
          if (InverseWarpXform) InverseWarpXform->Refine();
          ++RefineGridCount;
...
ImagePairNonrigidRegistration::GetReformattedFloatingImage(
    Interpolators::InterpolationEnum interpolator) const {
