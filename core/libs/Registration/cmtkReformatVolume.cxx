...
void ReformatVolume::SetReferenceVolume(
    const UniformVolume::SmartConstPtr &referenceVolume) {
  this->ReferenceVolume = referenceVolume;
}

void ReformatVolume::SetFloatingVolume(
    const UniformVolume::SmartConstPtr &floatingVolume) {
  FloatingVolume = floatingVolume;
}

void ReformatVolume::SetAffineXform(const AffineXform::SmartPtr &affineXform) {
  this->m_AffineXform = affineXform;
}

void ReformatVolume::SetWarpXform(const WarpXform::SmartPtr &warpXform) {
  this->m_WarpXform = warpXform;
}

const UniformVolume::SmartPtr ReformatVolume::MakeTargetVolume() const {
  return UniformVolume::SmartPtr(ReferenceVolume->CloneGrid());
}

const UniformVolume::SmartPtr ReformatVolume::PlainReformat() {
  UniformVolume::SmartPtr targetVolume = this->MakeTargetVolume();

  if (targetVolume) {
    Progress::Begin(0, targetVolume->GetDims()[AXIS_Z], 1,
                    "Volume reformatting");

    TypedArray::SmartPtr targetData(
        TypedArray::Create(FloatingVolume->GetData()->GetType(),
                           targetVolume->GetNumberOfPixels()));
    if (this->m_UsePaddingValue) {
      ...
      if (FloatingVolume->GetData()->GetPaddingFlag()) {
        targetData->SetPaddingValue(
            FloatingVolume->GetData()->GetPaddingValue());
      }
    }

    UniformVolumeInterpolatorBase::SmartPtr interpolator(
        this->CreateInterpolator(this->FloatingVolume));
    Vector3D pFlt;
    ...
    for (int pZ = 0; pZ < dims[2]; ++pZ) {
      ...
      const SplineWarpXform::SmartConstPtr &splineWarp =
          SplineWarpXform::SmartConstPtr::DynamicCastFrom(this->m_WarpXform);
      if (splineWarp) {
        SplineWarpXformUniformVolume xformVolume(*(this->ReferenceVolume),
                                                 splineWarp);
        ...
        for (int pY = 0; pY < dims[1]; ++pY) {
          for (int pX = 0; pX < dims[0]; ++pX, ++offset) {
            pFlt = this->m_AffineXform->Apply(
                ReferenceVolume->GetGridLocation(pX, pY, pZ));
...
TypedArray::SmartPtr ReformatVolume::PlainReformat(const int plane,
                                                   TypedArray::SmartPtr &target,
  ...
  if (!result) {
    ...
    if (this->m_UsePaddingValue) result->SetPaddingValue(this->m_PaddingValue);
  }
  ...
  const SplineWarpXform::SmartConstPtr splineWarp =
      SplineWarpXform::SmartConstPtr::DynamicCastFrom(this->m_WarpXform);
  if (splineWarp) {
    const SplineWarpXformUniformVolume xformVolume(*(this->ReferenceVolume),
                                                   splineWarp);
    ...
    for (int pY = 0; pY < DimsY; ++pY) {
      for (int pX = 0; pX < DimsX; ++pX, ++offset) {
        pMod = this->m_AffineXform->Apply(
            ReferenceVolume->GetGridLocation(pX, pY, plane));
...
ReformatVolume::GetTransformedReferenceGrey(void *const arg) {
  GetTransformedReferenceTP *params =
      static_cast<GetTransformedReferenceTP *>(arg);

  TypedArray::SmartPtr dataArray = params->dataArray;
  const SplineWarpXform *splineXform = params->splineXform;
  const UniformVolumeInterpolatorBase *interpolator =
      params->referenceInterpolator;
        ...
        if (toIdx && success) {
          ...
          for (unsigned int idx = 0; idx < toIdx; ++idx) {
            for (unsigned int corner = 0; corner < 8; ++corner) {
              labelCount[static_cast<int>(probe[idx].Values[corner])] +=
                  probe[idx].GetWeight(corner);
            }
          }
...
UniformVolume *ReformatVolume::GetTransformedReferenceJacobianAvg(
    const std::vector<SplineWarpXform::SmartPtr> *xformList,
    Types::Coordinate *const volumeOffset, const bool includeReferenceData) {
  const SplineWarpXform *splineXform =
      dynamic_cast<const SplineWarpXform *>(this->m_WarpXform.GetConstPtr());
  if (!splineXform) {
    StdErr << "ERROR: ReformatVolume::GetTransformedReferenceJacobian supports "
              "spline warp only.\n";
    return NULL;
  }
  ...
  UniformVolume *result =
      this->CreateTransformedReference(bbFrom, delta, volumeOffset);

  TypedArray::SmartPtr dataArray(
      TypedArray::Create(TYPE_FLOAT, result->GetNumberOfPixels()));

  if (this->m_UsePaddingValue) dataArray->SetPaddingValue(this->m_PaddingValue);
  ...
  Threads::RunThreads(GetTransformedReferenceJacobianAvgThread, numberOfThreads,
                      &params[0]);
...
CMTK_THREAD_RETURN_TYPE
ReformatVolume::GetTransformedReferenceJacobianAvgThread(void *const arg) {
  GetTransformedReferenceTP *params =
      static_cast<GetTransformedReferenceTP *>(arg);
  ...
  const int czFrom =
      params->ThisThreadIndex * dims[2] / params->NumberOfThreads;
  const int czTo =
      std::min<int>(dims[2], (1 + params->ThisThreadIndex) * dims[2] /
                                 params->NumberOfThreads);

  Vector<Types::Coordinate> values(
      params->IncludeReferenceData ? numberOfXforms + 1 : numberOfXforms);
  ...
  for (int cz = czFrom; cz < czTo; ++cz, xyz[2] += delta[2]) {
    ...
    for (int cy = 0; cy < dims[1]; ++cy, xyz[1] += delta[1]) {
      ...
      for (int cx = 0; cx < dims[0]; ++cx, xyz[0] += delta[0], ++offset) {
        ...
        if (success) {
          const Types::Coordinate refJacobian =
              splineXform->GetGlobalScaling() /
              splineXform->GetJacobianDeterminant(u);
          ...
          switch (params->avgMode) {
            case MODE_MEAN: {
              ...
              for (unsigned int img = 0; img < numberOfXforms; ++img)
                sum += xforms[img]->GetJacobianDeterminant(u) /
                       xforms[img]->GetGlobalScaling();
              dataArray->Set(static_cast<Types::DataItem>(refJacobian * sum /
                                                          numberOfXforms),
                             offset);
              break;
            }
            ...
            case MODE_ROBUST90: {
              for (unsigned int img = 0; img < numberOfXforms; ++img)
                values[img] = xforms[img]->GetJacobianDeterminant(u) /
                              xforms[img]->GetGlobalScaling();
              if (params->IncludeReferenceData) values[numberOfXforms] = 1.0;
              ...
              if (params->avgMode == MODE_MEDIAN) {
                ...
                if (numberOfXforms & 1)
                  dataArray->Set(
                      static_cast<Types::DataItem>(
                          refJacobian * values[numberOfXforms / 2 + 1]),
                      offset);
                else
                  dataArray->Set(static_cast<Types::DataItem>(
                                     0.5 * refJacobian *
                                     (values[numberOfXforms / 2] +
                                      values[numberOfXforms / 2 + 1])),
                                 offset);
              } else {
                // robust average of center 90% average
                Types::Coordinate sum = 0;
                for (unsigned int img = margin; img < numberOfXforms - margin;
                     ++img)
                  sum += values[img];
                dataArray->Set(
                    static_cast<Types::DataItem>(refJacobian * sum /
                                                 (numberOfXforms - 2 * margin)),
                    offset);
              }
              break;
            }
...
UniformVolume *ReformatVolume::CreateTransformedReference(
...
UniformVolumeInterpolatorBase::SmartPtr ReformatVolume::CreateInterpolator(
    const cmtk::Interpolators::InterpolationEnum interpolation,
    const UniformVolume::SmartConstPtr &volume) {
  switch (interpolation) {
    default:
    case cmtk::Interpolators::LINEAR: {
      typedef UniformVolumeInterpolator<cmtk::Interpolators::Linear>
          TInterpolator;
      return TInterpolator::SmartPtr(new TInterpolator(*volume));
    }
    case cmtk::Interpolators::NEAREST_NEIGHBOR: {
      typedef UniformVolumeInterpolator<cmtk::Interpolators::NearestNeighbor>
          TInterpolator;
      return TInterpolator::SmartPtr(new TInterpolator(*volume));
    }
    case cmtk::Interpolators::CUBIC: {
      typedef UniformVolumeInterpolator<cmtk::Interpolators::Cubic>
          TInterpolator;
      return TInterpolator::SmartPtr(new TInterpolator(*volume));
    }
    case cmtk::Interpolators::COSINE_SINC: {
      typedef UniformVolumeInterpolator<cmtk::Interpolators::CosineSinc<>>
          TInterpolator;
      return TInterpolator::SmartPtr(new TInterpolator(*volume));
    }
