 ...
 * This class template adds to its base class all basic functionality that
 * depends on the coordinate transformation model (affine vs. nonrigid) but does
 * not require implementation by explicit specialization. In other words, this
 * class provides the interface that is common to all transformation models.
 *
 * The next level of derived classes exist in several specialized variants
 * that implement the transformation-dependent interfaces, i.e., member
 * functions that exist only for certain transformation models.
 */
...
class GroupwiseRegistrationFunctionalXformTemplateBase :
 ...
 public:
  ...
  virtual const XformType *GetActiveXformByIndex(const size_t idx) const {
    return XformType::SmartPtr::DynamicCastFrom(
        this->m_XformVector[idx + this->m_ActiveXformsFrom]);
  }
 ...
 protected:
  ...
  /** Threshold for maximum fraction of reformatted pixels from any given image
   * that may be outside FOV. If the number of outside pixels for any one image
   * exceeds this threshold (as a fraction of total number of reformatted
   * pixels) then an exception is thrown.
   */
  float m_MaxRelativeNumberOutsidePixels;

  /** User-defined background value from parent class, transformed to histogram
   * bin index. */
  byte m_PrivateUserBackgroundValue;

 private:
  /// Crop image histograms to get rid of high-intensity low-probability
  /// samples.
