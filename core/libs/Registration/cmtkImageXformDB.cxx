...
cmtk::ImageXformDB ::ImageXformDB(const std::string &dbPath,
  ...
  if (!this->TableExists("images")) {
    this->Exec(
        "CREATE TABLE images(id INTEGER PRIMARY KEY, space INTEGER, "
        "path TEXT)");
  }

  if (!this->TableExists("xforms")) {
    this->Exec(
        "CREATE TABLE xforms(id INTEGER PRIMARY KEY, path TEXT, invertible "
        "INTEGER, level INTEGER, spacefrom INTEGER, spaceto INTEGER)");
  }
}
...
void cmtk::ImageXformDB ::AddImage(const std::string &imagePath,
  ...
  if (imageKey != Self::NOTFOUND) return;

  if ((spacePath == "")) {
    this->Exec("INSERT INTO images (path) VALUES ('" + imagePath + "')");
    this->Exec("UPDATE images SET space=(SELECT id FROM images WHERE path='" +
               imagePath + "') WHERE path='" + imagePath + "'");
  } else {
    ...
    if ((spaceKey == Self::NOTFOUND)) {
      ...
      this->Exec("UPDATE images SET space=(SELECT id FROM images WHERE path='" +
                 spacePath + "') WHERE path='" + spacePath + "'");
      spaceKey = this->FindImageSpaceID(spacePath);
    }

    std::ostringstream sql;
    sql << "INSERT INTO images (space,path) VALUES ( " << spaceKey << ", '"
        << imagePath << "')";
    this->Exec(sql.str());
  }
}
...
bool cmtk::ImageXformDB ::AddImagePairXform(const std::string &xformPath,
  ...
  if (spaceKeyTrg == spaceKeySrc) {
    StdErr << "WARNING - cmtk::ImageXformDB::AddXform - source and target "
              "image of transformation are in the same space; bailing out.\n";
    return false;
  }

  std::ostringstream sql;
  sql << "INSERT INTO xforms (path,invertible,level,spacefrom,spaceto) VALUES "
         "( '"
      << xformPath << "', " << (invertible ? 1 : 0) << ", 0, " << spaceKeySrc
      << ", " << spaceKeyTrg << ")";
  this->Exec(sql.str());
...
bool cmtk::ImageXformDB ::AddRefinedXform(const std::string &xformPath,
                                          ...
                                          const bool initInverse) {
  const std::string sql =
      "SELECT level,spacefrom,spaceto FROM xforms WHERE ( path='" +
      xformInitPath + "' )";
  ...
  if (spacefrom == Self::NOTFOUND || spaceto == Self::NOTFOUND) {
    StdErr << "WARNING - cmtk::ImageXformDB::AddXform - given initializing "
              "transformation has invalid space ID(s). Bailing out.\n";
    return false;
  }

  std::ostringstream sqlAdd;
  sqlAdd << "INSERT INTO xforms (path,invertible,level,spacefrom,spaceto) "
            "VALUES ( '"
         << xformPath << "', " << (invertible ? 1 : 0) << ", " << level << ", ";
  if (initInverse)
...
cmtk::ImageXformDB::PrimaryKeyType cmtk::ImageXformDB ::FindImageSpaceID(
    const std::string &imagePath) const {
  if (imagePath != "") {
    const std::string sql =
        "SELECT space FROM images WHERE path='" + imagePath + "'";

    SQLite::TableType table;
    this->Query(sql, table);

    if (table.size() && table[0].size()) return atoi(table[0][0].c_str());
  }

  return Self::NOTFOUND;
}

const std::vector<std::string> cmtk::ImageXformDB ::GetSpaceImageList(
    const Self::PrimaryKeyType &spaceKey, const bool sortById) {
  ...
  for (size_t i = 0; i < table.size(); ++i) {
    if (table[i].size()) results.push_back(table[i][0]);
  }
...
bool cmtk::ImageXformDB ::FindXform(const std::string &imagePathSrc,
  ...
  if ((spaceKeySrc == Self::NOTFOUND) || (spaceKeyTrg == Self::NOTFOUND)) {
    // if either space is not in the database, then clearly no transformation
    // can reach it
    return false;
  }
  ...
  std::ostringstream sql;
  sql << "SELECT path FROM xforms WHERE ( spacefrom=" << spaceKeySrc
      << " AND spaceto=" << spaceKeyTrg
      << " ) ORDER BY level DESC, invertible ASC";
  ...
  sql.str("");
  sql << "SELECT path FROM xforms WHERE ( spacefrom=" << spaceKeyTrg
      << " AND spaceto=" << spaceKeySrc
      << " ) ORDER BY level DESC, invertible ASC";
...
const std::vector<std::string> cmtk::ImageXformDB ::FindAllXforms(
    const std::string &imagePathSrc, const std::string &imagePathTrg) const {
  ...
  this->Query(sql.str(), table);
  for (size_t i = 0; i < table.size(); ++i) {
    if (table[i].size()) result.push_back(table[i][0]);
  }
  return result;
}
...
int cmtk::ImageXformDB ::FindXformLevel(const std::string &xformPath) const {
  const std::string sql =
      "SELECT level FROM xforms WHERE ( path='" + xformPath + "' )";
