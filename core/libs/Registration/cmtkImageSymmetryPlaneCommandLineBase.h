 ...
 */
class ImageSymmetryPlaneCommandLineBase : private CannotBeCopied {
 public:
  /// Default constructor.
  ImageSymmetryPlaneCommandLineBase();

  /** Run the symmetry plane computation based on provided command line
   *arguments. \return The return code for the symmetry plane program. This
   *should be returned from main() via either "return" or "exit()".
   */
 ...
 protected:
  ...
  virtual ImageSymmetryPlaneFunctionalBase::SmartPtr CreateFunctional(
      UniformVolume::SmartPtr &volume) = 0;

  /// Create functional for volume and value range.
  virtual ImageSymmetryPlaneFunctionalBase::SmartPtr CreateFunctional(
      UniformVolume::SmartPtr &volume, const Types::DataItemRange &range) = 0;
 ...
 private:
  ...
  /// Optional output path for file with subtraction between input and mirrored
  /// input.
