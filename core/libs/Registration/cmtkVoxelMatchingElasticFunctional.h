...
class VoxelMatchingElasticFunctional :
 ...
 protected:
  /// Constructor.
  VoxelMatchingElasticFunctional(UniformVolume::SmartPtr &reference,
...
class VoxelMatchingElasticFunctional_WarpTemplate :
 ...
 public:
  ...
  void SetInverseTransformation(typename W::SmartPtr &inverseTransformation) {
    this->InverseTransformation =
        W::SmartPtr::DynamicCastFrom(inverseTransformation);
  }
 ...
 protected:
  ...
  typename Self::ReturnType WeightedTotal(
      const typename Self::ReturnType metric, const W *warp) const {
    double result = metric;
    if (this->m_JacobianConstraintWeight > 0) {
      result -=
          this->m_JacobianConstraintWeight * warp->GetJacobianConstraint();
    }

    if (this->m_RigidityConstraintWeight > 0) {
      if (this->m_RigidityConstraintMap) {
        result -= this->m_RigidityConstraintWeight *
                  warp->GetRigidityConstraint(this->m_RigidityConstraintMap);
      } else {
        result -=
            this->m_RigidityConstraintWeight * warp->GetRigidityConstraint();
      }
    }
    ...
    if (InverseTransformation) {
      result -= this->InverseConsistencyWeight *
                warp->GetInverseConsistencyError(this->InverseTransformation,
                                                 this->ReferenceGrid);
    }
  ...
  void WeightedDerivative(double &lower, double &upper, W &warp,
 ...
 public:
  ...
  virtual size_t ParamVectorDim() const { return Warp->ParamVectorDim(); }
  ...
  virtual void GetParamVector(CoordinateVector &v) { Warp->GetParamVector(v); }
...
class VoxelMatchingElasticFunctional_Template
 ...
 public:
  ...
  /// Superclass.
  typedef VoxelMatchingElasticFunctional_WarpTemplate<SplineWarpXform>
      Superclass;
   ...
   */
  VoxelMatchingElasticFunctional_Template(UniformVolume::SmartPtr &reference,
                                          UniformVolume::SmartPtr &floating)
      : VoxelMatchingFunctional_Template<VM>(reference, floating),
        VoxelMatchingElasticFunctional_WarpTemplate<SplineWarpXform>(reference,
                                                                     floating),
    ...
    this->ThreadVectorCache =
        Memory::ArrayC::Allocate<Vector3D *>(this->m_NumberOfThreads);
    for (size_t thread = 0; thread < this->m_NumberOfThreads; ++thread)
      this->ThreadVectorCache[thread] =
          Memory::ArrayC::Allocate<Vector3D>(this->ReferenceDims[0]);
  }
  ...
  virtual ~VoxelMatchingElasticFunctional_Template() {
    ...
    if (WarpedVolume) Memory::ArrayC::Delete(WarpedVolume);
  }
   ...
   */
  typename Self::ReturnType EvaluateIncremental(const SplineWarpXform &warp,
                                                VM *const localMetric,
                                                const DataGrid::RegionType &voi,
                                                Vector3D *const vectorCache) {
    Vector3D *pVec;
    Types::GridIndexType pX, pY, pZ, offset, r;
    Types::GridIndexType fltIdx[3];
    Types::Coordinate fltFrac[3];

    Types::GridIndexType endLineIncrement =
        (voi.From()[0] + (this->DimsX - voi.To()[0]));
    Types::GridIndexType endPlaneIncrement =
        this->DimsX * (voi.From()[1] + (this->DimsY - voi.To()[1]));

    const typename VM::Exchange unsetY = this->Metric->DataY.padding();
    *localMetric = *this->Metric;
    r = voi.From()[0] +
        this->DimsX * (voi.From()[1] + this->DimsY * voi.From()[2]);
    for (pZ = voi.From()[2]; pZ < voi.To()[2]; ++pZ) {
      for (pY = voi.From()[1]; pY < voi.To()[1]; ++pY) {
        pVec = vectorCache;
        warp.GetTransformedGridRow(voi.To()[0] - voi.From()[0], pVec,
                                   voi.From()[0], pY, pZ);
        for (pX = voi.From()[0]; pX < voi.To()[0]; ++pX, ++r, ++pVec) {
          // Remove this sample from incremental metric according to "ground
          // warp" image.
          ...
          // Tell us whether the current location is still within the floating
          ...
          *pVec *= this->FloatingInverseDelta;
          if (this->FloatingGrid->FindVoxelByIndex(*pVec, fltIdx, fltFrac)) {
            ...
            offset = fltIdx[0] +
                     this->FltDimsX * (fltIdx[1] + this->FltDimsY * fltIdx[2]);

            // Continue metric computation.
            localMetric->Increment(sampleX,
                                   this->Metric->GetSampleY(offset, fltFrac));
          } else {
  ...
  virtual typename Self::ReturnType EvaluateWithGradient(
    ...
    const size_t numberOfTasks =
        std::min<size_t>(this->m_NumberOfTasks, this->Dim);
  ...
  virtual typename Self::ReturnType Evaluate() {
    ...
    if (!this->WarpedVolume)
      this->WarpedVolume = Memory::ArrayC::Allocate<typename VM::Exchange>(
          this->DimsX * this->DimsY * this->DimsZ);

    const size_t numberOfTasks =
        std::min<size_t>(this->m_NumberOfTasks, this->DimsY * this->DimsZ);
    for (size_t taskIdx = 0; taskIdx < numberOfTasks; ++taskIdx) {
    ...
    ThreadPool::GetGlobalThreadPool().Run(
        EvaluateCompleteThread, this->InfoTaskComplete, numberOfTasks);
 ...
 private:
  ...
  std::vector<JointHistogram<Types::GridIndexType>::SmartPtr>
      m_ThreadConsistencyHistograms;
#endif  // #ifdef _OPENMP
   ...
   */
  static void EvaluateGradientThread(void *arg, const size_t taskIdx,
                                     const size_t taskCnt,
                                     const size_t threadIdx, const size_t) {
    typename Self::EvaluateGradientTaskInfo *info =
        static_cast<typename Self::EvaluateGradientTaskInfo *>(arg);
    ...
    for (size_t dim = taskIdx; dim < me->Dim; dim += taskCnt, voi += taskCnt) {
      if (me->StepScaleVector[dim] <= 0) {
        ...
        const typename Self::ParameterType thisStep =
            info->Step * me->StepScaleVector[dim];

        pOld = p[dim];

        p[dim] += thisStep;
        upper =
            me->EvaluateIncremental(myWarp, threadMetric, *voi, vectorCache);
        p[dim] = pOld - thisStep;
        lower =
            me->EvaluateIncremental(myWarp, threadMetric, *voi, vectorCache);
        ...
        if ((upper > info->BaseValue) || (lower > info->BaseValue)) {
          // strictly mathematically speaking, we should divide here by
          // step*StepScaleVector[dim], but StepScaleVector[idx] is either zero
          // or a constant independent of idx
  ...
  static void EvaluateCompleteThread(void *arg, const size_t taskIdx,
                                     const size_t taskCnt,
                                     const size_t threadIdx, const size_t) {
    typename Self::EvaluateCompleteTaskInfo *info =
        static_cast<typename Self::EvaluateCompleteTaskInfo *>(arg);
    ...
    Types::GridIndexType rowCount = (me->DimsY * me->DimsZ);
    Types::GridIndexType rowFrom = (rowCount / taskCnt) * taskIdx;
    Types::GridIndexType rowTo = (taskIdx == (taskCnt - 1))
                                     ? rowCount
                                     : (rowCount / taskCnt) * (taskIdx + 1);
    Types::GridIndexType rowsToDo = rowTo - rowFrom;

    Types::GridIndexType pYfrom = rowFrom % me->DimsY;
    Types::GridIndexType pZfrom = rowFrom / me->DimsY;

    Types::GridIndexType r = rowFrom * me->DimsX;
    for (pZ = pZfrom; (pZ < me->DimsZ) && rowsToDo; ++pZ) {
      for (pY = pYfrom; (pY < me->DimsY) && rowsToDo;
           pYfrom = 0, ++pY, --rowsToDo) {
        ...
        for (pX = 0; pX < me->DimsX; ++pX, ++r, ++pVec) {
          ...
          *pVec *= me->FloatingInverseDelta;
          if (me->FloatingGrid->FindVoxelByIndex(*pVec, fltIdx, fltFrac)) {
            ...
            const size_t offset =
                fltIdx[0] +
                me->FltDimsX * (fltIdx[1] + me->FltDimsY * fltIdx[2]);
            ...
            if (me->m_ForceOutsideFlag) {
              ...
              threadMetric->Increment(me->Metric->GetSampleX(r),
                                      warpedVolume[r]);
            } else {
 ...
 * template.
 */
VoxelMatchingElasticFunctional *CreateElasticFunctional(
