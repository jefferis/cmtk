...
GroupwiseRegistrationFunctionalXformTemplate<
    SplineWarpXform>::GroupwiseRegistrationFunctionalXformTemplate()
    : m_MaximumNumberOfPixelsVOI(0),
      m_MaximumNumberOfPixelsPerLineVOI(0),
...
void GroupwiseRegistrationFunctionalXformTemplate<
    SplineWarpXform>::SetTemplateGrid(UniformVolume::SmartPtr &templateGrid,
  ...
  if (this->m_XformVector.size()) {
    for (size_t i = 0; i < this->m_XformVector.size(); ++i) {
      dynamic_cast<SplineWarpXform &>(*(this->m_XformVector[i]))
          .RegisterVolume(*(this->m_TemplateGrid));
    }
    this->UpdateVolumesOfInfluence();
  }
}

void GroupwiseRegistrationFunctionalXformTemplate<SplineWarpXform>::
    InitializeXformsFromAffine(
        const Types::Coordinate gridSpacing,
        std::vector<AffineXform::SmartPtr> initialAffineXformsVector,
        const bool exactSpacing) {
  ...
  for (size_t i = 0; i < this->m_ImageVector.size(); ++i) {
    SplineWarpXform::SmartPtr xform(
        new SplineWarpXform(this->m_TemplateGrid->m_Size, gridSpacing,
                            initialAffineXformsVector[i], exactSpacing));
    xform->RegisterVolume(*(this->m_TemplateGrid));
    this->m_XformVector[i] = xform;

    this->m_InitialRotationsVector[i] =
        AffineXform::SmartPtr(initialAffineXformsVector[i]);
...
void GroupwiseRegistrationFunctionalXformTemplate<
    SplineWarpXform>::RefineTransformationGrids() {
  for (size_t i = 0; i < this->m_XformVector.size(); ++i) {
...
void GroupwiseRegistrationFunctionalXformTemplate<
    SplineWarpXform>::UpdateVolumesOfInfluence() {
  const UniformVolume::CoordinateRegionType templateDomain(
      this->m_TemplateGrid->m_Offset,
      this->m_TemplateGrid->m_Offset + this->m_TemplateGrid->m_Size);
  ...
  for (size_t param = 0; param < this->m_ParametersPerXform; param += 3) {
    ...
    voi = this->m_TemplateGrid->GetGridRange(
        xform0.GetVolumeOfInfluence(param, templateDomain));

    this->m_MaximumNumberOfPixelsVOI =
        std::max<size_t>(voi.Size(), this->m_MaximumNumberOfPixelsVOI);
    this->m_MaximumNumberOfPixelsPerLineVOI = std::max<size_t>(
        voi.To()[0] - voi.From()[0], this->m_MaximumNumberOfPixelsPerLineVOI);
  }
}

bool GroupwiseRegistrationFunctionalXformTemplate<
    SplineWarpXform>::UpdateParamStepArray() {
  bool changed = false;

  this->m_ParamStepArray.resize(this->ParamVectorDim());

  if ((this->m_DeactivateUninformativeMode ||
       this->m_DisableControlPointsMask) &&
      (this->m_ActiveControlPointFlags.size() ==
    ...
    for (size_t param = 0; param < this->ParamVectorDim(); ++param) {
      ...
      if (!this->m_ActiveControlPointFlags
               [(param % this->m_ParametersPerXform) / 3]) {
        this->m_ParamStepArray[param] = 0;
      }
      if (pOld != this->m_ParamStepArray[param]) changed = true;
    }
  } else {
    for (size_t param = 0; param < this->ParamVectorDim(); ++param) {
      const Types::Coordinate pOld = this->m_ParamStepArray[param];
...
void GroupwiseRegistrationFunctionalXformTemplate<
    SplineWarpXform>::ForceZeroSumGradient(CoordinateVector &g) const {
  ...
  if (this->m_ForceZeroSumNoAffine) {
    for (size_t xform = 0; xform < numberOfXforms; ++xform) {
      ...
      const AffineXform *affineXform =
          this->m_InitialRotationsVector[xform]->GetInverse();
      if (affineXform) {
#pragma omp parallel for
        for (int param = 0;
             param < static_cast<int>(this->m_ParametersPerXform); param += 3) {
          const FixedVector<3, Types::Coordinate> projected(
              affineXform->RotateScaleShear(
                  FixedVector<3, Types::Coordinate>::FromPointer(gX + param)));
          for (size_t i = 0; i < 3; ++i) gX[param + i] = projected[i];
        }
      }
    }
  }
...
bool GroupwiseRegistrationFunctionalXformTemplate<SplineWarpXform>::Wiggle() {
...
void GroupwiseRegistrationFunctionalXformTemplate<
    SplineWarpXform>::InterpolateImage(const size_t idx,
...
void GroupwiseRegistrationFunctionalXformTemplate<
    SplineWarpXform>::InterpolateImageThread(void *args, const size_t taskIdx,
  ...
  const Types::GridIndexType rowTo = (taskIdx == (taskCnt - 1))
                                         ? rowCount
...
void GroupwiseRegistrationFunctionalXformTemplate<
    SplineWarpXform>::UpdateActiveControlPoints() {
  ...
  if (numberOfControlPoints) {
    ...
    std::fill(this->m_ActiveControlPointFlags.begin(),
              this->m_ActiveControlPointFlags.end(), true);
  ...
  if (this->m_DisableControlPointsMask) {
    ...
    for (size_t cp = 0; cp < numberOfControlPoints; ++cp) {
      const DataGrid::RegionType maskRegion =
          this->m_DisableControlPointsMask->GetGridRange(
              xform0.GetVolumeOfInfluence(3 * cp, templateDomain,
                                          false /*force slow, accurate mode*/));
      for (RegionIndexIterator<DataGrid::RegionType> it(maskRegion);
           it != it.end(); ++it) {
        if (this->m_DisableControlPointsMask->GetDataAt(
                this->m_DisableControlPointsMask->GetOffsetFromIndex(
                    it.Index())) > 0) {
    ...
    DebugOutput(2) << "Disabled " << cntDisabled
                   << " control points due to provided mask.\n";
  }
}
