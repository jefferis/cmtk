...
#include <cmtkAffineGroupwiseAverageImageEntropyFunctional.h>

#include <cmtkMathUtil.h>
#include <cmtkVolumeAxesHash.h>

AffineGroupwiseAverageImageEntropyFunctional::AffineGroupwiseAverageImageEntropyFunctional()
{
...
void
AffineGroupwiseAverageImageEntropyFunctional
...
void
AffineGroupwiseAverageImageEntropyFunctional::InitializeXforms
...
void
AffineGroupwiseAverageImageEntropyFunctional::SetXforms
( const std::vector<AffineXform::SmartPtr>& xformVector )
{
  ...
  for ( size_t i = 0; i < this->m_XformVector.size(); ++i )
    {
    AffineXform::SmartPtr xform( Memory::AllocateArray<AffineXform( *(xformVector>( i )) ) );
    xform->SetNumberDOFs( this->m_XformNumberDOFs );
    xform->SetUseLogScaleFactors( true );
...
void
AffineGroupwiseAverageImageEntropyFunctional::InterpolateImage
( const size_t idx, byte* const destination )
{
  ...
  if ( (m_ProbabilisticSampleDensity > 0) && (m_ProbabilisticSampleDensity < 1) )
...
AffineGroupwiseAverageImageEntropyFunctional::InterpolateImageThread
( void* args )
{
...
CMTK_THREAD_RETURN_TYPE
AffineGroupwiseAverageImageEntropyFunctional::InterpolateImageProbabilisticThread
( void* args )
{
  InterpolateImageThreadParameters* threadParameters = static_cast<InterpolateImageThreadParameters*>( args );
  ...
  const size_t endIdx = ( threadID == numberOfThreads ) ?
    This->m_ProbabilisticSamples.size() :
    (threadID+1) * (This->m_ProbabilisticSamples.size() / numberOfThreads);
