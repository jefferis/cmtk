...
ElasticRegistrationCommandLine ::ElasticRegistrationCommandLine(
  ...
  try {
    ...
    cl.AddOption(Key("initial"), &initialTransformationFile,
    ...
    cl.AddOption(Key('g', "grid-spacing"), &this->m_GridSpacing,
    ...
    cl.AddSwitch(Key('S', "switch"), &this->ForceSwitchVolumes, true,
                 "Switch reference and floating image");
    cl.EndGroup();

    cl.BeginGroup("Regularization", "Regularization parameters");
    cl.AddOption(
        Key("jacobian-weight"), &this->m_JacobianConstraintWeight,
        "Weight for Jacobian-based local volume preservation constraint");
    cl.AddOption(Key("energy-weight"), &this->m_GridEnergyWeight,
                 "Weight for grid bending energy constraint");
    cl.AddOption(Key("rigidity-weight"), &this->m_RigidityConstraintWeight,
                 "Weight for local rigidity constraint");
    cl.AddOption(Key("ic-weight"), &this->m_InverseConsistencyWeight,
                 "Weight for inverse consistency constraint");
    cl.AddOption(Key("relax"), &this->m_RelaxWeight,
    ...
    cl.AddOption(Key('e', "exploration"), &this->m_Exploration,
                 "Search space exploration (initial step size)");
    cl.AddOption(Key('a', "accuracy"), &this->m_Accuracy,
                 "Search accuracy (initial step size)");
    cl.AddOption(Key('f', "stepfactor"), &this->OptimizerStepFactor,
    ...
    cl.AddSwitch(Key("no-maxnorm"), &this->UseMaxNorm, false,
    ...
    cl.AddOption(Key("coarsest"), &this->CoarsestResolution,
    ...
    cl.AddOption(Key('p', "protocol"), &this->Protocol,
  ...
  if ((OptimizerStepFactor <= 0) || (OptimizerStepFactor >= 1)) {
    StdErr << "ERROR: step factor value " << OptimizerStepFactor
  ...
  if (!clArg2.empty()) {
    ...
    if (!Study2.empty()) {
      ...
      try {
        ...
        StdErr << "ERROR: singular matrix read from initialization file in "
                  "cmtk::ElasticRegistrationCommandLine constructor\n";
        throw ExitException(1);
      }
    }
  ...
  if (!initialTransformationFile.empty()) {
    try {
      ...
      StdErr << "Initializing transformation from " << initialTransformationFile
             << " failed: " << ex.what() << "\n";
      throw(ExitException(1));
    }
  }
  ...
  if (this->ForceSwitchVolumes) {
    ...
    this->SetInitialTransformation(
        AffineXform::SmartPtr(dynamic_cast<AffineXform *>(
            this->m_InitialTransformation->MakeInverse())));
  }
  ...
  if (!this->RigidityConstraintMapFilename.empty()) {
    UniformVolume::SmartPtr rigidityWeightMap(
        VolumeIO::ReadOriented(this->RigidityConstraintMapFilename));
    if (rigidityWeightMap) {
      this->SetRigidityConstraintMap(rigidityWeightMap);
    } else {
      StdErr << "ERROR: rigidity constraint mapcould not be read from "
             << this->RigidityConstraintMapFilename << "\n";
      throw ExitException(1);
    }
  }
...
CallbackResult ElasticRegistrationCommandLine ::InitRegistration() {
...
void ElasticRegistrationCommandLine ::OutputResult(const CoordinateVector *,
                                                   const CallbackResult irq) {
  if (!this->Studylist.empty()) {
    std::string path(this->Studylist);
    if (irq != CALLBACK_OK) path.append("-partial");
      ...
      if (!this->Studylist.empty()) {
        if (!this->InputStudylist.empty()) {
          db.AddRefinedXform(this->Studylist, true /*invertible*/,
                             this->InputStudylist, this->ForceSwitchVolumes);
        } else {
...
void ElasticRegistrationCommandLine ::EnterResolution(
...
int ElasticRegistrationCommandLine::DoneResolution(
    CoordinateVector::SmartPtr &v, Functional::SmartPtr &f, const int index,
    const int total) {
  if (this->m_OutputIntermediate) this->OutputIntermediate();
...
void ElasticRegistrationCommandLine::OutputWarp(const std::string &path) const {
  ...
  classStream.WriteDouble("rigidity_constraint_weight",
                          this->m_RigidityConstraintWeight);
  if (!this->RigidityConstraintMapFilename.empty()) {
    classStream.WriteString("rigidity_constraint_map_filename",
                            RigidityConstraintMapFilename);
  }
...
void ElasticRegistrationCommandLine::OutputIntermediate(
...
void cmtkElasticRegistrationCommandLineDispatchSIGUSR1(int sig) {
  fprintf(stderr,
  ...
  cmtk::ElasticRegistrationCommandLine::StaticThis->OutputIntermediate(
