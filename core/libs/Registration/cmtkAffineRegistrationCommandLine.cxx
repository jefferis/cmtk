...
AffineRegistrationCommandLine ::AffineRegistrationCommandLine(
  ...
  try {
    ...
    cl.AddOption(
        Key('f', "stepfactor"), &this->OptimizerStepFactor,
    ...
    cl.AddSwitch(Key('n', "no-switch"), &this->m_NoSwitch, true,
                 "Do not auto-switch reference and floating image for improved "
                 "computational performance");
    cl.AddSwitch(Key('i', "initxlate"), &InitXlate, true,
                 "Initialized transformation by translating floating image FOV "
                 "center onto reference image FOV center");
    ...
    cl.AddOption(Key("out-matrix"), &this->OutMatrixName,
                 "Output path for final transformation in matrix format");
    cl.AddOption(
        Key("out-parameters"), &this->OutParametersName,
    ...
    cl.AddOption(Key("out-itk"), &this->m_OutputPathITK,
  ...
  if (!clArg2.empty()) {
    ...
    ClassStreamInput typedStream(
        MountPoints::Translate(this->m_InitialXformPath), "registration");
    if (!typedStream.IsValid()) {
      StdErr << "ERROR: could not open studylist archive "
      ...
      try {
        ...
        *affine = toStandardSpace.GetTransformation();
        ...
        StdErr << "ERROR: singular matrix cannot be inverted to change "
                  ...
                  "AffineRegistrationCommandLine constructor.\n";
        throw ExitException(1);
      }
    }
  ...
  if (InitXlate) {
    if (!this->m_InitialXformPath.empty()) {
      StdErr << "WARNING: Initial transformation was taken from studylist. "
                "Switch --initxlate / -i will be ignored.\n";
    } else {
...
CallbackResult AffineRegistrationCommandLine::InitRegistration() {
  CallbackResult Result = AffineRegistration::InitRegistration();
  return Result;
}

void AffineRegistrationCommandLine::OutputResultMatrix(
...
void AffineRegistrationCommandLine::OutputResultParameters(
...
void AffineRegistrationCommandLine::OutputResultList(
...
void AffineRegistrationCommandLine::OutputResult(const CoordinateVector *v,
  ...
  if (!this->m_OutputPathITK.empty()) {
    try {
      ...
      StdErr << "ERROR: singular matrix cannot be inverted to change "
                "transformation to standard space in "
                "AffineRegistrationCommandLine::OutputResult\n";
      throw ExitException(1);
    }
  }
