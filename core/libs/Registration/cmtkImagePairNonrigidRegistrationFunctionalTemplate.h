 ...
 * time-consuming tasks performed by ImagePairNonrigidRegistrationFunctional and
 * its derived classes.
 */
template <class VM>
class ImagePairNonrigidRegistrationFunctionalTemplate
 ...
 public:
  ...
  /// Constructor.
  ImagePairNonrigidRegistrationFunctionalTemplate<VM>(
    ...
    this->m_TaskMetric.resize(this->m_NumberOfThreads,
  ...
  typename Self::ReturnType Evaluate() {
    ...
    if (!this->m_WarpedVolume) {
      this->m_WarpedVolume = Memory::ArrayC::Allocate<Types::DataItem>(
          this->m_DimsX * this->m_DimsY * this->m_DimsZ);
    }

    const size_t numberOfTasks =
        std::min<size_t>(this->m_NumberOfTasks, this->m_DimsY * this->m_DimsZ);
    ...
    ThreadPool::GetGlobalThreadPool().Run(
        EvaluateCompleteThread, this->m_InfoTaskComplete, numberOfTasks);
   ...
   *\param vectorCache Pre-allocated cache storage for locally used transformed
   *vectors. \return The metric after recomputation over the given
   *volume-of-influence.
   */
  typename Self::ReturnType EvaluateIncremental(const SplineWarpXform &warp,
    ...
    Types::GridIndexType endLineIncrement =
        (voi.From()[0] + (this->m_DimsX - voi.To()[0]));
    Types::GridIndexType endPlaneIncrement =
        this->m_DimsX * (voi.From()[1] + (this->m_DimsY - voi.To()[1]));

    const Types::DataItem unsetY =
        DataTypeTraits<Types::DataItem>::ChoosePaddingValue();
    localMetric = dynamic_cast<VM &>(*this->m_Metric);
    r = voi.From()[0] +
        this->m_DimsX * (voi.From()[1] + this->m_DimsY * voi.From()[2]);
        ...
        for (pX = voi.From()[0]; pX < voi.To()[0]; ++pX, ++r, ++pVec) {
          ...
          if (this->m_Metric->GetSampleX(sampleX, r)) {
            ...
            *pVec *= this->m_FloatingInverseDelta;
            if (this->m_FloatingGrid->FindVoxelByIndex(*pVec, fltIdx,
                                                       fltFrac)) {
              // Continue metric computation.
              localMetric.Increment(
                  sampleX, this->m_Metric->GetSampleY(fltIdx, fltFrac));
            } else {
  ...
  virtual typename Self::ReturnType EvaluateWithGradient(
    ...
    ThreadPool::GetGlobalThreadPool().Run(
        EvaluateGradientThread, this->m_InfoTaskGradient, numberOfTasks);
 ...
 private:
    ...
    for (size_t dim = taskIdx; dim < me->Dim; dim += taskCnt, voi += taskCnt) {
      if (me->m_StepScaleVector[dim] <= 0) {
        ...
        const typename Self::ParameterType thisStep =
            info->Step * me->m_StepScaleVector[dim];
  ...
  static void EvaluateCompleteThread(void *arg, const size_t taskIdx,
    ...
    const Types::DataItem unsetY =
        (me->m_ForceOutsideFlag)
            ? me->m_ForceOutsideValueRescaled
            : DataTypeTraits<Types::DataItem>::ChoosePaddingValue();
    ...
    for (pZ = pZfrom; (pZ < me->m_DimsZ) && rowsToDo; ++pZ) {
      for (pY = pYfrom; (pY < me->m_DimsY) && rowsToDo;
