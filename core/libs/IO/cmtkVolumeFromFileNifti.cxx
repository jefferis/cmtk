...
void __matrixToNiftiQform(nifti_1_header &header,
...
/**\note If the imported NIFTI file contains a qform and/or sform, and the
 *resulting image object is later written back to a new NIFTI file (via
 *VolumeFromFile::WriteNifti), then these transformations will be stored in the
 *output file's qform and sform fields, respectively. \note If the header
 *contains only a qform, then the qform will be used to initialize
 *cmtk::UniformVolume::m_IndexToPhysicalMatrix, which determines the
 *anatomy-based gross alignment of the image, regardless of qform_code \note If
 *the header contains only an sform, then the sform will be used instead,
 *regardless of sform_code. \note If the header contains both sform and qform,
 *then the transformation is used whose code is NIFTI_XFORM_SCANNER_ANAT. If
 *both sform_code and qform_code are NIFTI_XFORM_SCANNER_ANAT, then qform is
 *used. \note If the header contains neither a qform or an sform, then it is
 *assumed that the image is oriented "RAS", i.e., fastest-moving array index
 *from Left to Right, second fastest from Posterior to Anterior, and third
 *fastest from Inferior to Superior. \warning In keeping with the nifti1.h
 *documentation, it is assumed that qform_code and sform_code will always be
 *non-negative. If this assumption is violated, qform and sform will be switched
 *in the output file (as written by VolumeFromFile::WriteNifti), or one of them
 *may be missing.
 */
const UniformVolume::SmartPtr VolumeFromFile::ReadNifti(
    const std::string &pathHdr, const bool detached, const bool readData) {
  ...
  if (sizeof(buffer) != hdrStream.Read(&buffer, 1, sizeof(buffer))) {
    StdErr << "ERROR: could not read " << sizeof(buffer)
           << " bytes from header file " << pathHdr << "\n";
    return UniformVolume::SmartPtr(NULL);
  }
  hdrStream.Close();
  ...
  if (ndims < 3) {
    StdErr << "ERROR: image dimension " << ndims
           << " is smaller than 3 in file " << pathHdr << "\n";
    return UniformVolume::SmartPtr(NULL);
  }

  const DataGrid::IndexType::ValueType dims[3] = {header.GetField<short>(42),
                                                  header.GetField<short>(44),
                                                  header.GetField<short>(46)};
  const int dims3 = header.GetField<short>(48);

  if ((ndims > 3) && (dims3 > 1)) {
    StdErr << "WARNING: dimension " << ndims << " is greater than 3 in file "
           << pathHdr << "\n";
  }
  ...
  UniformVolume::SmartPtr volume(new UniformVolume(
      DataGrid::IndexType::FromPointer(dims), fabs(pixelDim[0]),
      fabs(pixelDim[1]), fabs(pixelDim[2])));
  ...
  const char xyzt_units =
      header.GetField<char>(offsetof(struct nifti_1_header, xyzt_units));
  ...
  const short qform_code =
      header.GetField<short>(offsetof(nifti_1_header, qform_code));
  const short sform_code =
      header.GetField<short>(offsetof(nifti_1_header, sform_code));
  ...
  if (sform_code > NIFTI_XFORM_UNKNOWN) {
    ...
    if ((sform_code == NIFTI_XFORM_SCANNER_ANAT) ||
        (qform_code == NIFTI_XFORM_UNKNOWN))
      volume->m_IndexToPhysicalMatrix = m4;

    // NIFTI says, sform_code must be > 0, so should be safe to use "abs" and
    // make negative to distinguish from qform (below)
    volume->m_AlternativeIndexToPhysicalMatrices[-abs(sform_code)] = m4;
  }

  if (qform_code > NIFTI_XFORM_UNKNOWN) {
    const float qb =
        header.GetField<float>(offsetof(nifti_1_header, quatern_b));
    const float qc =
        header.GetField<float>(offsetof(nifti_1_header, quatern_c));
    const float qd =
        header.GetField<float>(offsetof(nifti_1_header, quatern_d));

    const float qx =
        header.GetField<float>(offsetof(nifti_1_header, qoffset_x));
    const float qy =
        header.GetField<float>(offsetof(nifti_1_header, qoffset_y));
    const float qz =
        header.GetField<float>(offsetof(nifti_1_header, qoffset_z));

    const double qfac =
        (header.GetField<float>(offsetof(nifti_1_header, pixdim)) >= 0) ? 1.0f
                                                                        : -1.0f;
    const mat44 m44 = nifti_quatern_to_mat44(
        qb, qc, qd, qx, qy, qz, pixelDim[0], pixelDim[1], pixelDim[2], qfac);
    ...
    if ((qform_code == NIFTI_XFORM_SCANNER_ANAT) ||
  ...
  AnatomicalOrientation::GetOrientationFromDirections(
      orientationImage, volume->m_IndexToPhysicalMatrix, niftiSpace);
  ...
  if (header.GetField<char>(148)) {
    ...
    volume->SetMetaInfo(META_IMAGE_DESCRIPTION,
                        std::string(header.GetFieldString(148, desc, 80)));
  }

  // don't read data, we're done here.
  if (!readData) return volume;
  ...
  switch (header.GetField<short>(70)) {
    ...
    case DT_ALL:
    default:
      StdErr << "ERROR: unsupported data type " << header.GetField<short>(70)
             << " in Nifti file " << pathHdr << "\n";
      return volume;
  ...
  if (detached) {
    ...
    if (period != std::string::npos) pathImg.replace(period, 4, ".img");
  ...
  if (stream.IsValid()) {
    ...
    TypedArray::SmartPtr data(
        TypedArray::Create(dtype, volume->GetNumberOfPixels()));
    if (data->GetDataSize() == stream.Read(data->GetDataPtr(),
                                           data->GetItemSize(),
                                           data->GetDataSize())) {
      if (byteSwap) data->ChangeEndianness();

      volume->SetData(data);
    } else {
      StdErr << "ERROR: could not read " << data->GetDataSize()
             << " pixels from Nifti image file " << pathImg << "\n";
    }
  } else {
    StdErr << "ERROR: could not open Nifti image file " << pathImg << "\n";
  }

  // if we read volume data (as opposed to grid geometry only) then check
  // intent_code and set DataClass accordingly.
  if (volume->GetData()) {
...
void VolumeFromFile::WriteNifti(const std::string &path,
  ...
  const std::string spaceUnits =
      writeVolume->GetMetaInfo(META_SPACE_UNITS_STRING);
  if (spaceUnits == "mm")
  ...
  for (std::map<int, cmtk::AffineXform::MatrixType>::const_iterator it =
           volume.m_AlternativeIndexToPhysicalMatrices.begin();
       it != volume.m_AlternativeIndexToPhysicalMatrices.end(); ++it) {
  ...
  // fallback - we want at least a generic qform to be set to the volume's
  // index-to-physical matrix
  if (!(header.qform_code || header.sform_code)) {
#ifdef IGNORE
    // This piece of code, when replacing the next two lines (qform stuff) would
    // make CMTK entirely backward-compatible (release 2.3 and earlier), but
    // non-NIFTI-compliant.
  ...
  if (volume.MetaKeyExists(META_IMAGE_DESCRIPTION)) {
    ...
    strncpy(header.descrip, volume.GetMetaInfo(META_IMAGE_DESCRIPTION).c_str(),
            sizeof(header.descrip) - 1);
  }

  // see if we have the Phase Encode Direction from DICOM - set axis information
  // in header if we do
  if (volume.MetaKeyExists(META_IMAGE_SLICE_PEDIRECTION)) {
    const std::string peDirection =
        volume.GetMetaInfo(META_IMAGE_SLICE_PEDIRECTION);
    if (peDirection == "COL")
  ...
  if (volume.MetaKeyExists(META_IMAGE_SLICEORDER)) {
    // here we assume that this is a volume straight out of the DICOM stacker,
    // which keeps slices along dim[3]
    if (header.dim_info ==
        0)  // see if we previously set dim_info based on PE direction
      header.dim_info = FPS_INTO_DIM_INFO(
          0, 0, 3);  // if not, we need to set at least the slice direction
    ...
    header.slice_duration =
        atof(volume.GetMetaInfo(META_IMAGE_SLICEDURATION).c_str()) /
        1000;  // convert from msec to s for AFNI
