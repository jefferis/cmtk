...
/// Environment variable that turns on legacy Analyze I/O with incorrect
/// anatomical orientations.
const char *const CMTK_LEGACY_ANALYZE_IO = "CMTK_LEGACY_ANALYZE_IO";

/// Legacy environment variable that turns on legacy Analyze I/O with incorrect
/// anatomical orientations.
...
const UniformVolume::SmartPtr VolumeFromFile::ReadAnalyzeHdr(
    const std::string &pathHdr, const bool bigEndian, const bool readData) {
#ifdef _MSC_VER
  ...
  UniformVolume::SmartPtr volume(new UniformVolume(
      dims, fabs(pixelDim[0]), fabs(pixelDim[1]), fabs(pixelDim[2])));
  ...
  if (legacyMode) {
    //
    // Legacy mode: read Analyze images with incorrect orientations to preserve
    // backward compatibility We do this by setting "apparent" anatomical axis
    // orientation codes so that the new reorientation function in DataGrid will
    // perform the same reordering as its obsolete SetData() member used to.
    //
    switch (orient) {
      default:
        StdErr.printf(
            "WARNING: unsupported slice orientation %d in Analyze file %s\n",
    ...
    StdErr
        << "INFO: reading Analyze hdr/img in legacy orientation mode, assuming "
        << orientString << " axes\n";
  } else {
    switch (orient) {
      default:
  ...
  if (stream.IsValid()) {
    ...
    if (data->GetDataSize() == stream.Read(data->GetDataPtr(),
      ...
      StdErr << "ERROR: could not read " << data->GetDataSize()
             << " pixels from Analyze image file " << pathImg << "\n";
    }
  } else {
...
void VolumeFromFile::WriteAnalyzeHdr(const std::string &pathHdr,
  ...
  std::string currentOrientation =
      writeVolume->GetMetaInfo(META_IMAGE_ORIENTATION);
  if (currentOrientation == "") {
    currentOrientation =
        "LAS";  // default: write as is, axial tag, no reorientation.
  }
  std::string originalOrientation =
      writeVolume->GetMetaInfo(META_IMAGE_ORIENTATION_ORIGINAL);
  ...
  const char *writeOrientation = AnatomicalOrientation::GetClosestOrientation(
      originalOrientation.c_str(), supportedOrientations);

  if (getenv(CMTK_LEGACY_ANALYZE_IO) || getenv(IGS_LEGACY_ANALYZE_IO)) {
    const char *const supportedOrientationsLegacy[] = {"RAS", "RIP", "AIR",
                                                       NULL};
    writeOrientation = AnatomicalOrientation::GetClosestOrientation(
        originalOrientation.c_str(), supportedOrientationsLegacy);
  }

  UniformVolume::SmartPtr reorientedVolume;
  if (strcmp(writeOrientation, currentOrientation.c_str())) {
    DebugOutput(2) << "INFO: WriteAnalyzeHdr will reorient output volume from '"
                   << currentOrientation << "' to '" << writeOrientation
                   << "'\n";
    reorientedVolume =
        UniformVolume::SmartPtr(volume.GetReoriented(writeOrientation));
  ...
  header.StoreField<float>(
      124, static_cast<float>(dataRange.m_UpperBound));  // cal_max
  header.StoreField<float>(
      128, static_cast<float>(dataRange.m_LowerBound));  // cal_min
  ...
  if (volume.MetaKeyExists(META_IMAGE_DESCRIPTION))
    header.StoreFieldString(
        148, volume.GetMetaInfo(META_IMAGE_DESCRIPTION).c_str(), 80);
  ...
  if (VolumeIO::GetWriteCompressed()) {
    ...
    if (!stat(pathImg.c_str(), &buf)) {
      StdErr << "WARNING: Analyze img file '" << pathImg
