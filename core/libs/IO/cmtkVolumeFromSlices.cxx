...
void VolumeFromSlices::InitSequence(const ScalarImage *image,
  ...
  VolumeDataArray =
      TypedArray::Create(image->GetPixelData()->GetType(), DataSize);
...
char *VolumeFromSlices::AllocDataArray(const int bytesPerPixel,
                                       const int dataSize) const {
  return Memory::ArrayC::Allocate<char>(bytesPerPixel * dataSize);
}

TypedArray::SmartPtr VolumeFromSlices::EncapDataArray(
    const ScalarDataType dtype, void *const data, const int data_size) const {
  return TypedArray::Create(dtype, data, data_size, Padding, &PaddingValue,
                            Memory::ArrayC::Delete);
}

const char *VolumeFromSlices::FillPlane(unsigned int &plane,
  ...
  for (int planeIdx = 0; planeIdx < image->GetNumberOfFrames();
       ++planeIdx, ++plane) {
    const char *check = this->CheckImage(plane, image, planeIdx);
    if (check) return check;

    memcpy(rawDataPtr + bytesPerBlock * plane,
           image->GetPixelData()->GetDataPtr(), bytesPerBlock);

    // set world coordinate of the plane just read
    Types::Coordinate slicePosition =
        (ImagePosition - FirstImagePosition).RootSumOfSquares();
...
UniformVolume::SmartPtr VolumeFromSlices::FinishVolume(
    Types::Coordinate &sliceOffset, int &sliceDirection) {
  ...
  *next_point = 0;
  ...
  if (!VolumeDataArray)
    VolumeDataArray = TypedArray::SmartPtr(this->EncapDataArray(
        SelectDataTypeInteger(BytesPerPixel, SignBit), RawData, DataSize));

  const Types::Coordinate *aux[] = {Points[0], Points[1], Points[2]};
  UniformVolume::SmartPtr Result =
      this->ConstructVolume(Dims, Size, aux, VolumeDataArray);
  ...
  for (idx = 0; idx < 3; ++idx) Memory::ArrayC::Delete(Points[idx]);

  Result->SetMetaInfo(META_SPACE, "LPS");
  Result->SetMetaInfo(META_SPACE_ORIGINAL, "LPS");

  // actual image directions
  const Types::Coordinate spacing[3] = {(Size[0] / (Dims[0] - 1)),
                                        (Size[1] / (Dims[1] - 1)),
                                        (Size[2] / (Dims[2] - 1))};

  this->ImageOrientation[0] *=
      spacing[0] / this->ImageOrientation[0].RootSumOfSquares();
  this->ImageOrientation[1] *=
      spacing[1] / this->ImageOrientation[1].RootSumOfSquares();
  this->IncrementVector *=
      spacing[2] / this->IncrementVector.RootSumOfSquares();

  const Types::Coordinate directions[3][3] = {
      {this->ImageOrientation[0][0], this->ImageOrientation[0][1],
       this->ImageOrientation[0][2]},
      {this->ImageOrientation[1][0], this->ImageOrientation[1][1],
       this->ImageOrientation[1][2]},
      {this->IncrementVector[0], this->IncrementVector[1],
       this->IncrementVector[2]}};

  const Matrix3x3<Types::Coordinate> m3(directions);
  Matrix4x4<Types::Coordinate> m4(m3);
  for (int i = 0; i < 3; ++i) m4[3][i] = this->FirstImagePosition[i];

  Result->m_IndexToPhysicalMatrix = m4;
  //  const std::string orientationString0 =
  //  Result->GetOrientationFromDirections();
  ...
  Result->SetMetaInfo(META_SPACE_UNITS_STRING,
                      "mm");  // seems to be implied in DICOM
...
const char *VolumeFromSlices::CheckImage(const int plane,
                                         const ScalarImage *image,
                                         const unsigned int frame) {
  if ((this->Dims[0] != image->GetDims()[AXIS_X]) ||
      (this->Dims[1] != image->GetDims()[AXIS_Y]))
  ...
  for (unsigned int dim = 0; dim < 3; ++dim) {
    if ((fabs(ImageOrientation[0][dim] - image->GetImageDirectionX()[dim]) >
         CMTK_MAX_CALIB_ERROR) ||
        (fabs(ImageOrientation[1][dim] - image->GetImageDirectionY()[dim]) >
         CMTK_MAX_CALIB_ERROR))
      return "Non-parallel image planes";
  }

  // Second++ slice: Compute slice-to-slice vector
  ScalarImage::SpaceVectorType imageToImage =
      image->GetImageOrigin(frame) - ImagePosition;

  if (imageToImage.MaxAbsValue() < CMTK_MAX_LOCALIZE_ERROR) {
    StdErr.printf("Two slices at position (%f,%f,%f)\n",
                  (float)ImagePosition[0], (float)ImagePosition[1],
                  (float)ImagePosition[2]);
...
UniformVolume::SmartPtr VolumeFromSlices::ConstructVolume(
    const DataGrid::IndexType &dims,
    const UniformVolume::CoordinateVectorType &size,
    const Types::Coordinate *points[3], TypedArray::SmartPtr &data) const {
  bool isUniform = true;
  ...
  for (unsigned int dim = 0; (dim < 3) && isUniform; ++dim) {
    ...
    for (int idx = 2; (idx < dims[dim]) && isUniform; ++idx) {
      if (fabs(delta - (points[dim][idx] - points[dim][idx - 1])) >
          (this->m_Tolerance * delta))
        isUniform = false;
