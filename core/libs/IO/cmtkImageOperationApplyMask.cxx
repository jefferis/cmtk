...
cmtk::UniformVolume::SmartPtr cmtk::ImageOperationApplyMask ::Apply(
    cmtk::UniformVolume::SmartPtr &volume) {
  const std::string maskOrientation =
      this->m_MaskVolume->GetMetaInfo(META_IMAGE_ORIENTATION);
  const std::string workingOrientation =
      volume->GetMetaInfo(META_IMAGE_ORIENTATION);
  if (maskOrientation != workingOrientation) {
    this->m_MaskVolume = cmtk::UniformVolume::SmartPtr(
        this->m_MaskVolume->GetReoriented(workingOrientation.c_str()));
  }

  for (int dim = 0; dim < 3; ++dim) {
    if (this->m_MaskVolume->m_Dims[dim] != volume->m_Dims[dim]) {
      cmtk::StdErr << "ERROR: mask volume dimensions do not match working "
                      "volume dimensions.\n";
      exit(1);
    }
  }
  ...
  for (size_t i = 0; i < nPixels; ++i)
    if (maskData.IsPaddingOrZeroAt(i)) volumeData.SetPaddingAt(i);
  return volume;
}

cmtk::UniformVolume::SmartPtr cmtk::ImageOperationApplyMask ::ReadMaskFile(
    const char *maskFileName, const bool inverse) {
  ...
  // binarize mask to 1/0, convert to char, and also consider "inverse" flag in
  // the process.
  ...
  maskVolume->SetData(
      cmtk::TypedArray::SmartPtr(maskData->Convert(cmtk::TYPE_BYTE)));
