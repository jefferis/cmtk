...
const char *cmtk::PhantomIO::WhitespaceWriteMiniXML(mxml_node_t *node,
                                                    int where) {
...
void cmtk::PhantomIO::Write(const DetectedPhantomMagphanEMR051 &phantom,
  ...
  Coverity::FakeFree(mxmlNewText(mxmlNewElement(x_phantom, "phantomType"), 0,
                                 "MagphanEMR051"));
  ...
  if (phantom.m_StatusFlags.m_FallbackCentroidCNR) {
    mxml_node_t *x_fallback_centroid =
  ...
  Coverity::FakeFree(mxmlNewReal(mxmlNewElement(x_phantom, "maxDimming"),
                                 phantom.m_MaxDimming));

  FixedVector<3, Types::Coordinate> scales =
      phantom.m_LinearFitXform.GetScales();
  ...
  for (std::list<LandmarkPair>::const_iterator it = lmPairs.begin();
       it != lmPairs.end(); ++it) {
    ...
    Coverity::FakeFree(mxmlNewText(mxmlNewElement(x_lm, "isPrecise"), 0,
                                   it->m_Precise ? "yes" : "no"));
    Coverity::FakeFree(
...
cmtk::DetectedPhantomMagphanEMR051::SmartPtr cmtk::PhantomIO::Read(
    const std::string &fpath) {
  ...
  mxml_node_t *x_landmarks =
      mxmlFindElement(x_root, x_root, "landmarkList", NULL, NULL, MXML_DESCEND);
  if (!x_landmarks) {
    cmtk::StdErr << "ERROR: could not file 'landmarks' XML element in file "
                 << fpath << "\n";
    mxmlDelete(x_root);
    return DetectedPhantomMagphanEMR051::SmartPtr(NULL);
  }
  ...
  AffineXform xform;
  DetectedPhantomMagphanEMR051::SmartPtr result(
      new DetectedPhantomMagphanEMR051(xform));

  for (mxml_node_t *x_fiducial = mxmlFindElement(
           x_landmarks, x_root, "landmark", NULL, NULL, MXML_DESCEND);
       x_fiducial != NULL;
       x_fiducial = mxmlFindElement(x_fiducial, x_root, "landmark", NULL, NULL,
                                    MXML_DESCEND)) {
    mxml_node_t *x_name =
        mxmlFindElement(x_fiducial, x_root, "name", NULL, NULL, MXML_DESCEND);
    if (!x_name || !x_name->child) continue;
    const std::string name = x_name->child->value.text.string;

    mxml_node_t *x_expected = mxmlFindElement(x_fiducial, x_root, "expected",
    ...
    mxml_node_t *x_detected = mxmlFindElement(x_fiducial, x_root, "detected",
                                              NULL, NULL, MXML_DESCEND);
    ...
    mxml_node_t *x_residual = mxmlFindElement(x_fiducial, x_root, "fitResidual",
                                              NULL, NULL, MXML_DESCEND);
    if (!x_residual || !x_residual->child) continue;
    const Types::Coordinate residual = static_cast<Types::Coordinate>(
        atof(x_residual->child->value.text.string));
