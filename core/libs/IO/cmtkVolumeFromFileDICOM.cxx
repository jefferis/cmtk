...
const UniformVolume::SmartPtr VolumeFromFile::ReadDICOM(
  ...
  try {
    ...
    TypedArray::SmartPtr pixelDataArray =
        dicom.GetPixelDataArray(totalImageSizePixels);
    ...
    UniformVolume::CoordinateVectorType sliceNormal =
        dicom.DemosaicAndGetNormal(imageOrientation, pixelSize, dims,
                                   pixelDataArray, imageOrigin);

    // Construct volume and set the DICOM coordinates
    volume = UniformVolume::SmartPtr(
        new UniformVolume(UniformVolume::IndexType(dims), pixelSize[0],
                          pixelSize[1], pixelSize[2], pixelDataArray));
    volume->SetMetaInfo(META_SPACE, "LPS");
    volume->SetMetaInfo(META_SPACE_ORIGINAL, "LPS");

    imageOrientation[0] *=
        pixelSize[0] / imageOrientation[0].RootSumOfSquares();
    imageOrientation[1] *=
        pixelSize[1] / imageOrientation[1].RootSumOfSquares();
    sliceNormal *= pixelSize[2] / sliceNormal.RootSumOfSquares();

    const Types::Coordinate directions[3][3] = {
        {imageOrientation[0][0], imageOrientation[0][1],
         imageOrientation[0][2]},
        {imageOrientation[1][0], imageOrientation[1][1],
