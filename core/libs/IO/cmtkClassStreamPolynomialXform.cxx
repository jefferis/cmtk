...
ClassStreamOutput &operator<<(ClassStreamOutput &stream,
  ...
  stream.WriteCoordinateArray("coefficients", xform.m_Parameters,
                              xform.m_NumberOfParameters);
  stream.End();
...
ClassStreamInput &operator>>(ClassStreamInput &stream, PolynomialXform &xform) {
  ...
  if (stream.Seek("polynomial_xform", true /*forward*/) !=
      TypedStream::CONDITION_OK) {
    stream.Rewind();
    if (stream.Seek("registration", true /*forward*/) !=
        TypedStream::CONDITION_OK) {
    ...
    if (stream.Seek("polynomial_xform", false /*forward*/) !=
        TypedStream::CONDITION_OK) {
  ...
  if (stream.ReadCoordinateArray("center", center, 3) !=
      TypedStream::CONDITION_OK) {
    throw Exception(
        "Could not read 'center' array from polynomial xform archive");
  }
  xform.SetCenter(PolynomialXform::SpaceVectorType::FromPointer(center));

  if (stream.ReadCoordinateArray("coefficients", xform.m_Parameters,
                                 xform.m_NumberOfParameters) !=
      TypedStream::CONDITION_OK) {
    throw Exception(
        "Could not read 'coeffients' array from polynomial xform archive");
  }
  stream.End();

  xform.SetMetaInfo(META_SPACE, AnatomicalOrientation::ORIENTATION_STANDARD);
  if (fixedImage) xform.SetMetaInfo(META_XFORM_FIXED_IMAGE_PATH, fixedImage);

  if (movingImage) xform.SetMetaInfo(META_XFORM_MOVING_IMAGE_PATH, movingImage);
