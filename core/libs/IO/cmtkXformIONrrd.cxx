...
Xform::SmartPtr XformIO::ReadNrrd(const std::string &path) {
  DeformationField::SmartPtr dfield(NULL);
  try {
    Nrrd *nrrd = nrrdNew();
    if (nrrdLoad(nrrd, path.c_str(), NULL)) throw biffGetDone(NRRD);

    if (nrrd->dim != 4) {
      StdErr
          << "ERROR: deformation field must be stored as 4-dimensional Nrrd.\n";
      return dfield;
    }

    if (nrrd->axis[0].kind != nrrdKindVector) {
      StdErr << "ERROR: deformation field vectors in Nrrd must be stored "
                "together.\n";
      return dfield;
    }

    if (nrrd->axis[0].size != 3) {
      StdErr << "ERROR: deformation field vectors in Nrrd must be three "
                "dimensional.\n";
      return dfield;
    }

    NrrdAxisInfo *nrrdSpaceAxes = nrrd->axis + 1;
    const int dims[3] = {static_cast<int>(nrrdSpaceAxes[0].size),
                         static_cast<int>(nrrdSpaceAxes[1].size),
                         static_cast<int>(nrrdSpaceAxes[2].size)};

    // for each axis, if spacing is NaN, use direction vector to compute
    // spacing.
    double spacing[3] = {1, 1, 1};
    for (size_t ax = 0; ax < 3; ++ax) {
      switch (nrrdSpacingCalculate(nrrd, ax + 1, spacing + ax,
                                   nrrd->axis[ax + 1].spaceDirection)) {
    ...
    const Types::Coordinate size[3] = {(dims[0] - 1) * spacing[0],
                                       (dims[1] - 1) * spacing[1],
                                       (dims[2] - 1) * spacing[2]};
    const Types::Coordinate origin[3] = {
        nrrd->spaceOrigin[0], nrrd->spaceOrigin[1], nrrd->spaceOrigin[2]};
    dfield = DeformationField::SmartPtr(new DeformationField(
        FixedVector<3, Types::Coordinate>::FromPointer(size),
        DeformationField::ControlPointIndexType::FromPointer(dims), origin));
    ...
    if (type != TYPE_NONE) {
      TypedArray::SmartPtr data(TypedArray::Create(
          type, nrrd->data, 3 * dims[0] * dims[1] * dims[2]));
      data->ConvertSubArray(dfield->m_Parameters, TYPE_COORDINATE, 0,
                            data->GetDataSize());
    } else {
    ...
    if (orientationSpace) {
      ...
      for (int i = 0; i < 3; ++i) m4[3][i] = nrrd->spaceOrigin[i];
      ...
      try {
        ...
        StdErr << "ERROR: space directions in Nrrd file cause singular matrix "
                  "exception in XformIO::ReadNrrd(). Using identity matrix "
                  "instead.\n";
      }

      char orientationImage[4];
      AnatomicalOrientation::GetOrientationFromDirections(orientationImage, m4,
                                                          orientationSpace);
...
void XformIO::WriteNrrd(const Xform *xform, const std::string &path) {
  const DeformationField *dfield =
      dynamic_cast<const DeformationField *>(xform);
  if (!dfield) {
    StdErr << "ERROR: XformIO::WriteNrrd can only write DeformationField "
              "objects so far.\n"
           << "       No data was written.\n";
    return;
  }
  ...
  const int type = (sizeof(Types::Coordinate) == sizeof(float))
                       ? nrrdTypeFloat
                       : nrrdTypeDouble;
  ...
  try {
    if (nrrdWrap_va(nval, val, type, 4, (size_t)3, (size_t)dfield->m_Dims[0],
                    (size_t)dfield->m_Dims[1], (size_t)dfield->m_Dims[2])) {
    ...
    if (dfield->MetaKeyExists(META_SPACE_UNITS_STRING)) {
      nval->spaceUnits[0] =
          strdup(dfield->GetMetaInfo(META_SPACE_UNITS_STRING).c_str());
    }

    int kind[NRRD_DIM_MAX] = {nrrdKindVector, nrrdKindDomain, nrrdKindDomain,
                              nrrdKindDomain};
    nrrdAxisInfoSet_nva(nval, nrrdAxisInfoKind, kind);
    nrrdAxisInfoSet_va(nval, nrrdAxisInfoLabel, "Vx;Vy;Vz", "x", "y", "z");

    double origin[NRRD_DIM_MAX] = {dfield->m_Offset[0], dfield->m_Offset[1],
                                   dfield->m_Offset[2]};
