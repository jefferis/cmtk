...
const UniformVolume::SmartPtr VolumeFromFile::ReadVanderbilt(
    const std::string &path) {
  FILE *fp = fopen(path.c_str(), "r");
  if (!fp) return UniformVolume::SmartPtr(NULL);
    ...
    if (2 == sscanf(line, "%32[a-zA-Z ]:= %64[0-9.: ]", key, value)) {
      if (!strcmp(key, "Columns ")) {
        ...
        if (2 != sscanf(value, "%20lf : %20lf", calib, calib + 1)) {
          StdErr << "WARNING: could not determine pixel size from line '"
                 << line << "'\n";
          calib[0] = calib[1] = 1.0;
        }
      ...
      char axes[3];
      if (3 == sscanf(line, "Patient orientation := %c : %c : %c", &axes[0],
                      &axes[1], &axes[2])) {
  ...
  UniformVolume::SmartPtr volume(new UniformVolume(
      DataGrid::IndexType::FromPointer(dims), calib[0], calib[1], calib[2]));
  ...
  if (imageStream.IsValid()) {
    TypedArray::SmartPtr data =
        TypedArray::Create(TYPE_SHORT, dims[0] * dims[1] * dims[2]);
    imageStream.Read(data->GetDataPtr(), data->GetItemSize(),
                     data->GetDataSize());
