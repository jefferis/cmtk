...
class StudyList :
 ...
 public:
  ...
  /// Find constant Study object by file system path.
  const Study *FindStudyName(
      const std::string &name /*!< Name of the study to find.*/) const;

  /// Find non-constant Study object by file system path.
  Study::SmartPtr FindStudyName(
      const std::string &name /*!< Name of the study to find.*/);

  /// Add a new study entry.
  Study::SmartPtr AddStudy(
      const std::string &fileSystemPath /*!< Name of the study to add.*/);

  /// Add an existing study object.
  void AddStudy(
      Study::SmartPtr
          &study /*!< Existing study object to add to the study list.*/);

  /// Add a coordinate transformation between two studies.
  void AddXform(Study::SmartPtr &fromStudy, Study::SmartPtr &toStudy,
                AffineXform::SmartPtr &affineXform,
                WarpXform::SmartPtr &warpXform = WarpXform::SmartPtr::Null());

  /// Add a coordinate transformation between two studies.
  void AddXform(const std::string &fromStudyPath /*!< Path of the study that the
                                                    transformation maps from.*/
                ,
                const std::string &toStudyPath /*!< Path of the study that the
                                                  transformation maps to.*/
                ,
                AffineXform::SmartPtr
                    &affineXform /*!< Affine coordinate transformation between
                                    the given studies. */
                ,
  ...
  void DeleteStudy(
      const Study *study /*!< Study object to delete from the list.*/);
