...
Xform::SmartPtr XformIO::Read(const std::string &path) {
  ...
  switch (FileFormat::Identify(realPath)) {
...
#else
      StdErr << "ERROR: " << realPath
             << " is a Nrrd file, but Nrrd support is not enabled.\n"
             << "  Please re-configure software using either '--with-nrrd' or "
                "'--with-nrrd-teem' switch.\n";
    ...
    case FILEFORMAT_TYPEDSTREAM:
      DebugOutput(1) << "Reading transformation from typedstream file "
                     << realPath << "\n";
      {
        ...
        if (warpXform) return Xform::SmartPtr(warpXform);
    ...
    case FILEFORMAT_NEXIST:
      StdErr << "The file/directory " << realPath
             << " does not exist or cannot be read\n";
      throw ExitException(1);
    default:
      StdErr << "The file/directory " << realPath
             << " does not seem to be in a supported transformation format\n";
      throw ExitException(1);
  }
  return Xform::SmartPtr(NULL);
}

void XformIO::Write(const Xform *xform, const std::string &path) {
  ...
  switch (fileFormat) {
    ...
    case FILEFORMAT_ITK_TFM: {
      ...
      if (affineXform) AffineXformITKIO::Write(path, *affineXform);
      break;
    }
    case FILEFORMAT_TYPEDSTREAM: {
      ...
      const PolynomialXform *polyXform =
          dynamic_cast<const PolynomialXform *>(xform);
      if (polyXform) stream << *polyXform;

      const SplineWarpXform *splineWarpXform =
          dynamic_cast<const SplineWarpXform *>(xform);
      if (splineWarpXform) stream << *splineWarpXform;
    } break;
    default:
