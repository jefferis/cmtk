...
bool ImageStackDICOM::Match(const ImageFileDICOM &newImage,
                            const Types::Coordinate numericalTolerance,
                            const bool disableCheckOrientation,
                            const bool ignoreAcquisitionNumber) const {
  if (this->empty()) return true;  // first image always matches

  ImageFileDICOM::SmartConstPtr check = this->front();
  if (check) {
    if (!check->Match(newImage, numericalTolerance, disableCheckOrientation,
                      ignoreAcquisitionNumber))
      return 0;

    for (const_iterator it = this->begin(); it != this->end(); ++it) {
      // if we already have an image in same location in this study,
      // then bump to next study
      if ((*it)->GetTagValue(DCM_ImagePositionPatient) ==
...
void ImageStackDICOM::AddImageFile(ImageFileDICOM::SmartConstPtr &newImage) {
  iterator it = begin();
  for (; it != end(); ++it)
...
std::vector<double> ImageStackDICOM::AssembleSliceTimes() const {
  std::vector<double> sliceTimes;
  for (const_iterator it = this->begin(); it != this->end(); ++it) {
    // add this file's slice times to stack slice time vector - this should be
    // safe even if file slice time vector is empty
    sliceTimes.insert(sliceTimes.end(), (*it)->m_SliceTimes.begin(),
                      (*it)->m_SliceTimes.end());
  }
...
const char *ImageStackDICOM::WhitespaceWriteMiniXML(mxml_node_t *node,
                                                    int where) {
  ...
  if ((where >= 0) && (where < 4)) {
    for (size_t idx = 0; wsLookup[idx].name; ++idx) {
      if (!strcmp(name, wsLookup[idx].name)) return wsLookup[idx].ws[where];
    }
  }
...
// wrap tolower() - on Mac, system function is not compatible with
// std::transform()
static int cmtkWrapToLower(const int c) { return tolower(c); }

void ImageStackDICOM::WriteXML(const std::string &fname,
                               const UniformVolume &volume,
                               const bool includeIdentifiers) const {
  ...
  if (includeIdentifiers) {
    ...
    Coverity::FakeFree(
        mxmlNewText(mxmlNewElement(x_device, "dicom:Manufacturer"), 0,
                    firstImage->GetTagValue(DCM_Manufacturer).c_str()));
    Coverity::FakeFree(mxmlNewText(
        mxmlNewElement(x_device, "dicom:ManufacturerModel"), 0,
        firstImage->GetTagValue(DCM_ManufacturerModelName).c_str()));
    Coverity::FakeFree(
        mxmlNewText(mxmlNewElement(x_device, "dicom:StationName"), 0,
                    firstImage->GetTagValue(DCM_StationName).c_str()));
    Coverity::FakeFree(
        mxmlNewText(mxmlNewElement(x_device, "dicom:DeviceSerialNumber"), 0,
                    firstImage->GetTagValue(DCM_DeviceSerialNumber).c_str()));
  }

  std::string modality = firstImage->GetTagValue(DCM_Modality);
  std::transform(modality.begin(), modality.end(), modality.begin(),
                 cmtkWrapToLower);
  ...
  if (modality == "mr") {
    ...
    mxmlNewReal(x_tr,
                atof(firstImage->GetTagValue(DCM_RepetitionTime).c_str()));
    ...
    Coverity::FakeFree(mxmlNewReal(
        mxmlNewElement(x_modality, "dicom:ImagingFrequency"),
        atof(firstImage->GetTagValue(DCM_ImagingFrequency).c_str())));
    ...
    const std::string phaseEncodeDirection =
        firstImage->GetTagValue(DCM_InPlanePhaseEncodingDirection);
    if (phaseEncodeDirection != "") {
      Coverity::FakeFree(
          mxmlNewText(mxmlNewElement(x_modality, "phaseEncodeDirection"), 0,
                      phaseEncodeDirection.c_str()));
    }

    if (firstImage->m_PhaseEncodeDirectionSign != "") {
      Coverity::FakeFree(
          mxmlNewText(mxmlNewElement(x_modality, "phaseEncodeDirectionSign"), 0,
                      firstImage->m_PhaseEncodeDirectionSign.c_str()));
    }

    if (firstImage->GetTagValue(DCM_GE_EffectiveEchoSpacing) != "") {
      Coverity::FakeFree(mxmlNewReal(
          mxmlNewElement(x_modality, "dicom:GE:EffectiveEchoSpacing"),
          atof(firstImage->GetTagValue(DCM_GE_EffectiveEchoSpacing).c_str())));
    }

    if (firstImage->GetTagValue(DCM_SequenceName) != "" && includeIdentifiers) {
      Coverity::FakeFree(
          mxmlNewText(mxmlNewElement(x_modality, "dicom:SequenceName"), 0,
                      firstImage->GetTagValue(DCM_SequenceName).c_str()));
    }

    if (firstImage->GetTagValue(DCM_GE_PulseSequenceName) != "" &&
        includeIdentifiers) {
      Coverity::FakeFree(mxmlNewText(
          mxmlNewElement(x_modality, "dicom:GE:PulseSequenceName"), 0,
          firstImage->GetTagValue(DCM_GE_PulseSequenceName).c_str()));
    }

    if (firstImage->GetTagValue(DCM_GE_PulseSequenceDate) != "" &&
        includeIdentifiers) {
      Coverity::FakeFree(mxmlNewText(
          mxmlNewElement(x_modality, "dicom:GE:PulseSequenceDate"), 0,
          firstImage->GetTagValue(DCM_GE_PulseSequenceDate).c_str()));
    }

    if (firstImage->GetTagValue(DCM_GE_InternalPulseSequenceName) != "" &&
        includeIdentifiers) {
      Coverity::FakeFree(mxmlNewText(
          mxmlNewElement(x_modality, "dicom:GE:InternalPulseSequenceName"), 0,
          firstImage->GetTagValue(DCM_GE_InternalPulseSequenceName).c_str()));
    }

    if (firstImage->m_RawDataType != "unknown") {
      Coverity::FakeFree(mxmlNewText(mxmlNewElement(x_modality, "type"), 0,
                                     firstImage->m_RawDataType.c_str()));
    }

    if (firstImage->m_IsDWI) {
      mxml_node_t *x_dwi = mxmlNewElement(x_modality, "dwi");

      Coverity::FakeFree(mxmlNewInteger(mxmlNewElement(x_dwi, "bValue"),
                                        firstImage->m_BValue));
      ...
      if (firstImage->m_HasBVector) {
        ...
        try {
          // Apply inverse of remaining image-to-space matrix to original
          // bVector
          const UniformVolume::CoordinateVectorType bVectorImage =
              firstImage->m_BVector *
              Matrix3x3<Types::Coordinate>(
                  gridLPS->GetImageToPhysicalMatrix().GetInverse());

          mxml_node_t *x_bvec_image = mxmlNewElement(x_dwi, "bVectorImage");
          mxmlElementSetAttr(
              x_bvec_image, "imageOrientation",
              gridLPS->GetMetaInfo(META_IMAGE_ORIENTATION).c_str());
          for (size_t idx = 0; idx < 3; ++idx) {
            Coverity::FakeFree(mxmlNewReal(x_bvec_image, bVectorImage[idx]));
          }
        } catch (const AffineXform::MatrixType::SingularMatrixException &) {
          StdErr
              << "WARNING: singular image-to-physical matrix; cannot determine "
                 "b vector orientation in image space (bVectorImage).\n";
        }
        ...
        try {
          // Apply inverse of remaining image-to-space matrix to original
          // bVector
          const UniformVolume::CoordinateVectorType bVectorStandard =
              firstImage->m_BVector *
              Matrix3x3<Types::Coordinate>(
                  gridRAS->GetImageToPhysicalMatrix().GetInverse());

          mxml_node_t *x_bvec_std = mxmlNewElement(x_dwi, "bVectorStandard");
          mxmlElementSetAttr(
              x_bvec_std, "imageOrientation",
              gridRAS->GetMetaInfo(META_IMAGE_ORIENTATION).c_str());
          for (size_t idx = 0; idx < 3; ++idx) {
          ...
          StdErr
              << "WARNING: singular image-to-physical matrix; cannot determine "
                 "b vector orientation in standard space (bVectorStandard).\n";
        }
      }
    }
  }
  ...
  if (includeIdentifiers) {
    Coverity::FakeFree(mxmlNewText(mxmlNewElement(x_stack, "dcmFileDirectory"),
                                   0, firstImage->m_FileDir.c_str()));
    Coverity::FakeFree(
        mxmlNewText(mxmlNewElement(x_stack, "dicom:StudyInstanceUID"), 0,
                    firstImage->GetTagValue(DCM_StudyInstanceUID).c_str()));
    Coverity::FakeFree(
        mxmlNewText(mxmlNewElement(x_stack, "dicom:SeriesInstanceUID"), 0,
                    firstImage->GetTagValue(DCM_SeriesInstanceUID).c_str()));

    if (firstImage->GetTagValue(DCM_FrameOfReferenceUID, "missing") !=
        "missing") {
      Coverity::FakeFree(mxmlNewText(
          mxmlNewElement(x_stack, "dicom:FrameOfReferenceUID"), 0,
          firstImage->GetTagValue(DCM_FrameOfReferenceUID).c_str()));
    }
  }

  Coverity::FakeFree(mxmlNewText(
      mxmlNewElement(x_stack, "dicom:ImageOrientationPatient"), 0,
      firstImage->GetTagValue(DCM_ImageOrientationPatient).c_str()));

  for (const_iterator it = this->begin(); it != this->end(); ++it) {
    mxml_node_t *x_image = mxmlNewElement(x_stack, "image");

    if (includeIdentifiers) {
      Coverity::FakeFree(mxmlNewText(mxmlNewElement(x_image, "dcmFile"), 0,
                                     (*it)->m_FileName.c_str()));
    }

    Coverity::FakeFree(
        mxmlNewText(mxmlNewElement(x_image, "dicom:AcquisitionTime"), 0,
                    (*it)->GetTagValue(DCM_AcquisitionTime).c_str()));
    Coverity::FakeFree(
        mxmlNewText(mxmlNewElement(x_image, "dicom:ImagePositionPatient"), 0,
                    (*it)->GetTagValue(DCM_ImagePositionPatient).c_str()));

    if ((*it)->GetTagValue(DCM_RescaleIntercept, "missing") != "missing") {
      Coverity::FakeFree(
          mxmlNewReal(mxmlNewElement(x_image, "dicom:RescaleIntercept"),
                      atof((*it)->GetTagValue(DCM_RescaleIntercept).c_str())));
    }

    if ((*it)->GetTagValue(DCM_RescaleSlope, "missing") != "missing") {
      Coverity::FakeFree(
          mxmlNewReal(mxmlNewElement(x_image, "dicom:RescaleSlope"),
                      atof((*it)->GetTagValue(DCM_RescaleSlope).c_str())));
    }
  }
  ...
  if (!stackSliceTimes.empty()) {
    const double baseTime =
        *std::min_element(stackSliceTimes.begin(), stackSliceTimes.end());
    for (size_t stackSlice = 0; stackSlice < stackSliceTimes.size();
         ++stackSlice) {
      mxml_node_t *x_slice_time = mxmlNewElement(x_stack, "sliceTime");
      Coverity::FakeFree(
          mxmlNewReal(x_slice_time, stackSliceTimes[stackSlice] - baseTime));
...
cmtk::UniformVolume::SmartConstPtr ImageStackDICOM::WriteImage(
    const std::string &fname, const Self::EmbedInfoEnum embedInfo) const {
...
#ifdef MSC_VER
    snprintf(fullPath, sizeof(fullPath), "%s\\%s", first->m_FileDir.c_str(),
             first->m_FileName.c_str());
#else
    snprintf(fullPath, sizeof(fullPath), "%s/%s", first->m_FileDir.c_str(),
             first->m_FileName.c_str());
#endif
  ...
  if (volume) {
    switch (embedInfo) {
      ...
      case EMBED_STUDYID_STUDYDATE:
        volume->SetMetaInfo(META_IMAGE_DESCRIPTION,
                            first->GetTagValue(DCM_StudyID) + "_" +
                                first->GetTagValue(DCM_StudyDate));
        break;
      case EMBED_PATIENTNAME:
        volume->SetMetaInfo(META_IMAGE_DESCRIPTION,
                            first->GetTagValue(DCM_PatientsName));
        break;
      case EMBED_SERIESDESCR:
        volume->SetMetaInfo(META_IMAGE_DESCRIPTION,
                            first->GetTagValue(DCM_SeriesDescription));
        break;
    }

    // see if we have Phase Encode direction and set metadata (for NIFTI only at
    // this time)
    const std::string phaseEncodeDirection =
        first->GetTagValue(DCM_InPlanePhaseEncodingDirection);
    if (!phaseEncodeDirection.empty()) {
      volume->SetMetaInfo(META_IMAGE_SLICE_PEDIRECTION, phaseEncodeDirection);
    }

    // see if we have slice times and set metadata accordingly (relevant for
    // NIFTI only at this time)
    const std::vector<double> sliceTimes = this->AssembleSliceTimes();
    if (sliceTimes.size() >
        1)  // need at least 2 slices for meaningful slice order
    {
      std::vector<double> sliceTimesSorted = sliceTimes;
      std::sort(sliceTimesSorted.begin(), sliceTimesSorted.end());

      // This next bit inspired by Xiangru Li's Matlab DICOM-to-NIfTI converter,
      ...
      const double difference = (sliceTimesSorted[sliceTimesSorted.size() - 1] -
                                 sliceTimesSorted[0]) /
                                (sliceTimes.size() - 1);
      ...
      if (fabs(difference - duration) < 1e-6) {
        ...
        if (sliceTimes[sliceTimes.size() - 1] <
            sliceTimes[sliceTimes.size() - 2]) {
          // EVEN slices acquired first.
    ...
    DebugOutput(1).GetStream().printf(
        ...
        "%.4fx%.4fx%.4f mm\n\n",
        fname.c_str(), volume->m_Dims[0], volume->m_Dims[1], volume->m_Dims[2],
  ...
  DebugOutput(1)
      ...
      << "  TR / TE:       " << first->GetTagValue(DCM_RepetitionTime)
      << "ms / " << first->GetTagValue(DCM_EchoTime) << "ms\n"
