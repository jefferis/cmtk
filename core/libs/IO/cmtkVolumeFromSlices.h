...
class VolumeFromSlices {
 ...
 protected:
   ...
   */
  void InitSequence(const ScalarImage *image,
                    const unsigned int numberOfSlices);

  /** Allocate memory for the 3D image data.
   */
  virtual char *AllocDataArray(const int bytesperpixel,
   ...
   * TypedArray object.
   */
  virtual TypedArray::SmartPtr EncapDataArray(const ScalarDataType dtype,
                                              void *const data,
                                              const int data_size) const;
   ...
   */
  UniformVolume::SmartPtr FinishVolume(Types::Coordinate &sliceOffset,
                                       int &sliceDirection);
   ...
   *\see Volume
   */
  virtual UniformVolume::SmartPtr ConstructVolume(
   ...
   */
  const char *CheckImage(const int plane, const ScalarImage *image,
