...
TypedArrayNoiseEstimatorBrummer::TypedArrayNoiseEstimatorBrummer(
  ...
  Histogram<unsigned int>::SmartPtr histogram(
      data->GetHistogram(histogramBinsActual));
  ...
  std::cout << " 1 / sampleCount: " << (1.0 / histogram->SampleCount())
            << std::endl;

  double sumSquaredSampleDiffs = 0;
  for (size_t i = 0; i < histogramBinsActual; i++) {
    sumSquaredSampleDiffs +=
        (*histogram)[i] * pow((histogram->BinToValue(i) - sampleMean), 2);
  }
  std::cout << " sumSquaredSampleDiffs: " << sumSquaredSampleDiffs << std::endl;

  const double sampleStdDev =
      sqrt((1.0 / histogram->SampleCount()) * sumSquaredSampleDiffs);
  std::cout << " sampleStdDev: " << sampleStdDev << std::endl;
  std::cout << " npow: " << pow((double)histogram->SampleCount(), 1 / 5)
            << std::endl;

  /* Find an initial estimate of sigma using the Chang method.
   * (Chang L-C et al 2005 "An automatic method for estimating noise-induced
   * signal variance in magnitude-reconstructed magnetic resonance images")
   */
  const double changH = 1.0 / (1.06 * sampleStdDev *
                               pow((double)histogram->SampleCount(), 1 / 5));
  double changSigma = 0;
  double sigmaUpperBound = 100;
  double valueToMaximize = std::numeric_limits<double>::min();
  for (double maybeChangSigma = 1.0; maybeChangSigma < sigmaUpperBound;
       maybeChangSigma += 1.5) {
    double changSum = 0;
    for (size_t i = 0; i < histogramBinsActual; i++) {
      const double valToAdd =
          ((*histogram)[i] * (1.0 / sqrt(2.0 * M_PI)) *
           exp((-1.0 *
                log(pow((double)(maybeChangSigma - histogram->BinToValue(i)) /
                            changH,
                        2) /
                    2.0))));
      changSum += valToAdd;
    ...
    double curValueToMaximize =
        (1.0 / (histogram->SampleCount() * changH)) * changSum;
    ...
    if (curValueToMaximize > valueToMaximize) {
      ...
      std::cout << "New max: " << curValueToMaximize
                << " New changSigma: " << changSigma << std::endl;
    }
  }
  ...
  for (double maybeSigma = 0; maybeSigma < histogramBinsActual;
       maybeSigma += 1.0) {
    for (double maybeAmplitude = 0; maybeAmplitude < maxAmplitude;
         maybeAmplitude += 10.0) {
      double brummerSum = 0;
      for (int i = 0; i <= 2 * sigma0; i++) {
        brummerSum +=
            log(pow((double)(*histogram)[i] -
                        maybeAmplitude *
                            (histogram->BinToValue(i) / pow(maybeSigma, 2)) *
                            exp(-1 * (pow((double)histogram->BinToValue(i), 2) /
                                      (2 * pow(maybeSigma, 2)))),
                    2));
      }
      std::cout << " sigma: " << maybeSigma << " K: " << maybeAmplitude
                << " brummerSum: " << brummerSum << std::endl;
...
double TypedArrayNoiseEstimatorBrummer::SijbersBiasHat(
  ...
  const double denom =
      exp(-1 *
          (log((double)pow(histogram->BinToValue(0), 2) / twoSigmaSquared))) -
      exp(-1 *
          (log((double)pow(histogram->BinToValue(K), 2) / twoSigmaSquared)));
  ...
  for (int i = 1; i < K; i++) {
    const double quotient =
        (exp(-1 * (log((double)pow(histogram->BinToValue(i - 1), 2))) /
             twoSigmaSquared) -
         exp(-1 * (log(pow((double)histogram->BinToValue(i), 2))) /
             twoSigmaSquared)) /
        denom;
   ...
   */
  for (size_t i = static_cast<size_t>(K); i < histogram->GetNumberOfBins();
       i++) {
    const double quotient =
        (exp(-1 * (log(pow((double)histogram->BinToValue(i - 1), 2))) /
             twoSigmaSquared) -
         exp(-1 * (log(pow((double)histogram->BinToValue(i), 2))) /
             twoSigmaSquared)) /
        denom;
   ...
   *  from above.
   */
  const double biasHat = (lambdaStar - (K - 2 + M)) / sqrt(K - 2 + M);
...
double TypedArrayNoiseEstimatorBrummer::SijbersLogLikelihood(
  ...
  for (int i = 0; i < K; i++) {
    ...
    std::cout << exp(-1 * (log(pow(histogram->BinToValue(i - 1), 2)) / s2))
              << std::endl;
    std::cout << exp(-1 * (log(pow(histogram->BinToValue(i), 2)) / s2))
              << std::endl;
    std::cout << exp(-1 * (log(pow(histogram->BinToValue(0), 2)) / s2))
              << std::endl;
    std::cout << exp(-1 * (log(pow(histogram->BinToValue(K), 2)) / s2))
              << std::endl;
...
double TypedArrayNoiseEstimatorBrummer::SijbersVarHat(
    const Histogram<unsigned int>::SmartPtr histogram, const double sigmaHat,
    const int numBinsToUse) {
  double h = 0.01;

  const double fXminusH =
      SijbersLogLikelihood(histogram, sigmaHat - h, numBinsToUse);
  const double fX = SijbersLogLikelihood(histogram, sigmaHat, numBinsToUse);
  const double fXplusH =
      SijbersLogLikelihood(histogram, sigmaHat + h, numBinsToUse);
  ...
  const double secondDerivOfLogLikelihood =
      (fXminusH - 2 * fX + fXplusH) / (h * h);
...
double TypedArrayNoiseEstimatorBrummer::EstimateNumBinsToUse(
  ...
  for (int curK = 1; curK < maxK; curK++) {
    const double curKMinimizer = SijbersBiasHat(histogram, sigmaHat, curK) +
                                 SijbersVarHat(histogram, sigmaHat, curK);
