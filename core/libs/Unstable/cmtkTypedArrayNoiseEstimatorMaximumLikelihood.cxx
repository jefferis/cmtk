...
TypedArrayNoiseEstimatorMaximumLikelihood::
    TypedArrayNoiseEstimatorMaximumLikelihood(const TypedArray *data,
                                              const size_t histogramBins) {
  //  Histogram<unsigned int>::SmartPtr histogram( data->GetHistogram(
  //  histogramBins ) );
  UNUSED(histogramBins);

  size_t overriddenHistogramBins = 255;
  Histogram<unsigned int>::SmartPtr histogram(
      data->GetHistogram(overriddenHistogramBins));

  // Find first maximum
  size_t idx = 0;
  while ((idx < (overriddenHistogramBins - 1)) &&
  ...
  while (sqrt(pow(maxLikelySigma - prevMaxLikelySigma, 2)) > sigmaThresh) {
    std::cout << "K: " << numBinsToUse
              << "\tprevMaxLikelySigma: " << prevMaxLikelySigma
              << "\tmaxLikelySigma: " << maxLikelySigma << std::endl;
    ...
    for (double sigma = 1; sigma < sigmaUpperBound; sigma += stepSize) {
      ...
      for (int j = 1; j <= numBinsToUse; j++) {
        const double curAddend =
            (*histogram)[j] *
            (exp(-1 * (pow((double)(histogram->BinToValue(j - 1)), 2) /
                       twoSigmaSquared)) -
             exp(-1 * (pow((double)(histogram->BinToValue(j)), 2) /
                       twoSigmaSquared)));
        sumForSearch += curAddend;
      }

      const double curSigmaMinimizer = (exp(-bin0Squared / twoSigmaSquared) -
                                        exp(-binKSquared / twoSigmaSquared)) -
                                       sumForSearch;
    ...
    numBinsToUse = static_cast<int>(
        Superclass::EstimateNumBinsToUse(data, histogram, maxLikelySigma));
  }
