 ...
 *
 * This class implements some side effects that are not strictly part of the
 *volume injection algorithm, but which supply functionality to the derived
 * igsInverseInterpolationVolumeReconstructionBase class. These include
 *computation of regional minimum/maximum intensity ranges, between-pass
 *registration, and KLD metric computation.
 *
...
class VolumeInjectionReconstruction {
 public:
   ...
   */
  VolumeInjectionReconstruction(const UniformVolume *originalImage,
                                const Types::GridIndexType numberOfPasses,
                                const int interleaveAxis);

  /** Constructor for general volume reconstruction from multiple acquired
   * images.
   */
  VolumeInjectionReconstruction(const UniformVolume *reconstructionGrid,
  ...
  static int GuessInterleaveAxis(
      const UniformVolume *image /*!< The interleaved image.*/, const int
                                                                    defaultAxis = 2 /*!< In case all guessing fails, this is the default axis we return.*/);

  /** Compute transformations between the reference image grid and the original
   *pass images. The resulting transformations are stored in the
   *m_TransformationsToPassImages vector. If a high-resolution reference image
   *is set in m_ReferenceImage, then all subimages are registered to it.
   * Otherwise, the 0th subimage is used as the reference and the remaining
   *subimages are registered to it. In this case, the transformation for the 0th
   *subimage is set as the identity transformation. \param registrationMetric
   *Similarity metric for registration of the passes to the reference image.
   */
  ...
  void SetTransformationsToPassImages(
      std::vector<Xform::SmartPtr> &transformations) {
  ...
  void VolumeInjectionIsotropic(
      const Types::Coordinate kernelSigma /*!< Gaussian kernel sigma (standard
                                             deviation) parameter*/
      ,
  ...
  void VolumeInjectionAnisotropic(const Types::Coordinate
                                      kernelSigmaFactor /*!< Gaussian kernel
                                                           sigma (standard
                                                           deviation) factor
                                                           (multiple of
                                                           per-dimension pass
  ...
  /** Set pass weight.
   * Each pass weight should be between 0 and 1. If the weight for a pass is
   * zero, then that pass is effectively excluded from the reconstruction. This
   * can be useful if one of the passes shows severe within-pass motion
   * artifacts that would otherwise disturb the across-pass correction.
   *
   * By default, all pass weights are set to 1, i.e., all passes contribute
   * equally.
   */
  ...
  /// Get Kullback-Leibler Divergence between intensity distributions in
  /// original and corrected image.
  ap::real_value_type GetOriginalToCorrectedImageKLD(
 ...
 protected:
  ...
  /// Relative weights of the passes in the correction; can be used to
  /// underweight or even exclude passes.
  ...
  /// Affine transformations that map FROM the corrected image TO each of the
  /// subimages.
   ...
   * Side effect: this function first computes the Laplacian image, which is
   * stored in m_CorrectedImageLaplacian for use in the
   * AddLaplacianGradientImage function.
   */
  ap::real_value_type
  ComputeCorrectedImageLaplacianNorm(const ap::
                                         real_1d_array &correctedImagePixels /*!< Current vector of corrected image pixels.*/);

  /// Add weighted gradient image of Laplacian to already computed cost function
  /// gradient.
  void AddLaplacianGradientImage(ap::real_1d_array &g,
