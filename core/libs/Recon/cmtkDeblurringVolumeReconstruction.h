...
class DeblurringVolumeReconstruction :
 ...
 public:
   ...
   *\param psfScale Optional spatial scale factor for the PSF. Values larger
   *than 1 increase PSF size, smaller than one decrease it.
   */
  DeblurringVolumeReconstruction(const UniformVolume *originalImage,
   ...
   * images.
   */
  DeblurringVolumeReconstruction(const UniformVolume *reconstructionGrid,
 ...
 private:
  ...
  void GetBoundingBoxOfXformedPassNeighborhood(
      ...
      const Vector3D &currentPassVoxel, const TPSF *psf,
