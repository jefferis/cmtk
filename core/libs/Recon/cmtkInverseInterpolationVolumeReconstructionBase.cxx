...
InverseInterpolationVolumeReconstructionBase ::
    ...
    : VolumeInjectionReconstruction(originalImage, interleaveFactor,
                                    interleaveAxis),
...
double
InverseInterpolationVolumeReconstructionBase ::ComputeApproximationError() {
  ...
  for (int pass = 0; pass < this->m_NumberOfPasses; ++pass) {
    UniformVolume::SmartPtr differencePassImage(
        this->m_InterpolatedPassImages[pass]->CloneGrid());
    differencePassImage->CreateDataArray(TYPE_FLOAT, true /*setToZero*/);

    const int numberOfPixels =
        this->m_InterpolatedPassImages[pass]->GetNumberOfPixels();
    ...
    for (int idx = 0; idx < numberOfPixels; ++idx) {
      ...
      if (this->m_InterpolatedPassImages[pass]->GetDataAt(interpolatedData,
                                                          idx)) {
        ...
        this->m_MaximumError =
            std::max<double>(fabs(difference), this->m_MaximumError);
        ++totalNumberOfPixels;
      } else {
...
void InverseInterpolationVolumeReconstructionBase ::Optimize(
  ...
  for (int i = 1; i <= numberOfPixels; ++i) {
    nbd(i) = nbdAll;
    if (this->m_NeighorhoodMinPixelValues(i) >
  ...
  int info;
  ap::lbfgsbminimize(this->m_FunctionAndGradient, numberOfPixels, 5, x,
                     1e-10 /*epsg*/, 1e-10 /*epsf*/, 1e-10 /*epsx*/,
                     numberOfIterations, nbd, this->m_NeighorhoodMinPixelValues,
                     this->m_NeighorhoodMaxPixelValues, info);
