 ...
 *
 * This class is specifically designed to use the L-BFFS-B Optimizer from
 *alglib.net.
 *
...
class InverseInterpolationVolumeReconstructionBase :
 ...
 public:
   ...
   * This function computed the difference images between the original and the
   *interpolated pass images, and it also computes from these the mean squared
   *and the maximum approximation errors.
   *
   *\return The approximation error, i.e., the sum of squared differences of the
   *intensities in the original pass images and the corresponding intensities in
   *the images interpolated from the current estimated corrected image.
   */
  ...
  /// Return MSD error.
  double GetMeanSquaredError() const { return this->m_MeanSquaredError; }
 ...
 protected:
  ...
  /// Difference Subimages between original and interpolated Subimages, used for
  /// error- and gradientcalculation.
  std::vector<UniformVolume::SmartPtr> m_DifferencePassImages;

  /** Flag for fourth order error optimization.
   * If this flag is set, the error optimized is the fourth order error,
   * otherwise the second order (square) error.
   */
  ...
  /// Mean squared error of the approximation, i.e., MSD between interpolated
  /// and actual pass images.
  double m_MeanSquaredError;

  /// Maximum error of the approximation, i.e., maximum difference between
  /// interpolated and actual pass images.
