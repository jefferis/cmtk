...
void lbfgsbbmv(const int &, const ap::real_2d_array &sy, ap::real_2d_array &wt,
...
void lbfgsbcauchy(const int &n, const ap::real_1d_array &x,
                  ...
                  const ap::real_1d_array &, const ap::real_1d_array &,
...
void lbfgsbformk(const int &n, const int &nsub, const ap::integer_1d_array &ind,
  ...
  if (updatd) {
    if (iupdat > m) {
      for (jy = 1; jy <= m - 1; jy++) {
        js = m + jy;
        ap::vmove(wn1.getcolumn(jy, jy, m - 1),
                  wn1.getcolumn(jy + 1, jy + 1, m));
        ap::vmove(wn1.getcolumn(js, js, js + m - jy - 1),
                  wn1.getcolumn(js + 1, js + 1, js + m - jy));
        ap::vmove(wn1.getcolumn(jy, m + 1, m + m - 1),
  ...
  for (j = 1; j <= col; j++) {
    ap::vmove(workmat.getrow(j, 1, col),
              wn.getrow(col + j, col + 1, col + col));
  }
  info = 0;
  ...
  for (j = 1; j <= col; j++) {
    ap::vmove(wn.getrow(col + j, col + 1, col + col),
              workmat.getrow(j, 1, col));
  }
}

void lbfgsbformt(const int &, ap::real_2d_array &wt,
...
void lbfgsblnsrlb(const int &n, const ap::real_1d_array &l,
  ...
  lbfgsbdcsrch(f, gd, stp, ftol, gtol, xtol, ap::real_value_type(0), stpmx,
               csave, isave, dsave, addinfo);
  xstep = stp * dnrm;
  if (csave != 4 && csave != 3) {
    task = 1;
    ifun = ifun + 1;
...
void lbfgsbsubsm(const int &, const int &m, const int &nsub,
...
void lbfgsbdcsrch(const ap::real_value_type &f, const ap::real_value_type &g,
  ...
  while (true) {
    ...
    if (stage == 1 && f <= fx && f > ftest) {
      ...
      gxm = gx - gtest;
      gym = gy - gtest;
      lbfgsbdcstep(stx, fxm, gxm, sty, fym, gym, stp, fm, gm, brackt, stmin,
                   stmax);
      fx = fxm + stx * gtest;
      fy = fym + sty * gtest;
      gx = gxm + gtest;
  ...
  if (fp > fx) {
    ...
    if (sgnd < 0) {
      ...
      if (fabs(dp) < fabs(dx)) {
        theta = 3 * (fx - fp) / (stp - stx) + dx + dp;
        s = ap::maxreal(fabs(theta), ap::maxreal(fabs(dx), fabs(dp)));
        gamma = s * sqrt(ap::maxreal(ap::real_value_type(0),
                                     ap::sqr(theta / s) - dx / s * (dp / s)));
        if (stp > stx) {
          gamma = -gamma;
        }
        p = gamma - dp + theta;
...
bool additionallbfgsbstoppingcriterion(int, const ap::real_1d_array &,
                                       ap::real_value_type,
...
void lbfgsbdtrsl(ap::real_2d_array &t, const int &n, ap::real_1d_array &b,
  ...
  if (cse == 3) {
    ...
    for (jj = 2; jj <= n; jj++) {
      j = n - jj + 1;
      v = ap::vdotproduct(t.getcolumn(j, j + 1, j + 1 + jj - 1 - 1),
...
void lbfgsbnewiteration(const ap::real_1d_array &, ap::real_value_type,
                        const ap::real_1d_array &) {}
...
void lbfgsbminimize(FunctionAndGradient *const functionAndGradient,
  ...
  while (true) {
    ...
    if (!cnstnd && col > 0) {
      ...
      ap::lbfgsbcauchy(n, x, l, u, nbd, g, indx2, iwhere, t, d, z, m, wy, ws,
      ...
      nintol = nintol + nint;
      ap::lbfgsbfreev(n, nfree, index, nenter, ileave, indx2, iwhere, wrk,
                      updatd, cnstnd, iter);
    }
    if (nfree != 0 && col != 0) {
      if (wrk) {
        ap::lbfgsbformk(n, nfree, index, nenter, ileave, indx2, iupdat, updatd,
                        wn, snd, m, ws, wy, sy, theta, col, head, internalinfo,
                        workvec, workmat);
      }
      if (internalinfo != 0) {
        internalinfo = 0;
      ...
      ap::lbfgsbcmprlb(n, m, x, g, ws, wy, sy, wt, z, r, wa, index, theta, col,
                       head, nfree, cnstnd, internalinfo, workvec, workvec2);
      if (internalinfo == 0) {
        ap::lbfgsbsubsm(n, m, nfree, index, l, u, nbd, z, r, ws, wy, theta, col,
                        head, iword, wa, wn, internalinfo);
      }
      if (internalinfo != 0) {
        internalinfo = 0;
        col = 0;
    ...
    task = 0;
    while (true) {
      ap::lbfgsblnsrlb(n, l, u, nbd, x, f, fold, gd, gdold, g, d, r, t, z, stp,
                       dnrm, dtd, xstep, stpmx, iter, ifun, iback, nfgv,
    ...
    ddum =
        ap::maxreal(fabs(fold), ap::maxreal(fabs(f), ap::real_value_type(1)));
    if (fold - f <= epsf * ddum) {
      info = 1;
      return;
    }
    ...
    if (dr <= ap::machineepsilon * ddum) {
      ...
      updatd = true;
      iupdat = iupdat + 1;
      ap::lbfgsbmatupd(n, m, ws, wy, sy, ss, d, r, itail, iupdat, col, head,
                       theta, rr, dr, stp, dtd);
