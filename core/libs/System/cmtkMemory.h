...
class ArrayC {
 public:
   ...
   * This function provides a universal signature that we can use for function
   * pointers.
   */
  static void DeleteWrapper(void *const array) { free(array); }
};
 ...
 */
template <class T>
inline void Set(T *const to, const T value, const size_t length) {
  for (size_t i = 0; i < length; ++i) to[i] = value;
}
 ...
 */
template <class T>
inline void Copy(T *const to, const T *from, const size_t length) {
