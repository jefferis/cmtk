...
void cmtk::CommandLine::KeyToAction ::SetProperties(const long int properties) {
  this->m_Properties = properties;
}

long int cmtk::CommandLine::KeyToAction ::GetProperties() const {
  return this->m_Properties;
}

mxml_node_t *cmtk::CommandLine::KeyToAction ::MakeXML(
    mxml_node_t *const node) const {
  if (!(this->m_Properties & PROPS_NOXML)) {
    // for some reason Slicer does not accept long options that contain hyphens
    // ("-"), so we replace them.
    std::string xmlKeyStr = this->m_Key.m_KeyString;
    for (size_t i = 0; i < xmlKeyStr.length(); ++i) {
      if (xmlKeyStr[i] == '-') xmlKeyStr[i] = '_';
    }
    ...
    if (this->m_Key.m_KeyString.length()) {
      Coverity::FakeFree(mxmlNewText(mxmlNewElement(node, "longflag"), 0,
                                     (std::string("--") + xmlKeyStr).c_str()));
    }
...
void cmtk::CommandLine::KeyToAction ::FormatHelp(
  ...
  if (this->m_Comment.length()) {
    ...
    if (fmt.str().length() >
        static_cast<size_t>(CommandLine::HelpTextIndent - 2))
      fmt << "\n";
    else {
      while (fmt.str().length() <
             static_cast<size_t>(CommandLine::HelpTextIndent))
        fmt << " ";
    }
...
void cmtk::CommandLine::KeyToAction ::PrintWikiWithPrefix(
...
void cmtk::CommandLine::KeyToAction ::PrintManWithPrefix(
    const std::string &prefix) const {
  if (this->m_Comment.length()) {
    const std::string &typeInfo = this->GetActionTypeInfo();
...
bool cmtk::CommandLine::KeyToAction ::MatchLongOption(
    const std::string &key) const {
  if (key.length() != this->m_Key.m_KeyString.length()) return false;

  for (size_t i = 0; i < key.length(); ++i) {
    if ((key[i] == '-' || key[i] == '_') &&
        (this->m_Key.m_KeyString[i] == '-' ||
         this->m_Key.m_KeyString[i] == '_'))
      continue;

    if (key[i] != this->m_Key.m_KeyString[i]) return false;
  }
  return true;
}
