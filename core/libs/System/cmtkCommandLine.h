 ...
 * This class provides functionality for command line argument parsing,
 *including automatic generation of help texts and XML self-description
 *according to the specification of the Slicer3 execution model
 ...
 *
 *  A command line switch does not have an argument itself but internally sets a
 *variable to a pre-defined value.
 *
 * Example:
 *
 * \code
 ...
 *
 * A command line option has an argument, which is evaluated and stored in a
 *variable.
 *
 * Example:
 *
 * \code
 * int iterations = 0;
 * cl.AddOption( CommandLine::Key( "iterations" ), &iterations, "Number of
 *iterations" ); \endcode
 *
 ...
 * }
 * cl.AddCallback( Key( "do-something" ), &callback, "Do something using a
 *callback function" ); \endcode
 *
 *\section secEnumeration Enumerations
 *
 * An enumeration is a group of options that modify the same variable by setting
 ...
 *"interleave-axis", &InterleaveAxis, "Define interleave axis." );
 *  interleaveGroup->AddSwitch( Key( "guess-from-input" ), -1, "Guess from input
 *image" ); interleaveGroup->AddSwitch( Key( 'a', "axial" ), 2, "Interleaved
 *axial images" ); interleaveGroup->AddSwitch( Key( 'c', "coronal" ), 1,
 *"Interleaved coronal images" ); interleaveGroup->AddSwitch( Key( 's',
 *"sagittal" ), 0, "Interleaved sagittal images" ); \endcode
 *
 * \code
 *  std::string channel( "spgr" );
 *  cmtk::CommandLine::EnumGroup<std::string>::SmartPtr channelGroup =
 *cl.AddEnum( "RegistrationChannel", &channel, "MR channel." );
 *  channelGroup->AddSwitch( Key( "spgr" ), "spgr", "SPGR (T1-weighted)
 *structural channel" ); channelGroup->AddSwitch( Key( "early-fse" ), "erly",
 *"Early-echo (PD-weighted) fast spin echo channel" ); channelGroup->AddSwitch(
 *Key( "late-fse" ), "late", "Late-echo (T2-weighted) fast spin echo channel" );
 * \endcode
 */
class CommandLine :
 ...
 public:
  ...
  typedef enum {
    ...
    PROPS_IMAGE = 32,
    /// When used with PROPS_IMAGE, this means the expected image is a label
    /// map.
    PROPS_LABELS = 64,
   ...
   * A key is the character (short options) or string (long options) that is
   * associated with a given action. When the key appears on the command line,
  ...
  class Key {
   public:
    ...
    explicit Key(const std::string &keyString)
        : m_KeyChar(0), m_KeyString(keyString) {}

    /// Short and long option constructor.
    explicit Key(const char keyChar, const std::string &keyString)
        : m_KeyChar(keyChar), m_KeyString(keyString) {}
  ...
  class Item {
   public:
    ...
    /// Get item properties.
    virtual long int GetProperties() const { return this->m_Properties; }

    /// Set an attribute.
    virtual Item *SetAttribute(const std::string &key,
    ...
    virtual void Evaluate(const size_t argc, const char *argv[],
                          size_t &index) = 0;

    /// Virtual function that returns an XML tree describing this option.
    virtual mxml_node_t *MakeXML(
        mxml_node_t *const
            parent /*!< Parent in the XML tree for the new node.*/) const = 0;

    /// Virtual function returns a string that describes the parameter type
    /// associated with this option (derived classes only)
    ...
    /// Return true if and only if this item is the default for the associated
    /// action or variable.
   ...
   protected:
    ...
    class Helper {
     public:
      /// Make a basic XML node for a generic item based on type, properties,
      /// attribute, etc.
      static mxml_node_t *MakeXML(const Item *item, mxml_node_t *const parent);

      /// Return a textual description of the parameter associated with this
      /// option
 ...
 private:
  ...
  class Switch :
   ...
   public:
    /// Constructor.
    Switch(T *const field, const T value) : m_Field(field), m_Value(value) {}
    ...
    virtual std::ostringstream& PrintHelp( std::ostringstream& fmt /*!< Stream that the additional help information is formatted into*/ ) const
    {
      if (this->IsDefault()) fmt << "\n[This is the default]";
      return fmt;
    }

    /// Format additional help information (e.g., default values).
    virtual void PrintWiki() const {
      if (this->IsDefault()) StdOut << " '''[This is the default]'''";
    }

    /// Format additional man page information (e.g., default values).
    virtual void PrintMan() const {
      if (this->IsDefault()) StdOut << "\\fB[This is the default]\\fR\n";
    }
  ...
  class Vector :
   ...
   private:
    ...
    /// Has this vector option been used already? This is so we clear the vector
    /// exactly once.
    bool m_HasBeenUsed;
  };
  ...
  class NonOptionParameter :
   ...
   public:
    ...
    /// Constructor.
    NonOptionParameter(std::string *const var, const std::string &name,
                       const std::string &comment, bool *const flag)
        : Superclass(var, flag), m_Name(name), m_Comment(comment){};
  ...
  class NonOptionParameterVector :
   ...
   public:
    ...
    /// Constructor.
    NonOptionParameterVector(std::vector<std::string> *pvec,
                             const std::string &name,
                             const std::string &comment, bool *const flag)
        : Superclass(pvec, flag), m_Name(name), m_Comment(comment){};
 ...
 public:
  ...
  /// Destructor: spit out a warning if there are unused extra arguments on the
  /// command line.
  ~CommandLine();
  ...
  class KeyToAction {
   public:
    ...
    /// Constructor.
    KeyToAction(const Key &key /*!< Key: long and/or short command line option
                                  for this action.*/
                ,
                const std::string
                    &comment /*!< Command line help comment for this action.*/)
        : m_Key(key), m_Comment(comment), m_Properties(PROPS_XML) {}

    /// Virtual destructor.
    virtual ~KeyToAction(){};

    /// Test long key from command line and execute if match.
    virtual bool MatchAndExecute(
        const std::string &key /*!< Key (long option) from the command line.*/,
    ...
    virtual bool MatchAndExecute(
        const char keyChar /*!< Key (long option) from the command line.*/,
        const size_t argc /*!< Total number of command line arguments.*/,
        const char *argv[] /*!< Command line argument list.*/,
        size_t &index /*!< Current index in command line list*/) = 0;
    ...
    virtual mxml_node_t *MakeXML(
        mxml_node_t *const
            parent /*!< Parent in the XML tree for the new node.*/) const;

    /// Print help for this item.
    virtual void PrintHelp(
        const size_t globalIndent = 0 /*!< Indent by this many characters.*/,
        const bool advanced =
            false /*!< Flag: include advanced help content. */) const = 0;
   ...
   protected:
    ...
    virtual std::string GetActionTypeInfo() const { return ""; }
    ...
    /** Match two long options but be tolerant to hyphens, i.e., consider '-'
     *and '_' the same. This allows us to be tolerant with Slicer's requirement
     *that there are no hyphens in long options, while maintaining the ability
     *to use them on the command line for compatibility. \return true is the two
     *string match, or their only differences are hyphens vs. underlines.
     */
  ...
  /// Local class that connects command line options with single action
  /// evaluators.
  class KeyToActionSingle :
   ...
   public:
    ...
    /// Constructor.
    KeyToActionSingle(
        const Key &key /*!< Key: long and/or short command line option for this
                          action.*/
        ,
        Item::SmartPtr
        ...
        const std::string
            &comment /*!< Command line help comment for this action.*/)
        : KeyToAction(key, comment), m_Action(action) {}

    /// Virtual destructor.
    virtual ~KeyToActionSingle(){};

    /// Test long key from command line and execute if match.
    bool MatchAndExecute(
        const std::string &key /*!< Key (long option) from the command line.*/,
        const size_t argc /*!< Total number of command line arguments.*/,
        const char *argv[] /*!< Command line argument list.*/,
    ...
    virtual void PrintHelp(
        const size_t globalIndent = 0 /*!< Indent by this many characters.*/,
        const bool advanced =
            false /*!< Flag: include advanced help content. */) const;
  ...
  class KeyToActionEnum :
   ...
   public:
     ...
     * An enumeration parameter group is a group of parameters that all modify
     * the same variable by setting it to different values. There is one command
     * line parameter (i.e., key/action pair) per value, plus a group parameter,
     * which sets the variable based on the name of one of the supported values.
     */
    KeyToActionEnum(
        const Key &key /*!< Key: long and/or short command line option for this
                          action.*/
        ,
        EnumGroupBase::SmartPtr
            keyToActionEnum /*!< The definition of the enumeration keys and
                               values.*/
        ,
        const std::string
            &comment /*!< Command line help comment for this action.*/)
        : KeyToAction(key, comment), m_EnumGroup(keyToActionEnum) {}
  ...
  class EnumGroup :
   ...
   public:
    ...
    /// Constructor.
    EnumGroup(TDataType *const
                  variable /*!< The variable handled by this enum group.*/)
        : m_Variable(variable) {}

    /// Add switch to this group.
    Item::SmartPtr &AddSwitch(const Key &key, const TDataType &value,
                              const std::string &comment) {
      KeyToActionSingle::SmartPtr keyToAction(new KeyToActionSingle(
          key, Item::SmartPtr(new Switch<TDataType>(this->m_Variable, value)),
          comment));
  ...
  /// Add an enum group.
  template <class TDataType>
  typename EnumGroup<TDataType>::SmartPtr AddEnum(const std::string &name,
                                                  TDataType *const variable,
                                                  const std::string &comment) {
    typename EnumGroup<TDataType>::SmartPtr enumGroup(
        new EnumGroup<TDataType>(variable));
    KeyToActionEnum::SmartPtr keyToAction(
        new KeyToActionEnum(Key(name), enumGroup, comment));
  ...
  /// Add switch.
  template <class T>
  Item::SmartPtr AddSwitch(const Key &key, T *const var, const T value,
                           const std::string &comment) {
    return this
        ->AddKeyAction(KeyToActionSingle::SmartPtr(new KeyToActionSingle(
            key, Item::SmartPtr(new Switch<T>(var, value)), comment)))
        ->m_Action;
  }

  /// Add option.
  template <class T>
  Item::SmartPtr AddOption(const Key &key, T *const var,
                           const std::string &comment,
                           bool *const flag = NULL) {
    return this
        ->AddKeyAction(KeyToActionSingle::SmartPtr(new KeyToActionSingle(
            key, Item::SmartPtr(new Option<T>(var, flag)), comment)))
        ->m_Action;
  }

  /// Add list option (put arguments from repeated uses into a FIFO list).
  template <class T>
  Item::SmartPtr AddList(const Key &key, std::list<T> &list,
                         const std::string &comment) {
    return this
        ->AddKeyAction(KeyToActionSingle::SmartPtr(new KeyToActionSingle(
            key, Item::SmartPtr(new List<T>(list)), comment)))
        ->m_Action;
  }

  /// Add vector option (breaks argument into elements of a vector).
  template <class T>
  Item::SmartPtr AddVector(const Key &key, std::vector<T> &vector,
                           const std::string &comment) {
    return this
        ->AddKeyAction(KeyToActionSingle::SmartPtr(new KeyToActionSingle(
            key, Item::SmartPtr(new Vector<T>(vector)), comment)))
        ->m_Action;
  }

  /// Add callback.
  Item::SmartPtr AddCallback(const Key &key, CallbackFunc func,
                             const std::string &comment) {
    return this
        ->AddKeyAction(KeyToActionSingle::SmartPtr(new KeyToActionSingle(
            key, Item::SmartPtr(new Callback(func)), comment)))
        ->m_Action;
  }

  /// Add callback with a single argument.
  template <class TArg>
  Item::SmartPtr AddCallback(const Key &key, void (*funcArg)(const TArg),
                             const std::string &comment) {
    return this
        ->AddKeyAction(KeyToActionSingle::SmartPtr(new KeyToActionSingle(
            key, Item::SmartPtr(new Callback(funcArg)), comment)))
        ->m_Action;
  }

  /// Add callback with multiple arguments.
  Item::SmartPtr AddCallback(const Key &key, CallbackFuncMultiArg funcMultiArg,
                             const std::string &comment) {
    return this
        ->AddKeyAction(KeyToActionSingle::SmartPtr(new KeyToActionSingle(
            key, Item::SmartPtr(new Callback(funcMultiArg)), comment)))
        ->m_Action;
  }

  /// Add single non-option parameter.
  Item::SmartPtr AddParameter(std::string *const var, const std::string &name,
                              const std::string &comment,
                              bool *const flag = NULL) {
    NonOptionParameter::SmartPtr parameter(
        new NonOptionParameter(var, name, comment, flag));
    this->m_NonOptionParameterList.push_back(parameter);
    return parameter;
  }

  /// Add vector of non-option parameters.
  Item::SmartPtr AddParameterVector(std::vector<std::string> *pvec,
                                    const std::string &name,
                                    const std::string &comment,
                                    bool *const flag = NULL) {
    NonOptionParameterVector::SmartPtr vparameter(
        new NonOptionParameterVector(pvec, name, comment, flag));
  ...
  /// Reference to complete key/action list (all groups combined), which is used
  /// for key lookup.
  KeyActionListType m_KeyActionListComplete;

  /// Add an item to applicable key/action lists.
  KeyToActionSingle::SmartPtr AddKeyAction(
      const KeyToActionSingle::SmartPtr &keyAction) {
  ...
  /// Add an item to applicable key/action lists.
  KeyToActionEnum::SmartPtr AddKeyAction(
      const KeyToActionEnum::SmartPtr &keyAction) {
    this->m_KeyActionList->push_back(keyAction);
    this->m_KeyActionListComplete.push_back(keyAction);
    return keyAction;
  }

  /// Type for action groups, which map a group name to the group's key-action
  /// list.
  class KeyActionGroupType {
   public:
    ...
    KeyActionGroupType(const std::string &name, const std::string &description)
        : m_Name(name), m_Description(description), m_Properties(PROPS_XML){};
  ...
  KeyActionGroupType::SmartPtr &BeginGroup(const std::string &name,
  ...
  /// Print help text.
  void PrintHelp(const bool advanced =
                     false /*!< Flag: include advanced help content.*/) const;
   ...
   */
  const char *GetNext() {
    if (Index >= ArgC) throw(Exception("Out of arguments.", Index));
    return ArgV[Index++];
  }
 ...
 private:
  ...
  /// Type for no-option parameter vector list. These come after the scalar
  /// non-option parameters.
  typedef std::vector<NonOptionParameterVector::SmartPtr>
      NonOptionParameterVectorListType;
  ...
  mxml_node_t *AddProgramInfoXML(
      mxml_node_t *const parent /*!< Parent node for new entry in XML tree.*/,
