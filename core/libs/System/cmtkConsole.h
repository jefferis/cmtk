...
class Console :
 ...
 public:
  /// Constructor.
  Console(std::ostream *stream) : m_StreamP(stream) { this->IndentLevel = 0; }

  /** Get terminal line width, if possible.
   * The line width is determined using an ioctl() call, if available. Line
   * width can be set (or overridden) by the user by setting the
   * "CMTK_CONSOLE_LINE_WIDTH" environment variable to the desired number of
   * characters per line.
   */
  ...
  Console &FormatText(
      const std::string &text /*!< The text to format with line breaks.*/, const size_t margin = 0 /*!< Left margin: this many space characters are printed at the beginning of each line.*/, const size_t width = 0 /*<! Line width including margin. Default: determine based on terminal width, or default to 80 if that fails.*/, const int firstLine = 0 /*!< Relative indent of first line, i.e., firstLine = -margin means no indentation of first line.*/);
