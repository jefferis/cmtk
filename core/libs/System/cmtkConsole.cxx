...
size_t Console::GetLineWidth() const {
  ...
  struct winsize sz;
  if (ioctl(0, TIOCGWINSZ, &sz) >= 0) return sz.ws_col;
#endif
  // default to 80
  return 80;
}

Console &Console::FormatText(const std::string &text, const size_t margin,
                             const size_t width, const int firstLine) {
  // Set current indentation to first line.
  size_t currentIndent =
      static_cast<size_t>(std::max<int>(0, margin + firstLine));
  ...
  size_t length =
      static_cast<size_t>(std::max<int>(1, actualWidth - currentIndent)) - 1;

  // loop while text left to write
  std::string remaining = text;
  while (remaining.length() > length) {
    // first, see if we have forced line breaks within the first "length"
    // characters.
...
void Console::Indent(size_t level) {
  if (!level) level = this->IndentLevel;

  for (size_t i = 0; i < level; ++i) (*this) << " ";
}
