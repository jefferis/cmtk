...
double cmtk::Timers::GetTimeProcess() {
  ...
  if (times(&t))
#ifdef _SC_CLK_TCK
    return (double)(t.tms_utime + t.tms_cutime + t.tms_stime + t.tms_cstime) /
           sysconf(_SC_CLK_TCK);
#else
    return (double)(t.tms_utime + t.tms_cutime + t.tms_stime + t.tms_cstime) /
           CLK_TCK;
...
double cmtk::Timers::GetWalltime() { return static_cast<double>(time(NULL)); }
...
double cmtk::Timers::GetTimeThread() {
  ...
  if (fp) {
    ...
    if (fread(&usage, sizeof(usage), 1, fp)) {
      ...
      return ((double)usage.pr_utime.tv_sec) +
             ((double)usage.pr_utime.tv_nsec) / NANOSEC +
             ((double)usage.pr_stime.tv_sec) +
             ((double)usage.pr_stime.tv_nsec) / NANOSEC;
    }
#endif
    fclose(fp);
  }
