...
/// This is the actual low-level thread function. It calls ThreadFunction() for
/// the cmtk::ThreadPoolThreads instance given as the function parameter.
extern "C" CMTK_THREAD_RETURN_TYPE cmtkThreadPoolThreadFunction(
    CMTK_THREAD_ARG_TYPE arg) {
  static_cast<cmtk::ThreadPoolThreads::ThreadPoolThreadsArg *>(arg)
      ->m_Pool->ThreadFunction(
          static_cast<cmtk::ThreadPoolThreads::ThreadPoolThreadsArg *>(arg)
              ->m_Index);
...
void ThreadPoolThreads::StartThreads() {
  if (!this->m_ThreadsRunning) {
    ...
    for (size_t idx = 0; idx < this->m_NumberOfThreads; ++idx) {
      ...
      const int status = pthread_create(
          &this->m_ThreadID[idx], &attr, cmtkThreadPoolThreadFunction,
          static_cast<CMTK_THREAD_ARG_TYPE>(&this->m_ThreadArgs[idx]));
...
ThreadPoolThreads::~ThreadPoolThreads() { this->EndThreads(); }

void ThreadPoolThreads::EndThreads() {
  if (this->m_ThreadsRunning) {
#ifdef CMTK_USE_PTHREADS
...
void ThreadPoolThreads::ThreadFunction(const size_t threadIdx) {
#ifdef _OPENMP
  ...
  while (this->m_ContinueThreads) {
    ...
    // call task function
    this->m_TaskFunction(this->m_TaskParameters[taskIdx], taskIdx,
                         this->m_NumberOfTasks, threadIdx,
...
ThreadPoolThreads &ThreadPoolThreads::GetGlobalThreadPool() {
