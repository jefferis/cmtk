...
class CompressedStream :
 ...
 public:
  ...
  bool IsValid() const { return (this->m_Reader != NULL); }

  /// Return flag whether this stream is actually using decompression or simply
  /// reads a file as-is.
   ...
   */
  void Rewind() { this->m_Reader->Rewind(); }
  ...
  int Get(char &c) { return this->m_Reader->Get(c); }

  /// Return number of bytes read from stream.
  int Tell() const { return this->m_Reader->Tell(); }

  /// Return 1 if and only if end of file reached.
  bool Feof() const { return this->m_Reader->Feof(); }
   ...
   *\return -1 if the file does not exist; 0 if the file exists under the given
   *name; 1 if the file exists with an additional suffix corresponding to one of
   *the supported compression schemes, e.g., ".gz"; 2 if the file exists with
   *both its plain name AND at least one compressed suffix. The last case
   *indicates a potential consistency problem because it is not clear, which
   *file should be read.
   */
 ...
 private:
   ...
   *\param compressedSuffix The file suffix corresponding to input files for the
   *given pipe command, e.g. ".gz" for gzip.
   */
  bool OpenDecompressionPipe(const std::string &filename,
                             const std::string &suffix, const char *command,
 ...
 private:
  ...
  class ReaderBase {
   public:
    ...
    virtual void Rewind() { this->m_BytesRead = 0; }

    /** Set filepointer.
     * This class implements a naive seek that optionally calls "this->Rewind()"
     *(if "whence" is SEEK_SET, then reads "offset" bytes from the input to
     *position read pointer. \param offset Offset the file pointer is set to,
     *depending on the value of whence. \param whence File pointer set mode as
     *defined for fseek.
     */
 ...
 public:
  ...
  class Zlib : public ReaderBase {
   public:
     ...
     * This is simply here as a service to other classes, which may use this as
     *a plug-in replacement for gzwrite() to work around a large-file problem in
     *zlib. \see http://bugs.debian.org/cgi-bin/bugreport.cgi?bug=587912 \remark
     *Yaroslav Halchenko <debian@onerussian.com> pointed this out.
     */
