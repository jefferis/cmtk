...
int RecursiveMkDir(const std::string &filename, const int permissions) {
...
int RecursiveMkPrefixDir(const std::string &filename, const int permissions) {
...
std::string Basename(const std::string &path, const std::string &suffix) {
  ...
  if (!suffix.empty() && (basename.length() >= suffix.length())) {
    if (basename.compare(basename.length() - suffix.length(), suffix.length(),
                         suffix)) {
