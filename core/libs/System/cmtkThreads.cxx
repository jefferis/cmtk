...
int Threads::GetNumberOfThreads() {
  if (!Threads::NumberOfThreads) Threads::CheckEnvironment();

  return Threads::NumberOfThreads;
}

void Threads::SetNumberOfThreads(const long int numberOfThreads) {
  Threads::SetNumberOfThreads(numberOfThreads, true /*force*/);
}

int Threads ::SetNumberOfThreads(const int numberOfThreads, const bool force) {
  if (numberOfThreads) {
    if (force) {
      NumberOfThreads = std::min(numberOfThreads, Threads::GetMaxThreads());
    } else {
      NumberOfThreads =
          std::min(numberOfThreads, Threads::GetNumberOfProcessors());
    }
  } else
    NumberOfThreads =
        std::min(Threads::GetNumberOfProcessors(), Threads::GetMaxThreads());
...
int Threads::GetNumberOfProcessors() {
#ifdef _MSC_VER
...
#ifdef CMTK_USE_GCD
void Threads::RunThreads(ThreadFunction threadCall,
                         const unsigned numberOfThreads, void *const parameters,
                         const size_t parameterSize) {
  dispatch_apply(numberOfThreads, dispatch_get_global_queue(0, 0), ^(size_t i) {
    threadCall(((char *)parameters) + i * parameterSize);
  });
}
#else
void Threads::RunThreads(ThreadFunction threadCall,
                         ...
                         const size_t parameterSize) {
#ifdef _OPENMP
  const int nThreadsOMP =
      std::max<int>(1, 1 + GetNumberOfThreads() - numberOfThreads);
...
#ifdef CMTK_USE_SMP
#ifdef _MSC_VER
    ThreadHandles[threadIdx] = CreateThread(
        NULL /*default security attributes*/, 0 /*use default stack size*/,
        (LPTHREAD_START_ROUTINE)threadCall, threadParameters,
...
#else   // _MSC_VER
    status =
        pthread_create(&Thread[threadIdx], &attr, threadCall, threadParameters);
#endif  // _MSC_VER
#else
...
void Threads::CheckEnvironment() {
  const char *env = getenv("CMTK_NUM_THREADS");
  // check legacy variable
  if (!env) env = getenv("IGS_NUM_THREADS");
  ...
  if (env) {
    ...
    if (numThreads) {
      ...
      std::cerr << "INFO: number of threads set to " << numThreads
                << " according to environment variable CMTK_NUM_THREADS\n";
    } else {
      // cannot use StdErr here, because it may not be initialized yet
      std::cerr << "WARNING: environment variable CMTK_NUM_THREADS is set but "
                   "does not seem to contain a number larger than 0.\n";
    }
  }
