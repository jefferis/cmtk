...
const CompressedStream::ArchiveLookupEntry CompressedStream::ArchiveLookup[] = {
#ifdef _MSC_VER
    {".Z", "gunzip -cd %s > %s"},
    {".gz", "gzip -cd %s > %s"},
    {".bz", "bzip -Q -cd %s > %s"},
...
CompressedStream::CompressedStream(const std::string &filename)
    : m_Reader(NULL), m_Compressed(false) {
  this->Open(MountPoints::Translate(filename));
}

CompressedStream::~CompressedStream() { this->Close(); }

bool CompressedStream::Open(const std::string &filename) {
  this->Close();

  if (Self::Stat(filename.c_str()) == 2) {
    StdErr << "WARNING: file '" << filename
           << "' exists both compressed and uncompressed!\n";
  }
  ...
  if (period != std::string::npos) {
    ...
    for (int i = 0; ArchiveLookup[i].suffix && !this->m_Compressed; ++i)
      this->m_Compressed =
          this->m_Compressed || (suffix == ArchiveLookup[i].suffix);
  }
  ...
  try {
    if (!this->m_Reader) {
      ...
      for (int i = 0; ArchiveLookup[i].suffix && !result; ++i)
        result = this->OpenDecompressionPipe(filename, suffix,
                                             ArchiveLookup[i].command,
                                             ArchiveLookup[i].suffix);
...
bool CompressedStream::OpenDecompressionPipe(const std::string &filename,
                                             const std::string &suffix,
                                             const char *command,
                                             const char *compressedSuffix) {
  std::string fname = filename;
  if (suffix != compressedSuffix) fname = fname + compressedSuffix;
...
int CompressedStream::Stat(const std::string &path, Self::StatType *buf) {
  const std::string baseName =
      CompressedStream::GetBaseName(MountPoints::Translate(path));
