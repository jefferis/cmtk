...
class PolynomialXform :
 ...
 public:
  ...
  /// Default constructor.
  PolynomialXform(const byte degree =
                      0 /*!< Polynomial degree - 0 through 4 are supported. */)
      : m_Degree(degree) {
    // Sort out how many monomials a polynomial of the given degree has.
    this->m_NumberOfMonomials =
        PolynomialHelper::GetNumberOfMonomials(this->m_Degree);
  ...
  byte Degree() const { return this->m_Degree; }
  ...
  const Self::SpaceVectorType &Center() const { return this->m_Center; }
  ...
  virtual Self::SpaceVectorType Apply(const Self::SpaceVectorType &v) const {
    ...
    size_t paramIdx = 0;
    for (size_t monomialIdx = 0; monomialIdx < this->m_NumberOfMonomials;
         ++monomialIdx) {
      const Types::Coordinate monomialValue =
          this->GetMonomialAt(monomialIdx, v);
  ...
  Types::Coordinate GetMonomialAt(
      const size_t idx /*!< Index of the monomial to get. */,
      const Self::SpaceVectorType &v) const {
    // remove "center" from input
    const Self::SpaceVectorType vRel = v - this->m_Center;
    return Polynomial<4, Types::Coordinate>::EvaluateMonomialAt(
        idx, vRel[0], vRel[1], vRel[2]);
  }

  /** Return inverse-transformed vector.
   * This uses the Jacobian-based search algorithm inherited from cmtk::Xform,
   * with the initial estimate of the iverse derived from the inverse of the
   * affine sub-transformation within this polynomial.
   */
   ...
   * The global linear matrix comprises the first-order components of the
   * polynomial transformation, i.e., the rotational/scale/shear components.
   */
   ...
   * components of the polynomial transformation, i.e., the translational as
   * well as rotational/scale/shear components.
   */
