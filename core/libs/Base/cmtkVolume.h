...
class Volume :
 ...
 public:
  ...
  /// Set volume offset.
  void SetOffset(const Vector3D &o) { this->m_Offset = o; }

  /** Spatial extent of the volume in world coordinates
   *\note This is the actual size of the volume between first and last pixel.
   *Therefore, if a non-zero volume coordinate offset is set in m_Offset, this
   *does not affect this field, because the volume size as the product of pixel
   *size times number of pixels per dimension minus one remains unaffected.
   */
  ...
  /// Copy constructor.
  Volume(const Self &other)
      : Superclass(other), m_Offset(other.m_Offset), m_Size(other.m_Size) {}

  /// Grid and data constructor.
  Volume(const Self::IndexType &dims, const Self::CoordinateVectorType &size,
         TypedArray::SmartPtr &data = TypedArray::SmartPtr::Null())
      : DataGrid(dims, data), m_Offset(0.0), m_Size(size) {}
  ...
  virtual Types::Coordinate AverageVoxelVolume() const {
    return ((m_Size[0] * m_Size[1] * m_Size[2]) /
            ((this->m_Dims[0] - 1) * (this->m_Dims[1] - 1) *
             (this->m_Dims[2] - 1)));
  }
 ...
 protected:
   ...
   */
  bool GetTrilinear(ProbeInfo &, const Types::GridIndexType,
                    const Types::GridIndexType, const Types::GridIndexType,
                    const Vector3D &, const Types::Coordinate *,
                    const Types::Coordinate *) const;
};
