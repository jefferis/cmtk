 ...
 * This class inherits from cmtk::FixedArray and, unlike its base class, is
 * suitable only for element types that support arithmetic operations.
 */
...
class FixedVector : public FixedArray<NDIM, T> {
 public:
  ...
  Self &operator+=(const Self &rhs) {
    for (size_t i = 0; i < NDIM; ++i) this->m_Data[i] += rhs.m_Data[i];
    return *this;
  }
  ...
  Self &operator*=(const T a) {
    for (size_t i = 0; i < NDIM; ++i) this->m_Data[i] *= a;
    return *this;
  }

  /// Elementwise multiplication with another vector.
  Self &operator*=(const Self &rhs) {
    for (size_t i = 0; i < NDIM; ++i) this->m_Data[i] *= rhs[i];
    return *this;
  }
  ...
  const Self operator-() {
    Self minus;
    for (size_t i = 0; i < NDIM; ++i) minus.m_Data = -this->m_Data;
  ...
  Self &AddScalar(const T &value) {
    for (size_t i = 0; i < NDIM; ++i) this->m_Data[i] += value;
    return *this;
  }

  /// Maximum value.
  T MaxValue() const { return *(std::max_element(this->begin(), this->end())); }
  ...
  /// Minimum value.
  T MinValue() const { return *(std::min_element(this->begin(), this->end())); }
  ...
  T Sum() const {
    T sum = this->m_Data[0];
    for (size_t i = 1; i < NDIM; ++i) sum += this->m_Data[i];
  ...
  T Product() const {
    ...
    for (size_t i = 1; i < NDIM; ++i) product *= this->m_Data[i];

    return product;
  }

  /// Calculate sum of squares of vector elements (that is, square of Euclid's
  /// norm).
  T SumOfSquares() const {
    T sq = 0;
    for (size_t i = 0; i < NDIM; ++i) sq += this->m_Data[i] * this->m_Data[i];
  ...
  T RootSumOfSquares() const { return sqrt(this->SumOfSquares()); }

  /// Get angle cosine between this and another vector.
  T GetAngleCosine(const Self &other) const {
    return (*this * other) /
           (this->RootSumOfSquares() * other.RootSumOfSquares());
  }
};

/// Addition operator.
template <size_t NDIM, typename T>
const FixedVector<NDIM, T> operator+(const FixedVector<NDIM, T> &lhs,
...
template <size_t NDIM, typename T>
const FixedVector<NDIM, T> ComponentDivide(const FixedVector<NDIM, T> &lhs,
                                           const FixedVector<NDIM, T> &rhs) {
  return FixedVector<NDIM, T>(lhs) /= rhs;
}
...
template <size_t NDIM, typename T>
const FixedVector<NDIM, T> ComponentMultiply(const FixedVector<NDIM, T> &lhs,
                                             const FixedVector<NDIM, T> &rhs) {
  return FixedVector<NDIM, T>(lhs) *= rhs;
}
...
T operator*(const FixedVector<NDIM, T> &lhs, const FixedVector<NDIM, T> &rhs) {
  ...
  for (size_t i = 1; i < NDIM; ++i) product += lhs[i] * rhs[i];
  return product;
}

/// Scalar multiplication operator.
template <size_t NDIM, typename T, typename T2>
const FixedVector<NDIM, T> operator*(const T2 lhs,
                                     const FixedVector<NDIM, T> &rhs) {
  FixedVector<NDIM, T> result(rhs);
  for (size_t i = 0; i < NDIM; ++i) result[i] *= static_cast<T>(lhs);
  return result;
}

/// Elementwise maximum operator.
template <size_t NDIM, typename T>
const FixedVector<NDIM, T> Max(const FixedVector<NDIM, T> &lhs,
                               const FixedVector<NDIM, T> &rhs) {
  FixedVector<NDIM, T> result;
  for (size_t i = 0; i < NDIM; ++i) result[i] = std::max(lhs[i], rhs[i]);
  return result;
}

/// Elementwise minimum operator.
template <size_t NDIM, typename T>
const FixedVector<NDIM, T> Min(const FixedVector<NDIM, T> &lhs,
                               const FixedVector<NDIM, T> &rhs) {
  FixedVector<NDIM, T> result;
  for (size_t i = 0; i < NDIM; ++i) result[i] = std::min(lhs[i], rhs[i]);
  return result;
}

/// Element-wise less-than operator.
template <size_t NDIM, typename T>
bool operator<(const FixedVector<NDIM, T> &lhs,
               const FixedVector<NDIM, T> &rhs) {
  for (size_t i = 0; i < NDIM; ++i)
