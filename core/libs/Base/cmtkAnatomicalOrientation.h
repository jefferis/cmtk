...
class AnatomicalOrientation : public AnatomicalOrientationBase {
 public:
  /// This class.
  ...
  /** Get closest anatomical orientation based on space dimensions and image
   *direction vectors. \param orientation The resulting orientation string
   *(three characters plus terminating zero) will be put into this string.
   *\param directions The image direction vectors. "directions[0]" is the 3-D
   *vector of the in-plane x image direction, "directions[1]" is in-plane y, and
   *"directions[2]" is the vector from one plane origin to the origin of the
   *next plane. \param spaceAxes Six-character string that defines the
   *orientation of the underlying space. The first two characters relate to the
   *x coordinate, the second two to the y coordinate, and the final two to the z
   *coordinate.
   */
  static void GetOrientationFromDirections(
      char *orientation, const AffineXform::MatrixType &directions,
      const char *spaceAxes);

  /// Get permutation table of coordinate axes from space axes and image
  /// orientation.
  static void GetImageToSpaceAxesPermutation(
      Types::GridIndexType (&imageToSpaceAxesPermutation)[3][3],
      const char *orientation, const char *spaceAxes);

  /// Class for permutation matrix that can be applied to pixel indexes, as well
  /// as dimension, and pixel size arrays.
  class PermutationMatrix {
   public:
    ...
    /// Constructor: determine axes permutation, flipping, and store local copy
    /// of reoriented dimensions array.
    PermutationMatrix(const FixedVector<3, Types::GridIndexType> &sourceDims,
     ...
     */
    template <class T>
    const FixedVector<3, T> GetPermutedArray(
        const FixedVector<3, T> &source) const {
    ...
    /** Get proper permutation matrix
     */
    AffineXform::MatrixType GetMatrix() const;

    /** Permute index-to-physical matrix
     */
    AffineXform::MatrixType GetPermutedMatrix(
     ...
     */
    void GetReorientedIndex(const Types::GridIndexType *origPoint,
                            Types::GridIndexType *newPoint) const {
      for (int i = 0; i < 3; ++i)
        newPoint[i] = this->m_Multipliers[i] * origPoint[this->m_Axes[i]] +
                      this->m_Offsets[i];
    }
    ...
    size_t NewOffsetFromPoint(const Types::GridIndexType *origPoint) const {
      return (this->m_Multipliers[0] * origPoint[this->m_Axes[0]] +
              this->m_Offsets[0]) +
             this->m_NewDims[0] *
                 ((this->m_Multipliers[1] * origPoint[this->m_Axes[1]] +
                   this->m_Offsets[1]) +
                  this->m_NewDims[1] *
                      (this->m_Multipliers[2] * origPoint[this->m_Axes[2]] +
                       this->m_Offsets[2]));
    }
