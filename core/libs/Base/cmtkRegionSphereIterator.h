...
class RegionSphereIterator {
 ...
 public:
  ...
  explicit RegionSphereIterator(
      const typename Self::IndexType
          radius /*!< Radius of the sphere in each dimension.*/) {
  ...
  explicit RegionSphereIterator(
      const typename Self::IndexType
          radius /*!< Radius of the sphere in each dimension.*/,
      const typename Self::IndexType
          center /*!< Center index of the sphere. */) {
    this->Populate(radius, center, 0, 1.0);
  }
  ...
  /// Iterator assignment.
  const Self &operator=(
      const typename Self::IndexListType::const_iterator &it) {
 ...
 protected:
  ...
  virtual void
  Populate(const typename Self::IndexType
               &radius /*!< Sphere radius in index steps by dimension.*/,
           const typename Self::IndexType &center /*!< Sphere center. */,
           const size_t dim /*!< Next dimension. */, const double remainSquare /*!< Remaining proportion of total squared sphere radius. */) {
    if (remainSquare >= 0) {
      typename Self::IndexType index = center;
      const int radiusThisDimension =
          static_cast<int>(sqrt(remainSquare) * radius[dim]);
      if (dim < Self::Dimension) {
        this->Populate(radius, index, dim + 1, remainSquare);

        for (int r = 1; r <= radiusThisDimension; ++r) {
          const double newRemainSquare =
              remainSquare - MathUtil::Square(1.0 * r / radius[dim]);
