...
class ParametricPlane {
 public:
  ...
  /// Get parameter Rho.
  Types::Coordinate GetRho() const { return Rho; }
  ...
  /// Get parameter Theta.
  const Units::Degrees GetTheta() const { return Theta; }
  ...
  /// Get parameter Phi.
  const Units::Degrees GetPhi() const { return Phi; }
  ...
  void SetParameters(const CoordinateVector &v) {
    Rho = v[0];
    Theta = Units::Degrees(v[1]);
    Phi = Units::Degrees(v[2]);
    this->m_Origin[0] = v[3];
    this->m_Origin[1] = v[4];
    this->m_Origin[2] = v[5];
    this->Update();
  ...
  char GetWhichSide(const Self::CoordinateVectorType &point) const {
    ...
    // compute line parameter of orthogonal projection of "point" onto this
    // plane
  ...
  void Mirror(Self::CoordinateVectorType &toPoint,
              const Self::CoordinateVectorType &fromPoint) const {
    toPoint = fromPoint;
  ...
  void MirrorInPlace(Self::CoordinateVectorType &point) const {
    ...
    const Types::Coordinate intersect =
        2 * ((Normal * point - Rho) / SquareNormal);

    // compute mirrored point
    for (int dim = 0; dim < 3; ++dim) point[dim] -= intersect * Normal[dim];
  ...
  void Project(Self::CoordinateVectorType &toPoint,
               const Self::CoordinateVectorType &fromPoint) const {
    toPoint = fromPoint;
   ...
   * axes.
   *\param normalAxis The index of the axis to align the normal vector with (0,
   *1, or 2 for x, y, or z, respectively).
   */
