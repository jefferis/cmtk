...
/// Description of a detected Magphan EMR051 structural imaging phantom (a.k.a.
/// ADNI Phantom) in an actual image.
class DetectedPhantomMagphanEMR051 {
 public:
  ...
  void AddLandmarkPair(
      const std::string &name /*!< Name of this sphere. */,
      const LandmarkPair::SpaceVectorType &
          expected /*!< Expected landmark position in physical image coordinates
                      based on linear (not necessarily rigid) fit. */
      ,
      const LandmarkPair::SpaceVectorType
          &actual /*!< Actual, detected landmark position in physical image
                     coordinates in the image. */
      ,
      const Types::Coordinate residual /*!< Residual of landmark fit. */, const bool reliable /*!< If true, this landmark is considered reliable, i.e., its expected position by phantom manufacturing is sufficiently precise to be used as a landmark. */) {
    this->m_LandmarkPairs.push_back(
        LandmarkPair(name, expected, actual, residual, reliable));
  }

  /** Apply a transformation to all landmarks.
   * The purpose of this function is mainly to bring all landmark images into a
   * new image space, e.g., to transform them from phantom image physical space
   * to unwarp image standard space.
   */
  void ApplyXformToLandmarks(const Xform &xform) {
    for (std::list<LandmarkPair>::iterator it = this->m_LandmarkPairs.begin();
         it != this->m_LandmarkPairs.end(); ++it) {
  ...
  /// Estimated maximum dimming (i.e., minimum fraction of intensity of a 10mm
  /// sphere relative to maximum)
  ...
  class StatusFlags {
   public:
    /// Default constructor.
    StatusFlags()
        : m_FallbackOrientationCNR(false),
          m_FallbackCentroidCNR(false),
          m_DistanceSNRtoCNR(0.0) {}

    /// Flag for using CNR orientation as a fallback for missing/undetected 15mm
    /// spheres.
