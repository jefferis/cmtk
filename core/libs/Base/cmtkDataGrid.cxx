...
DataGrid::DataGrid(const Self &other)
  ...
  if (other.m_Data) this->m_Data = other.m_Data->Clone();

  this->ComputeGridIncrements();
}

DataGrid *DataGrid::CloneVirtual(const bool copyData) {
  if (copyData) {
...
DataGrid *DataGrid::CloneVirtual() const {
...
const DataGrid::RegionType DataGrid::GetWholeImageRegion() const {
  return Self::RegionType(Self::IndexType(0), Self::IndexType(this->m_Dims));
}

const DataGrid::RegionType DataGrid::GetSliceRegion(
    const int axis, const Types::GridIndexType slice) const {
...
TypedArray::SmartPtr DataGrid::CreateDataArray(const ScalarDataType dataType,
                                               const bool setToZero) {
  TypedArray::SmartPtr data(
      TypedArray::Create(dataType, this->GetNumberOfPixels()));
  if (setToZero) data->ClearArray();
  this->SetData(data);
  return data;
}

DataGrid *DataGrid::GetDownsampledAndAveraged(
    const Types::GridIndexType (&downsample)[3]) const {
  const Types::GridIndexType newDims[3] = {
      (this->m_Dims[0] - 1) / downsample[0] + 1,
      (this->m_Dims[1] - 1) / downsample[1] + 1,
      (this->m_Dims[2] - 1) / downsample[2] + 1};

  DataGrid *newDataGrid = new DataGrid(Self::IndexType::FromPointer(newDims));

  const TypedArray *thisData = this->GetData();
  if (thisData) {
    TypedArray::SmartPtr newData = TypedArray::Create(
        thisData->GetType(), newDataGrid->GetNumberOfPixels());
...
#pragma omp parallel for
    for (Types::GridIndexType z = 0; z < newDims[2]; ++z) {
      ...
      for (Types::GridIndexType y = 0; y < newDims[1];
           ++y, oldY += downsample[1]) {
        Types::GridIndexType oldX = 0;
        for (Types::GridIndexType x = 0; x < newDims[0];
             ++x, oldX += downsample[0], ++toOffset) {
          Types::DataItem sum = 0;
          Types::GridIndexType count = 0;
          for (Types::GridIndexType zz = 0;
               (zz < downsample[2]) && (oldZ + zz < this->m_Dims[2]); ++zz) {
            const Types::GridIndexType ofsZ = (oldZ + zz) * this->nextK;
            for (Types::GridIndexType yy = 0;
                 (yy < downsample[1]) && (oldY + yy < this->m_Dims[1]); ++yy) {
              const Types::GridIndexType ofsYZ =
                  ofsZ + (oldY + yy) * this->nextJ;
              for (Types::GridIndexType xx = 0;
                   (xx < downsample[0]) && (oldX + xx < this->m_Dims[0]);
                   ++xx) {
...
DataGrid *DataGrid::GetDownsampled(
    const Types::GridIndexType (&downsample)[3]) const {
  const Types::GridIndexType newDims[3] = {
      (this->m_Dims[0] - 1) / downsample[0] + 1,
      (this->m_Dims[1] - 1) / downsample[1] + 1,
...
const DataGrid::SmartPtr DataGrid::GetReoriented(
  ...
  if (oldData) {
    ...
    for (fromPoint[2] = 0; fromPoint[2] < this->m_Dims[2]; fromPoint[2]++) {
      for (fromPoint[1] = 0; fromPoint[1] < this->m_Dims[1]; fromPoint[1]++) {
        for (fromPoint[0] = 0; fromPoint[0] < this->m_Dims[0];
             fromPoint[0]++, fromPtr += bytesPerPixel) {
          memcpy(toPtr + bytesPerPixel * pmatrix.NewOffsetFromPoint(fromPoint),
...
void DataGrid::ComputeGridIncrements() {
  this->m_GridIncrements[0] = 1;
  for (int i = 1; i < 3; ++i)
    this->m_GridIncrements[i] =
        this->m_GridIncrements[i - 1] * this->m_Dims[i - 1];
...
bool DataGrid::TrilinearInterpolation(Types::DataItem &value,
  ...
  if (data_present) {
    const Types::Coordinate deltaX = 1 / (to[0] - from[0]),
                            deltaY = 1 / (to[1] - from[1]),
                            deltaZ = 1 / (to[2] - from[2]);
    ...
    value = static_cast<Types::DataItem>(
        offsZ * (offsY * (offsX * corners[0] + revX * corners[1]) +
...
TypedArray::SmartPtr DataGrid::GetDataMirrorPlane(const int axis) const {
...
void DataGrid::ApplyMirrorPlane(const int axis) {
  this->MirrorPlaneInPlace(*(this->GetData()), this->m_Dims, axis);
}

void DataGrid::MirrorPlaneInPlace(TypedArray &data, const Self::IndexType &dims,
                                  const int axis) {
  switch (axis) {
    case AXIS_X: {
    ...
    case AXIS_Y: {
      ...
      for (Types::GridIndexType z = 0; z < dims[2];
           ++z, zOffset += dims[0] * dims[1]) {
        for (Types::GridIndexType y = 0; y < (dims[1] / 2); ++y)
          data.BlockSwap(zOffset + y * dims[0],
                         zOffset + (dims[1] - 1 - y) * dims[0], dims[0]);
      }
    } break;
    case AXIS_Z: {
...
ScalarImage::SmartPtr DataGrid::GetOrthoSlice(
  ...
  const TypedArray *data = this->GetData();
  TypedArray::SmartPtr sliceData =
      TypedArray::Create(data->GetType(), dims[0] * dims[1]);
  ...
  if ((plane < 0) || (plane >= depth)) {
    ...
    for (Types::GridIndexType y = 0; y < dims[1]; ++y) {
      ...
      for (Types::GridIndexType x = 0; x < dims[0]; ++x, ++sliceOffset) {
        ...
        memcpy(sliceData->GetDataPtr(sliceOffset), data->GetDataPtr(offset),
               itemSize);
        offset = offsetX;
      }
      offset = offsetY;
...
DataGrid::SmartPtr DataGrid::ExtractSlice(
    const int axis, const Types::GridIndexType plane) const {
...
void DataGrid::SetOrthoSlice(const int axis, const Types::GridIndexType idx,
...
FixedVector<3, Types::Coordinate> DataGrid ::GetCenterOfMassGrid() const {
    ...
    for (Types::GridIndexType y = 0; y < this->m_Dims[1]; ++y)
      for (Types::GridIndexType x = 0; x < this->m_Dims[0]; ++x, ++ofs) {
        ...
        if (this->GetDataAt(value, x, y, z) && MathUtil::IsFinite(value)) {
          const Types::Coordinate pixelCOM[3] = {value * x, value * y,
                                                 value * z};
...
FixedVector<3, Types::Coordinate> DataGrid ::GetCenterOfMassGrid(
    FixedVector<3, Types::Coordinate> &firstOrderMoment) const {
  FixedVector<3, Types::Coordinate> com =
      this->Self::GetCenterOfMassGrid();  // do not use overloaded function
  ...
  for (Types::GridIndexType z = 0; z < this->m_Dims[2]; ++z)
    for (Types::GridIndexType y = 0; y < this->m_Dims[1]; ++y)
      for (Types::GridIndexType x = 0; x < this->m_Dims[0]; ++x, ++ofs) {
        Types::DataItem value;
        if (this->GetDataAt(value, x, y, z) && MathUtil::IsFinite(value)) {
          const Types::Coordinate pixelMoment[3] = {value * fabs(x - com[0]),
                                                    value * fabs(y - com[1]),
                                                    value * fabs(z - com[2])};
