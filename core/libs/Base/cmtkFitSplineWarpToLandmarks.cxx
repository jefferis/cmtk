...
cmtk::FitSplineWarpToLandmarks::FitSplineWarpToLandmarks(
    const LandmarkPairList &landmarkList)
    : m_Landmarks(landmarkList.begin(), landmarkList.end()) {}

cmtk::Types::Coordinate cmtk::FitSplineWarpToLandmarks::ComputeResiduals(
...
#pragma omp parallel for reduction(+ : msResidual)
  for (int i = 0; i < static_cast<int>(this->m_Landmarks.size()); ++i) {
    splineWarp.PrecomputeLocationSpline(this->m_Landmarks[i].m_Location,
                                        this->m_LandmarksGrid[i],
                                        this->m_LandmarksSpline[i]);
    this->m_Residuals[i] = this->m_Landmarks[i].m_TargetLocation -
                           splineWarp.Apply(this->m_Landmarks[i].m_Location);
...
cmtk::SplineWarpXform::SmartPtr cmtk::FitSplineWarpToLandmarks::Fit(
    const SplineWarpXform::SpaceVectorType &domain,
    const SplineWarpXform::ControlPointIndexType &finalDims,
    const AffineXform *affineXform, const Self::Parameters &parameters) {
  ...
  for (int level = 1; level < actualParameters.m_Levels; ++level) {
                                       ...
                                       // will be left in each dimension
    {
      ...
      DebugOutput(2) << "INFO: adjusted number of levels to " << level
                     << " from " << parameters.m_Levels
  ...
  AffineXform::SmartPtr initialAffine(
      affineXform ? new AffineXform(*affineXform) : new AffineXform);
  SplineWarpXform *splineWarp = new SplineWarpXform(
      domain, initialDims, CoordinateVector::SmartPtr::Null(), initialAffine);
...
cmtk::SplineWarpXform::SmartPtr cmtk::FitSplineWarpToLandmarks::Fit(
    const SplineWarpXform::SpaceVectorType &domain,
    const Types::Coordinate finalSpacing, const AffineXform *affineXform,
    const Self::Parameters &parameters) {
  ...
  SplineWarpXform *splineWarp =
      new SplineWarpXform(domain, spacing, initialAffine);
...
void cmtk::FitSplineWarpToLandmarks::FitSpline(
    SplineWarpXform &splineWarp, const Self::Parameters &parameters) {
  ...
  for (int level = 0; level < parameters.m_Levels; ++level) {
    ...
    for (int iteration = 0; iteration < parameters.m_IterationsPerLevel;
         ++iteration) {
      // loop over all control points to compute deltas as the spline
      // coefficients that fit current residuals
      ...
      for (size_t i = 0; i < this->m_Landmarks.size(); ++i) {
        ...
        for (int m = 0; m < 4; ++m) {
          for (int l = 0; l < 4; ++l) {
            const Types::Coordinate wlm = this->m_LandmarksSpline[i][1][l] *
                                          this->m_LandmarksSpline[i][2][m];
            for (int k = 0; k < 4; ++k) {
              sumOfSquares +=
                  (w2klm[m][l][k] = MathUtil::Square(
                       wklm[m][l][k] = this->m_LandmarksSpline[i][0][k] * wlm));
            }
          }
        }
        ...
        for (int m = 0; m < 4; ++m) {
          const size_t mOfs =
              splineWarp.m_Dims[1] * (this->m_LandmarksGrid[i][2] + m);
          for (int l = 0; l < 4; ++l) {
            const size_t mlOfs =
                splineWarp.m_Dims[0] * (this->m_LandmarksGrid[i][1] + l + mOfs);
            for (int k = 0; k < 4; ++k) {
...
#pragma omp parallel for
      for (int cp = 0;
           cp < static_cast<int>(splineWarp.m_NumberOfControlPoints); ++cp) {
        if (weight[cp] != 0) {
          delta[cp] /= weight[cp];
      ...
      if (((rmsResidualBefore - rmsResidualAfter) / rmsResidualBefore) <
          parameters.m_ResidualThreshold)
        break;
