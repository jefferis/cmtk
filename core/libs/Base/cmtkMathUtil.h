...
class MathUtil {
 ...
 public:
   ...
   */
  template <class T>
  static T Variance(const unsigned int nValues, const T *values, T mean,
   ...
   */
  template <class T>
  static T Variance(const std::vector<T> &values, T mean,
                    const bool unbiased = false);
  ...
  static double TStatFromCorrelation(
      const double r /*!< Coefficient of correlation as computed by
                        MathUtil::Correlation function.*/
      ,
      const size_t df /*!< Number of degrees of freedom*/);

  /// Compute probability from T-statistic.
  static double ProbabilityFromTStat(
      const double t /*!< T-statistic as returned for example from
                        MathUtil::TStatFromCorrelation function.*/
      ,
      const size_t df /*!< Number of degrees of freedom.*/);
   ...
   */
  template <class T>
  static T TTest(const std::vector<T> &valuesX, const std::vector<T> &valuesY,
                 T &t);
   ...
   */
  template <class T>
  static T PairedTTest(const std::vector<T> &valuesX,
                       const std::vector<T> &valuesY, T &t, T &avgX, T &avgY);
  ...
  static void SVD(Matrix2D<double> &U, std::vector<double> &W,
                  Matrix2D<double> &V);

  /// Linear Regression using SVD results
  static void SVDLinearRegression(const Matrix2D<double> &U,
