...
ActiveDeformationModel<W>::ActiveDeformationModel(
    ...
    const unsigned int numberOfModes, const bool includeScaleInModel,
  ...
  if (IncludeReferenceInModel) ++numberOfSamples;

  Types::Coordinate **samplePoints =
      Memory::ArrayC::Allocate<Types::Coordinate *>(numberOfSamples);
  unsigned int numberOfPoints = 0;

  typename std::list<SmartPointer<W>>::const_iterator it =
      deformationList.begin();
  ...
  while (it != deformationList.end()) {
    ...
    samplePoints[sample++] =
        (*it)->GetPureDeformation(this->IncludeScaleInModel);
    globalScaling +=
        static_cast<Types::Coordinate>(log((*it)->GetGlobalScaling()));
    ++it;
  }
...
template <class W>
Types::Coordinate *ActiveDeformationModel<W>::MakeSamplePointsReference(
    const W *deformation) {
  ...
  for (unsigned int pointIdx = 0; pointIdx < numberOfParameters / 3;
       ++pointIdx, ptr += 3) {
    // get original (undeformed) control point position
    const Vector3D v =
        deformation->GetOriginalControlPointPositionByOffset(pointIdx);
...
template <class W>
Types::Coordinate *ActiveDeformationModel<W>::MakeSamplePoints(
    const W *deformation) {
  const unsigned int numberOfParameters = deformation->m_NumberOfParameters;
  Types::Coordinate *points =
      Memory::ArrayC::Allocate<Types::Coordinate>(numberOfParameters);
  memcpy(points, deformation->m_Parameters,
         sizeof(*points) * numberOfParameters);

  AffineXform::SmartPtr xform(
      deformation->GetInitialAffineXform()->MakeInverse());
  ...
  for (unsigned int pointIdx = 0; pointIdx < numberOfParameters / 3;
    ...
    const FixedVector<3, Types::Coordinate> v =
        xform->Apply(FixedVector<3, Types::Coordinate>::FromPointer(ptr));
...
template <class W>
W *ActiveDeformationModel<W>::Compose(const Types::Coordinate *weights) {
...
template <class W>
float ActiveDeformationModel<W>::Decompose(
    const W *input, Types::Coordinate *const weights) const {
  CoordinateVector inputVector(
      this->GetNumberOfPoints(),
      input->GetPureDeformation(this->IncludeScaleInModel));
