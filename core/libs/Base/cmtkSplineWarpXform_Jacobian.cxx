...
void SplineWarpXform::GetJacobianRow(CoordinateMatrix3x3 *const array,
                                     const int x, const int y, const int z,
                                     const size_t numberOfPoints) const {
  const Types::Coordinate *spX = &this->m_GridSpline[0][x << 2],
                          *spY = &this->m_GridSpline[1][y << 2],
                          *spZ = &this->m_GridSpline[2][z << 2];
  const Types::Coordinate *dspX = &this->m_GridDerivSpline[0][x << 2],
                          *dspY = &this->m_GridDerivSpline[1][y << 2],
                          *dspZ = &this->m_GridDerivSpline[2][z << 2];
  const Types::Coordinate *coeff =
      this->m_Parameters + this->m_GridOffsets[0][x] +
      this->m_GridOffsets[1][y] + this->m_GridOffsets[2][z];
  ...
  const int numberOfCells = (this->m_GridOffsets[0][x + numberOfPoints - 1] -
                             this->m_GridOffsets[0][x]) /
                                nextI +
                            4;
  ...
  for (const int lastPoint = x + numberOfPoints; i < lastPoint;) {
    ...
    do {
      ...
      // performance.
      J[0][0] = this->m_InverseSpacing[0] *
                (dspX[0] * phiPtrX[0] + dspX[1] * phiPtrX[3] +
                 dspX[2] * phiPtrX[6] + dspX[3] * phiPtrX[9]);
      J[0][1] = this->m_InverseSpacing[0] *
                (dspX[0] * phiPtrX[1] + dspX[1] * phiPtrX[4] +
                 dspX[2] * phiPtrX[7] + dspX[3] * phiPtrX[10]);
      J[0][2] = this->m_InverseSpacing[0] *
                (dspX[0] * phiPtrX[2] + dspX[1] * phiPtrX[5] +
                 dspX[2] * phiPtrX[8] + dspX[3] * phiPtrX[11]);

      J[1][0] = this->m_InverseSpacing[1] *
                (spX[0] * phiPtrY[0] + spX[1] * phiPtrY[3] +
                 spX[2] * phiPtrY[6] + spX[3] * phiPtrY[9]);
      J[1][1] = this->m_InverseSpacing[1] *
                (spX[0] * phiPtrY[1] + spX[1] * phiPtrY[4] +
                 spX[2] * phiPtrY[7] + spX[3] * phiPtrY[10]);
      J[1][2] = this->m_InverseSpacing[1] *
                (spX[0] * phiPtrY[2] + spX[1] * phiPtrY[5] +
                 spX[2] * phiPtrY[8] + spX[3] * phiPtrY[11]);

      J[2][0] = this->m_InverseSpacing[2] *
                (spX[0] * phiPtrZ[0] + spX[1] * phiPtrZ[3] +
                 spX[2] * phiPtrZ[6] + spX[3] * phiPtrZ[9]);
      J[2][1] = this->m_InverseSpacing[2] *
                (spX[0] * phiPtrZ[1] + spX[1] * phiPtrZ[4] +
                 spX[2] * phiPtrZ[7] + spX[3] * phiPtrZ[10]);
      J[2][2] = this->m_InverseSpacing[2] *
                (spX[0] * phiPtrZ[2] + spX[1] * phiPtrZ[5] +
                 spX[2] * phiPtrZ[8] + spX[3] * phiPtrZ[11]);
...
const CoordinateMatrix3x3 SplineWarpXform::GetJacobianAtControlPoint(
  ...
  for (int i = 0; i < 3; ++i) {
    for (int j = 0; j < 3; ++j) J[i][j] *= this->m_InverseSpacing[i];
  }

  return J;
}

const CoordinateMatrix3x3 SplineWarpXform::GetJacobian(
...
Types::Coordinate SplineWarpXform::GetJacobianDeterminant(const int x,
                                                          const int y,
                                                          const int z) const {
  const Types::Coordinate *spX = &this->m_GridSpline[0][x << 2],
                          *spY = &this->m_GridSpline[1][y << 2],
  ...
  return this->m_InverseSpacing[0] * this->m_InverseSpacing[1] *
         this->m_InverseSpacing[2] *
         (J[0][0] * (J[1][1] * J[2][2] - J[1][2] * J[2][1]) +
          J[0][1] * (J[1][2] * J[2][0] - J[1][0] * J[2][2]) +
          J[0][2] * (J[1][0] * J[2][1] - J[1][1] * J[2][0]));
}

Types::Coordinate SplineWarpXform::GetJacobianDeterminant(
    const Self::SpaceVectorType &v) const {
  Types::Coordinate r[3], f[3];
  int grid[3];
...
void SplineWarpXform::GetJacobianDeterminantRow(
  ...
  const Types::Coordinate globalInverseSpacing = this->m_InverseSpacing[0] *
                                                 this->m_InverseSpacing[1] *
                                                 this->m_InverseSpacing[2];
  ...
  for (const int lastPoint = x + numberOfPoints; i < lastPoint;) {
    ...
    do {
      ...
      // and adding x. The loops to do this have been unrolled for increased
      // performance.
      JXX = dspX[0] * phiPtrX[0] + dspX[1] * phiPtrX[3] + dspX[2] * phiPtrX[6] +
            dspX[3] * phiPtrX[9];
      JXY = dspX[0] * phiPtrX[1] + dspX[1] * phiPtrX[4] + dspX[2] * phiPtrX[7] +
            dspX[3] * phiPtrX[10];
      JXZ = dspX[0] * phiPtrX[2] + dspX[1] * phiPtrX[5] + dspX[2] * phiPtrX[8] +
            dspX[3] * phiPtrX[11];

      JYX = spX[0] * phiPtrY[0] + spX[1] * phiPtrY[3] + spX[2] * phiPtrY[6] +
            spX[3] * phiPtrY[9];
      JYY = spX[0] * phiPtrY[1] + spX[1] * phiPtrY[4] + spX[2] * phiPtrY[7] +
            spX[3] * phiPtrY[10];
      JYZ = spX[0] * phiPtrY[2] + spX[1] * phiPtrY[5] + spX[2] * phiPtrY[8] +
            spX[3] * phiPtrY[11];

      JZX = spX[0] * phiPtrZ[0] + spX[1] * phiPtrZ[3] + spX[2] * phiPtrZ[6] +
            spX[3] * phiPtrZ[9];
      JZY = spX[0] * phiPtrZ[1] + spX[1] * phiPtrZ[4] + spX[2] * phiPtrZ[7] +
            spX[3] * phiPtrZ[10];
      JZZ = spX[0] * phiPtrZ[2] + spX[1] * phiPtrZ[5] + spX[2] * phiPtrZ[8] +
            spX[3] * phiPtrZ[11];

      values[i - x] = globalInverseSpacing * (JXX * (JYY * JZZ - JYZ * JZY) +
                                              JXY * (JYZ * JZX - JYX * JZZ) +
                                              JXZ * (JYX * JZY - JYY * JZX));
...
Types::Coordinate SplineWarpXform::JacobianDeterminant(
    const Types::Coordinate *cp) const {
...
void SplineWarpXform ::GetJacobianConstraintThread(void *const args,
                                                   const size_t taskIdx,
                                                   const size_t taskCnt,
                                                   const size_t, const size_t) {
  Self::JacobianConstraintThreadInfo *info =
      static_cast<Self::JacobianConstraintThreadInfo *>(args);
  ...
  const int rowTo = (taskIdx == (taskCnt - 1))
                        ? rowCount
  ...
  for (int z = zFrom; (z < me->VolumeDims[2]) && rowsToDo; ++z) {
    for (int y = yFrom; (y < me->VolumeDims[1]) && rowsToDo;
         yFrom = 0, ++y, --rowsToDo) {
...
Types::Coordinate SplineWarpXform::GetJacobianConstraint() const {
  ThreadPool &threadPool = ThreadPool::GetGlobalThreadPool();
  const size_t numberOfThreads = threadPool.GetNumberOfThreads();
  const size_t numberOfTasks =
      std::min<size_t>(4 * numberOfThreads - 3, this->m_Dims[2]);

  // Info blocks for parallel tasks that evaulate the constraint.
  std::vector<Self::JacobianConstraintThreadInfo> constraintTaskInfo(
      numberOfTasks);
...
void SplineWarpXform::GetJacobianConstraintDerivative(
  ...
  for (int k = voi.From()[2]; k < voi.To()[2]; ++k)
    for (int j = voi.From()[1]; j < voi.To()[1]; ++j) {
      this->GetJacobianDeterminantRow(&(valuesJ[0]), voi.From()[0], j, k,
                                      pixelsPerRow);
  ...
  for (int k = kFrom; k < kTo; ++k)
    for (int j = jFrom; j < jTo; ++j)
      for (int i = iFrom; i < iTo; ++i)
        ground += fabs(
            log(this->GetJacobianDeterminant(Self::SpaceVectorType::FromPointer(
                    coeff + i * nextI + j * nextJ + k * nextK)) /
                this->m_GlobalScaling));
...
void SplineWarpXform::RelaxToUnfold() {
  std::vector<byte> cpList(this->m_NumberOfControlPoints);
  std::vector<double> jacobiansRow(this->VolumeDims[0]);

  bool isFolded = true;
  while (isFolded) {
    // check all Jacobian determinant values to see if grid is folded, and what
    // control points are affected
    isFolded = false;
    std::fill(cpList.begin(), cpList.end(), 0);

    for (int k = 0; k < this->VolumeDims[2]; ++k) {
      for (int j = 0; j < this->VolumeDims[1]; ++j) {
        this->GetJacobianDeterminantRow(&(jacobiansRow[0]), 0, j, k,
                                        this->VolumeDims[0]);
        for (int i = 0; i < this->VolumeDims[0]; ++i) {
          if (jacobiansRow[i] <= 0) {
            isFolded = true;
            cpList[(this->m_GridOffsets[0][i] + this->m_GridOffsets[1][j] +
                    this->m_GridOffsets[2][k]) /
                   3] = 1;
          }
        }
      }
    ...
    if (isFolded) {
      // Expand affected control points to their 4x4x4 neighbourhoods on all
      // sides.
      for (int k = 0; k < this->m_Dims[2]; ++k) {
        for (int j = 0; j < this->m_Dims[1]; ++j) {
          for (int i = 0; i < this->m_Dims[0]; ++i) {
            if (cpList[(i * this->nextI + j * this->nextJ + k * this->nextK) /
                       3] == 1) {
              for (int kk = std::max(0, k - 3);
                   kk < std::min(k + 4, this->m_Dims[2]); ++kk) {
                for (int jj = std::max(0, j - 3);
                     jj < std::min(j + 4, this->m_Dims[1]); ++jj) {
                  for (int ii = std::max(0, i - 3);
                       ii < std::min(i + 4, this->m_Dims[0]); ++ii) {
                    const size_t cpIdx = (ii * this->nextI + jj * this->nextJ +
                                          kk * this->nextK) /
                                         3;
                    if (cpList[cpIdx] != 1) {
                      cpList[cpIdx] = 2;
      ...
      std::vector<Types::Coordinate> pureDeformation(
          3 * this->m_NumberOfControlPoints);

      size_t param = 0;
      for (int k = 0; k < this->m_Dims[2]; ++k) {
        for (int j = 0; j < this->m_Dims[1]; ++j) {
          for (int i = 0; i < this->m_Dims[0]; ++i, param += 3) {
            const Self::SpaceVectorType cp =
                this->m_InitialAffineXform->GetInverse()->Apply(
                    Self::SpaceVectorType::FromPointer(this->m_Parameters +
                                                       param)) -
                this->GetOriginalControlPointPosition(i, j, k);
            for (int dim = 0; dim < 3; ++dim)
                ...
                for (int kk = std::max(0, k - 3);
                  ...
                  for (int jj = std::max(0, j - 3);
                    ...
                    for (int ii = std::max(0, i - 3);
                      ...
                      smoothed[cp] =
                          w *
                          pureDeformation[dim + ii * this->nextI +
                                          jj * this->nextJ + kk * this->nextK];
                      weight += w;
                    }
                  }
                }
                if (weight > 0) smoothed[cp] /= weight;
              } else {
                // if this control point does not need regularizing, keep it as
                // is
      ...
      for (int k = 0; k < this->m_Dims[2]; ++k) {
        for (int j = 0; j < this->m_Dims[1]; ++j) {
          for (int i = 0; i < this->m_Dims[0]; ++i, param += 3) {
            Self::SpaceVectorType cp = Self::SpaceVectorType::FromPointer(
                &(pureDeformation[0]) + param);
