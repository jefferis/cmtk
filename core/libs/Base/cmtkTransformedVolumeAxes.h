...
class TransformedVolumeAxes {
 public:
   ...
   *\param xform Coordinate transformation. If this pointer is NULL, identity is
   *assumed. \param deltas If this parameter is given, it is used as a pointer
   *to a three-element coordinate array defining the voxel size in the other
   *volume to obtain fractional voxel indices rather than actual coordinates.
   *
   * Alternatively, this parameter can also be used to provide the volume size
   *(Volume::Size), which creates normalized coordinates (0..1) for each volume
   *axis.
   *
   *\param otherOrigin If this parameter is given, it is used as a pointer to a
   * three-element coordinate array defining an m_Origin vector for the
   *transformed coordinates. \return Pointer to an array of three pointers. Each
   *of these points to an array of Vector3D objects that contain the vector
   *hashes for the X-, Y-, and Z-axis.
   */
  TransformedVolumeAxes(const UniformVolume &volume,
  ...
  /// Get dimensions.
  const FixedVector<3, int> &Dims() const { return this->m_Dims; }
 ...
 private:
  ...
  /// Create the actual hash: allocate and fill according to given offset and
  /// delta vectors.
  void MakeHash(const UniformVolume &volume,
