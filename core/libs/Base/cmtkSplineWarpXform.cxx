...
SplineWarpXform::SplineWarpXform() { this->Init(); }
...
SplineWarpXform::SplineWarpXform(const Self::SpaceVectorType &domain,
...
SplineWarpXform::SplineWarpXform(const Self::SpaceVectorType &domain,
  ...
  if (!parameters) this->InitControlPoints(this->m_InitialAffineXform);
}

void SplineWarpXform ::Init(const Self::SpaceVectorType &domain,
                            const Types::Coordinate delta,
                            const AffineXform *initialXform,
                            const bool exactDelta) {
  this->Init();
  ...
  if (exactDelta) {
    for (int dim = 0; dim < 3; ++dim) {
      ...
      this->m_Dims[dim] =
          static_cast<int>(4 + (this->m_Domain[dim] / this->m_Spacing[dim]));
      this->m_Domain[dim] = (this->m_Dims[dim] - 3) * this->m_Spacing[dim];
    }
  } else {
    for (int dim = 0; dim < 3; ++dim)
      this->m_Dims[dim] =
          2 + std::max(2, 1 + static_cast<int>(domain[dim] / delta));
  }
...
void SplineWarpXform::InitControlPoints(const AffineXform *affineXform) {
  ...
  for (int z = 0; z < this->m_Dims[2]; ++z, pZ += this->m_Spacing[2]) {
    ...
    for (int y = 0; y < this->m_Dims[1]; ++y, pY += this->m_Spacing[1]) {
      ...
      for (int x = 0; x < this->m_Dims[0];
           ++x, pX += this->m_Spacing[0], ofs += 3) {
        ofs[0] = pX;
        ofs[1] = pY;
        ofs[2] = pZ;
      }
  ...
  if (affineXform) {
    ...
    for (unsigned int idx = 0; idx < this->m_NumberOfControlPoints;
         ++idx, ofs += 3) {
      const Self::SpaceVectorType p =
          affineXform->Apply(Self::SpaceVectorType::FromPointer(ofs));
      ofs[0] = p[0];
      ofs[1] = p[1];
      ofs[2] = p[2];
    }
...
void SplineWarpXform::Update(const bool exactDelta) {
  ...
  for (int dim = 0; dim < 3; ++dim) {
    ...
    if (exactDelta) {
      ...
      this->m_InverseSpacing[dim] =
          1.0 * (this->m_Dims[dim] - 3) / this->m_Domain[dim];
    }
...
SplineWarpXform *SplineWarpXform::CloneVirtual() const {
  SplineWarpXform *newXform = new SplineWarpXform();

  newXform->m_ParameterVector =
      CoordinateVector::SmartPtr(this->m_ParameterVector->Clone());
  ...
  if (this->m_InitialAffineXform) {
    newXform->m_InitialAffineXform =
        AffineXform::SmartPtr(this->m_InitialAffineXform->Clone());
  }
  ...
  memcpy(newXform->GridPointOffset, this->GridPointOffset,
         sizeof(this->GridPointOffset));
...
void SplineWarpXform::Refine() {
  ...
  for (int z = 0; z < newDims[2]; ++z) {
    for (int y = 0; y < newDims[1]; ++y) {
      for (int x = 0; x < newDims[0]; ++x) {
        const int oldX = ((x + 1) / 2), oldY = ((y + 1) / 2),
                  oldZ = ((z + 1) / 2);
        const int oddX = x % 2, oddY = y % 2, oddZ = z % 2;

        const Types::Coordinate *coeff =
            m_Parameters + oldX * nextI + oldY * nextJ + oldZ * nextK;
        ...
        for (int dim = 0; dim < 3; ++dim, ++coeff, ++ncoeff) {
          for (int k = 0; k < 3; ++k) {
            ...
            for (int j = 0; j < 3; ++j, ofsJK += nextJ) {
              if ((oddY || j) && (oddZ || k)) {
                if (oddX) {
                  level0[k][j] = (coeff[ofsJK - nextI] + 6 * coeff[ofsJK] +
                                  coeff[ofsJK + nextI]) /
                                 8;
                } else {
          ...
          for (int k = 0; k < 3; ++k) {
            if (oddZ || k) {
              if (oddY) {
                level1[k] =
                    (level0[k][0] + 6 * level0[k][1] + level0[k][2]) / 8;
              } else {
  ...
  if (this->m_IgnoreEdge) this->m_IgnoreEdge = 2 * this->m_IgnoreEdge - 1;

  this->UnRegisterVolume();
}

UniformVolume::CoordinateRegionType SplineWarpXform::GetVolumeOfInfluence(
    const size_t idx, const UniformVolume::CoordinateRegionType &domain,
    const bool fastMode) const {
  ...
  if (fastMode) {
    for (int dim = 0; dim < 3; ++dim) {
      ...
      xyzUp[dim] =
          this->m_Spacing[dim] * std::min(this->m_Dims[dim] - 3, xyz[dim]);
    }
  } else {
    for (int dim = 0; dim < 3; ++dim) {
      xyzLow[dim] = this->m_Spacing[dim] * std::max(0, xyz[dim] - 3);
      xyzUp[dim] =
          this->m_Spacing[dim] * std::min(this->m_Dims[dim] - 2, xyz[dim] + 1);
    }
  }

  for (int dim = 0; dim < 3; ++dim) {
    regionFrom[dim] =
        std::min(domain.To()[dim], std::max(xyzLow[dim], domain.From()[dim]));
    regionTo[dim] =
        std::max(domain.From()[dim], std::min(xyzUp[dim], domain.To()[dim]));
  }

  return UniformVolume::CoordinateRegionType(regionFrom, regionTo);
}

void SplineWarpXform::RegisterVolumeAxis(
    const DataGrid::IndexType::ValueType dim, const Types::Coordinate delta,
    const Types::Coordinate origin, const int cpgDim, const size_t ofs,
    const Types::Coordinate invCpgSpacing, std::vector<int> &gIdx,
    std::vector<int> &gOfs, std::vector<Types::Coordinate> &spline,
...
void SplineWarpXform::RegisterVolumePoints(
    const DataGrid::IndexType &volDims, const Self::SpaceVectorType &delta,
    const Self::SpaceVectorType &origin) {
  const int ijk[3] = {this->nextI, this->nextJ, this->nextK};
  for (int axis = 0; axis < 3; ++axis)
    this->RegisterVolumeAxis(
        volDims[axis], delta[axis], origin[axis], this->m_Dims[axis], ijk[axis],
        this->m_InverseSpacing[axis], this->m_GridIndexes[axis],
        this->m_GridOffsets[axis], this->m_GridSpline[axis],
        this->m_GridDerivSpline[axis]);
...
SplineWarpXform::GetDeformedControlPointPosition(const int x, const int y,
                                                 const int z) const {
  Self::SpaceVectorType v;

  // Create a pointer to the front-lower-left corner of the c.p.g. cell.
  const Types::Coordinate *coeff =
      m_Parameters + (x - 1) * nextI + (y - 1) * nextJ + (z - 1) * nextK;
...
SplineWarpXform::SpaceVectorType SplineWarpXform ::GetTransformedGrid(
    const int idxX, const int idxY, const int idxZ) const {
  Self::SpaceVectorType v;

  const Types::Coordinate *coeff =
      this->m_Parameters + this->m_GridOffsets[0][idxX] +
      this->m_GridOffsets[1][idxY] + this->m_GridOffsets[2][idxZ];
  const Types::Coordinate *spX = &this->m_GridSpline[0][idxX << 2],
                          *spY = &this->m_GridSpline[1][idxY << 2],
                          *spZ = &this->m_GridSpline[2][idxZ << 2];
...
void SplineWarpXform::GetTransformedGridRow(const int numPoints,
                                            Self::SpaceVectorType *const vIn,
                                            const int idxX, const int idxY,
                                            const int idxZ) const {
  Self::SpaceVectorType *v = vIn;
  const Types::Coordinate *coeff = m_Parameters + this->m_GridOffsets[0][idxX] +
                                   this->m_GridOffsets[1][idxY] +
                                   this->m_GridOffsets[2][idxZ];
  const Types::Coordinate *spX = &this->m_GridSpline[0][idxX << 2],
                          *spY = &this->m_GridSpline[1][idxY << 2],
                          *spZ = &this->m_GridSpline[2][idxZ << 2];
  ...
  const int numberOfCells = (this->m_GridOffsets[0][idxX + numPoints - 1] -
                             this->m_GridOffsets[0][idxX]) /
                                nextI +
                            4;
  ...
  Types::Coordinate
      phiHat[3 * numberOfCells];  // GNU compiler can have variable-sized
                                  // automatic arrays
#else
  ...
  for (const int lastPoint = idxX + numPoints; i < lastPoint;) {
    ...
    do {
      ...
      // performance.
      vRef[0] = spX[0] * phiPtr[0] + spX[1] * phiPtr[3] + spX[2] * phiPtr[6] +
                spX[3] * phiPtr[9];
      vRef[1] = spX[0] * phiPtr[1] + spX[1] * phiPtr[4] + spX[2] * phiPtr[7] +
                spX[3] * phiPtr[10];
      vRef[2] = spX[0] * phiPtr[2] + spX[1] * phiPtr[5] + spX[2] * phiPtr[8] +
                spX[3] * phiPtr[11];
    ...
    } while ((this->m_GridOffsets[0][i - 1] == this->m_GridOffsets[0][i]) &&
             (i < lastPoint));
...
Types::Coordinate SplineWarpXform ::GetGridEnergy() const {
  double energy = 0;
  ...
  for (int z = 1; z < this->m_Dims[2] - 1; ++z) {
    for (int y = 1; y < this->m_Dims[1] - 1; ++y) {
      for (int x = 1; x < this->m_Dims[0] - 1; ++x) {
        const Types::Coordinate *coeff =
            this->m_Parameters + x * nextI + y * nextJ + z * nextK;
        energy += this->GetGridEnergy(coeff);
      }
    }
  }

  return energy / ((this->m_Dims[0] - 2) * (this->m_Dims[1] - 2) *
                   (this->m_Dims[2] - 2));
}

Types::Coordinate SplineWarpXform ::GetGridEnergy(
  ...
  for (int dim = 0; dim < 3; ++dim) {
    ...
    f[dim] = std::max<Types::Coordinate>(
        0, std::min<Types::Coordinate>(1.0, r[dim] - grid[dim]));
  }

  const Types::Coordinate *coeff =
      this->m_Parameters +
      3 * (grid[0] + this->m_Dims[0] * (grid[1] + this->m_Dims[1] * grid[2]));
    ...
    for (int m = 0; m < 3; ++m) {
      ...
      for (int l = 0; l < 3; ++l) {
        ...
        for (int k = 0; k < 3; ++k, coeff_kk += nextI) {
          const Types::Coordinate tmp =
              CubicSpline::ApproxSpline(k, f[0]) * (*coeff_kk);
          kkJ[0] += CubicSpline::SecondDerivApproxSpline(k, f[0]) * (*coeff_kk);
          kkJ[1] += tmp;
          kkJ[2] += tmp;

          const Types::Coordinate tmp2 =
              CubicSpline::DerivApproxSpline(k, f[0]) * (*coeff_kk);
          kkK[0] += tmp2;
...
void SplineWarpXform::GetGridEnergyDerivative(
  ...
  for (int k = kFrom; k < kTo; ++k)
    for (int j = jFrom; j < jTo; ++j)
      for (int i = iFrom; i < iTo; ++i) {
        ground +=
            this->GetGridEnergy(coeff + i * nextI + j * nextJ + k * nextK);
      }
...
Types::Coordinate SplineWarpXform::GetInverseConsistencyError(
    const WarpXform *inverse, const UniformVolume *volume,
    const DataGrid::RegionType *voi) const {
  ...
  const int dX =
      1 + static_cast<int>(this->m_Spacing[0] / 2 * volume->m_Delta[AXIS_X]);
  const int dY =
      1 + static_cast<int>(this->m_Spacing[1] / 2 * volume->m_Delta[AXIS_Y]);
  const int dZ =
      1 + static_cast<int>(this->m_Spacing[2] / 2 * volume->m_Delta[AXIS_Z]);
...
Types::Coordinate *SplineWarpXform::GetPureDeformation(
    const bool includeScale) const {
  const size_t numberOfParameters = this->m_NumberOfParameters;
  Types::Coordinate *points =
      Memory::ArrayC::Allocate<Types::Coordinate>(numberOfParameters);
  ...
  for (size_t pointIdx = 0; pointIdx < numberOfParameters / 3;
       ++pointIdx, ptr += 3) {
    // undo affine transformation component
    const Self::SpaceVectorType v =
        xform->Apply(Self::SpaceVectorType::FromPointer(ptr));

    // copy the result into ouput array
    for (unsigned int dim = 0; dim < 3; ++dim) ptr[dim] = v[dim];
  }
