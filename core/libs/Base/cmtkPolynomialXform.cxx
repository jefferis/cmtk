...
bool PolynomialXform::ApplyInverse(const Self::SpaceVectorType &v,
                                   Self::SpaceVectorType &u,
                                   const Types::Coordinate accuracy) const {
  return this->ApplyInverseWithInitial(
      v, u, v * this->GetGlobalAffineMatrix().GetInverse(), accuracy);
}

const CoordinateMatrix3x3 PolynomialXform::GetJacobian(
  ...
  for (size_t monomialIdx = 0; monomialIdx < this->m_NumberOfMonomials;
       ++monomialIdx) {
    const Types::Coordinate monomialValueDX =
        Polynomial<4, Types::Coordinate>::EvaluateMonomialDXAt(
            monomialIdx, vRel[0], vRel[1], vRel[2]);
    const Types::Coordinate monomialValueDY =
        Polynomial<4, Types::Coordinate>::EvaluateMonomialDYAt(
            monomialIdx, vRel[0], vRel[1], vRel[2]);
    const Types::Coordinate monomialValueDZ =
        Polynomial<4, Types::Coordinate>::EvaluateMonomialDZAt(
            monomialIdx, vRel[0], vRel[1], vRel[2]);
...
const CoordinateMatrix3x3 PolynomialXform::GetLinearMatrix() const {
...
const AffineXform::MatrixType PolynomialXform::GetGlobalAffineMatrix() const {
