...
cmtk::FitSplineWarpToDeformationField::FitSplineWarpToDeformationField(
    DeformationField::SmartConstPtr dfield, const bool absolute)
    : m_FitAbsolute(absolute),
      m_DeformationField(dfield),
      m_DeformationFieldFOV(dfield->m_Offset, dfield->m_Domain) {}

void cmtk::FitSplineWarpToDeformationField::ComputeResiduals(
  ...
  for (int z = 0; z < dims[2]; ++z) {
    ...
    for (int y = 0; y < dims[1]; ++y) {
      for (int x = 0; x < dims[0]; ++x, ++ofs) {
        const Xform::SpaceVectorType xT =
            this->m_DeformationField->GetShiftedControlPointPositionByOffset(
                ofs);
          ...
          if (this->m_FitAbsolute)
            this->m_Residuals[ofs] +=
                this->m_DeformationField
                    ->GetOriginalControlPointPositionByOffset(ofs);
        } else {
          this->m_Residuals[ofs] = Xform::SpaceVectorType(
              std::numeric_limits<Types::Coordinate>::quiet_NaN());
        }
      }
    }
  }
...
cmtk::SplineWarpXform::SmartPtr cmtk::FitSplineWarpToDeformationField::Fit(
  ...
  SplineWarpXform *splineWarp =
      new SplineWarpXform(this->m_DeformationField->m_Domain, initialDims,
...
cmtk::SplineWarpXform::SmartPtr cmtk::FitSplineWarpToDeformationField::Fit(
  ...
  SplineWarpXform *splineWarp = new SplineWarpXform(
      this->m_DeformationField->m_Domain, spacing, initialAffine);
...
void cmtk::FitSplineWarpToDeformationField::FitSpline(
    SplineWarpXform &splineWarp, const int nLevels) {
  ...
  for (int level = 0; level < nLevels; ++level) {
    ...
    // compute residuals
    splineWarp.RegisterVolumePoints(this->m_DeformationField->m_Dims,
                                    this->m_DeformationField->m_Spacing,
                                    this->m_DeformationField->m_Offset);
    ...
    for (RegionIndexIterator<WarpXform::ControlPointRegionType> voxelIt(
             this->m_DeformationField->GetAllControlPointsRegion());
         voxelIt != voxelIt.end(); ++voxelIt) {
      const DataGrid::IndexType voxelIdx = voxelIt.Index();
      const Xform::SpaceVectorType residual =
          this->m_Residuals[this->m_DeformationField->GetOffsetFromIndex(
                                voxelIdx) /
                            3];
        ...
        for (int m = 0; m < 4; ++m) {
          ...
          for (int l = 0; l < 4; ++l) {
            ...
            for (int k = 0; k < 4; ++k) {
              ...
              delta[cpOfs] +=
                  w2klm[m][l][k] * wklm[m][l][k] / sumOfSquares * residual;
