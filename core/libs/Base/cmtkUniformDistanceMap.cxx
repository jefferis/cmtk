...
UniformDistanceMap<TDistanceDataType>::UniformDistanceMap(
    const UniformVolume &volume, const byte flags, const Types::DataItem value,
    const Types::DataItem window) {
...
#pragma omp parallel for
  ...
  this->m_DistanceMap->m_AlternativeIndexToPhysicalMatrices =
...
template <class TDistanceDataType>
void UniformDistanceMap<TDistanceDataType>::BuildDistanceMap(
    const UniformVolume &volume, const byte flags, const Types::DataItem value,
    const Types::DataItem window) {
  this->m_DistanceMap =
      UniformVolume::SmartPtr(new UniformVolume(volume.m_Dims, volume.m_Size));

  TypedArray::SmartPtr distanceArray = TypedArray::SmartPtr(
      TypedArray::Create(DataTypeTraits<DistanceDataType>::DataTypeID,
                         volume.GetNumberOfPixels()));
  DistanceDataType *Distance =
      static_cast<DistanceDataType *>(distanceArray->GetDataPtr());
...
template <class TDistanceDataType>
void UniformDistanceMap<TDistanceDataType>::ComputeEDT(
    DistanceDataType *const distance) {
...
template <class TDistanceDataType>
void UniformDistanceMap<TDistanceDataType>::ComputeEDTThreadPhase1(
  ...
  const size_t nXY =
      ThisConst->m_DistanceMap->m_Dims[0] * ThisConst->m_DistanceMap->m_Dims[1];

  /* compute D_2 */
  /* call edtComputeEDT_2D for each plane */
  DistanceDataType *p = params->m_Distance + nXY * taskIdx;
  for (int k = taskIdx; k < ThisConst->m_DistanceMap->m_Dims[2];
       k += taskCnt, p += nXY * taskCnt) {
    This->ComputeEDT2D(p, This->m_G[threadIdx], This->m_H[threadIdx]);
  }
}

template <class TDistanceDataType>
void UniformDistanceMap<TDistanceDataType>::ComputeEDTThreadPhase2(
    void *const args, const size_t taskIdx, const size_t taskCnt,
    const size_t threadIdx, const size_t) {
  ThreadParametersEDT *params = static_cast<ThreadParametersEDT *>(args);
  Self *This = params->thisObject;
  ...
  for (size_t i = taskIdx; i < nXY; i += taskCnt) {
    ...
    for (int k = 0; k < ThisConst->m_DistanceMap->m_Dims[2];
         k++, p += nXY, q++) {
      *q = *p;
    }
    ...
    /* call edtVoronoiEDT */
    if (This->VoronoiEDT(
            &f[0], ThisConst->m_DistanceMap->m_Dims[2],
            static_cast<DistanceDataType>(ThisConst->m_DistanceMap->m_Delta[2]),
            This->m_G[threadIdx], This->m_H[threadIdx])) {
...
template <class TDistanceDataType>
void UniformDistanceMap<TDistanceDataType>::ComputeEDT2D(
    DistanceDataType *const plane, std::vector<DistanceDataType> &gTemp,
 ...
 */
{
  ...
  for (int j = 0; j < this->m_DistanceMap->m_Dims[1]; j++) {
    ...
    DistanceDataType d =
        static_cast<DistanceDataType>(Self::EDT_MAX_DISTANCE_SQUARED);
    for (int i = 0; i < this->m_DistanceMap->m_Dims[0]; i++, p++) {
      ...
      /* increment distance ... */
      else if (d != Self::EDT_MAX_DISTANCE_SQUARED) {
        *p = ++d;
      }
    ...
    if (*(--p) != Self::EDT_MAX_DISTANCE_SQUARED) {
      ...
      for (int i = this->m_DistanceMap->m_Dims[0] - 1; i >= 0; i--, p--) {
        ...
        *p =
            static_cast<DistanceDataType>(*p * this->m_DistanceMap->m_Delta[0]);
        *p *= *p;
      }
    }
  }
  ...
  for (int i = 0; i < this->m_DistanceMap->m_Dims[0]; i++) {
    ...
    p = plane + i;
    DistanceDataType *q = &f[0];
    for (int j = 0; j < this->m_DistanceMap->m_Dims[1];
         j++, p += this->m_DistanceMap->m_Dims[0], q++) {
      *q = *p;
    }

    /* call edtVoronoiEDT */
    if (this->VoronoiEDT(
            &f[0], this->m_DistanceMap->m_Dims[1],
            static_cast<DistanceDataType>(this->m_DistanceMap->m_Delta[1]),
            gTemp, hTemp)) {
      p = plane + i;
      q = &f[0];
      for (int j = 0; j < this->m_DistanceMap->m_Dims[1];
...
template <class TDistanceDataType>
bool UniformDistanceMap<TDistanceDataType>::VoronoiEDT(
    DistanceDataType *const distanceSoFar, const int nSize,
    const DistanceDataType delta, std::vector<DistanceDataType> &gTemp,
    std::vector<DistanceDataType> &hTemp) {
  long i, l, n_S;
    ...
    if (distanceSoFar[i] != Self::EDT_MAX_DISTANCE_SQUARED) {
      ...
      /* line 7 */
        ...
        while (l >= 1) {
          ...
          c = a + b;
          /* compute Eq. 2 */
          if ((c * g[l] - b * g[l - 1] - a * distanceSoFar[i] - a * b * c) >
              0) {
            /* line 9 */
            l--;
          } else {
