...
TransformedVolumeAxes::TransformedVolumeAxes(
    ...
    const Types::Coordinate *deltas, const Types::Coordinate *otherOrigin) {
  // define volume corners
  UniformVolume::CoordinateVectorType dX =
      FixedVectorStaticInitializer<3, Types::Coordinate>::Init(1, 0, 0);
  UniformVolume::CoordinateVectorType dY =
      FixedVectorStaticInitializer<3, Types::Coordinate>::Init(0, 1, 0);
  UniformVolume::CoordinateVectorType dZ =
  ...
  if (deltas) {
    const UniformVolume::CoordinateVectorType deltasV =
        UniformVolume::CoordinateVectorType::FromPointer(deltas);
    dX /= deltasV;
    dY /= deltasV;
    dZ /= deltasV;
    V /= deltasV;
...
TransformedVolumeAxes::TransformedVolumeAxes(const UniformVolume &volume,
                                             const ParametricPlane &mirrorPlane,
                                             const Types::Coordinate *deltas) {
  // define volume corners
  UniformVolume::CoordinateVectorType dX =
      FixedVectorStaticInitializer<3, Types::Coordinate>::Init(1, 0, 0);
  UniformVolume::CoordinateVectorType dY =
...
void TransformedVolumeAxes::MakeHash(
    const UniformVolume &volume, const UniformVolume::SpaceVectorType &offset,
  ...
  for (int dim = 0; dim < 3; ++dim) {
    this->m_Hash[dim] =
        Memory::ArrayC::Allocate<UniformVolume::SpaceVectorType>(
            this->m_Dims[dim]);
