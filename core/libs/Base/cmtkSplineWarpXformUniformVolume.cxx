...
cmtk::SplineWarpXformUniformVolume::SplineWarpXformUniformVolume(
    const UniformVolume &volume, const SplineWarpXform::SmartConstPtr &xform)
    : m_Xform(xform) {
  this->RegisterVolume(volume);
}

void cmtk::SplineWarpXformUniformVolume::RegisterVolumeAxis(
    const int dim, const Types::Coordinate delta,
    const Types::Coordinate origin, const int cpgDim,
    const Types::Coordinate invCpgSpacing, std::vector<int> &g,
...
void cmtk::SplineWarpXformUniformVolume::RegisterVolume(
    const UniformVolume &volume) {
  const SplineWarpXform &xform = *(this->m_Xform);

  this->RegisterVolumeAxis(
      volume.m_Dims[0], volume.m_Delta[0], volume.m_Offset[0], xform.m_Dims[0],
      xform.m_InverseSpacing[0], this->gX, this->splineX, this->dsplineX);
  this->RegisterVolumeAxis(
      volume.m_Dims[1], volume.m_Delta[1], volume.m_Offset[1], xform.m_Dims[1],
      xform.m_InverseSpacing[1], this->gY, this->splineY, this->dsplineY);
  this->RegisterVolumeAxis(
      volume.m_Dims[2], volume.m_Delta[2], volume.m_Offset[2], xform.m_Dims[2],
      xform.m_InverseSpacing[2], this->gZ, this->splineZ, this->dsplineZ);

  for (int idx = 0; idx < volume.m_Dims[0]; ++idx) gX[idx] *= xform.nextI;
  for (int idx = 0; idx < volume.m_Dims[1]; ++idx) gY[idx] *= xform.nextJ;
  for (int idx = 0; idx < volume.m_Dims[2]; ++idx) gZ[idx] *= xform.nextK;
}

void cmtk::SplineWarpXformUniformVolume ::GetTransformedGrid(
    Vector3D &v, const int idxX, const int idxY, const int idxZ) const {
  const SplineWarpXform &xform = *(this->m_Xform);

  const Types::Coordinate *coeff =
      xform.m_Parameters + gX[idxX] + gY[idxY] + gZ[idxZ];
  const Types::Coordinate *spX = &splineX[idxX << 2],
                          *spY = &splineY[idxY << 2],
                          *spZ = &splineZ[idxZ << 2];
...
void cmtk::SplineWarpXformUniformVolume::GetTransformedGridRow(
    Vector3D *const vIn, const size_t numPoints, const int idxX, const int idxY,
  ...
  const int numberOfCells =
      (gX[idxX + numPoints - 1] - gX[idxX]) / xform.nextI + 4;
