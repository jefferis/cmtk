...
class HypothesisTests {
 public:
  ...
  static TypedArray::SmartPtr GetPairedCorrelation(
      std::vector<TypedArray::SmartPtr> &dataX,
      std::vector<TypedArray::SmartPtr> &dataY,
      TypedArray::SmartPtr *pData = NULL, const TypedArray *mask = NULL);

  /// Test mean of Jacobian map of a single population for difference from zero.
  static TypedArray::SmartPtr GetOneSampleTTest(
      std::vector<TypedArray::SmartPtr> &dataX, TypedArray::SmartPtr *tstatData,
      TypedArray::SmartPtr *avgXData, const TypedArray *mask = NULL);
   ...
   */
  static TypedArray::SmartPtr GetZScores(
      std::vector<TypedArray::SmartPtr> &dataX,
      std::vector<TypedArray::SmartPtr> &dataY, const TypedArray *mask = NULL);

  /// Get pixelwise genetic covariance from MZ and DZ twin data.
  static TypedArray::SmartPtr GetGeneticCovariance(
      std::vector<TypedArray::SmartPtr> &dataMZ,
      std::vector<TypedArray::SmartPtr> &dataDZ, const TypedArray *mask = NULL);
};
