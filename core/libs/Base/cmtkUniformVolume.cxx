...
UniformVolume::UniformVolume(const UniformVolume &other)
    : Volume(other),
      m_Delta(other.m_Delta),
      m_IndexToPhysicalMatrix(other.m_IndexToPhysicalMatrix),
      m_AlternativeIndexToPhysicalMatrices(
          other.m_AlternativeIndexToPhysicalMatrices) {}

UniformVolume::UniformVolume(const DataGrid::IndexType &dims,
                             const Self::CoordinateVectorType &size,
                             TypedArray::SmartPtr &data)
    : Volume(dims, size, data) {
  for (int i = 0; i < 3; ++i) {
    this->m_Delta[i] = ((this->m_Dims[i] > 1) && (this->m_Size[i] > 0))
                           ? this->m_Size[i] / (this->m_Dims[i] - 1)
                           : 1.0;
  }
...
UniformVolume *UniformVolume::CloneVirtual(const bool copyData) {
  if (copyData) {
...
UniformVolume *UniformVolume::CloneVirtual() const {
  UniformVolume *result = this->CloneGridVirtual();
...
UniformVolume *UniformVolume::CloneGridVirtual() const {
  ...
  clone->m_AlternativeIndexToPhysicalMatrices =
      this->m_AlternativeIndexToPhysicalMatrices;
  return clone;
}

UniformVolume *UniformVolume::GetDownsampledAndAveraged(
    const Types::GridIndexType downsample, const bool approxIsotropic) const {
  if (approxIsotropic) {
    const Types::Coordinate minDelta = std::min<Types::Coordinate>(
        this->m_Delta[0],
        std::min<Types::Coordinate>(this->m_Delta[1], this->m_Delta[2]));
    const Types::GridIndexType downsampleByAxis[3] = {
        std::max<Types::GridIndexType>(
            1, downsample / std::max<Types::GridIndexType>(
                                1, static_cast<Types::GridIndexType>(
                                       this->m_Delta[0] / minDelta))),
        std::max<Types::GridIndexType>(
            1, downsample / std::max<Types::GridIndexType>(
                                1, static_cast<Types::GridIndexType>(
                                       this->m_Delta[1] / minDelta))),
        std::max<Types::GridIndexType>(
            1, downsample / std::max<Types::GridIndexType>(
                                1, static_cast<Types::GridIndexType>(
                                       this->m_Delta[2] / minDelta)))};
    return this->GetDownsampledAndAveraged(downsampleByAxis);
  } else {
    const Types::GridIndexType downsampleByAxis[3] = {downsample, downsample,
                                                      downsample};
    return this->GetDownsampledAndAveraged(downsampleByAxis);
  }
}
...
UniformVolume *UniformVolume::GetDownsampledAndAveraged(
  ...
  DataGrid::SmartPtr newDataGrid(
      this->DataGrid::GetDownsampledAndAveraged(downsample));
  TypedArray::SmartPtr newData = newDataGrid->GetData();

  // create downsample grid
  UniformVolume *dsVolume = new UniformVolume(
      newDataGrid->GetDims(), downsample[0] * this->m_Delta[0],
      downsample[1] * this->m_Delta[1], downsample[2] * this->m_Delta[2],
      newData);

  // compute shift of volume origin
  const Types::Coordinate shift[3] = {
      (downsample[0] - 1) * this->m_Delta[0] / 2,
      (downsample[1] - 1) * this->m_Delta[1] / 2,
      (downsample[2] - 1) * this->m_Delta[2] / 2};
  ...
  for (int axis = 0; axis < 3; ++axis) {
    for (int i = 0; i < 3; ++i) {
      dsVolume->m_IndexToPhysicalMatrix[3][i] +=
          (downsample[i] - 1) * dsVolume->m_IndexToPhysicalMatrix[axis][i] / 2;
  ...
  dsVolume->m_AlternativeIndexToPhysicalMatrices =
      this->m_AlternativeIndexToPhysicalMatrices;
  for (std::map<int, AffineXform::MatrixType>::iterator it =
           dsVolume->m_AlternativeIndexToPhysicalMatrices.begin();
       it != dsVolume->m_AlternativeIndexToPhysicalMatrices.end(); ++it) {
    // apply offset shift to index-to-physical matrix
    for (int axis = 0; axis < 3; ++axis) {
      for (int i = 0; i < 3; ++i) {
...
UniformVolume *UniformVolume::GetInterleavedSubVolume(
  ...
  if (this->m_Dims[axis] % factor > idx) ++dims[axis];
  ...
  for (int i = 0; i < 3; ++i)
    volume->m_IndexToPhysicalMatrix[3][i] +=
        idx * volume->m_IndexToPhysicalMatrix[axis][i];
  // scale direction vector along axis
  for (int i = 0; i < 3; ++i)
    volume->m_IndexToPhysicalMatrix[axis][i] *= factor;

  // do the same for any alternative matrices
  volume->m_AlternativeIndexToPhysicalMatrices =
      this->m_AlternativeIndexToPhysicalMatrices;
  for (std::map<int, AffineXform::MatrixType>::iterator it =
    ...
    // scale direction vector along axis
    for (int i = 0; i < 3; ++i) (it->second)[axis][i] *= factor;
  }
...
UniformVolume *UniformVolume::GetInterleavedPaddedSubVolume(
    const int axis, const Types::GridIndexType factor,
    const Types::GridIndexType idx) const {
  Types::GridIndexType sDims = this->m_Dims[axis] / factor;
  if (this->m_Dims[axis] % factor > idx) ++sDims;
...
const UniformVolume::RegionType UniformVolume::GetGridRange(
    const Self::CoordinateRegionType &region) const {
  Self::IndexType from, to;

  for (size_t i = 0; i < 3; ++i) {
    from[i] = std::max<IndexType::ValueType>(
        0, static_cast<IndexType::ValueType>(
               (region.From()[i] - this->m_Offset[i]) / this->m_Delta[i]));
    to[i] = 1 + std::min(this->m_Dims[i] - 1,
                         1 + static_cast<IndexType::ValueType>(
                                 (region.To()[i] - this->m_Offset[i]) /
                                 this->m_Delta[i]));
  }
...
void UniformVolume::Mirror(const int axis) {
  this->DataGrid::ApplyMirrorPlane(axis);

  this->CropRegion().From()[axis] =
      this->m_Dims[axis] - 1 - this->CropRegion().From()[axis];
  this->CropRegion().To()[axis] =
      this->m_Dims[axis] - 1 - this->CropRegion().To()[axis];
}

ScalarImage::SmartPtr UniformVolume::GetOrthoSlice(
...
UniformVolume::SmartPtr UniformVolume::ExtractSlice(
    const int axis, const Types::GridIndexType plane) const {
  DataGrid::SmartPtr sliceGrid(this->DataGrid::ExtractSlice(axis, plane));
  Self::SmartPtr sliceVolume(new Self(sliceGrid->m_Dims, this->m_Delta[0],
                                      this->m_Delta[1], this->m_Delta[2],
                                      sliceGrid->m_Data));
...
ScalarImage::SmartPtr UniformVolume::GetNearestOrthoSlice(
    const int axis, const Types::Coordinate location) const {
  return this->GetOrthoSlice(axis, this->GetCoordIndex(axis, location));
}

ScalarImage::SmartPtr UniformVolume::GetOrthoSliceInterp(
    const int axis, const Types::Coordinate location) const {
  Types::GridIndexType baseSliceIndex = this->GetCoordIndex(axis, location);

  const Types::Coordinate baseSliceLocation =
      this->GetPlaneCoord(axis, baseSliceIndex);
  const Types::Coordinate nextSliceLocation =
      this->GetPlaneCoord(axis, baseSliceIndex + 1);

  // only bother to interpolate if we're more than 1% towards the next slice.
  if (((location - baseSliceLocation) /
       (nextSliceLocation - baseSliceLocation)) < 0.01)
    return this->GetOrthoSlice(axis, baseSliceIndex);

  // only bother to interpolate if we're more than 1% towards the next slice.
  if (((nextSliceLocation - location) /
       (nextSliceLocation - baseSliceLocation)) < 0.01)
  ...
  Types::Coordinate weight0 =
      (nextSliceLocation - location) / (nextSliceLocation - baseSliceLocation);
  ...
  image0->SetImageOrigin(weight0 * image0->GetImageOrigin() +
                         (1 - weight0) * image1->GetImageOrigin());

  return image0;
}

void UniformVolume ::GetPrincipalAxes(
    Matrix3x3<Types::Coordinate> &directions,
  ...
  const EigenSystemSymmetricMatrix3x3<Types::Coordinate> eigensystem(
      inertiaMatrix);
  for (int n = 0; n < 3; ++n) {
  ...
  for (int i = 0; i < 3; i++) {
    const Types::Coordinate norm = sqrt(directions[i][0] * directions[i][0] +
