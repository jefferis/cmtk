...
void AffineXform::MakeIdentityXform() {
  this->m_ParameterVector->Clear();
  Types::Coordinate *scales = this->RetScales();
  if (!m_LogScaleFactors) scales[0] = scales[1] = scales[2] = 1;
  try {
    this->ComposeMatrix();
  } catch (const Self::MatrixType::SingularMatrixException &) {
    StdErr << "ERROR: identify matrix construction created singular matrix - "
...
AffineXform::AffineXform(const AffineXform &other)
    : Xform(other), m_LogScaleFactors(false) {
  ...
  try {
    ...
    StdErr << "ERROR: construction from existing AffineXform object created "
              "singular matrix - this should not happen! Aborting...\n";
    throw ExitException(1);
  }
}

AffineXform::AffineXform(const Types::Coordinate matrix[4][4],
                         const Types::Coordinate *center)
    : Matrix(&matrix[0][0]), m_LogScaleFactors(false), InverseXform(NULL) {
  this->AllocateParameterVector(TotalNumberOfParameters);
...
AffineXform::AffineXform(const MatrixType &matrix,
                         const Types::Coordinate *center)
    : Matrix(matrix), m_LogScaleFactors(false), InverseXform(NULL) {
  this->AllocateParameterVector(TotalNumberOfParameters);
  this->NumberDOFs = this->DefaultNumberOfDOFs();
  if (center)
    memcpy(this->RetCenter(), center, 3 * sizeof(Types::Coordinate));
...
AffineXform &AffineXform::operator=(const AffineXform &other) {
  (*this->m_ParameterVector) = (*other.m_ParameterVector);
  this->NumberDOFs = other.NumberDOFs;
  this->m_LogScaleFactors = other.m_LogScaleFactors;
  try {
    ...
    StdErr << "ERROR: assignment from existing AffineXform object created "
              "singular matrix - this should not happen! Aborting...\n";
    throw ExitException(1);
  }
...
void AffineXform::SetNumberDOFs(const int numberDOFs) {
...
void AffineXform::SetUseLogScaleFactors(const bool logScaleFactors) {
...
void AffineXform::ComposeMatrix() {
...
bool AffineXform::DecomposeMatrix() {
  return this->Matrix.Decompose(this->m_Parameters, this->RetCenter(),
                                this->m_LogScaleFactors);
}

AffineXform::SpaceVectorType AffineXform::RotateScaleShear(
...
AffineXform *AffineXform::MakeInverse() const {
  ...
  const Self::SpaceVectorType newCenter =
      Self::SpaceVectorType::FromPointer(this->RetCenter()) * this->Matrix;
  inverseXform->ChangeCenter(newCenter);

  if (this->NumberDOFs == 7) {
    inverseXform->m_Parameters[8] =
        (inverseXform->m_Parameters[7] = inverseXform->m_Parameters[6]);
    inverseXform->Matrix.Compose(inverseXform->m_Parameters,
                                 this->m_LogScaleFactors);
  }
...
void AffineXform::ChangeCenter(const Self::SpaceVectorType &newCenter) {
  Types::Coordinate *const xlate = this->RetXlate();
  Types::Coordinate *const center = this->RetCenter();
  Self::SpaceVectorType deltaCenter =
      newCenter - Self::SpaceVectorType::FromPointer(center);

  for (size_t i = 0; i < 3; ++i) xlate[i] -= deltaCenter[i];
...
void AffineXform::SetMatrix(const MatrixType &matrix) {
  this->Matrix = matrix;
  this->DecomposeMatrix();
  this->UpdateInverse();
}

void AffineXform::SetParamVector(CoordinateVector &v) {
...
void AffineXform::SetParamVector(const CoordinateVector &v) {
  Superclass::SetParamVector(v);
  this->CanonicalRotationRange();
  this->ComposeMatrix();
}

void AffineXform::SetParameter(const size_t idx, const Types::Coordinate p) {
  Superclass::SetParameter(idx, p);
  this->CanonicalRotationRange();
  this->ComposeMatrix();
}

std::set<short> AffineXform::GetSupportedDOFs() {
  const short supportedDOFs[] = {0, 3, 6, 7, 9, 12, 3003, 3033, 3303, -1};
  return std::set<short>(&supportedDOFs[0], &supportedDOFs[9]);
}

Types::Coordinate AffineXform::GetParamStep(
    const size_t idx, const Self::SpaceVectorType &volSize,
    const Types::Coordinate mmStep) const {
  ...
  switch (idx) {
    ...
    case 2:
      return mmStep;
    case 3:
      if (this->NumberDOFs == 3003 ||
          this->NumberDOFs == 3033) {  // special case: 3 DOFs translation plus
                                       // 3 DOFs shear, but no rotation
        return 0;
      } else {
        return mmStep * 180 /
               (M_PI * sqrt(MathUtil::Square(volSize[1]) +
                            MathUtil::Square(volSize[2])));
      }
    case 4:
      if (this->NumberDOFs == 3003 ||
          this->NumberDOFs == 3033) {  // special case: 3 DOFs translation plus
        ...
        return mmStep * 180 /
               (M_PI * sqrt(MathUtil::Square(volSize[0]) +
                            MathUtil::Square(volSize[2])));
      }
    case 5:
      if (this->NumberDOFs == 3003 ||
        ...
        return mmStep * 180 /
               (M_PI * sqrt(MathUtil::Square(volSize[0]) +
                            MathUtil::Square(volSize[1])));
      }
    case 6:
    case 7:
    case 8:
      if (this->NumberDOFs == 3303 ||
          this->NumberDOFs == 3003) {  // special case: 6 DOFs rigid plus 3 DOFs
                                       // shear, but no scale
        return 0;
      } else {
...
void AffineXform::Concat(const Self &other) {
  this->Matrix *= other.Matrix;
  try {
    this->DecomposeMatrix();
  } catch (const Self::MatrixType::SingularMatrixException &) {
    StdErr << "ERROR: singular matrix in cmtk::AffineXform::Concat; this "
              "should not happen.\n";
    throw ExitException(1);
  }
}

void AffineXform::Insert(const Self &other) {
  ...
  try {
    ...
    StdErr << "ERROR: singular matrix in cmtk::AffineXform::Insert; this "
              "should not happen.\n";
    throw ExitException(1);
  }
}

void AffineXform::RotateWXYZ(const Units::Radians angle,
                             const Self::SpaceVectorType &direction,
                             const Types::Coordinate *center,
                             Self::MatrixType *const accumulate) {
  ...
  try {
    ...
    StdErr << "ERROR: singular matrix in cmtk::AffineXform::WXYZ; this should "
              "not happen.\n";
    throw ExitException(1);
  }
...
AffineXform::SmartPtr &AffineXform::GetInverse() {
  if (!InverseXform) {
...
const AffineXform::SmartPtr &AffineXform::GetInverse() const {
  if (!InverseXform) {
    InverseXform = AffineXform::SmartPtr(this->MakeInverse());
  } else {
    this->UpdateInverse();
...
void AffineXform::UpdateInverse() const {
  if (InverseXform) {
...
void AffineXform::CanonicalRotationRange() {
