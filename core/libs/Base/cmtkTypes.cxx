...
const char *DataClassString[] = {"grey", "label", "unknown", NULL};

const char *DataTypeName[] = {"byte (8bit unsigned)", "char (8bit signed)",
                              "short (16bit signed)", "ushort (16bit unsigned)",
                              "int (32bit signed)",   "uint (32bit unsigned)",
                              "float (32bit)",        "double (64bit)"};

DataClass StringToDataClass(const char *dataClassStr) {
  if (dataClassStr) {
    for (int idx = 0; DataClassString[idx]; ++idx) {
      if (!strcmp(dataClassStr, DataClassString[idx])) return (DataClass)idx;
    }
  }
