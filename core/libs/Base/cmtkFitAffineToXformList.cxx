...
cmtk::AffineXform::SmartPtr cmtk::FitAffineToXformList::Fit(
    const bool fitRigid) {
  ...
  size_t ofs = 0;
  for (RegionIndexIterator<DataGrid::RegionType> it =
  ...
  Matrix3x3<Types::Coordinate> matrix =
      fitRigid ? this->GetMatrixRigidSVD(cFrom, cTo)
               : this->GetMatrixAffinePseudoinverse(cFrom, cTo);
...
cmtk::FitAffineToXformList::GetMatrixAffinePseudoinverse(
  ...
  for (RegionIndexIterator<DataGrid::RegionType> it =
    ...
    if (this->m_XformValidAt[ofs]) {
      const cmtk::FixedVector<3, cmtk::Types::Coordinate> x =
          this->m_XformField.GetGridLocation(it.Index()) - cFrom;
      const cmtk::FixedVector<3, cmtk::Types::Coordinate> t =
          this->m_XformField[ofs] - cTo;
...
cmtk::Matrix3x3<cmtk::Types::Coordinate>
cmtk::FitAffineToXformList::GetMatrixRigidSVD(
    const cmtk::FixedVector<3, cmtk::Types::Coordinate> &cFrom,
    const cmtk::FixedVector<3, cmtk::Types::Coordinate> &cTo) {
  // compute the transformation matrix for rotation, scale, shear, using the
  // previously computed centroids for reference
