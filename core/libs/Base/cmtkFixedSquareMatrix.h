...
class FixedSquareMatrix {
 ...
 public:
  ...
  FixedSquareMatrix(
      const FixedSquareMatrix<N2, T2> &other /*!< Source matrix object */,
      const size_t iOfs = 0 /*!< Sub-matrix offset in i index */,
      const size_t jOfs = 0 /*!< Sub-matrix offset in i index */);
  ...
  FixedSquareMatrix(const T2 (&matrix)[NDIM][NDIM]) {
    for (size_t j = 0; j < NDIM; ++j)
      for (size_t i = 0; i < NDIM; ++i) this->m_Matrix[j][i] = matrix[j][i];
  }
 ...
 protected:
  ...
  friend FixedSquareMatrix<N, TYPE> operator+(
      const FixedSquareMatrix<N, TYPE> &, const FixedSquareMatrix<N, TYPE> &);
  template <size_t N, class TYPE>
  friend FixedSquareMatrix<N, TYPE> operator-(
      const FixedSquareMatrix<N, TYPE> &, const FixedSquareMatrix<N, TYPE> &);
};
...
FixedVector<NDIM, T> &operator*=(FixedVector<NDIM, T> &u,
                                 const FixedSquareMatrix<NDIM, T> &M) {
...
template <size_t NDIM, class T>
FixedVector<NDIM, T> &operator*=(FixedVector<NDIM, T> &u,
                                 const FixedSquareMatrix<NDIM + 1, T> &M) {
  FixedVector<NDIM, T> v;
  for (size_t i = 0; i < NDIM; ++i) {
    v[i] = u[0] * M[0][i];
    for (size_t j = 1; j < NDIM; ++j) {
...
inline Console &operator<<(Console &stream,
                           const FixedSquareMatrix<NDIM, TSCALAR> &m) {
  stream << NDIM << "x" << NDIM << " Matrix:\n";
  for (int i = 0; i < NDIM; ++i) {
    for (int j = 0; j < NDIM; ++j) stream << m[i][j] << "\t";
    stream << "\n";
  }
  return stream;
}
