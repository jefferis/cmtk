...
bool UniformVolumeInterpolatorPartialVolume ::GetDataAt(
    const Vector3D &v, Types::DataItem &value) const {
  value = 0;
  ...
  for (int n = 0; n < 3; ++n) {
    ...
    if ((imageGridPoint[n] < 0) ||
        (imageGridPoint[n] >= this->m_VolumeDims[n] - 1))
      return false;
  }

  const size_t offset = this->GetOffsetFromIndex(
      imageGridPoint[0], imageGridPoint[1], imageGridPoint[2]);
  ...
  for (Types::GridIndexType k = 0; k < 2; ++k) {
    for (Types::GridIndexType j = 0; j < 2; ++j) {
      for (Types::GridIndexType i = 0; i < 2; ++i, ++idx) {
        corners[idx] =
            this->m_VolumeDataArray[offset + this->GetOffsetFromIndex(i, j, k)];
  ...
  if (dataPresent) {
    ...
    const Types::Coordinate weights[8] = {
        offsX * offsY * offsZ, revX * offsY * offsZ, offsX * revY * offsZ,
        revX * revY * offsZ,   offsX * offsY * revZ, revX * offsY * revZ,
        offsX * revY * revZ,   revX * revY * revZ};
...
Types::DataItem UniformVolumeInterpolatorPartialVolume ::GetDataDirect(
