...
cmtk::DataGridMorphologicalOperators::GetBinaryConnectedComponents() const {
  ...
  // this vector will hold the component index per pixel; since we move strictly
  // forward and only look back in the algorithm below, there is no need to
        ...
        if (this->m_DataGrid->GetDataAt(offset) != 0) {
          ...
          for (int dim = 2; dim >= 0; --dim) {
            ...
            if (index[dim]) {
              ...
              if (existing) {
                // did we already get a different component ID from another
                // neighbor?
                if (component &&
                    (component !=
                     existing))  // note: this implies "component != 0" because
                                 // "existing" is at least 1 in this branch.
                {
                  // link old and new component via this pixel
                  connected.Union(connected.Find(component),
                                  connected.Find(existing));
                }
          ...
          // none of the neighbors is foreground, so use next available
          // component ID.
          if (!component) {
  ...
  TypedArray::SmartPtr resultArray(
      TypedArray::Create(TYPE_INT, numberOfPixels));
...
cmtk::DataGridMorphologicalOperators::GetRegionsRenumberedBySize() const {
  ...
  for (size_t px = 0; px < numberOfPixels; ++px) {
    ...
    if (value) ++regionSizeMap[value];
  }

  // now create list sorted by region size
  std::list<std::pair<const int, int>> sortedList;
  for (std::map<int, int>::const_iterator it = regionSizeMap.begin();
       it != regionSizeMap.end(); ++it) {
    std::list<std::pair<const int, int>>::iterator ins = sortedList.begin();
    while ((ins != sortedList.end()) && (ins->second >= it->second)) ++ins;

    sortedList.insert(ins,
                      *it);  // need to explicitly create new pair because some
                             // STL implementation have it->first as "const".
  }

  // create renumbering lookup map
  std::map<int, int> renumberMap;
  int component = 1;
  for (std::list<std::pair<const int, int>>::iterator it = sortedList.begin();
  ...
  for (size_t px = 0; px < numberOfPixels; ++px) {
    resultArray->Set(
        renumberMap[static_cast<int>(this->m_DataGrid->GetDataAt(px))], px);
  }
