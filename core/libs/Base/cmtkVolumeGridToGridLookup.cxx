...
VolumeGridToGridLookup ::VolumeGridToGridLookup(const UniformVolume &fromVolume,
                                                const UniformVolume &toVolume)
  ...
  for (int dim = 0; dim < 3; ++dim) {
    ...
    for (int toIdx = 0; toIdx < toVolume.m_Dims[dim]; ++toIdx) {
      const Types::Coordinate toGridLo =
          std::max<Types::Coordinate>(0.0, (toIdx - 0.5) * toGridDelta);
      const Types::Coordinate toGridHi = std::min<Types::Coordinate>(
          toVolume.m_Size[dim], (0.5 + toIdx) * toGridDelta);
      this->m_Length[dim][toIdx] = toGridHi - toGridLo;

      Types::Coordinate fromGridHi = std::min<Types::Coordinate>(
          toVolume.m_Size[dim], (0.5 + fromIdx) * fromGridDelta);
      ...
      fromGridHi =
          std::min<Types::Coordinate>(fromVolume.m_Size[dim], fromGridHi);

      int idx = 0;
      Types::Coordinate fromGridLo =
          std::max<Types::Coordinate>(0.0, (fromIdx - 0.5) * fromGridDelta);
      for (int p = fromIdx;
           (p < fromVolume.m_Dims[dim]) && (fromGridLo < toGridHi);
           ++p, ++idx) {
        weightList[idx] =
            MathUtil::Intersect(toGridLo, toGridHi, fromGridLo, fromGridHi);
