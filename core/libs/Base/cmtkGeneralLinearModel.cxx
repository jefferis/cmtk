...
#define TOL 1.0e-5
GeneralLinearModel::GeneralLinearModel(const size_t nParameters,
                                       const size_t nData,
...
void GeneralLinearModel::LeastSquares() {
  ...
  for (size_t j = 0; j < NParameters; ++j) {
    ...
    this->VariableSD[j] =
        MathUtil::Variance<double>(data, this->VariableMean[j]);
...
Matrix2D<double> *GeneralLinearModel::GetCorrelationMatrix() const {
  Matrix2D<double> *CC =
      new Matrix2D<double>(this->NParameters, this->NParameters);
...
void GeneralLinearModel::InitResults(const size_t nPixels) {
  Model.clear();
  TStat.clear();
  for (size_t p = 0; (p < this->NParameters); ++p) {
...
void GeneralLinearModel::FitModel(std::vector<TypedArray::SmartPtr> &y,
                                  const bool normalizeParameters) {
  ...
  for (size_t n = 0; n < nPixels; ++n) {
    ...
    if (missing) {
      ...
      // use SVD of design matrix to compute model parameters lm_params[] from
      // data b[]
      MathUtil::SVDLinearRegression(*(this->U), *(this->W), *(this->V), b,
                                    lm_params);
      ...
      for (size_t p = 0; (p < this->NParameters); ++p) {
        ...
        if (normalizeParameters)
          ...
          //	  Model[p]->Set( lm_params[p] * this->GetNormFactor( p ) / sqrt(
          // varY ), n );
      ...
      for (size_t p = 0; p < this->NParameters; ++p) {
        ...
        MathUtil::SVDLinearRegression(*(this->Up[p]), *(this->Wp[p]),
                                      *(this->Vp[p]), b, lm_params_P);
        ...
        for (size_t i = 0; i < this->NData; i++) {
          ...
          if (!MathUtil::IsFinite(tStat)) tStat = 0;
