...
class UniformVolume :
 ...
 public:
  ...
  const CoordinateVectorType &Deltas() const { return this->m_Delta; }

  /// Get pixel size vector.
  CoordinateVectorType &Deltas() { return this->m_Delta; }
   ...
   */
  UniformVolume(const DataGrid::IndexType &dims,
   ...
   *\param data An existing TypedArray containing the scalar voxel data.
   */
  UniformVolume(const DataGrid::IndexType &dims, const Types::Coordinate deltaX,
                const Types::Coordinate deltaY, const Types::Coordinate deltaZ,
                TypedArray::SmartPtr &data = TypedArray::SmartPtr::Null());

  /// Test whether this grid matches another one, i.e., has the same pixel
  /// sizes.
  bool GridMatches(const Self &other) const {
    return Superclass::GridMatches(other) &&
           ((this->m_Delta - other.m_Delta).MaxAbsValue() < 1e-5) &&
           ((this->m_Offset - other.m_Offset).MaxAbsValue() < 1e-5);
  }
   ...
   */
  virtual UniformVolume *GetResampled(const Types::Coordinate resolution,
                                      const bool allowUpsampling = false) const;

  /** Resample volume to an exact given resolution.
   * Unlike GetResampled(), this function forces the pixel size of the resampled
   *image to be exactly the given "resolution." Grid dimensions and volume size
   *are adjusted accordingly. \param resolution Resolution of the newly created
   *volume in world units. \return Newly created, resampled volume.
   */
  virtual UniformVolume *GetResampledExact(
  ...
  /** Some images (notably those read from NIFTI files) may have several
   * alternative transformations to different spaces (e.g., physical and atlas
   * spaces). The map key is an integer that identifies the type of
   * transformation by some coding system (e.g., we use positive values for
   * NIFTI qforms, negative ones for sforms).
   */
  std::map<int, AffineXform::MatrixType> m_AlternativeIndexToPhysicalMatrices;

  /** Change volume coordinate space.
   * Re-arrange volume's direction vectors to refer to a different coordinate
   *space. \invariant This should keep the result of
   *GetOrientationFromDirections invariant.
   */
   ...
   * The new matrix is the identity matrix with diagonal elements scaled
   ...
   */
  virtual void SetImageToPhysicalMatrix(
      const AffineXform::MatrixType
          &matrix /*!< Existing image-to-physical matrix */);
  ...
  const UniformVolume::SmartPtr GetReoriented(
      const char *newOrientation =
          AnatomicalOrientation::ORIENTATION_STANDARD) const;
   ...
   */
  virtual UniformVolume *GetDownsampledAndAveraged(
      const Types::GridIndexType downsample,
      const bool approxIsotropic = false) const;

  /** Downsampling and pixel averaging constructor function.
   *\param downsample Array of per-dimension downsampling factors.
   */
  virtual UniformVolume *GetDownsampledAndAveraged(
      const Types::GridIndexType (&downsample)[3]) const;
   ...
   * is as close to isotropic as possible without interpolation.
   */
  virtual UniformVolume *GetDownsampled(
      const Types::GridIndexType downsample,
      const bool approxIsotropic = false) const;
  ...
  /** Get interleaved sub-volume along given axis and with given interleave
   *offset. \param axis Coordinate axis along which the image is interleaved.
   *\param factor Interleave factor, i.e., the number of interleaved
   *sub-volumes. \param idx Index of interleaved sub-volume to extract.
   */
  UniformVolume *GetInterleavedSubVolume(const int axis,
                                         const Types::GridIndexType factor,
                                         const Types::GridIndexType idx) const;

  /** Get interleaved sub-volume along given axis and with given interleave
   *offset, padded with empty image planes. \param axis Coordinate axis along
   *which the image is interleaved. \param factor Interleave factor, i.e., the
   *number of interleaved sub-volumes. \param idx Index of interleaved
   *sub-volume to extract.
   */
  UniformVolume *GetInterleavedPaddedSubVolume(
      const int axis, const Types::GridIndexType factor,
      const Types::GridIndexType idx) const;

  /// Mirror volume and associated data.
  virtual void Mirror(
      const int axis =
          AXIS_X /*!< Mirror with respect to this coordinate axis.*/);
   ...
   */
  virtual ScalarImage::SmartPtr GetOrthoSlice(
      const int axis, const Types::GridIndexType plane) const;

  /** Extract orthogonal slice as a new volume.
   */
  Self::SmartPtr ExtractSlice(const int axis,
                              const Types::GridIndexType plane) const;
   ...
   * each axis -- non-uniform spacings will lead to incorrect results.
   */
  virtual ScalarImage::SmartPtr GetOrthoSliceInterp(
   ...
   * each axis -- non-uniform spacings will lead to incorrect results.
   */
  virtual ScalarImage::SmartPtr GetNearestOrthoSlice(
      const int axis, const Types::Coordinate location) const;
   ...
   */
  virtual const Self::CoordinateVectorType GetGradientAt(
   ...
   * This function cannot be called for pixels within a two-pixel distance from
   *the volume boundary, i.e., we require that 1 < i,j,k <
   *[Dims[0]-2,Dims[1]-2,Dims[2]-2].
   *
   *\warning No parameter range checking is currently performed!
   */
  virtual Matrix3x3<Types::DataItem> GetHessianAt(const Types::GridIndexType i,
                                                  const Types::GridIndexType j,
                                                  const Types::GridIndexType k);

  /// Get data value at specified coordinate.
  template <class TData>
  inline bool ProbeData(TData &result, const TData *dataPtr,
                        const Self::CoordinateVectorType &location) const;

  /// Return linearly interpolated voxel without applying a transformation.
  inline bool ProbeNoXform(ProbeInfo &,
                           const Self::CoordinateVectorType &) const;
   ...
   */
  inline bool FindVoxel(const Self::CoordinateVectorType &location,
                        Types::GridIndexType *const idx,
                        Types::Coordinate *const from,
                        Types::Coordinate *const to) const;
   ...
   */
  inline bool FindVoxel(const Self::CoordinateVectorType &location,
                        Types::GridIndexType *const idx) const;
   ...
   * ModelDims[dim-1].
   */
  inline void GetVoxelIndexNoBounds(const Self::CoordinateVectorType &location,
                                    Types::GridIndexType *const idx) const;
   ...
   * and all other output values (see parameters) are invalid.
   */
  inline bool FindVoxelByIndex(const Self::CoordinateVectorType &fracIndex,
                               Types::GridIndexType *const idx,
                               Types::Coordinate *const frac) const;

  /// Get 3D grid region from continuous lower and upper corner.
  const Self::RegionType GetGridRange(
      const Self::CoordinateRegionType &region /*!< The coordinate region*/)
      const;

  /// Get plane coordinate.
  virtual Types::Coordinate GetPlaneCoord(
      const int axis, const Types::GridIndexType plane) const {
   ...
   * @param location The location along the selected coordinate axis in the
   * range from 0 to Size[axis].
   */
  virtual Types::GridIndexType GetCoordIndex(
      const int axis, const Types::Coordinate location) const {
    return std::max<Types::GridIndexType>(
        0, std::min<Types::GridIndexType>(
               (Types::GridIndexType)((location - this->m_Offset[axis]) /
                                      this->m_Delta[axis]),
               this->m_Dims[axis] - 1));
  }
   ...
   * range from 0 to Size[axis].
   */
  virtual Types::GridIndexType GetClosestCoordIndex(
      const int axis, const Types::Coordinate location) const {
    const Types::GridIndexType idx = (Types::GridIndexType)MathUtil::Round(
        (location - this->m_Offset[axis]) / this->m_Delta[axis]);
    return std::max<Types::GridIndexType>(
        0, std::min<Types::GridIndexType>(idx, this->m_Dims[axis] - 1));
  }

  /** Get grid index corresponding (as close as possible) to coordinate.
   *\return True if given point is inside image, false if outside.
   */
  virtual bool GetClosestGridPointIndex(const Self::CoordinateVectorType v,
                                        Self::IndexType &idx) const {
    for (int dim = 0; dim < 3; ++dim) {
      idx[dim] = static_cast<Types::GridIndexType>(
          MathUtil::Round((v[dim] - this->m_Offset[dim]) / this->m_Delta[dim]));
      if ((idx[dim] < 0) || (idx[dim] > this->m_Dims[dim] - 1)) return false;
    }
    return true;
  }

  /** Get grid index corresponding to coordinate by truncation, not rounding.
   * @param axis The coordinate dimension that the location parameters refers
   * to.
   * @param location The location in the range from 0 to Size[axis].
   */
  virtual Types::GridIndexType GetTruncCoordIndex(
      const int axis, const Types::Coordinate location) const {
    const Types::GridIndexType idx = static_cast<Types::GridIndexType>(
        (location - this->m_Offset[axis]) / this->m_Delta[axis]);
    return std::max<Types::GridIndexType>(
        0, std::min<Types::GridIndexType>(idx, this->m_Dims[axis] - 1));
  }
   ...
   *\return True if given point is inside image, false if outside.
   */
  virtual bool GetTruncGridPointIndex(
      const Self::CoordinateVectorType v /*!< Location to find in the grid. */, Self::IndexType
                                                                                    &idx /*!< Truncated grid point index (i.e., nearest grid point between location and grid coordinate origin. */)
      const {
    for (int dim = 0; dim < 3; ++dim) {
      idx[dim] = static_cast<Types::GridIndexType>(
          (v[dim] - this->m_Offset[dim]) / this->m_Delta[dim]);
      if ((idx[dim] < 0) || (idx[dim] > this->m_Dims[dim] - 1)) return false;
    }
    return true;
  }
   ...
   *\return The location in image coordinates of the given grid element as a
   *Self::CoordinateVectorType.
   */
  virtual const Self::CoordinateVectorType GetGridLocation(
      const Types::GridIndexType x, const Types::GridIndexType y,
      const Types::GridIndexType z) const {
    const Types::Coordinate loc[3] = {this->m_Offset[0] + x * this->m_Delta[0],
                                      this->m_Offset[1] + y * this->m_Delta[1],
                                      this->m_Offset[2] + z * this->m_Delta[2]};
  ...
  virtual const Self::CoordinateVectorType GetGridLocation(
      const Self::CoordinateVectorType
          &xyz /*!< Grid index (possibly fractional) */) const {
   ...
   * three coordinate axes. Valid range is from 0 to Dims[...]-1. Fractional
   *coordinates are permitted. \return The location in image coordinates of the
   *given grid element as a Self::CoordinateVectorType.
   */
  virtual const Self::CoordinateVectorType IndexToPhysical(
      const Self::CoordinateVectorType &idxV) const {
   ...
   */
  virtual const Self::CoordinateVectorType PhysicalToIndex(
      const Self::CoordinateVectorType &physical) const {
   ...
   *\return The location of the given grid element as a
   *Self::CoordinateVectorType.
   */
  virtual const Self::CoordinateVectorType GetGridLocation(
      const Types::GridIndexType idx) const {
    const Types::Coordinate loc[3] = {
        this->m_Offset[0] + (idx % this->nextJ) * this->m_Delta[0],
        this->m_Offset[1] +
            (idx % this->nextK) / this->nextJ * this->m_Delta[1],
  ...
  /// Get center of mass of pixel data.
  virtual Self::CoordinateVectorType GetCenterOfMass(
      Self::CoordinateVectorType &firstOrderMoment) const {
    Self::CoordinateVectorType com =
        this->Superclass::GetCenterOfMassGrid(firstOrderMoment);
    for (int dim = 0; dim < 3; ++dim) {
      (com[dim] *= this->m_Delta[dim]) += this->m_Offset[dim];
  ...
  /** Get principal axes.
   *\return This function returns two types of information: the principal
   *directions (axes) of the image data, and the relative scales. The principal
   *axes are returned as a 3x3 array, such that directions[0] is the 3D array of
   *the major principal direction, i.e. the one with the largest norm. Then,
   *directions[0][1] is the y-component of that vector, etc. Accordingly,
   *directions[1] and directions[2] are the two remaining principal directions,
   *with the norm of directions[1] larger than, or equal to, the norm of
   *directions[2].
   */
  void GetPrincipalAxes(Matrix3x3<Types::Coordinate> &directions,
                        Self::CoordinateVectorType &centerOfMass) const;
 ...
 private:
   ...
   *
   *\note The crop region includes the volume offset, m_Offset. Its upper limit
   *can, therefore, be larger than m_Size, which does NOT include m_Offset.
   */
   ...
   * each thread created.
   */
  class ResampleTaskInfo : public ThreadParameters<const Self> {
   public:
  ...
  static void ResampleThreadPoolExecuteGrey(void *const arg,
  ...
  static void ResampleThreadPoolExecuteLabels(void *const arg,
                                              const size_t taskIdx,
