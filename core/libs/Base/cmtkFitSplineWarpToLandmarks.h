 ...
 *
 *\attention This class does not compile with Solaris Studio, unless STLport
 *support is activated via the "-library=stlport4" compiler command line option.
 */
class FitSplineWarpToLandmarks {
 public:
  ...
  class Parameters {
   public:
    /// Default constructor.
    Parameters()
        : m_Levels(1), m_IterationsPerLevel(100), m_ResidualThreshold(0) {}
    ...
    /// Threshold for relative RMS residual improvement. Iteration terminates if
  ...
  SplineWarpXform::SmartPtr Fit(
      const SplineWarpXform::SpaceVectorType
          &domain /*!< Domain of the deformation field. This should be the size
                     of the fixed image grid to be used with the resulting
                     deformation */
      ,
      ...
      const AffineXform *initialAffine =
          NULL /*!< Optional affine transformation to initialize the spline
                  control points.*/
      ,
      const Self::Parameters &parameters =
          Self::DefaultParameters /*!< Fitting parameters.*/);
  ...
  SplineWarpXform::SmartPtr Fit(
      ...
      const Types::Coordinate
          finalSpacing /*!< Final control point spacing of the fitted B-spline
 ...
 private:
  ...
  std::vector<FixedArray<3, FixedVector<4, Types::Coordinate>>>
      m_LandmarksSpline;
   ...
   *transformation and deformation field. \return Root-of-mean-squared residual
   *over all landmarks.
   */
  ...
  void FitSpline(
      SplineWarpXform &splineWarp /*!< Fitted spline warp is returned here.*/,
      const Self::Parameters &parameters =
          Self::DefaultParameters /*!< Fitting parameters.*/);
};
