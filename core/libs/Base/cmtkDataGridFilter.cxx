...
DataGridFilter ::DataGridFilter(DataGrid::SmartConstPtr dataGrid)
    : m_DataGrid(dataGrid) {}

cmtk::Types::DataItem cmtk::DataGridFilter::MedianOperator::Reduce(
    std::vector<Types::DataItem> &regionData) {
  std::sort(regionData.begin(), regionData.end());

  if (regionData.size() % 2)
    return regionData[regionData.size() / 2];
  else
    return (Types::DataItem)(0.5 * (regionData[regionData.size() / 2] +
                                    regionData[regionData.size() / 2 - 1]));
}

TypedArray::SmartPtr DataGridFilter::GetDataMedianFiltered(
    const Types::GridIndexType radiusX, const Types::GridIndexType radiusY,
    const Types::GridIndexType radiusZ) const {
  return this->ApplyRegionFilter<Self::MedianOperator>(radiusX, radiusY,
                                                       radiusZ);
}

cmtk::Types::DataItem cmtk::DataGridFilter::MeanOperator::Reduce(
    std::vector<Types::DataItem> &regionData) {
...
TypedArray::SmartPtr DataGridFilter::RegionMeanFilter(
    const Types::GridIndexType radiusX, const Types::GridIndexType radiusY,
    const Types::GridIndexType radiusZ) const {
  return this->ApplyRegionFilter<Self::MeanOperator>(radiusX, radiusY, radiusZ);
}

TypedArray::SmartPtr DataGridFilter::FastRegionMeanFilter(
    const Types::GridIndexType radiusX, const Types::GridIndexType radiusY,
    const Types::GridIndexType radiusZ) const {
  const DataGrid &dataGrid = *(this->m_DataGrid);
  if (!dataGrid.GetData()) return TypedArray::SmartPtr(NULL);
  ...
  for (int dim = 0; dim < 3; ++dim) {
    ...
    for (RegionIndexIterator<DataGrid::RegionType> fIt =
             RegionIndexIterator<DataGrid::RegionType>(face);
         fIt != fIt.end(); ++fIt) {
      double sum = 0;
      ...
      //
      // PASS 2 - compute differences between upper and lower end of kernel
      // window
      //
      idx0 = 0;
      for (idx[dim] = columnFrom; idx[dim] < columnTo; ++idx[dim], ++idx0) {
        ...
        const Types::GridIndexType upper =
            static_cast<Types::GridIndexType>(std::min<Types::GridIndexType>(
                idx0 + radius[dim], nPixelsColumn - 1));
        sums[offset] = sumsColumn[upper];
        cnts[offset] = cntsColumn[upper];

        // if lower end of window is outside range, implicitly subtract zero
        // from sums and counts
...
TypedArray::SmartPtr DataGridFilter::FastRegionVarianceFilter(
    const Types::GridIndexType radiusX, const Types::GridIndexType radiusY,
    const Types::GridIndexType radiusZ) const {
  if (!this->m_DataGrid->GetData()) return TypedArray::SmartPtr(NULL);

  //
  // see http://en.wikipedia.org/wiki/Algorithms_for_calculating_variance
  //
  TypedArray::SmartPtr mean =
      this->FastRegionMeanFilter(radiusX, radiusY, radiusZ);
...
cmtk::Types::DataItem cmtk::DataGridFilter::VarianceOperator::Reduce(
    std::vector<Types::DataItem> &regionData) {
...
TypedArray::SmartPtr DataGridFilter::RegionVarianceFilter(
    const Types::GridIndexType radiusX, const Types::GridIndexType radiusY,
    const Types::GridIndexType radiusZ) const {
  return this->ApplyRegionFilter<Self::VarianceOperator>(radiusX, radiusY,
                                                         radiusZ);
}

cmtk::Types::DataItem cmtk::DataGridFilter::StandardDeviationOperator::Reduce(
    std::vector<Types::DataItem> &regionData) {
  return sqrt(VarianceOperator::Reduce(regionData));
}

TypedArray::SmartPtr DataGridFilter::RegionStandardDeviationFilter(
    const Types::GridIndexType radiusX, const Types::GridIndexType radiusY,
    const Types::GridIndexType radiusZ) const {
  return this->ApplyRegionFilter<Self::StandardDeviationOperator>(
      radiusX, radiusY, radiusZ);
}

cmtk::Types::DataItem cmtk::DataGridFilter::SmoothnessOperator::Reduce(
    std::vector<Types::DataItem> &regionData) {
  return (1.0 - 1.0 / (1.0 + VarianceOperator::Reduce(regionData)));
}

TypedArray::SmartPtr DataGridFilter::RegionSmoothnessFilter(
    const Types::GridIndexType radiusX, const Types::GridIndexType radiusY,
    const Types::GridIndexType radiusZ) const {
  return this->ApplyRegionFilter<Self::SmoothnessOperator>(radiusX, radiusY,
                                                           radiusZ);
}

cmtk::Types::DataItem cmtk::DataGridFilter::ThirdMomentOperator::Reduce(
    std::vector<Types::DataItem> &regionData) {
  const Types::DataItem mean = MeanOperator::Reduce(regionData);
...
TypedArray::SmartPtr DataGridFilter::RegionThirdMomentFilter(
    const Types::GridIndexType radiusX, const Types::GridIndexType radiusY,
    const Types::GridIndexType radiusZ) const {
  return this->ApplyRegionFilter<Self::ThirdMomentOperator>(radiusX, radiusY,
                                                            radiusZ);
}

TypedArray::SmartPtr DataGridFilter::GetDataSobelFiltered() const {
  const TypedArray *data = this->m_DataGrid->GetData();
  if (!data) return TypedArray::SmartPtr(NULL);

  TypedArray::SmartPtr result =
      TypedArray::Create(data->GetType(), data->GetDataSize());
  ...
  for (Types::GridIndexType z = 0; z < this->m_DataGrid->m_Dims[2]; ++z) {
    ...
    for (Types::GridIndexType y = 0; y < this->m_DataGrid->m_Dims[1]; ++y)
      for (Types::GridIndexType x = 0; x < this->m_DataGrid->m_Dims[0];
           ++x, ++offset) {
        if (x && y && z && (x < this->m_DataGrid->m_Dims[0] - 1) &&
            (y < this->m_DataGrid->m_Dims[1] - 1) &&
            (z < this->m_DataGrid->m_Dims[2] - 1)) {
          for (Types::GridIndexType dz = -1; dz < 2; ++dz)
            for (Types::GridIndexType dy = -1; dy < 2; ++dy)
              for (Types::GridIndexType dx = -1; dx < 2; ++dx)
                if (!data->Get(fov[1 + dx][1 + dy][1 + dz],
                               offset + this->m_DataGrid->GetOffsetFromIndex(
                                            dx, dy, dz)))
                  fov[1 + dx][1 + dy][1 + dz] = 0;

          value = (Types::DataItem)(fabs(fov[0][0][1] - fov[2][0][1] +
...
TypedArray::SmartPtr DataGridFilter::GetDataKernelFiltered(
    const std::vector<Types::DataItem> &filterX,
    const std::vector<Types::DataItem> &filterY,
    const std::vector<Types::DataItem> &filterZ, const bool normalize) const {
  if (!this->m_DataGrid->GetData()) return TypedArray::SmartPtr(NULL);

  // create and initialize result (data are copied at this point from the source
  // array)
  TypedArray::SmartPtr result(this->m_DataGrid->GetData()->Clone());

  ThreadPool &threadPool = ThreadPool::GetGlobalThreadPool();
  const Types::GridIndexType numberOfTasks =
      4 * threadPool.GetNumberOfThreads() - 3;
...
void DataGridFilter ::GetFilteredDataThreadX(void *args, const size_t taskIdx,
  ...
  for (Types::GridIndexType z = taskIdx; z < dims[2]; z += taskCnt) {
    for (Types::GridIndexType y = 0; y < dims[1]; ++y) {
      ...
      Types::GridIndexType ofs =
          ThisConst->m_DataGrid->GetOffsetFromIndex(0, y, z);
      for (Types::GridIndexType x = 0; x < dims[0]; ++x, ++ofs)
        if (!result->Get(pixelBufferFrom[x], ofs)) pixelBufferFrom[x] = 0;
      ...
      for (Types::GridIndexType x = 0; x < dims[0]; ++x) {
        ...
        for (Types::GridIndexType t = 1; t < (Types::GridIndexType)filterSize;
             ++t) {
...
void DataGridFilter ::GetFilteredDataThreadY(void *args, const size_t taskIdx,
                                             const size_t taskCnt, const size_t,
                                             const size_t) {
  FilterThreadParameters *params = static_cast<FilterThreadParameters *>(args);
  const Self *ThisConst = params->thisObject;
  ...
  for (Types::GridIndexType z = taskIdx; z < dims[2]; z += taskCnt) {
    for (Types::GridIndexType x = 0; x < dims[0]; ++x) {
      for (Types::GridIndexType y = 0; y < dims[1]; ++y)
        if (!result->Get(pixelBufferFrom[y],
                         ThisConst->m_DataGrid->GetOffsetFromIndex(x, y, z)))
          pixelBufferFrom[y] = 0;
      ...
      for (Types::GridIndexType y = 0; y < dims[1]; ++y)
        result->Set(pixelBufferTo[y],
                    ThisConst->m_DataGrid->GetOffsetFromIndex(x, y, z));
    }
  }
}

void DataGridFilter ::GetFilteredDataThreadZ(void *args, const size_t taskIdx,
                                             const size_t taskCnt, const size_t,
                                             const size_t) {
  FilterThreadParameters *params = static_cast<FilterThreadParameters *>(args);
  const Self *ThisConst = params->thisObject;
  ...
  for (Types::GridIndexType y = taskIdx; y < dims[1]; y += taskCnt) {
    for (Types::GridIndexType x = 0; x < dims[0]; ++x) {
      for (Types::GridIndexType z = 0; z < dims[2]; ++z)
        if (!result->Get(pixelBufferFrom[z],
                         ThisConst->m_DataGrid->GetOffsetFromIndex(x, y, z)))
          pixelBufferFrom[z] = 0;
