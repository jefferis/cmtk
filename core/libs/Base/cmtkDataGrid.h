 ...
 * need to be handled by derived classes. Thus, this class provides the
 * coordinate independent services such as median filtering and, to a certain
...
class DataGrid :
 ...
 public:
  ...
  /// Offset increments when moving to the next pixel in each of the three grid
  /// dimensions.
 ...
 public:
  ...
  virtual DataGrid *GetDownsampledAndAveraged(
      const Types::GridIndexType (&downsample)[3]) const;

  /// Downsampling without averaging constructor function.
  virtual DataGrid *GetDownsampled(
      const Types::GridIndexType (&downsample)[3]) const;

  /** Reorientation constructor function.
   *\param newOrientation Three letter orientation code that specifies the
   *anatomically-based orientation of the reoriented volume. Each letter can be
   *one of the following: R, L, A, P, I, S. These stand for Right, Left,
   *Anterior, Posterior, Inferior, Superior.
   *
   * The three letters in the orientation string define the directions of the
   *positive x, y, and z axes, in this order. For example, "RAS", the standard
   *orientation for this software, means that the pixels along the x axis are
   *arranged from the subject's Left to the Right side, along the y axis from
   *the subject's Posterior (back) to Anterior (front), and along the z axis
   *from Inferior (feet) to Superior (head).
   *
   * The current orientation of this volume is to be taken from its meta
   *information, as this->m_MetaInformation[CMTK_META_IMAGE_ORIENTATION]. This
   *is also a three-letter string of the same form as the one given to this
   *function as a parameter.
   *
   * If the current orientation is not set, a warning message should be printed
   *to StdErr, and a NULL pointer returned.
   *
   *\return Reoriented data grid with permuted pixels in this->Data and permuted
   *grid dimensions in this->Dims. The returned pointers points to a newly
   *allocated object, which can be wrapped in an SmartPointer.
   */
  const DataGrid::SmartPtr GetReoriented(
  ...
  /** Create data array.
   *\param dataType ID of scalar data type for the array. This is the image
   *pixel type. \param setToZero If this flag is set, all values in the newly
   *created array will be initialized to zero.
   */
  virtual TypedArray::SmartPtr CreateDataArray(const ScalarDataType dataType,
                                               const bool setToZero = false);

  /// Get number of data items in the volume.
  size_t GetNumberOfPixels() const {
    return this->m_Dims[0] * this->m_Dims[1] * this->m_Dims[2];
  }

  /// Check whether given pixel index is inside grid.
  bool IndexIsInRange(const Types::GridIndexType x,
                      const Types::GridIndexType y,
                      const Types::GridIndexType z) const {
    return (x >= 0) && (x < this->m_Dims[0]) && (y >= 0) &&
           (y < this->m_Dims[1]) && (z >= 0) && (z < this->m_Dims[2]);
  }

  /// Get offset of a pixel.
  Types::GridIndexType GetOffsetFromIndex(const Types::GridIndexType x,
                                          const Types::GridIndexType y,
                                          const Types::GridIndexType z) const {
  ...
  void GetIndexFromOffset(const size_t offset, Types::GridIndexType &x,
                          Types::GridIndexType &y,
                          Types::GridIndexType &z) const {
    z = offset / nextK;
  ...
  bool GetDataAt(Types::DataItem &data, const Types::GridIndexType x,
                 const Types::GridIndexType y,
                 const Types::GridIndexType z) const {
  ...
  void SetDataAt(const Types::DataItem data, const Types::GridIndexType x,
                 const Types::GridIndexType y, const Types::GridIndexType z) {
    this->SetDataAt(data, this->GetOffsetFromIndex(x, y, z));
  }

  /// Return data at specified grid point, or a given default value if no data
  /// exists there.
  Types::DataItem GetDataAt(const Types::GridIndexType x,
                            const Types::GridIndexType y,
                            const Types::GridIndexType z,
                            const Types::DataItem defaultValue = 0.0) const {
  ...
  /// Return data at specified grid offset, or a given default value if no data
  /// exists there.
  Types::DataItem GetDataAt(const size_t offset,
                            const Types::DataItem defaultValue = 0.0) const {
 ...
 public:
   ...
   */
  Self::RegionType &CropRegion() { return this->m_CropRegion; }

  /** Set cropped region.
   * This function can deal with negative crop region values, which refer to the
   * upper grid boundary and are automatically converted.
   */
  virtual void SetCropRegion(const Self::RegionType &region);

  /** Get cropped region reference.
   */
  const Self::RegionType &CropRegion() const { return this->m_CropRegion; }

  /// Get whole image region.
  const Self::RegionType GetWholeImageRegion() const;

  /// Get region covering one slice of the image.
  const Self::RegionType GetSliceRegion(
      const int axis /*!< Coordinate axis perpendicular to the slice, i.e., 0
                        for x, 1 for y, 2 for z.*/
      ,
      const Types::GridIndexType slice /*!< Index of selected slice. */) const;
   ...
   *\param margin Width of additional margin added around the threshold-cropped
   *region. \return The crop region that was applied.
   */
  Self::RegionType AutoCrop(const Types::DataItem threshold,
                            const bool recrop = false,
                            const Types::GridIndexType margin = 0);
  ...
  virtual FixedVector<3, Types::Coordinate> GetCenterOfMassGrid(
      FixedVector<3, Types::Coordinate> &firstOrderMoment) const;
   ...
   */
  Self::SmartPtr ExtractSlice(
      const int axis /*!< Coordinate axis perpendicular to extracted plane*/,
      const Types::GridIndexType plane /*!< Index of extracted plane */) const;

  /** Set orthogonal slice from a 2D image.
   */
  virtual void SetOrthoSlice(const int axis, const Types::GridIndexType idx,
 ...
 protected:
   ...
   * False otherwise.
   */
  bool TrilinearInterpolation(Types::DataItem &data,
                              const Types::GridIndexType x,
                              const Types::GridIndexType y,
                              const Types::GridIndexType z,
   ...
   * of interpolations from a given data volume of known pixel type are
   *required.
   *
  ...
  inline TData TrilinearInterpolation(const TData *dataPtr,
                                      const Types::GridIndexType x,
                                      const Types::GridIndexType y,
                                      const Types::GridIndexType z,
  ...
  /** Utility function for trilinear interpolation from multiple primitive data
   * arrays of identical grid structure. This function is provided for
   * computational efficiency when a large number of interpolations from a given
   * data volume of known pixel type are required.
   */
  ...
  inline void TrilinearInterpolation(
      ...
      const Types::Coordinate
          fracZ /*!< Fractional coordinate Z within pixel */) const;
 ...
 private:
  ...
  static void MirrorPlaneInPlace(TypedArray &data, const Self::IndexType &dims,
