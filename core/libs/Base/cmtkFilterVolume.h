...
class FilterVolume {
 public:
   ...
   */
  static TypedArray::SmartPtr GaussianFilter(
      const UniformVolume *volume, const Units::GaussianSigma &width,
      const Types::Coordinate radius = 1.0, const TypedArray *maskData = NULL);
  ...
  /** Apply Torsten Rohlfing's single-image intensity-consistent Gaussian
   *filter. \param volume Input 3D image. \param subjectData Pixel array of the
   *individual grey image from this subject. \param maskData Optional binary
   *mask data array. \param iFilterSigma Width (standard deviation of the
   *Gaussian kernel. \param filterWidth Width (standard deviation of the
   *Gaussian kernel. \param filterRadius Filter radius in multiples of the
   *filter width. Outside the radius (Euclidean distance) the filter is
   *truncated. \return A newly allocated TypedArray object that can be used, for
   * example, to replace the one held by the input image. The data type of the
   * array is identical to the input array.
   */
