...
class Xform :
 ...
 public:
  ...
  Xform(const Xform &other)
    ...
    this->SetMetaInfo(cmtk::META_SPACE,
                      cmtk::AnatomicalOrientationBase::ORIENTATION_STANDARD);
  }

  /// Default constructor.
  Xform() : m_Parameters(NULL), m_NumberOfParameters(0) {
    this->SetMetaInfo(cmtk::META_SPACE,
                      cmtk::AnatomicalOrientationBase::ORIENTATION_STANDARD);
  }
  ...
  virtual bool ApplyInverse(const Self::SpaceVectorType &,
                            ...
                            const Types::Coordinate = 0.01) const = 0;

  /** Return origin of warped vector.
   * Note that since not every class of transformation is closed under
   *inversion, this function computes only a more or less accurate numerical
   * approximation to the actual origin of a transformed vector. Note also that
   *this computation is everything but computationally efficient. \note This
   *function expects that derived classes will implement the GetJacobian member
   *function, since the iterative search relies on the Jacobian matrix. \param v
   *Input location; is replaced with the inverse transformation applied to it
   *upon return. \param initial Initial estimate for the original location.
   *Search goes from here. This is useful for looking up the original locations
   *of a large number of closely located vectors, for example all pixels in an
   * image.
   *\param accuracy Accuracy of the inversion, i.e., residual inverse
   *consistency error threshold. \return True is the given inverse was
   *succesfully comuted, false if the given warped vector was outside the target
   *domain of this transformation.
   */
  virtual bool ApplyInverseWithInitial(
      const Self::SpaceVectorType &v, Self::SpaceVectorType &u,
  ...
  virtual size_t ParamVectorDim() const { return this->m_NumberOfParameters; }
  ...
  virtual CoordinateVector &GetParamVector(CoordinateVector &v,
                                           const size_t targetOffset = 0) const;
  ...
  virtual const CoordinateMatrix3x3 GetJacobian(
      const Self::SpaceVectorType &) const = 0;
