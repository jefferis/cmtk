...
ActiveShapeModel::ActiveShapeModel(CoordinateVector::SmartPtr &mean,
...
float ActiveShapeModel::Construct(const Types::Coordinate *const *trainingSet,
                                  const unsigned int numberOfSamples,
                                  const unsigned int numberOfPoints,
                                  const unsigned int numberOfModes) {
  if (numberOfSamples < numberOfModes) {
    StdErr << "WARNING: number of modes of an ASM can be no higher than number "
              "of training samples.\n";
  ...
  for (unsigned int sampleY = 0; sampleY < numberOfSamples; ++sampleY) {
    for (unsigned int sampleX = 0; sampleX <= sampleY; ++sampleX) {
      ...
      for (unsigned int point = 0; point < NumberOfPoints;
           ++point, ++meanPtr2) {
        ccXY += (trainingSet[sampleX][point] - (*meanPtr2)) *
                (trainingSet[sampleY][point] - (*meanPtr2));
      }
  ...
  const std::vector<Types::Coordinate> eigenvalues =
      eigensystem.GetEigenvalues();
  std::vector<unsigned int> permutation(numberOfSamples);
  // initialize permutation array
  for (unsigned int i = 0; i < numberOfSamples; ++i) permutation[i] = i;
    ...
    for (unsigned int point = 0; point < NumberOfPoints; ++point, ++modePtr) {
      ...
      *modePtr = 0;
      for (unsigned int sample = 0; sample < numberOfSamples; ++sample)
        *modePtr += (eigensystem.EigenvectorElement(sample, fromMode) *
                     (trainingSet[sample][point] - meanValue));
    }

    // finally, normalize mode vectors... if Geremy is right ;)
    (*(*Modes)[mode]) *=
        (sqrt(eigenvalues[permutation[mode]]) / (*Modes)[mode]->EuclidNorm());
  }

  return 0;
}

Types::Coordinate *ActiveShapeModel::Generate(
    Types::Coordinate *const instance,
    const Types::Coordinate *modeWeights) const {
...
float ActiveShapeModel::Decompose(const CoordinateVector *input,
                                  Types::Coordinate *const weights) const {
...
#define RETURN_PDF
  ...
  for (size_t mode = 0; mode < this->NumberOfModes; ++mode) {
    ...
    pdf *= static_cast<float>(exp(-(w[mode] * w[mode]) / (2.0 * variance)) /
  ...
  if (weights) memcpy(weights, &w[0], this->NumberOfModes * sizeof(*weights));
...
void ActiveShapeModel::Allocate(const unsigned int numberOfPoints,
                                const unsigned int numberOfModes) {
  ...
  for (unsigned int mode = 0; mode < NumberOfModes; ++mode)
    Modes->push_back(
        CoordinateVector::SmartPtr(new CoordinateVector(NumberOfPoints)));
  ModeVariances =
      CoordinateVector::SmartPtr(new CoordinateVector(NumberOfModes));
