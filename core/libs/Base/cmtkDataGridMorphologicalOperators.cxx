...
DataGridMorphologicalOperators ::DataGridMorphologicalOperators(
    const DataGrid::SmartConstPtr &dataGrid)
    : m_DataGrid(dataGrid) {}

TypedArray::SmartPtr DataGridMorphologicalOperators::GetEroded(
    const int iterations) const {
  TypedArray::SmartPtr dataArray = this->m_DataGrid->GetData();
  if (!dataArray) return TypedArray::SmartPtr(NULL);

  // if not byte array, convert first (copy will be de-allocated automatically
  ...
  for (int i = 0; i < iterations; ++i) {
    ...
    for (int z = 0; z < this->m_DataGrid->m_Dims[2]; ++z) {
      const int dzFrom = z ? -1 : 0,
                dzTo = (z < this->m_DataGrid->m_Dims[2] - 1) ? 1 : 0;
      for (int y = 0; y < this->m_DataGrid->m_Dims[1]; ++y) {
        const int dyFrom = y ? -1 : 0,
                  dyTo = (y < this->m_DataGrid->m_Dims[1] - 1) ? 1 : 0;
        for (int x = 0; x < this->m_DataGrid->m_Dims[0]; ++x, ++offset) {
          const int dxFrom = x ? -1 : 0,
                    dxTo = (x < this->m_DataGrid->m_Dims[0] - 1) ? 1 : 0;
          if (eroded[offset]) {
            ...
            for (int dz = dzFrom; (dz <= dzTo) && !erodePixel; ++dz)
              for (int dy = dyFrom; (dy <= dyTo) && !erodePixel; ++dy)
                for (int dx = dxFrom; (dx <= dxTo) && !erodePixel; ++dx)
                  if (dx || dy || dz)
                    if (!eroded[offset + this->m_DataGrid->GetOffsetFromIndex(
                                             dx, dy, dz)])
                      erodePixel = true;
            if (erodePixel)
              tmp[offset] = 0;
...
TypedArray::SmartPtr DataGridMorphologicalOperators::GetDilated(
    const int iterations) const {
  TypedArray::SmartPtr dataArray = this->m_DataGrid->GetData();
  if (!dataArray) return TypedArray::SmartPtr(NULL);
  ...
  ByteArray::SmartPtr dilatedArray =
      ByteArray::Create(dataArray->GetDataSize());
            ...
            for (int dz = dzFrom; (dz <= dzTo) && !dilatePixel; ++dz)
              for (int dy = dyFrom; (dy <= dyTo) && !dilatePixel; ++dy)
                for (int dx = dxFrom; (dx <= dxTo) && !dilatePixel; ++dx)
                  if (dx || dy || dz)
                    dilatePixel =
                        dilated[offset + this->m_DataGrid->GetOffsetFromIndex(
                                             dx, dy, dz)];
            if (dilatePixel)
...
TypedArray::SmartPtr DataGridMorphologicalOperators::GetBoundaryMap(
    const bool multiValued) const {
  TypedArray::SmartPtr dataArray = this->m_DataGrid->GetData();
  if (!dataArray) return TypedArray::SmartPtr(NULL);

  ShortArray::SmartPtr boundaryArray =
      ShortArray::Create(dataArray->GetDataSize());
  short *boundary = boundaryArray->GetDataPtrConcrete();

#pragma omp parallel for
  for (int z = 0; z < this->m_DataGrid->m_Dims[2]; ++z) {
    size_t offset =
        z * this->m_DataGrid->m_Dims[0] * this->m_DataGrid->m_Dims[1];
              ...
              for (int dx = dxFrom; (dx <= dxTo) && !bp; ++dx)
                if (dx || dy || dz)
                  if (dataArray->Get(
                          neighbor,
                          offset +
                              this->m_DataGrid->GetOffsetFromIndex(dx, dy, dz)))
                    bp = (value != neighbor);
        } else {
          value = 0;
        }
