...
cmtk::ScalarImageGradientField::ScalarImageGradientField(
    const UniformVolume &volume)
    : m_GradientField(
          new Self::GradientFieldType(volume.m_Dims, volume.m_Size)) {
  const DataGrid::RegionType wholeImageRegion = volume.GetWholeImageRegion();

  size_t ofsPlusMinus = 1;
  for (int dim = 0; dim < 3; ++dim) {
    size_t ofs = 0;
    for (RegionIndexIterator<DataGrid::RegionType> it(wholeImageRegion);
         it != it.end(); ++it, ++ofs) {
      const DataGrid::IndexType idx = it.Index();
      Types::Coordinate div = 0;

      if (idx[dim] + 1 < wholeImageRegion.To()[dim]) {
        (*this->m_GradientField)[ofs][dim] =
            volume.GetDataAt(ofs + ofsPlusMinus);
        div += 1.0;
      } else
