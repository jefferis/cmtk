...
const CoordinateMatrix3x3 DeformationField::GetJacobian(
  ...
  CoordinateMatrix3x3 J =
      CoordinateMatrix3x3::Identity();  // note that deformation field is offset
                                        // vector field, thus start with
                                        // identity matrix, not zero matrix
      ...
      for (int l = 0; l < 4; ++l, coeff_ll += nextJ) {
        ...
        for (int k = 0; k < 4; ++k, coeff_kk += 3) {
          ...
          const Types::Coordinate tmp =
              CubicSpline::InterpSpline(k, f[0]) * (*coeff_kk);
          kk[1] += tmp;
          // dT / dz
          kk[2] += tmp;
        }
