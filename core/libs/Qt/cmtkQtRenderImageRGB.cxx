...
QtRenderImageRGB::QtRenderImageRGB(QWidget *const parent, Qt::WindowFlags f)
    : QWidget(parent, f),
...
void QtRenderImageRGB::paintEvent(QPaintEvent *const) {
...
void QtRenderImageRGB::Execute() {
  if (Input == NULL) return;
}

void QtRenderImageRGB::RenderTo(QPaintDevice *pd) {
  if (!Input) return;
  ...
  this->setFixedSize(ZoomFactorPercent * width / 100,
                     ZoomFactorPercent * height / 100);
  Image = QImage(width, height, QImage::Format_RGB32);
  memcpy(Image.bits(), imageDataRGB, width * height * 4);

  if (FlipX || FlipY) Image = Image.mirrored(FlipX, FlipY);

  if (ZoomFactorPercent != 100) {
    Image = Image.scaled(width * ZoomFactorPercent / 100,
                         height * ZoomFactorPercent / 100);
  }

  QPainter painter(pd);
  painter.drawImage(0, 0, Image);

  if (CrosshairMode) this->DrawCrosshair(painter, width, height);
}

void QtRenderImageRGB::DrawCrosshair(QPainter &painter,
                                     const unsigned int width,
                                     const unsigned int height) const {
  int crosshairX =
      (FlipX) ? width - 1 - CrosshairPosition[0] : CrosshairPosition[0];
  crosshairX = static_cast<int>((0.5 + crosshairX) * ZoomFactorPercent / 100);

  int crosshairY =
      (FlipY) ? height - 1 - CrosshairPosition[1] : CrosshairPosition[1];
  ...
  const int realHeight =
      static_cast<int>(1.0 * height * ZoomFactorPercent / 100);
...
QPixmap QtRenderImageRGB::GetPixmap() {
  if (Input == NULL) return QPixmap();

  QPixmap capture(ZoomFactorPercent * Input->GetDims(AXIS_X) / 100,
                  ZoomFactorPercent * Input->GetDims(AXIS_Y) / 100);
  this->RenderTo(&capture);
  return capture;
}

/// React to mouse clicks (generate a signal).
void QtRenderImageRGB ::mousePressEvent(QMouseEvent *e) {
  unsigned int scaledX = (e->x() - static_cast<int>(ZoomFactorPercent / 200)) *
                         100 / ZoomFactorPercent;
  if (Input && this->FlipX) {
    scaledX = Input->GetDims(AXIS_X) - 1 - scaledX;
  }

  unsigned int scaledY = (e->y() - static_cast<int>(ZoomFactorPercent / 200)) *
                         100 / ZoomFactorPercent;
  if (Input && this->FlipY) {
...
void QtRenderImageRGB ::mouseMoveEvent(QMouseEvent *e) {
  unsigned int scaledX = (e->x() - static_cast<int>(ZoomFactorPercent / 200)) *
                         100 / ZoomFactorPercent;
  if (Input && this->FlipX) {
    scaledX = Input->GetDims(AXIS_X) - 1 - scaledX;
