...
QtTriplanarWindow::QtTriplanarWindow()
    : QWidget(NULL), m_Study(NULL), m_ZoomFactor(100), m_BatchMode(false) {
  ...
  this->m_ZoomActions->addAction(action);
  ViewMenu->addSeparator();
  (this->m_InterpolateAction = ViewMenu->addAction(
       "&Interpolation", this, SLOT(slotViewInterpolation())))
      ->setCheckable(true);
  this->m_InterpolateAction->setChecked(false);
  (this->m_CrosshairAction =
       ViewMenu->addAction("&Crosshair", this, SLOT(slotViewCrosshair())))
      ->setCheckable(true);
  this->m_CrosshairAction->setChecked(true);
  (this->m_CheckerboxAction =
       ViewMenu->addAction("C&heckerbox", this, SLOT(slotViewCheckerbox())))
      ->setCheckable(true);
  ...
  QObject::connect(ExportMenu, SIGNAL(triggered(QAction *)), this,
                   SLOT(slotExportMenuCmd(QAction *)));
  ...
  ScrollRenderViewAx->GetRenderImage()->SetCrosshairMode(
      this->m_CrosshairAction->isChecked());
  ScrollRenderViewAx->GetRenderImage()->SetCrosshairColors(QColor(255, 0, 0),
                                                           QColor(0, 255, 0));
  ...
  ScrollRenderViewSa->GetRenderImage()->SetCrosshairMode(
      this->m_CrosshairAction->isChecked());
  ScrollRenderViewSa->GetRenderImage()->SetCrosshairColors(QColor(0, 255, 0),
                                                           QColor(0, 0, 255));
  ...
  ScrollRenderViewCo->GetRenderImage()->SetCrosshairMode(
      this->m_CrosshairAction->isChecked());
  ScrollRenderViewCo->GetRenderImage()->SetCrosshairColors(QColor(255, 0, 0),
                                                           QColor(0, 0, 255));
  ...
  QObject::connect(LocationEntryX, SIGNAL(returnPressed()), this,
                   SLOT(slotGoToLocation()));
  ...
  QObject::connect(LocationEntryY, SIGNAL(returnPressed()), this,
                   SLOT(slotGoToLocation()));
  ...
  QObject::connect(LocationEntryZ, SIGNAL(returnPressed()), this,
                   SLOT(slotGoToLocation()));
  ...
  QObject::connect(GoToLocationButton, SIGNAL(clicked()), this,
                   SLOT(slotGoToLocation()));

  LandmarkBox = new QComboBox(landmarksTab);
  LandmarksLayout->addWidget(LandmarkBox, 2, 0, 1, 2);
  LandmarkBox->setEnabled(false);
  QObject::connect(LandmarkBox, SIGNAL(currentIndexChanged(int)), this,
                   SLOT(slotGoToLandmark()));
  ...
  QObject::connect(GoToLandmarkButton, SIGNAL(clicked()), this,
                   SLOT(slotGoToLandmark()));
  ...
  QObject::connect(DeleteLandmarkButton, SIGNAL(clicked()), this,
                   SLOT(slotDeleteLandmark()));
  ...
  QObject::connect(AddLandmarkButton, SIGNAL(clicked()), this,
                   SLOT(slotAddLandmark()));
  ...
  QObject::connect(ExportLandmarksButton, SIGNAL(clicked()), this,
                   SLOT(slotExportLandmarks()));
  ...
  QObject::connect(ImportLandmarksButton, SIGNAL(clicked()), this,
                   SLOT(slotImportLandmarks()));
  ...
  QObject::connect(WindowLevelControls, SIGNAL(colormap(Study::SmartPtr &)),
                   this, SLOT(slotColormapChanged(Study::SmartPtr &)));
  ...
  QObject::connect(ScrollRenderViewAx, SIGNAL(indexChanged(int)), this,
                   SLOT(slotSwitchImageAx(int)));
  QObject::connect(ScrollRenderViewAx,
                   SIGNAL(signalMouse3D(Qt::MouseButton, const Vector3D &)),
                   this, SLOT(slotMouseAx(Qt::MouseButton, const Vector3D &)));
  ...
  QObject::connect(ScrollRenderViewSa, SIGNAL(indexChanged(int)), this,
                   SLOT(slotSwitchImageSa(int)));
  QObject::connect(ScrollRenderViewSa,
                   SIGNAL(signalMouse3D(Qt::MouseButton, const Vector3D &)),
                   this, SLOT(slotMouseSa(Qt::MouseButton, const Vector3D &)));
  ...
  QObject::connect(ScrollRenderViewCo, SIGNAL(indexChanged(int)), this,
                   SLOT(slotSwitchImageCo(int)));
  QObject::connect(ScrollRenderViewCo,
                   SIGNAL(signalMouse3D(Qt::MouseButton, const Vector3D &)),
                   this, SLOT(slotMouseCo(Qt::MouseButton, const Vector3D &)));

  this->m_ProgressReporter = new QtProgress(this);
}

void QtTriplanarWindow::slotSetZoom(const int zoomPercent) {
...
void QtTriplanarWindow::slotSetCheckerboardMode(const bool mode) {
  this->m_CheckerboxAction->setChecked(mode);
  this->slotViewCheckerbox();
}

void QtTriplanarWindow::slotSetCrosshairMode(const bool mode) {
  this->m_CrosshairAction->setChecked(mode);
  this->slotViewCrosshair();
}

void QtTriplanarWindow::slotSetInterpolateMode(const bool mode) {
  this->m_InterpolateAction->setChecked(mode);
  this->slotViewInterpolation();
}

void QtTriplanarWindow::slotExportMenuCmd(QAction *action) {
  ...
  filename = QFileDialog::getSaveFileName(
      this, title, filename,
      "Portable Network Graphic (*.png);; Tagged Image File Format (*.tif);; "
      "Portable Pixmap (*.ppm *.pgm);; JPEG (*.jpg)");
...
void QtTriplanarWindow::slotExportImage(const QString &filename,
                                        const int command) {
  QPixmap pixmap;
  switch (command) {
    case 1:  // Axial image.
    ...
    case 4: {  // Panel.
      ...
      pixmap = QPixmap(pixmapCo.width() + pixmapSa.width(),
                       pixmapCo.height() + pixmapAx.height());
      QPainter painter(&pixmap);

      painter.drawPixmap(0, 0, pixmapCo.width(), pixmapCo.height(), pixmapCo);
      painter.drawPixmap(pixmapCo.width(), 0, pixmapSa.width(),
                         pixmapSa.height(), pixmapSa);
      painter.drawPixmap(0, pixmapCo.height(), pixmapAx.width(),
                         pixmapAx.height(), pixmapAx);
      painter.fillRect(pixmapCo.width(), pixmapCo.height(), pixmapSa.width(),
                       pixmapAx.height(), Qt::black);
      break;
    }
  }
...
void QtTriplanarWindow::slotRenderAll() {
  ScrollRenderViewAx->slotRender();
  ScrollRenderViewCo->slotRender();
  ScrollRenderViewSa->slotRender();
}

void QtTriplanarWindow::slotSwitchToStudy(Study::SmartPtr &study) {
  this->m_Study = study;
  if (this->m_Study) {
    ...
    if (!this->m_BatchMode) {
      while (!this->m_Study->GetVolume()) {
        int button = QMessageBox::warning(
            NULL, "Error", "Could not read image data for this study.",
    ...
    if (this->m_Study->GetVolume()) {
      ...
      if (this->m_BatchMode)
        StdErr << "ERROR: could not read image "
               << this->m_Study->GetFileSystemPath() << "\n";
    }
...
void QtTriplanarWindow::slotSwitchToStudyInternal(Study::SmartPtr &study) {
  this->m_Study = study;
  if (this->m_Study) {
    this->m_Study->ReadVolume(false /* reload */,
                              AnatomicalOrientation::ORIENTATION_STANDARD);
...
void QtTriplanarWindow::UpdateDialog() {
  if (this->m_Study) {
    ...
    if (volume) {
      ...
      qWarning(
          "QtTriplanarWindow::UpdateDialog called with no image data "
          "loaded.\n");
    }
    ...
    QString caption;
    this->setWindowTitle(
        (std::string("CMTK Triplanar Viewer: ") + this->m_Study->GetName())
            .c_str());
    this->show();
  }
}

void QtTriplanarWindow::slotDataChanged(Study::SmartPtr &study) {
  if (study != this->m_Study) return;
  this->slotGoToLocation();
}

void QtTriplanarWindow::slotSwitchImageAx(int imageIndex) {
  const UniformVolume *volume = this->m_Study->GetVolume();
  ...
  if (volume) {
    ...
    if (sliceImage) {
      ...
      sliceImage->AdjustToIsotropic(volume->GetMinDelta(),
                                    this->m_InterpolateAction->isChecked());
      PipelineImageAx->SetFromScalarImage(*sliceImage);
    }
    sliceImage = ScalarImage::SmartPtr::Null();

    LocationEntryZ->setText(
        QString::number(volume->GetPlaneCoord(AXIS_Z, imageIndex)));
    ...
    qWarning(
        "QtTriplanarWindow::slotSwitchImageAx called with no image data "
        "loaded.\n");
  }
}

void QtTriplanarWindow::slotSwitchImageSa(int imageIndex) {
  const UniformVolume *volume = this->m_Study->GetVolume();

  if (volume) {
    ScalarImage::SmartPtr sliceImage =
        volume->GetOrthoSlice(AXIS_X, imageIndex);
    ...
    LocationEntryX->setText(
        QString::number(volume->GetPlaneCoord(AXIS_X, imageIndex)));
    ...
    qWarning(
        "QtTriplanarWindow::slotSwitchImageSa called with no image data "
        "loaded.\n");
  }
}

void QtTriplanarWindow::slotSwitchImageCo(int imageIndex) {
  const UniformVolume *volume = this->m_Study->GetVolume();

  if (volume) {
    ScalarImage::SmartPtr sliceImage =
        volume->GetOrthoSlice(AXIS_Y, imageIndex);
    ...
    LocationEntryY->setText(
        QString::number(volume->GetPlaneCoord(AXIS_Y, imageIndex)));
    ...
    qWarning(
        "QtTriplanarWindow::slotSwitchImageCo called with no image data "
        "loaded.\n");
  }
}

void QtTriplanarWindow ::slotGoToPixel(const QString &xyz) {
  int x, y, z;
  if (3 != sscanf(xyz.toLatin1(), "%10d,%10d,%10d", &x, &y, &z)) {
    qWarning(
        "QtTriplanarWindow::slotGoToPixel needs pixel index as 'x,y,z'.\n");
  } else {
...
void QtTriplanarWindow::slotGoToLocation(const QString &xyz) {
  float v[3];
  if (3 != sscanf(xyz.toLatin1(), "%15f,%15f,%15f", v, v + 1, v + 2)) {
    qWarning(
        "QtTriplanarWindow::slotGoToLocation needs 3D coordinate as "
        "'x,y,z'.\n");
  } else {
    this->slotMouse3D(Qt::LeftButton,
                      UniformVolume::CoordinateVectorType::FromPointer(v));
  }
}

void QtTriplanarWindow ::slotSetColormap(const QString &cmap) {
...
void QtTriplanarWindow ::slotSetWindowLevel(const QString &wl) {
  float window, level;
...
void QtTriplanarWindow ::slotMouse3D(Qt::MouseButton, const Vector3D &v) {
  ...
  if (volume) {
    const unsigned int sliceSa =
        volume->GetClosestCoordIndex(AXIS_X, v[AXIS_X]);
    ScrollRenderViewSa->slotSetSlice(sliceSa);
    ScrollRenderViewSa->slotRender();

    const unsigned int sliceCo =
        volume->GetClosestCoordIndex(AXIS_Y, v[AXIS_Y]);
    ScrollRenderViewCo->slotSetSlice(sliceCo);
    ScrollRenderViewCo->slotRender();

    const unsigned int sliceAx =
        volume->GetClosestCoordIndex(AXIS_Z, v[AXIS_Z]);
    ScrollRenderViewAx->slotSetSlice(sliceAx);
    ScrollRenderViewAx->slotRender();
  }
}

void QtTriplanarWindow ::slotMouseAx(Qt::MouseButton, const Vector3D &v) {
  // if we don't have a study yet, simply ignore.
  if (!this->m_Study) return;
...
void QtTriplanarWindow ::slotMouseSa(Qt::MouseButton, const Vector3D &v) {
  // if we don't have a study yet, simply ignore.
  if (!this->m_Study) return;
...
void QtTriplanarWindow ::slotMouseCo(Qt::MouseButton, const Vector3D &v) {
  // if we don't have a study yet, simply ignore.
  if (!this->m_Study) return;
...
void QtTriplanarWindow::slotCenter() {
  const UniformVolume *volume = this->m_Study->GetVolume();
  if (!volume) return;
...
void QtTriplanarWindow::slotGoToLocation() {
  const UniformVolume *volume = this->m_Study->GetVolume();
  if (!volume) return;

  // Pretend there was a button event at the given location
  const double location[3] = {LocationEntryX->text().toDouble(),
                              LocationEntryY->text().toDouble(),
                              LocationEntryZ->text().toDouble()};
  this->slotMouse3D(Qt::LeftButton,
                    UniformVolume::CoordinateVectorType::FromPointer(location));
}

void QtTriplanarWindow::slotGoToLandmark() {
  if (!this->m_Study) return;

  const LandmarkList *ll = this->m_Study->GetLandmarkList();
  if (!ll) return;

  LandmarkList::ConstIterator lm =
      ll->FindByName(LandmarkBox->currentText().toStdString());
  if (lm != ll->end()) {
    this->slotMouse3D(Qt::LeftButton, lm->m_Location);
  }
}

void QtTriplanarWindow::slotDeleteLandmark() {}

void QtTriplanarWindow::slotExportLandmarks() {
  if (!this->m_Study) return;
  ...
  if (!path.isEmpty()) {
    ...
    if (stream.good()) {
      for (LandmarkList::ConstIterator it = ll->begin(); it != ll->end();
           ++it) {
        stream << it->m_Location[0] << "\t" << it->m_Location[1] << "\t"
               << it->m_Location[2] << "\t" << it->m_Name << std::endl;
      }
      stream.close();
    } else {
      QMessageBox::critical(NULL, "Error", "Could not open file for writing.",
                            QMessageBox::Ok, Qt::NoButton, Qt::NoButton);
    }
  }
}

void QtTriplanarWindow::slotImportLandmarks() {
  if (!this->m_Study) return;
  ...
  QString path = QFileDialog::getOpenFileName(this, "Open Landmarks File",
                                              QString(), "All Files (*.*)");
  ...
  if (!path.isEmpty()) {
    ...
    if (stream.good()) {
      ...
      if (lm != ll->end()) {
        this->LandmarkBox->setCurrentIndex(
            this->LandmarkBox->findText(lm->m_Name.c_str()));
        this->slotMouse3D(Qt::LeftButton, lm->m_Location);
      }
      ...
      QMessageBox::critical(NULL, "Error", "Could not open file for reading.",
                            QMessageBox::Ok, Qt::NoButton, Qt::NoButton);
    }
  }
}

void QtTriplanarWindow::slotAddLandmark() {
  if (!this->m_Study) return;
  ...
  QString name = QInputDialog::getText(
      this, "Add New Landmark", "Enter new landmark name:", QLineEdit::Normal,
      QString::null, &ok);
  if (ok && !name.isEmpty()) {
    Types::Coordinate location[3] = {LocationEntryX->text().toDouble(),
                                     LocationEntryY->text().toDouble(),
                                     LocationEntryZ->text().toDouble()};
    ll->push_back(Landmark(name.toStdString(),
                           Landmark::SpaceVectorType::FromPointer(location)));
    LandmarkBox->addItem(name);
...
void QtTriplanarWindow::slotColormapChanged(Study::SmartPtr &study) {
...
void QtTriplanarWindow::UpdateGridInfo() {
  ...
  if (volume->IndexIsInRange(GridIndex[0], GridIndex[1], GridIndex[2])) {
    ...
    if (volume->GetDataAt(value, GridIndex[0], GridIndex[1], GridIndex[2]))
      str.sprintf("Pixel Index: [%d,%d,%d] RAS: [%g,%g,%g] Value: %g",
                  GridIndex[0], GridIndex[1], GridIndex[2], v[0], v[1], v[2],
                  value);
    else
      str.sprintf("Pixel Index: [%d,%d,%d] RAS: [%g,%g,%g]", GridIndex[0],
                  GridIndex[1], GridIndex[2], v[0], v[1], v[2]);
  }
