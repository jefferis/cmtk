...
QtScrollRenderView::QtScrollRenderView(QWidget *parentWidget,
                                       const QString &title)
    : QGroupBox(parentWidget), RenderImage(NULL) {
  if (!parentWidget)
  ...
  ScrollView = new QScrollArea(
      this);  //, "ScrollView", Qt::WResizeNoErase|Qt::WStaticContents  );
  ...
  QObject::connect(RenderImage,
                   SIGNAL(signalMousePressed(Qt::MouseButton, int, int)),
                   SIGNAL(signalMousePressed(Qt::MouseButton, int, int)));
  QObject::connect(RenderImage,
  ...
  QObject::connect(ImageIndexSlider, SIGNAL(valueChanged(int)),
...
void QtScrollRenderView::slotSetNumberOfSlices(unsigned int nSlices) {
  if (nSlices) {
    ...
    if (ImageIndexSlider->value() < 0 ||
        ImageIndexSlider->value() >= (int)nSlices) {
