...
ImageToImageRGB::ImageToImageRGB() : CheckerboxPadding(true) {
  this->m_Image = NULL;
...
ImageToImageRGB::~ImageToImageRGB() {
  if (this->m_Image) this->m_Image->Unregister();
  if (this->m_Colormap) this->m_Colormap->Unregister();
}
  ...
  switch (AlphaMode) {
    case AlphaModeNone:
      ...
      if (inPtr->GetPaddingFlag())
        this->MarkPaddingData(output->GetDims(AXIS_X), output->GetDims(AXIS_Y),
                              static_cast<RGB *>(outPtr), inPtr);
      break;
    case AlphaModeConst:
      this->m_Colormap->Apply(outPtr, inPtr, true /* generateAlpha */);
      if (inPtr->GetPaddingFlag())
        this->MarkPaddingData(output->GetDims(AXIS_X), output->GetDims(AXIS_Y),
                              static_cast<RGBA *>(outPtr), inPtr);
      break;
  }
...
template <class T>
void ImageToImageRGB::MarkPaddingData(const unsigned int dimsx,
                                      const unsigned int dimsy, T *const rgba,
                                      const TypedArray *data) const {
  T *p = rgba;
  unsigned int idx = 0;

  byte bright = 170;
  byte dark = 80;
  if (!this->CheckerboxPadding) bright = dark = 0;
