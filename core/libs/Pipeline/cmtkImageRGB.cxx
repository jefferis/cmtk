...
ImageRGB *ImageRGB::New() { return new ImageRGB; }
...
ImageRGB::~ImageRGB() { Memory::ArrayC::Delete(this->Data); }

byte *ImageRGB::GetDataPtr(const bool forceAlloc) {
...
void ImageRGB::SetAlphaChannel(const ImageAlphaToggle alphaChannel,
  ...
  if (alphaChannel != AlphaChannel) {
    ...
    if (convertData) {
      ...
      if (AlphaChannel == IMAGE_RGB) {
        ...
        for (unsigned int i = 0; i < numberOfPixels;
             ++i, fromPtr += 4, toPtr += 3) {
          toPtr[0] = fromPtr[0];
          toPtr[1] = fromPtr[1];
          toPtr[2] = fromPtr[2];
...
void ImageRGB::SetPixel(const int index, const RGBA &rgb) {
  ...
  pixelPtr[1] = rgb.G;
  pixelPtr[2] = rgb.B;
  if (AlphaChannel == IMAGE_RGBA) pixelPtr[3] = rgb.Alpha;
}
