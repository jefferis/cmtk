...
class Colormap :
 ...
 public:
   ...
   */
  void Apply(void *const outPtr, const TypedArray *inPtr,
  ...
  static void HSV2RGB(RGB &rgb, Types::DataItem H, Types::DataItem S,
                      Types::DataItem V);
 ...
 private:
   ...
   *\param paddingData Padding value. Values equal to this are ignored if
   *"paddingFlag" is true. \see Apply
   */
  template <class T>
  void ApplyPrimitive(RGB *const outPtr, const T *inPtr,
   ...
   */
  template <class T>
  void ApplyPrimitive(RGBA *const outPtr, const T *inPtr,
                      const unsigned int count, const bool paddingFlag,
                      const T paddingData) const;
