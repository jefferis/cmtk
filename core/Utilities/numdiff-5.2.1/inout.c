...
static void
find_and_hash_each_line (struct file_data *current, char* ifs, int index, argslist *argl)
{
  hash_value h;
  ...
  while ((char const *) p < suffix_begin)
    {
      ...
      for (fieldno = 0; *p != '\n'; fieldno++)
        {
          /* 
	     The first time we enter in this cycle we have
	     *p != '\n' && strchr (ifs, *p) == NULL
         
	     But (see the last line of the cycle) this
	     is true also later 
          */
          for (eof = (unsigned char*)p + 1; 
	       *eof != '\n' && !strchr (ifs, *eof); eof++);
          /*
	  ...
	  if ( (ghostmask[fieldno >> 3] & 0x80 >> (fieldno & 0x7)) )
	      ...
	      else
		{
          ...
          for (*eof = c; *p != '\n' && (strchr (ifs, *p)); p++);
          /* And now to the begin of the next field */
        }
      p++;
