...
cmtk::UniformVolumeInterpolatorBase *MakeInterpolator(
    const cmtk::UniformVolume &volume,
...
int doMain(const int argc, const char *argv[]) {
  ...
  try {
    ...
    cl.SetProgramInfo(cmtk::CommandLine::PRG_DESCR,
                      "This tool prints pixel values or symbolic labels at a "
                      "list of user-provided image coordinates.");

    typedef cmtk::CommandLine::Key Key;
    cmtk::CommandLine::EnumGroup<CoordinateModeEnum>::SmartPtr modeGroup =
        cl.AddEnum("coordinates", &mode, "Coordinate specification mode.");
    modeGroup->AddSwitch(Key("absolute"), COORDINATES_ABSOLUTE,
                         "Use absolute volume coordinates. For each dimension, "
                         "the valid range is [0,FOV].");
    modeGroup->AddSwitch(Key("indexed"), COORDINATES_INDEXED,
                         "Use grid indexes to specify coordinates. For each "
                         "dimension, the valid value range is [0,Dims-1].");
    modeGroup->AddSwitch(Key("relative"), COORDINATES_RELATIVE,
    ...
    cl.AddOption(Key("sinc-window-radius"), &interpolationWindowRadius,
  ...
  cmtk::UniformVolumeInterpolatorBase::SmartPtr interpolator(
      MakeInterpolator(*volume, interpolation, interpolationWindowRadius));
  ...
  try {
    ...
    cmtk::StdErr
        << "ERROR: singular image-to-physical space matrix encountered\n";
    throw cmtk::ExitException(1);
  }
  ...
  while (!feof(infile)) {
    ...
    if (3 == fscanf(infile, "%20lf %20lf %20lf", xyz, xyz + 1, xyz + 2)) {
      cmtk::UniformVolume::SpaceVectorType v =
          cmtk::UniformVolume::SpaceVectorType::FromPointer(xyz);
