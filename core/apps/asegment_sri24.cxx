...
int doMain(const int argc, const char *argv[]) {
  ...
  try {
    ...
    cl.SetProgramInfo(cmtk::CommandLine::PRG_DESCR,
                      "Register a target image to a selected channel of the "
                      "SRI24 human brain atlas, then reformat one of the atlas "
                      "label maps to the target image. "
                      "Note: it is assume that the target image is "
                      "skull-stripped, i.e., contains only the brain.");
    ...
    cmtk::CommandLine::EnumGroup<std::string>::SmartPtr channelGroup =
        cl.AddEnum(
            "registration-channel", &channelSRI24,
            "The SRI24 channel used for registration to the target image.");
    channelGroup->AddSwitch(Key("spgr"), "spgr",
                            "SPGR (T1-weighted) structural channel");
    channelGroup->AddSwitch(Key("early-fse"), "erly",
                            "Early-echo (PD-weighted) fast spin echo channel");
    channelGroup->AddSwitch(Key("late-fse"), "late",
                            "Late-echo (T2-weighted) fast spin echo channel");
    channelGroup->AddSwitch(
        Key("fa"), "fa",
    ...
    cmtk::CommandLine::EnumGroup<std::string>::SmartPtr labelsGroup =
        cl.AddEnum(
            "label-map", &labelsSRI24,
            "The SRI24 label map that is reformatted to the target image.");
    labelsGroup->AddSwitch(Key("tzo116plus"), "tzo116plus",
                           "Extended cortical parcellation template based on "
                           "Tzourio-Mazoyer AAL 116 region template.");
    labelsGroup->AddSwitch(Key("lpba40"), "lpba40",
                           "Template based on the 40 subject LONI "
                           "Probabilistic Brain Atlas segmentation.");
    labelsGroup->AddSwitch(Key("tissue"), "tissue",
                           "SRI24 maximum likelihood three compartment (GM, "
                           "WM, CSF) tissue segmentation map.");
  ...
  std::string atlasImageName =
      std::string(CMTK_ROOT_PATH_SRI24) + "/" + channelSRI24 + ".nii";
  ...
  std::string atlasLabelName =
      std::string(CMTK_ROOT_PATH_SRI24) + "/" + labelsSRI24 + ".nii";
