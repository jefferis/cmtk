...
void ComputeHistogram(std::list<cmtk::TypedArray::SmartPtr> &dataList,
                      cmtk::Histogram<double> &histogram) {
  ...
  for (; it != dataList.end(); ++it) {
    ...
    for (size_t offset = 0; offset < size; ++offset)
      if (data->Get(value, offset)) {
        if (!Truncate ||
            ((value >= range.m_LowerBound) && (value <= range.m_UpperBound)))
...
void ComputeHistogram(std::list<cmtk::TypedArray::SmartPtr> &dataList,
                      const cmtk::TypedArray *mask, const int maskValue,
                      cmtk::Histogram<double> &histogram) {
  const cmtk::Types::DataItemRange range = histogram.GetRange();
...
void WriteHistogram(const cmtk::Histogram<double> &histogram,
                    const char *outfile) {
  ...
  if (outfile) {
    ...
    for (unsigned int bin = 0; bin < histogram.GetNumberOfBins(); ++bin) {
      ...
      stream << bin << "\t" << histogram.BinToValue(bin) << "\t"
             << histogram[bin] << "\t" << cumulative << "\n";
    }
  } else {
    for (unsigned int bin = 0; bin < histogram.GetNumberOfBins(); ++bin) {
      cumulative += histogram[bin];
      std::cout << bin << "\t" << histogram.BinToValue(bin) << "\t"
                << histogram[bin] << "\t" << cumulative << "\n";
    }
  }
}
...
int doMain(const int argc, const char *argv[]) {
  try {
    ...
    cl.SetProgramInfo(cmtk::CommandLine::PRG_DESCR,
                      "Create a histogram of image intensities and write as "
                      "tab-separated text file to standard output");
    ...
    cl.AddOption(Key('n', "nbins"), &NumberOfBins,
                 "Number of histogram bins (default: number of grey levels)");
    cl.AddSwitch(Key('N', "normalize"), &NormalizeBins, true,
                 "Normalize histogram to maximum 1.0");

    cl.AddOption(Key("min"), &UserMinValue, "User-defined minimum value",
                 &UserMinMaxValue);
    cl.AddOption(Key("max"), &UserMaxValue, "User-defined maximum value",
                 &UserMinMaxValue);
    cl.AddSwitch(Key('t', "truncate"), &Truncate, true,
                 "Truncate histogram (do not enter values outside range into "
                 "first/last bin)");
    cl.AddOption(Key("pad"), &PaddingValue, "Image padding value",
                 &PaddingFlag);

    cl.AddOption(Key('o', "outfile"), &outFile,
                 "File name pattern for histograms");
    cl.AddOption(Key('m', "mask"), &maskFile,
                 "File name for multi-valued mask file");
  ...
  if (!maskFile) {
    ...
    if (mask) {
      ...
      for (size_t i = 0; i < maskData->GetDataSize(); ++i) {
        ...
        if (maskData->Get(l, i)) labelFlags[static_cast<byte>(l)] = true;
      }
