...
void GetNormalizationCoefficients(const cmtk::TypedArray *floatingData,
                                  const cmtk::Types::DataItem refMean,
                                  const cmtk::Types::DataItem refVariance,
                                  cmtk::Types::DataItem &scale,
                                  cmtk::Types::DataItem &offset) {
  ...
  cmtk::DebugOutput(1).GetStream().printf(
      "Converted grey values: %f +- %f -> %f +- %f\n", fltMean,
      sqrt(fltVariance), refMean, sqrt(refVariance));
}
...
int doMain(const int argc, const char *argv[]) {
  try {
    ...
    cl.SetProgramInfo(cmtk::CommandLine::PRG_DESCR,
                      "This tool computes pixelwiase average, variance, "
                      "standard deviation, z-score, or entropy images from a "
                      "list of user-provided intensity images.");
    cl.SetProgramInfo(cmtk::CommandLine::PRG_SYNTX,
                      "average_images [options] image0 ...");

    typedef cmtk::CommandLine::Key Key;

    cmtk::CommandLine::EnumGroup<ModeEnum>::SmartPtr modeGroup =
        cl.AddEnum("mode", &Mode, "Mode of averaging operation");
    modeGroup->AddSwitch(Key("avg"), MODE_AVG,
                         "Compute average (i.e., mean) image");
    modeGroup->AddSwitch(Key("var"), MODE_VAR, "Compute variance image");
    modeGroup->AddSwitch(Key("stdev"), MODE_STDEV,
                         "Compute standard deviation image");
    modeGroup->AddSwitch(Key("zscore"), MODE_ZSCORE, "Compute z-score image");
    modeGroup->AddSwitch(Key("entropy"), MODE_ENTROPY,
  ...
  for (it = imagePathList.begin(); it != imagePathList.end(); ++it) {
    ...
    if (Normalize) {
      if (firstImage) {
        ...
        GetNormalizationCoefficients(data, refMean, refVariance, normFactor,
                                     normOffset);
    ...
    if (firstImage) {
      ...
      imagesValueRange.m_LowerBound =
          std::min(imagesValueRange.m_LowerBound, dataRange.m_LowerBound);
      imagesValueRange.m_UpperBound =
          std::min(imagesValueRange.m_UpperBound, dataRange.m_UpperBound);
    }

    dataList.push_back(data);
  }

  // this is only used in "Entropy" mode, but we'll instantiate it anyway to
  // save time
  cmtk::Histogram<float> histogram(NumberHistogramBins);
  histogram.SetRange(imagesValueRange);

  if (!outputData) {
    outputData = cmtk::TypedArray::SmartPtr(
  ...
  for (size_t i = 0; i < volume->GetNumberOfPixels(); ++i) {
    ...
    if (actualSize) {
      ...
      switch (Mode) {
        case MODE_AVG: {
          const cmtk::Types::DataItem avg =
              cmtk::MathUtil::Mean<cmtk::Types::DataItem>(pixelData);
        ...
        case MODE_VAR: {
          const cmtk::Types::DataItem avg =
              cmtk::MathUtil::Mean<cmtk::Types::DataItem>(pixelData);
          const cmtk::Types::DataItem var =
              cmtk::MathUtil::Variance<cmtk::Types::DataItem>(pixelData, avg);
        ...
        case MODE_ENTROPY: {
          ...
          for (size_t idx = 0; idx < actualSize; ++idx)
            histogram.IncrementFractional(
                histogram.ValueToBinFractional(pixelData[idx]));
