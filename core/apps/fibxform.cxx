...
int doMain(const int argc, const char *argv[]) {
  ...
  try {
    ...
    cl.SetProgramInfo(
        ...
        "A file with fiber tracking results from the UNC Fiber Tracking tool "
        "is read from Standard Input and one or more (concatenated) coordinate "
        "transformations are applied to all fiber point coordinates. "
        "The result is written to Standard Output, again in UNC fiber file "
        "format.");
    ...
    cl.AddOption(
          Key("source-image"), &sourceImagePath,
          "Set source image of the transformation (i.e., an image defining the "
          "space in which fiber tracking was performed) "
          "to correct for differences in orientation and coordinate space.")
        ->SetProperties(cmtk::CommandLine::PROPS_IMAGE);
    cl.AddOption(
          Key("target-image"), &targetImagePath,
          "Set target image of the transformation (i.e., the image that the "
          "fiber track points are mapped into) "
          "to correct for differences in orientation and coordinate space.")
        ->SetProperties(cmtk::CommandLine::PROPS_IMAGE);
  ...
  if (sourceImagePath) {
    cmtk::UniformVolume::SmartPtr sourceImage(
    ...
    sourceImage->ChangeCoordinateSpace(
        sourceImage->GetMetaInfo(cmtk::META_SPACE_ORIGINAL));
    try {
  ...
  if (targetImagePath) {
    ...
    targetImage->ChangeCoordinateSpace(
        targetImage->GetMetaInfo(cmtk::META_SPACE_ORIGINAL));
    try {
      xformList.Add(cmtk::AffineXform::SmartPtr(new cmtk::AffineXform(
          targetImage->GetImageToPhysicalMatrix().GetInverse())));
    } catch (const cmtk::AffineXform::MatrixType::SingularMatrixException &) {
    ...
    if (line.compare(0, 7, "NPoints") != 0) {
      ...
      for (size_t n = 0; n < npoints; ++n) {
        ...
        if (valid) {
          ...
          ;
          outputPointLines[npointsOut++] =
              std::string(sout.str() + " " + restOfLine);
        }
      }
