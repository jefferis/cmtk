...
void ParseArgsString(const std::string &in, std::vector<double> &args) {
  std::string arg = in;
  while (arg.length()) {
...
void ParseArgsWarpString(const std::string &in, std::string &fname,
                         std::vector<double> &args) {
  size_t colon = in.find(':');
  if (colon == std::string::npos) {
    cmtk::StdErr << "ERROR: could not parse argument string '" << in
                 << "'; must be ID:x0:[x1...]\n";
...
int doMain(const int argc, const char *argv[]) {
  ...
  try {
    ...
    cl.SetProgramInfo(cmtk::CommandLine::PRG_DESCR,
                      "Linear (and higher-order polynomial) regression of "
                      "deformation fields and images.");
    cl.SetProgramInfo(cmtk::CommandLine::PRG_SYNTX,
                      "regress [options] controlFile");

    typedef cmtk::CommandLine::Key Key;
    cl.AddOption(Key('o', "output"), &outFileName, "File name for output.");
    cl.AddOption(Key('p', "parameters"), &sParameters,
                 "Model parameters for regression instantiation");
    cl.AddOption(Key('s', "substitution"), &pathPrintf,
                 "Printf format string for ID-to-path substitition");
    cl.AddOption(Key('O', "order"), &order,
                 "Polynonial order of the regression [default: 1=linear]");
    ...
    if (line.length()) {
      ...
      if (nActualParameters) {
        if (nActualParameters != args.size()) {
          cmtk::StdErr << "ERROR: number of model parameters changed from "
                       << nActualParameters << " to " << args.size()
                       << " in argument " << line << "\n";
          throw cmtk::ExitException(1);
        }
      } else {
  ...
  if (!nData) {
    cmtk::StdErr
        << "ERROR: control file must contain at least one valid entry.\n";
    throw cmtk::ExitException(1);
  }
  ...
  if (cmtk::FileFormat::Identify(pathList.begin()->c_str()) ==
  ...
  // xform mode
  {
    ...
    for (std::list<std::string>::const_iterator it = pathList.begin();
         it != pathList.end(); ++it) {
      cmtk::SplineWarpXform::SmartPtr xform =
          cmtk::SplineWarpXform::SmartPtr::DynamicCastFrom(
              cmtk::Xform::SmartPtr(cmtk::XformIO::Read(*it)));
      if (!xform) {
        cmtk::StdErr << "ERROR: transformation '" << *it
                     << "' is either invalid or not a spline warp xform\n";
        throw cmtk::ExitException(1);
      }
    ...
    for (size_t i = 0; i < vWarpXform.size(); ++i) {
      try {
        ...
        cmtk::StdErr << "ERROR: encountered singular matrix replacing warp "
                        "initial affine transformation with identity.\n";
        throw cmtk::ExitException(1);
      }
    }
    ...
    for (size_t i = 0; i < vWarpXform.size(); ++i) {
      cmtk::Types::Coordinate *deformation =
          vWarpXform[i]->GetPureDeformation();
      cmtk::TypedArray::SmartPtr data(cmtk::TypedArray::Create(
          cmtk::TYPE_COORDINATE, deformation, nWarpParameters));
      ...
      vWarpXform[i] =
          cmtk::SplineWarpXform::SmartPtr::Null();  // no longer needed
    }

    for (size_t n = 0; n < mean.size(); ++n) mean[n] /= nData;
    ...
    if (outFileName) cmtk::XformIO::Write(referenceWarp, outFileName);
  } else
  // image mode
  {
    ...
    if (outFileName) cmtk::VolumeIO::Write(*referenceVolume, outFileName);
  }
