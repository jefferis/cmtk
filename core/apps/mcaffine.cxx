...
void DoRegistration() {
  typedef cmtk::AffineMultiChannelRegistrationFunctional<TMetricFunctional>
      FunctionalType;
  ...
  if (initialXformPath) {
    ...
    if (xform) {
      const cmtk::AffineXform::SmartPtr affine =
          cmtk::AffineXform::SmartPtr::DynamicCastFrom(xform);
      if (affine) {
        affine->GetParamVector(params);
      } else {
        const cmtk::WarpXform::SmartPtr warp =
            cmtk::WarpXform::SmartPtr::DynamicCastFrom(xform);
        if (warp) {
      ...
      cmtk::StdErr << "ERROR: unable to read initial transformation from "
                   << initialXformPath << "\n";
  ...
  for (int downsample = std::max(downsampleFrom, downsampleTo);
       downsample >= std::min(downsampleFrom, downsampleTo); --downsample) {
    ...
    for (std::vector<int>::const_iterator itDOF = numberDOFs.begin();
      ...
      const cmtk::Types::Coordinate effectiveMinPixelSize =
          std::max(1, downsample) * minPixelSize;
      optimizer.Optimize(params, initialStepSize * effectiveMinPixelSize,
                         finalStepSize * effectiveMinPixelSize);
    }
  }
  ...
  if (outArchive) {
    ...
    if (stream.IsValid()) {
      ...
      cmtk::StdErr << "ERROR: could not open archive " << outArchive
                   << " for writing.\n";
    }
  }
}
...
int doMain(const int argc, const char *argv[]) {
  try {
    ...
    cl.SetProgramInfo(
        ...
        "Multi-channel affine image registration using histogram-based or "
        "covariance-based joint entropy measures");
    cl.SetProgramInfo(cmtk::CommandLine::PRG_SYNTX,
                      "mcaffine [options] refChannel0 [refChannel1 ...] -- "
                      "fltChannel0 [fltChannel1 ...]");
    ...
    cl.AddOption(Key("initial-xform"), &initialXformPath,
                 "Optional path of a file with the initial transformation.");
    ...
    cl.AddOption(Key('D', "downsample-to"), &downsampleTo,
                 "Final downsampling factor [1]. Factor 0 is full resolution "
                 "with smoothing turned off");
    ...
    cl.AddVector(Key("dofs"), numberDOFs,
                 "Set sequence of numbers of DOFs for optimization schedule "
                 "[can be repeated]. Supported values are: 0, 3, 6, 7, 9, 12.");

    cl.AddSwitch(Key("nmi"), &metricNMI, true,
                 "Use normalized mutual information metric [default]");
    ...
    cl.AddOption(Key("crop-reference-from-index"), &cropReferenceFromIndex,
                 "Crop reference image from index x,y,z.");
    cl.AddOption(Key("crop-reference-to-index"), &cropReferenceToIndex,
                 "Crop reference image to index x,y,z.");
  ...
  for (std::list<const char *>::const_iterator refIt = fileListRef.begin();
       refIt != fileListRef.end(); ++refIt) {
  ...
  if (cropReferenceFromIndex) {
    int xyz[3];
    if (3 != sscanf(cropReferenceFromIndex, "%6d,%6d,%6d", &xyz[0], &xyz[1],
                    &xyz[2])) {
      cmtk::StdErr << "ERROR: reference crop from index could not parse index '"
                   << cropReferenceFromIndex << "' as valid x,y,z index.\n";
      throw cmtk::ExitException(1);
    }
    for (std::list<cmtk::UniformVolume::SmartPtr>::iterator refIt =
             refChannelList.begin();
         refIt != refChannelList.end(); ++refIt) {
      (*refIt)->CropRegion() = cmtk::DataGrid::RegionType(
          cmtk::DataGrid::IndexType::FromPointer(xyz),
          (*refIt)->CropRegion().To());
    }
  }
  ...
  if (cropReferenceToIndex) {
    int xyz[3];
    if (3 != sscanf(cropReferenceToIndex, "%6d,%6d,%6d", &xyz[0], &xyz[1],
                    &xyz[2])) {
      cmtk::StdErr << "ERROR: reference crop to index could not parse index '"
                   << cropReferenceToIndex << "' as valid x,y,z index.\n";
      throw cmtk::ExitException(1);
    }
    for (std::list<cmtk::UniformVolume::SmartPtr>::iterator refIt =
             refChannelList.begin();
         refIt != refChannelList.end(); ++refIt) {
      (*refIt)->CropRegion() = cmtk::DataGrid::RegionType(
          (*refIt)->CropRegion().From(),
          cmtk::DataGrid::IndexType::FromPointer(xyz));
    }
  }

  for (std::list<const char *>::const_iterator fltIt = fileListFlt.begin();
       fltIt != fileListFlt.end(); ++fltIt) {
