...
void ReadMatrix(cmtk::Types::Coordinate (&matrix)[4][4], std::istream &stream,
                const bool matrix3x3) {
...
int doMain(const int argc, const char *argv[]) {
  ...
  try {
    ...
    cl.AddSwitch(Key('t', "transpose"), &Transpose, true,
                 "Transpose input matrix.");
    cl.AddOption(Key("pixel-size"), &PixelSizeStr,
                 "For matrices in pixel space (i.e., mapping from grid index "
                 "to physical coordinate), set image grid pixel size.");
    cl.EndGroup();

    cl.BeginGroup("Modifiers", "Transformation Modifiers");
    cl.AddOption(Key("center"), &CenterStr,
                 "Set center x,y,z for rotation and scaling.");
    cl.AddOption(Key("offset"), &OffsetStr,
                 "Set offset dx,dy,dz for translation.");
    cl.AddOption(Key("xlate"), &TranslateStr,
                 "Translate result relative by given vector.");
    cl.AddSwitch(Key("inverse"), &Inverse, true,
                 "Output inverse transformation. Inversion happens after all "
                 "other modifiers have been applied.");
    cl.EndGroup();

    cl.BeginGroup("Output", "Output Options");
    cl.AddOption(Key('o', "output"), &OutXform,
                 "Write output transformation to this file");
    cl.AddOption(Key("list"), &OutList,
                 "Write output in list format to this archive (directory "
                 "including 'registration' and 'studylist' files).");
    cl.AddSwitch(Key("append"), &AppendToOutput, true,
                 "Append to output file rather than create a new one and "
  ...
  if (PixelSizeStr) {
    ...
    if (3 == sscanf(PixelSizeStr, "%15f,%15f,%15f", pixel + 0, pixel + 1,
                    pixel + 2)) {
      for (int row = 0; row < 4; ++row)
        for (int col = 0; col < 4; ++col) matrix[col][row] /= pixel[row];
    }
  }
  ...
  try {
    ...
    cmtk::StdErr << "ERROR: singular matrix encountered in cmtk::AffineXform "
                    "constructor.\n";
    throw cmtk::ExitException(1);
  }
  ...
  if (CenterStr) {
    float center[3];
    if (3 == sscanf(CenterStr, "%15f,%15f,%15f", center + 0, center + 1,
                    center + 2)) {
      xform->ChangeCenter(
          cmtk::FixedVector<3, cmtk::Types::Coordinate>::FromPointer(center));
    }
  }

  if (OffsetStr) {
    float offset[3];
    if (3 == sscanf(OffsetStr, "%15f,%15f,%15f", offset + 0, offset + 1,
                    offset + 2)) {
      xform->Translate(xform->RotateScaleShear(
          cmtk::FixedVector<3, cmtk::Types::Coordinate>::FromPointer(offset)));
    }
  }

  if (TranslateStr) {
    float xlate[3];
    if (3 == sscanf(TranslateStr, "%15f,%15f,%15f", xlate + 0, xlate + 1,
                    xlate + 2)) {
      xform->Translate(
          cmtk::FixedVector<3, cmtk::Types::Coordinate>::FromPointer(xlate));
    }
  }
  ...
  if (Inverse) {
    try {
      ...
      cmtk::StdErr << "ERROR: affine transformation with singular matrix "
                      "cannot be inverted\n";
      throw cmtk::ExitException(1);
    }
  }
  ...
  if (OutList) {
    if (AppendToOutput) {
      cmtk::ClassStreamOutput outStream(OutList, "registration",
                                        cmtk::ClassStreamOutput::MODE_APPEND);
      outStream << *xform;
    } else {
  ...
  if (OutXform) {
    cmtk::ClassStreamOutput outStream(
        OutXform, AppendToOutput ? cmtk::ClassStreamOutput::MODE_APPEND
