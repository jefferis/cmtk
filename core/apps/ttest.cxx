...
int doMain(const int argc, const char *argv[]) {
  ...
  try {
    ...
    cl.SetProgramInfo(cmtk::CommandLine::PRG_DESCR,
                      "Pixelwise tests of statistical significance. Also "
                      "compute correlations and z-scores");
    cl.SetProgramInfo(
        cmtk::CommandLine::PRG_SYNTX,
        "ttest [options] imageX0 [imageX1 ...] -- imageY0 [imageY1] ...\n"
        "ttest [options] --symmetric imageX0 [imageX1 ...]");
    cl.SetProgramInfo(cmtk::CommandLine::PRG_CATEG,
                      "CMTK.Statistics and Modeling");

    typedef cmtk::CommandLine::Key Key;
    cl.AddSwitch(Key('l', "log"), &UseLogData, true,
                 "Use log data for testing");
    cl.AddSwitch(Key('a', "abs"), &UseAbsData, true,
                 "Use absolute data for testing");

    cl.AddSwitch(Key('A', "abs-out"), &AbsoluteOutput, true,
                 "Generate absolute (unsigned) data");
    cl.AddSwitch(Key('i', "invert"), &Invert, true,
                 "Invert data, i.e., subtract from 1.0");
    cl.AddSwitch(Key('s', "short"), &ConvertToShort, true,
                 "Convert probabilities to short (and scale by 1000)");

    cl.AddSwitch(Key('p', "paired"), &Mode, TTEST_PAIRED,
                 "Compute paired t-test [default: groupwise]");
    cl.AddSwitch(Key('c', "cross-correlation"), &Mode, CORRELATION_PAIRED,
                 "Compute paired cross-correlation");
    cl.AddSwitch(Key('Z', "zscores"), &Mode, ZSCORES,
                 "Compute z-scores of test (y) distribution averages");

    cl.AddSwitch(Key('t', "text"), &TextFileMode, true,
    ...
    cl.AddOption(
        Key("tstats-file"), &TStatFileName,
        "T-statistics output file name (for correlation: p-value output file)");
  ...
  if (TextFileMode) {
    ...
    for (; fnameIt != FileListX.end(); ++fnameIt) {
      ...
      for (size_t i = 0; i < data.size(); ++i) array->Set(data[i], i);
    ...
    for (; fnameIt != FileListX.end(); ++fnameIt) {
      ...
      cmtk::UniformVolume::SmartPtr volume(
          cmtk::VolumeIO::ReadOriented(*fnameIt));
      if (volume) {
  ...
  if (MaskFileName) {
    if (TextFileMode) {
      ...
      if (volume) maskData = volume->GetData();
      if (!maskData) {
  ...
  if (TextFileMode || refVolume) {
    ...
    switch (Mode) {
      case TTEST: {
        ...
        if (TextFileMode) {
          if (dataY.empty()) {
            probData = cmtk::TypedArray::SmartPtr(
                cmtk::HypothesisTests::GetOneSampleTTest(dataX, &tstatsData,
                                                         &avgXData, maskData));
          } else {
            probData = cmtk::TypedArray::SmartPtr(
                cmtk::HypothesisTests::GetUnpairedTwoTailedTTest(
                    dataX, dataY, &tstatsData, &avgXData, &avgYData, maskData));
          }
        } else {
          if (dataY.empty()) {
            probData = cmtk::TypedArray::SmartPtr(
                cmtk::HypothesisTests::GetOneSampleTTest(
                    dataX, &tstatsData, NULL /*avgXData*/, maskData));
          } else {
            probData = cmtk::TypedArray::SmartPtr(
                cmtk::HypothesisTests::GetUnpairedTwoTailedTTest(
                    dataX, dataY, &tstatsData, NULL /*avgXData*/,
                    NULL /*avgYData*/, maskData));
          }
        }

        if (refVolume && TStatFileName) {
          cmtk::DebugOutput(1)
              << "Writing T-statistics to file " << TStatFileName << "\n";
        ...
        if (ConvertToShort) {
          ...
          probData =
              cmtk::TypedArray::SmartPtr(probData->Convert(cmtk::TYPE_SHORT));
        }
        ...
        if (TextFileMode) {
          ...
          cmtk::DebugOutput(1)
              << "Writing T-probablities to file " << OutFileName << "\n";
      ...
      case TTEST_PAIRED: {
        ...
        if (!dataY.empty()) {
          try {
            probData = cmtk::TypedArray::SmartPtr(
                cmtk::HypothesisTests::GetPairedTwoTailedTTest(
                    dataX, dataY, &tstatsData, NULL /*avgXData*/,
                    NULL /*avgYData*/, maskData));
      ...
      case CORRELATION_PAIRED: {
        ...
        if (!dataY.empty()) {
          try {
            probData = cmtk::TypedArray::SmartPtr(
                cmtk::HypothesisTests::GetPairedCorrelation(dataX, dataY,
                                                            &pData, maskData));
          } catch (const cmtk::Exception &ex) {
            cmtk::StdErr << "ERROR: " << ex.what() << "\n";
            throw cmtk::ExitException(1);
        ...
        if (refVolume && TStatFileName) {
          cmtk::DebugOutput(1)
              << "Writing probabilities to file " << TStatFileName << "\n";
          refVolume->SetData(cmtk::TypedArray::SmartPtr(pData));
          cmtk::VolumeIO::Write(*refVolume, TStatFileName);
        }

        cmtk::DebugOutput(1)
            << "Writing correlations to file " << OutFileName << "\n";
        refVolume->SetData(probData);
        cmtk::VolumeIO::Write(*refVolume, OutFileName);
        break;
      }
      case ZSCORES: {
        cmtk::TypedArray::SmartPtr zscoreData = cmtk::TypedArray::SmartPtr(
            cmtk::HypothesisTests::GetZScores(dataX, dataY, maskData));

        if (AbsoluteOutput)
          zscoreData->ApplyFunctionDouble(cmtk::Wrappers::Abs);
        if (Invert) zscoreData->Rescale(-1.0, 1.0);

        if (ConvertToShort) {
          zscoreData->Rescale(1000, 0);
          zscoreData =
              cmtk::TypedArray::SmartPtr(zscoreData->Convert(cmtk::TYPE_SHORT));
        }

        cmtk::DebugOutput(1)
            << "Writing z-scores map to file " << OutFileName << "\n";
