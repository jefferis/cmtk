...
int doMain(const int argc, const char *argv[]) {
  try {
    ...
    cl.AddOption(Key('p', "pad-in"), &PaddingInValue,
                 "Define padding value for input images", &PaddingInFlag);
    ...
    cl.AddOption(Key('N', "num-labels"), &NumberOfLabels,
                 "Number of label values [default: autodetect]");
    cl.AddOption(Key('f', "first-label"), &FirstLabel,
                 "Index of first label value [default: 0]; labels below this "
                 "one are ignored");
    cl.AddSwitch(Key("by-label"), &ByLabel, true,
                 "Analysis by label, i.e., separately for each label in given "
                 "(detected) range");
    ...
    while (next) {
      ...
      if (!vectorOfDataArrays.empty()) {
        if (vectorOfDataArrays[0]->GetDataSize() !=
            volume->GetNumberOfPixels()) {
          cmtk::StdErr << "ERROR: pixel count of image " << next
                       << " does not match other images.\n";
          throw cmtk::ExitException(1);
        }
      }
  ...
  if (ByLabel) {
    ...
    for (unsigned int label = FirstLabel; label < FirstLabel + NumberOfLabels;
         ++label) {
      if (overlapMeasures.ComputeGroupwiseOverlap(label, 1 /*numberOfLabels*/,
                                                  overlapEqual, overlapVolume,
                                                  overlapInverse) > 0) {
        fprintf(stdout, "%u\t%lf\t%lf\t %lf\n", label, overlapEqual,
                overlapVolume, overlapInverse);
      }
    }
  } else {
    overlapMeasures.ComputeGroupwiseOverlap(FirstLabel, NumberOfLabels,
                                            overlapEqual, overlapVolume,
                                            overlapInverse);
    fprintf(stdout,
            "O[equal] = %lf\nO[volume] = %lf\nO[inverse_volume] = %lf\n",
