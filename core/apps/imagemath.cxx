...
/// If this flag is set, PaddingValue defines the padding value for images read
/// from files.
bool PaddingFlag = false;

void CallbackSetPaddingValue(const double paddingValue) {
  cmtk::DebugOutput(2) << "CallbackSetPaddingValue\n";
  PaddingValue = paddingValue;
  PaddingFlag = true;
}

void CallbackUnsetPaddingValue() {
...
/// If this flag is set, the next single-image operation will be applied to all
/// images on the stack.
bool ApplyNextToAll = false;

void CheckStackOneImage(const char *function) {
  if (ImageStack.empty()) {
...
void CheckStackTwoMatchingImages(const char *function) {
  if (ImageStack.size() < 2) {
    cmtk::StdErr
        << "ERROR: at least two images are required to perform operation '"
        << function << "'\n";
    throw cmtk::ExitException(1);
  }
  ...
  const bool pixelCountsMatch =
      (top->GetNumberOfPixels() == ImageStack.front()->GetNumberOfPixels());
  ImageStack.push_front(top);

  if (!pixelCountsMatch) {
    cmtk::StdErr << "ERROR: two top images do not have equal pixel counts as "
...
void CallbackIn(const char **argv, int &argsUsed) {
  cmtk::DebugOutput(2) << "CallbackIn\n";
  argsUsed = 0;
  while (argv[argsUsed][0] != '-') {
    cmtk::UniformVolume::SmartPtr volume(
        cmtk::VolumeIO::ReadOriented(argv[argsUsed]));
    ...
    if (PaddingFlag) volume->GetData()->SetPaddingValue(PaddingValue);
...
void CallbackOut(const char *argv) {
...
void CallbackDup() {
  ...
  ImageStack.push_front(
      cmtk::UniformVolume::SmartPtr(ImageStack.front()->Clone()));
}

void CallbackAll() { ApplyNextToAll = true; }

void CallbackFill(const double value) {
  ...
  while (it != ImageStack.end()) {
    (*it)->SetData(
        cmtk::TypedArray::SmartPtr((*it)->GetData()->Convert(ResultType)));
...
void CallbackThreshBelow(const double threshold) {
...
void CallbackThreshAbove(const double threshold) {
...
void CallbackScalarMul(const double c) {
  ...
  while (it != ImageStack.end()) {
    ...
    cmtk::TypedArray::SmartPtr mul(
        cmtk::TypedArray::Create(ResultType, numberOfPixels));
    cmtk::TypedArray *mulRef =
        mul.GetPtr();  // need this to work around GCD / clang limitations

#ifdef CMTK_USE_GCD
    const cmtk::Threads::Stride stride(numberOfPixels);
    dispatch_apply(stride.NBlocks(), dispatch_get_global_queue(0, 0),
                   ^(size_t b) {
...
void CallbackScalarAdd(const double c) {
  ...
  while (it != ImageStack.end()) {
    ...
    cmtk::TypedArray::SmartPtr add(
        cmtk::TypedArray::Create(ResultType, numberOfPixels));
    cmtk::TypedArray *addRef =
        add.GetPtr();  // need this to work around GCD / clang limitations
...
void CallbackScalarXor(const long int c) {
  ...
  while (it != ImageStack.end()) {
    ...
    cmtk::TypedArray::SmartPtr out(
        cmtk::TypedArray::Create(ResultType, numberOfPixels));
    cmtk::TypedArray *outRef =
        out.GetPtr();  // need this to work around GCD / clang limitations
...
void CallbackScalarAnd(const long int c) {
...
void CallbackOneOver() {
  ...
  while (it != ImageStack.end()) {
    ...
    cmtk::TypedArray::SmartPtr inv(
        cmtk::TypedArray::Create(ResultType, numberOfPixels));
    cmtk::TypedArray *invRef =
        inv.GetPtr();  // need this to work around GCD / clang limitations
...
void CallbackDiv() {
  ...
  const size_t numberOfPixels = p->GetNumberOfPixels();
  cmtk::TypedArray::SmartPtr div(
      cmtk::TypedArray::Create(ResultType, numberOfPixels));
  cmtk::TypedArray *divRef =
      div.GetPtr();  // need this to work around GCD / clang limitations
...
void CallbackComplexDiv() {
  ...
  cmtk::TypedArray::SmartPtr divP(
      cmtk::TypedArray::Create(ResultType, numberOfPixels));
  cmtk::TypedArray::SmartPtr divQ(
      cmtk::TypedArray::Create(ResultType, numberOfPixels));

#pragma omp parallel for
  for (int i = 0; i < static_cast<int>(numberOfPixels); ++i) {
    cmtk::Types::DataItem pv0, qv0, pv1, qv1;
    if (p0->GetDataAt(pv0, i) && q0->GetDataAt(qv0, i) &&
        p1->GetDataAt(pv1, i) && q1->GetDataAt(qv1, i) &&
        ((pv1 != 0) || (qv1 != 0))) {
...
void CallbackAtan2() {
  ...
  const size_t numberOfPixels = p->GetNumberOfPixels();
  cmtk::TypedArray::SmartPtr result(
      cmtk::TypedArray::Create(ResultType, numberOfPixels));
  cmtk::TypedArray *resultRef =
      result.GetPtr();  // need this to work around GCD / clang limitations
...
void CallbackMatchHistograms() {
  if (ImageStack.size() < 2) {
    cmtk::StdErr << "ERROR: need at least two images on stack for histogram "
                    "intensity matching\n";
    return;
  }
  ...
  cmtk::TypedArray::SmartPtr result(
      ImageStack.front()->GetData()->Convert(ResultType));
  result->ApplyFunctionObject(cmtk::TypedArrayFunctionHistogramMatching(
      *(ImageStack.front()->GetData()), *(ref->GetData())));
  ImageStack.front()->SetData(result);
}

void CallbackMatchMeanSDev() {
  if (ImageStack.size() < 2) {
    cmtk::StdErr << "ERROR: need at least two images on stack for histogram "
                    "intensity matching\n";
    return;
...
void CallbackMatchMeanSDevThree() {
  if (ImageStack.size() < 3) {
    cmtk::StdErr << "ERROR: need at least three images on stack for histogram "
                    "intensity matching using separate reference images\n";
    return;
  }
...
void CallbackMaskAverage() {
  if (ImageStack.size() < 2) {
    cmtk::StdErr
        << "ERROR: need at last two images on stack for mask averaging\n";
    return;
  }
...
void CallbackAverage() {
  ...
  while (!ImageStack.empty()) {
    if (ImageStack.size() > 1) CheckStackTwoMatchingImages("Average");

    volPtrs.push_back(ImageStack.front());
    ImageStack.pop_front();
  }

  const std::vector<cmtk::UniformVolume::SmartPtr> &vPtrs =
      volPtrs;  // using const reference, we prevent GCD from segfaulting
...
void CallbackVariance() {
  CheckStackTwoMatchingImages("Variance");

  std::vector<cmtk::UniformVolume::SmartPtr> volPtrs;
  while (!ImageStack.empty()) {
    if (ImageStack.size() > 1) CheckStackTwoMatchingImages("Variance");
  ...
  const size_t numberOfPixels = volPtrs[0]->GetNumberOfPixels();
  cmtk::TypedArray::SmartPtr varianceArray(
      cmtk::TypedArray::Create(ResultType, numberOfPixels));
  cmtk::TypedArray *varianceArrayRef =
      varianceArray
          .GetPtr();  // need this to work around GCD / clang limitations
  ...
  for (int i = 0; i < static_cast<int>(numberOfPixels); ++i)
#endif
        {
          ...
          if (nvalues) {
            ...
            varianceArrayRef->Set(
                (nvalues * mean * mean - 2 * mean * sum + sum2) / nvalues, i);
          } else
...
void CallbackVoteCombination() {
  ...
  while (!ImageStack.empty()) {
    if (ImageStack.size() > 1) CheckStackTwoMatchingImages("Vote");
...
void CallbackSTAPLE(const long int maxIterations) {
  ...
  std::vector<cmtk::TypedArray::SmartPtr> dataPtrs;
  while (!ImageStack.empty()) {
    if (ImageStack.size() > 1) CheckStackTwoMatchingImages("STAPLE");
...
void MultiClassSTAPLE(const long int maxIterations, const bool disputedOnly) {
  ...
  std::vector<cmtk::TypedArray::SmartPtr> dataPtrs;
  while (!ImageStack.empty()) {
    if (ImageStack.size() > 1) CheckStackTwoMatchingImages("MultiClassSTAPLE");
...
void CallbackMultiClassSTAPLE(const long int maxIterations) {
  MultiClassSTAPLE(maxIterations, false /*disputedOnly*/);
}

void CallbackMultiClassSTAPLEDisputed(const long int maxIterations) {
  MultiClassSTAPLE(maxIterations, true /*disputedOnly*/);
}

void CallbackStackEntropyLabels() {
  ...
  const size_t numberOfPixels = volPtrs[0]->GetNumberOfPixels();
  cmtk::TypedArray::SmartPtr entropyArray(
      cmtk::TypedArray::Create(ResultType, numberOfPixels));
  cmtk::TypedArray *entropyArrayRef =
      entropyArray
          .GetPtr();  // need this to work around GCD / clang limitations
  ...
  for (int i = 0; i < static_cast<int>(numberOfPixels); ++i)
#endif
                   {
                     ...
                     if (totalCount) {
                       ...
                       double entropy = 0;
                       for (std::map<int, unsigned int>::const_iterator it =
                                labelCount.begin();
...
void CallbackMaxIndex() {
  ...
  const size_t numberOfPixels = volPtrs[0]->GetNumberOfPixels();
  cmtk::TypedArray::SmartPtr maxArray(
      cmtk::TypedArray::Create(cmtk::TYPE_SHORT, numberOfPixels));
  cmtk::TypedArray *maxArrayRef =
      maxArray.GetPtr();  // need this to work around GCD / clang limitations
...
void CallbackMaxValue() {
  CheckStackTwoMatchingImages("MaxValue");

  std::vector<cmtk::UniformVolume::SmartPtr> volPtrs;
  while (!ImageStack.empty()) {
    if (ImageStack.size() > 1) CheckStackTwoMatchingImages("MaxValue");
...
void CallbackMinValue() {
  CheckStackTwoMatchingImages("MinValue");

  std::vector<cmtk::UniformVolume::SmartPtr> volPtrs;
  while (!ImageStack.empty()) {
    if (ImageStack.size() > 1) CheckStackTwoMatchingImages("MinValue");
  ...
  const size_t numberOfPixels = volPtrs[0]->GetNumberOfPixels();
  cmtk::TypedArray::SmartPtr minArray(
      cmtk::TypedArray::Create(ResultType, numberOfPixels));
  cmtk::TypedArray *minArrayRef =
      minArray.GetPtr();  // need this to work around GCD / clang limitations
...
void CallbackContractLabels() {
  CheckStackTwoMatchingImages("ContractLabels");

  std::vector<cmtk::UniformVolume::SmartPtr> volPtrs;
  while (!ImageStack.empty()) {
    if (ImageStack.size() > 1) CheckStackTwoMatchingImages("ContractLabels");
  ...
  const size_t numberOfPixels = volPtrs[0]->GetNumberOfPixels();
  cmtk::TypedArray::SmartPtr outArray(
      cmtk::TypedArray::Create(ResultType, numberOfPixels));
...
#pragma omp parallel for
  for (int i = 0; i < static_cast<int>(numberOfPixels); ++i)
#endif
                   {
                     ...
                     for (size_t curVol = 0; curVol < vPtrs.size(); ++curVol) {
                       if (vPtrs[curVol]->GetDataAt(v, i) && v) break;
                     }
...
void CallbackCombinePCA() {
  std::vector<cmtk::UniformVolume::SmartPtr> volPtrs;
  while (!ImageStack.empty()) {
    if (ImageStack.size() > 1) CheckStackTwoMatchingImages("CombinePCA");
  ...
  for (size_t imageY = 0; imageY < numberOfImages; ++imageY) {
    for (size_t imageX = 0; imageX < numberOfImages; ++imageX) {
      ...
      for (size_t pixel = 0; pixel < numberOfPixels; ++pixel) {
        ...
        if (volPtrs[imageX]->GetDataAt(valueX, pixel) &&
            volPtrs[imageY]->GetDataAt(valueY, pixel)) {
  ...
  const std::vector<cmtk::Types::DataItem> eigenvalues =
      eigensystem.GetEigenvalues();
  ...
  cmtk::TypedArray::SmartPtr output(
      cmtk::TypedArray::Create(ResultType, numberOfPixels));

  cmtk::Vector<cmtk::Types::DataItem> v(numberOfImages);
  for (size_t pixel = 0; pixel < numberOfPixels; ++pixel) {
    for (size_t image = 0; image < numberOfImages; ++image) {
      if (!volPtrs[image]->GetDataAt(v[image], pixel)) v[image] = 0;
...
int doMain(const int argc, const char *argv[]) {
  try {
    ...
    cl.SetProgramInfo(
        ...
        "the stack can be processed and combined via different operators. "
        "Results of all operations are put back onto the stack, where they can "
        "be further processed or written back to image files.");

    typedef cmtk::CommandLine::Key Key;
    cl.BeginGroup("Input/output", "Input/output operations");
    cl.AddCallback(Key("in"), CallbackIn,
                   "Read input image(s) to top of stack");
    cl.AddCallback(
        Key("out"), CallbackOut,
        "Write output image from top of stack (but leave it on the stack)");
    cl.AddCallback(Key("set-padding-value"), CallbackSetPaddingValue,
                   "Set the value that is interpreted as padding value in "
                   "subsequently read images.");
    cl.AddCallback(Key("unset-padding"), CallbackUnsetPaddingValue,
                   "Disable padding. All values in subsequently read images "
                   "will be interpreted as actual data.");
    cl.EndGroup();

    cl.BeginGroup("Internal", "Internal settings");
    cl.AddSwitch(Key("float"), &ResultType, cmtk::TYPE_FLOAT,
                 "Use single precision for computations and results");
    cl.AddSwitch(Key("double"), &ResultType, cmtk::TYPE_DOUBLE,
                 "Use double precision for computations and results");
    cl.EndGroup();

    cl.BeginGroup("Stack", "Stack operations");
    cl.AddCallback(Key("pop"), CallbackPop,
                   "Pop (discard) top image from stack.");
    cl.AddCallback(Key("dup"), CallbackDup,
    ...
    cl.AddCallback(Key("fill"), CallbackFill,
                   "Fill top image with constant value (i.e., assign value to "
                   "all pixels)");
    cl.AddCallback(Key("abs"), CallbackAbs,
                   "Apply abs() function to top image");
    cl.AddCallback(Key("log"), CallbackLog,
                   "Apply log() function to top image");
    cl.AddCallback(Key("logit"), CallbackLogit,
                   "Apply log(x/(1-x)) function to top image");
    cl.AddCallback(Key("logistic"), CallbackLogistic,
                   "Apply 1/(1+exp(-x)) function to top image");
    cl.AddCallback(Key("exp"), CallbackExp,
                   "Apply exp() function to top image");
    cl.AddCallback(Key("sqr"), CallbackSqr,
                   "Apply square operator to top image");
    cl.AddCallback(Key("sqrt"), CallbackSqrt,
                   "Apply square root operator to top image");
    cl.AddCallback(Key("trunc"), CallbackTrunc,
                   "Truncate all values in top image to integer");
    cl.AddCallback(Key("one-over"), CallbackOneOver,
                   "For each pixel, replace its value x with 1.0/x");
    cl.AddCallback(Key("scalar-mul"), CallbackScalarMul,
                   "Multiply top image with a scalar value");
    cl.AddCallback(Key("scalar-add"), CallbackScalarAdd,
    ...
    cl.AddCallback(Key("match-histograms"), CallbackMatchHistograms,
                   ...
                   "and the modified image is pushed onto the stack.");
    cl.AddCallback(
        Key("match-mean-sdev"), CallbackMatchMeanSDev,
        "Scale intensities of one image to match mean and standard deviation "
        "of another. The last image pushed onto the stack provides the "
        "reference intensity distribution, the preceding image will be "
        "modified. Both input images are removed from the stack and the "
        "modified image is pushed onto the stack.");
    cl.AddCallback(
        Key("match-mean-sdev3"), CallbackMatchMeanSDevThree,
        "Scale intensities of an image by a factor and offset computed from "
        "two other images to match their mean and standard deviations. The "
        "last image pushed onto the stack provides the reference intensity "
        "distribution, the preceding image provides the intensity distribution "
        "to match to the reference image's, and the third image on the stack "
        "will be modified. All three input images are removed from the stack "
        "and the modified image is pushed onto the stack.");
    cl.AddCallback(
        Key("mask-average"), CallbackMaskAverage,
        "Mask averaging: the top image is taken as a multi-label mask. The "
        "pixels in the second image are averaged by mask labels, and then "
        "replaced with the average value for each mask label.");
    cl.EndGroup();

    cl.BeginGroup("Complex", "Complex Arithmetic");
    cl.AddCallback(
        Key("complex-div"), CallbackComplexDiv,
        "Complex division, (a+ib)/(c+id), assuming four values were put on the "
        "stack in order a, b, c, d. Place result on stack, real first, "
        "imaginary second (i.e., imaginary is top).");
    cl.EndGroup();

    cl.BeginGroup(
        "Contract multiple images",
        "Operators that contract the entire stack into a single image");
    cl.AddCallback(Key("sum"), CallbackSum,
                   "Sum all images on stack, place result on stack");
    cl.AddCallback(
        Key("product"), CallbackProduct,
        "Compute product of all images on stack, place result on stack");
    cl.AddCallback(Key("average"), CallbackAverage,
                   "Average all images on stack, place result on stack");
    cl.AddCallback(Key("variance"), CallbackVariance,
                   "For each pixel, compute variance over all images on stack, "
                   "place result on stack");
    cl.AddCallback(Key("combine-pca"), CallbackCombinePCA,
                   "Combine images using PCA by projecting onto direction of "
                   "largest correlation");
    cl.AddCallback(Key("max-value"), CallbackMaxValue,
                   "For each pixel, compute maximum VALUE over all images, "
                   "place result on stack");
    cl.AddCallback(Key("min-value"), CallbackMinValue,
                   "For each pixel, compute minimum VALUE over all images, "
                   "place result on stack");
    cl.AddCallback(Key("max-index"), CallbackMaxIndex,
                   "For each pixel, compute INDEX of image with maximum value, "
                   "place result on stack");
    cl.EndGroup();

    cl.BeginGroup("Contract multiple label images",
                  "Operators that contract a stack of label images into a "
                  "single label image");
    cl.AddCallback(
        Key("vote"), CallbackVoteCombination,
        "Merge all images on stack with voting, place result on stack");
    cl.AddCallback(
        Key("staple"), CallbackSTAPLE,
        "Combine binary maps on the stack using [arg] iterations of the STAPLE "
        "algorithm. "
        "The result of this operation is the spatial map of 'weights' W, which "
        "are the probabilities of image foreground at each pixel. In 'verbose' "
        "mode, estimated expert parameters p (sensitivity) and q (specificity) "
        "are also written to standard output.");
    cl.AddCallback(
        Key("contract-labels"), CallbackContractLabels,
        "Contract multiple label maps into one by selecting the first (over "
        "all images on the stack) non-zero label at each pixel");
    cl.AddCallback(Key("mstaple"), CallbackMultiClassSTAPLE,
                   "Combine multi-label maps on the stack using [arg] "
                   "iterations of the multi-class STAPLE algorithm."
                   "The result of this operation is the combined "
                   "maximum-likeliood multi-label map.");
    cl.AddCallback(
        Key("mstaple-disputed"), CallbackMultiClassSTAPLEDisputed,
        "Like previous operation, apply multi-class STAPLE algorithm, but "
        "restrict computation to 'disputed'"
        "voxels, i.e., those where the input label maps disagree. This often "
        "improves results by reducing, e.g., background effects.");
    cl.AddCallback(Key("stack-entropy-labels"), CallbackStackEntropyLabels,
  ...
  if (ImageStack.size() > 1) {
    cmtk::StdErr << "WARNING: more than one image left on stack. Are you sure "
