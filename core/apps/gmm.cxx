...
int doMain(const int argc, const char *argv[]) {
  ...
  try {
    ...
    cl.SetProgramInfo(cmtk::CommandLine::PRG_DESCR,
                      "Segment an image into c classes using the EM algorithm "
                      "for Gaussian mixtures with optional priors.");

    typedef cmtk::CommandLine::Key Key;

    cl.BeginGroup("General", "General Classification Parameters");
    cl.AddOption(Key('m', "mask"), &maskImagePath,
                 "Path to foreground mask image. If this is not provided, the "
                 "input image is used as its own mask, but this does not work "
                 "properly if the input image itself has pixels with "
                 "zero or negative values.");
    cl.AddOption(Key('c', "classes"), &nClasses, "Number of classes.");
    cl.AddOption(Key('n', "iterations"), &nIterations,
                 "Number of EM iterations.");
    cl.EndGroup();

    cl.BeginGroup("Priors", "Handling of Priors");
    cl.AddSwitch(Key("priors-init-only"), &priorsInitOnly, true,
                 "Use priors for initialization only.");
    cl.AddOption(
        Key('e', "prior-epsilon"), &priorEpsilon,
        "Small value to add to all class priors to eliminate zero priors.");

    cl.BeginGroup("Output", "Output Parameters");
    cl.AddSwitch(Key('p', "probability-maps"), &writeProbMaps, true,
                 "Write probability maps. The file names for these maps will "
                 "be generated from the output image path by inserting "
                 "'_prob#' before the file format suffix, "
                 "where '#' is the index of the respective class, numbered "
                 "starting at 1 (zero is background).");
    cl.EndGroup();
    ...
    cl.AddParameterVector(&priorImagePaths, "PriorImages", "Prior image paths")
        ->SetProperties(cmtk::CommandLine::PROPS_IMAGE);
  ...
  if (!maskImagePath.empty()) {
    ...
    if (!inputImage->GridMatches(*(maskImage))) {
      cmtk::StdErr << "ERROR: mask image must have the same discrete grid as "
                      "the input image.\n";
      throw cmtk::ExitException(1);
    }
  }
  ...
  for (size_t k = 0; k < nClasses; ++k) {
    ...
    if (!inputImage->GridMatches(*(priorImages[k]))) {
      cmtk::StdErr << "ERROR: all prior images must have the same discrete "
...
#pragma omp parallel for
    for (int k = 0; k < nClasses; ++k) {
      ...
      for (size_t n = 0; n < nPixels; ++n) {
        if (maskImage->GetDataAt(n) > 0) {
          const double w = pMaps[k]->GetDataAt(n);
          classSigma[k] +=
              w * cmtk::MathUtil::Square(classMu[k] - inputImage->GetDataAt(n));
        }
      }
    ...
    for (size_t k = 0; k < nClasses; ++k) {
      cmtk::StdOut.printf("Class %u: %f +/- %f\t", k, classMu[k],
                          classSigma[k]);
    }
    cmtk::StdOut << "\n";
...
#pragma omp parallel for
    for (int n = 0; n < static_cast<int>(nPixels); ++n) {
      if (maskImage->GetDataAt(n) > 0) {
        ...
        for (size_t k = 0; k < nClasses; ++k) {
          double kernel = cmtk::GaussianKernel<double>::GetValue(
              inputImage->GetDataAt(n), classMu[k], classSigma[k]);
          if (!priorsInitOnly)
  ...
  if (writeProbMaps) {
    ...
    for (size_t k = 0; k < nClasses; ++k) {
      ...
      if (!period) period = path + strlen(path);

      snprintf(period, PATH_MAX - (period - path), "_prob%d%s",
               static_cast<int>(1 + k),
               outputImagePath.c_str() + (period - path));
  ...
  for (int n = 0; n < static_cast<int>(nPixels); ++n) {
    if (maskImage->GetDataAt(n) > 0) {
      ...
      for (size_t k = 1; k < nClasses; ++k) {
        ...
        if ((pClass > maxValue) ||
            ((pClass == maxValue) && (priorImages[k]->GetDataAt(n) >
                                      priorImages[maxLabel]->GetDataAt(n)))) {
          maxLabel = k;
          maxValue = pClass;
        }
      }
