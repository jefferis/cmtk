...
int doMain(const int argc, const char *argv[]) {
  ...
  try {
    ...
    cl.SetProgramInfo(cmtk::CommandLine::PRG_DESCR,
                      "This tool converts coordinate transformations from CMTK "
                      "format to ITK format and, in the process, also correct "
                      "for differences in image coordinate conventions");

    typedef cmtk::CommandLine::Key Key;
    cl.BeginGroup("Input", "Input parameters")
        ->SetProperties(cmtk::CommandLine::PROPS_NOXML);
    cl.AddOption(Key("fixed-space"), &fixedImageSpace,
                 "Change fixed image coordinate space (e.g., 'RAS', 'LPS', "
                 "...). This defaults to ITK standard space.");
    cl.AddOption(Key("moving-space"), &movingImageSpace,
                 "Change moving image coordinate space (e.g., 'RAS', 'LPS', "
                 "...). This defaults to ITK standard space.");
    cl.AddSwitch(Key("invert-input"), &invertInputXform, true,
    ...
    cl.AddOption(Key("fixed-image"), &fixedImagePath,
                 "Override transformation's fixed/reference image (if any) "
                 "with this one.");
    cl.AddOption(Key("moving-image"), &movingImagePath,
                 "Override transformation's moving/floating image (if any) "
                 "with this one.");
    cl.EndGroup();

    cl.AddParameter(&inputPath, "InputPath", "CMTK input transformation path")
        ->SetProperties(cmtk::CommandLine::PROPS_XFORM);
    cl.AddParameter(&outputPath, "OutputPath", "ITK output transformation path")
        ->SetProperties(cmtk::CommandLine::PROPS_XFORM |
  ...
  cmtk::AffineXform::SmartConstPtr xform =
      cmtk::AffineXform::SmartConstPtr::DynamicCastFrom(
  ...
  if (fixedImagePath.empty()) {
    if (!xform->MetaKeyExists(cmtk::META_XFORM_FIXED_IMAGE_PATH)) {
      cmtk::StdErr
          << "ERROR: could not deduce fixed image path from transformation; "
             "must be explicitly provided on the command line instead\n";
      throw cmtk::ExitException(1);
    }
  ...
  if (movingImagePath.empty()) {
    if (!xform->MetaKeyExists(cmtk::META_XFORM_MOVING_IMAGE_PATH)) {
      cmtk::StdErr
          << "ERROR: could not deduce moving image path from transformation; "
             "must be explicitly provided on the command line instead\n";
      throw cmtk::ExitException(1);
    }
  ...
  cmtk::UniformVolume::SmartPtr fixedImage = cmtk::VolumeIO::ReadGridOriented(
      cmtk::MountPoints::Translate(fixedImagePath));
  if (!fixedImage) {
  ...
  cmtk::UniformVolume::SmartPtr movingImage = cmtk::VolumeIO::ReadGridOriented(
      cmtk::MountPoints::Translate(movingImagePath));
  if (!movingImage) {
  ...
  cmtk::TransformChangeToSpaceAffine toNative(*(xform), *(fixedImage),
                                              *(movingImage));
