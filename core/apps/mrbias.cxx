...
int doMain(const int argc, const char *argv[]) {
  try {
    ...
    cl.SetProgramInfo(
        ...
        "This program corrects intensity inhomogeneity artifacts in MR images "
        "using a bias field estimated via entropy minimization.");
    ...
    cl.AddOption(Key('A', "degree-add"), &PolynomialDegreeAdd,
                 "Polynomial degree for additive correction.");
    cl.AddOption(Key('M', "degree-mul"), &PolynomialDegreeMul,
                 "Polynomial degree for multiplicative correction.");
    cl.AddSwitch(Key('I', "incremental"), &IncrementalPolynomials, true,
                 "Incrementally increase polynomial degrees.");
    cl.EndGroup();

    cl.BeginGroup("Preprocessing", "Input Image Preprocessing");
    cl.AddOption(Key("set-padding-value"), &PaddingValue,
                 "Set padding value for input intensity image. Pixels with "
                 "this value will be ignored.",
                 &PaddingFlag);
    cl.AddOption(Key('m', "mask"), &FNameMaskImage,
                 "Binary mask image filename.")
        ->SetProperties(cmtk::CommandLine::PROPS_IMAGE |
                        cmtk::CommandLine::PROPS_LABELS);
    cl.AddOption(Key('t', "thresh-min"), &ThresholdForegroundMin,
                 "Minimum intensity threshold for image foreground.",
                 &ThresholdForegroundFlag);
    cl.AddOption(Key('T', "thresh-max"), &ThresholdForegroundMax,
                 "Minimum intensity threshold for image foreground.",
                 &ThresholdForegroundFlag);
    cl.AddSwitch(Key("thresh-auto"), &ThresholdAuto, true,
                 "Automatic minimum intensity threshold selection for image "
                 "foreground using an estimate of image noise level.");
    cl.AddOption(Key("thresh-otsu-nbins"), &ThresholdOtsuNBins,
                 "If this is a positive integer, use automatic minimum "
                 "intensity threshold selection for image foreground by Otsu "
                 "thresholding with given number of histogram bins.");
    cl.EndGroup();

    cl.BeginGroup("Entropy Estimation", "Entropy Estimation Settings")
        ->SetProperties(cmtk::CommandLine::PROPS_ADVANCED);
    cl.AddSwitch(Key('L', "log-intensities"), &LogIntensities, true,
    ...
    cl.AddOption(Key('s', "sampling-density"), &SamplingDensity,
                 ...
                 &UseSamplingDensity);
    cl.AddOption(Key('n', "num-bins"), &NumberOfHistogramBins,
    ...
    cl.BeginGroup("Optimization", "Optimization Algorithm Settings")
        ->SetProperties(cmtk::CommandLine::PROPS_ADVANCED);
    ;
    cl.AddOption(Key("step-max"), &StepMax,
                 "Maximum (initial) search step size.");
    cl.AddOption(Key("step-min"), &StepMin,
                 "Minimum (final) search step size.");
    cl.EndGroup();

    cl.BeginGroup("I/O", "Input and Output Options")
        ->SetProperties(cmtk::CommandLine::PROPS_ADVANCED);
    ;
    cl.AddOption(Key("import-bias-add"), &ImportBiasFieldAdd,
                 "Import additive bias field (disables optimization).")
        ->SetProperties(cmtk::CommandLine::PROPS_IMAGE);
    cl.AddOption(Key("import-bias-mul"), &ImportBiasFieldMul,
                 "Import multiplicative bias field (disables optimization).")
        ->SetProperties(cmtk::CommandLine::PROPS_IMAGE);
    cl.AddOption(Key("write-bias-add"), &FNameBiasFieldAdd,
                 "File name for output of additive bias field.")
        ->SetProperties(cmtk::CommandLine::PROPS_IMAGE |
                        cmtk::CommandLine::PROPS_OUTPUT);
    cl.AddOption(Key("write-bias-mul"), &FNameBiasFieldMul,
                 "File name for output of multiplicative bias field.")
        ->SetProperties(cmtk::CommandLine::PROPS_IMAGE |
                        cmtk::CommandLine::PROPS_OUTPUT);
    cl.AddSwitch(Key('F', "write-float"), &OutputFloatImage, true,
                 "Write output image with floating point pixel data. If this "
                 "is not given, the input data type is used.");
    cl.EndGroup();
  ...
  cmtk::UniformVolume::SmartPtr inputImage(
      cmtk::VolumeIO::ReadOriented(FNameInputImage));
  ...
  if (!FNameMaskImage.empty()) {
    maskImage = cmtk::UniformVolume::SmartPtr(
        cmtk::VolumeIO::ReadOriented(FNameMaskImage));
    ...
    if (ThresholdAuto) {
      ThresholdForegroundMin = static_cast<float>(
          cmtk::TypedArrayNoiseEstimatorNaiveGaussian(*(inputImage->GetData()))
              .GetNoiseThreshold());
      ThresholdForegroundFlag = true;

      cmtk::DebugOutput(1)
          << "INFO: estimated foreground threshold from noise level as "
          << ThresholdForegroundMin << "\n";
    }

    if (ThresholdOtsuNBins > 0) {
      ThresholdForegroundMin = static_cast<float>(
          cmtk::HistogramOtsuThreshold<cmtk::Histogram<unsigned int>>(
              *(inputImage->GetData()->GetHistogram(ThresholdOtsuNBins)))
              .Get());
      ThresholdForegroundFlag = true;

      cmtk::DebugOutput(1) << "INFO: Otsu thresholding at "
                           << ThresholdForegroundMin << "\n";
    }

    if (ThresholdForegroundFlag) {
      maskImage =
          cmtk::UniformVolume::SmartPtr(inputImage->Clone(true /*copyData*/));
      maskImage->GetData()->SetPaddingValue(0.0);
      maskImage->GetData()->ThresholdToPadding(cmtk::Types::DataItemRange(
          ThresholdForegroundMin, ThresholdForegroundMax));
    }
  }

  typedef cmtk::EntropyMinimizationIntensityCorrectionFunctionalBase::SmartPtr
      FunctionalPointer;
  ...
  if (!(ImportBiasFieldAdd.empty() && ImportBiasFieldMul.empty())) {
    functional =
        cmtk::CreateEntropyMinimizationIntensityCorrectionFunctional(1, 1);

    if (SamplingDensity > 0) functional->SetSamplingDensity(SamplingDensity);
    ...
    if (!ImportBiasFieldAdd.empty()) {
      cmtk::UniformVolume::SmartPtr biasAdd(
          cmtk::VolumeIO::ReadOriented(ImportBiasFieldAdd));
      if (!biasAdd || !biasAdd->GetData()) {
        cmtk::StdErr << "ERROR: Could not read additive bias field image "
                     << ImportBiasFieldAdd << "\n";
        throw cmtk::ExitException(1);
      }
    ...
    if (!ImportBiasFieldMul.empty()) {
      cmtk::UniformVolume::SmartPtr biasMul(
          cmtk::VolumeIO::ReadOriented(ImportBiasFieldMul));
      if (!biasMul || !biasMul->GetData()) {
        cmtk::StdErr << "ERROR: Could not read multiplicative bias field image "
                     << ImportBiasFieldMul << "\n";
        throw cmtk::ExitException(1);
      }
    ...
    size_t polynomialDegreeFrom =
        std::max(PolynomialDegreeAdd, PolynomialDegreeMul);
    const size_t polynomialDegreeTo = polynomialDegreeFrom + 1;
    if (IncrementalPolynomials) polynomialDegreeFrom = 1;

    for (size_t polynomialDegree = polynomialDegreeFrom;
         polynomialDegree < polynomialDegreeTo; ++polynomialDegree) {
      const size_t degreeAdd =
          std::min<size_t>(polynomialDegree, PolynomialDegreeAdd);
      const size_t degreeMul =
          std::min<size_t>(polynomialDegree, PolynomialDegreeMul);

      functional = cmtk::CreateEntropyMinimizationIntensityCorrectionFunctional(
          degreeAdd, degreeMul, functional);
      ...
      cmtk::DebugOutput(1).GetStream().printf(
          "Estimating bias field with order %u multiplicative / %u additive "
      ...
      if ((PolynomialDegreeAdd > 0) || (PolynomialDegreeMul > 0)) {
        try {
          ...
          optimizer =
              cmtk::Optimizer::SmartPtr(new cmtk::BestDirectionOptimizer);
  ...
  if (!FNameOutputImage.empty()) {
    if (!OutputFloatImage) {
      ...
      cmtk::TypedArray::SmartPtr convertedData(
          outputImage->GetData()->Convert(inputImage->GetData()->GetType()));
  ...
  if (PolynomialDegreeAdd && !FNameBiasFieldAdd.empty()) {
    cmtk::UniformVolume::SmartPtr biasField(
        functional->GetBiasFieldAdd(true /*completeImage*/));
  ...
  if (PolynomialDegreeMul && !FNameBiasFieldMul.empty()) {
    cmtk::UniformVolume::SmartPtr biasField(
        functional->GetBiasFieldMul(true /*completeImage*/));
