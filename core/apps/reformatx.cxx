...
void CallbackTargetVolume(const char *arg) {
  ...
  if ((numArgs != 6) && (numArgs != 9)) {
    cmtk::StdErr << "ERROR: target volume definition must be "
                    "int,int,int:float,float,float or "
                    "int,int,int:float,float,float:float,float,float\n";
    throw cmtk::ExitException(1);
  }

  UserDefinedTargetVolume =
      cmtk::UniformVolume::SmartPtr(new cmtk::UniformVolume(
          cmtk::UniformVolume::IndexType::FromPointer(gridDims), gridDelta[0],
          gridDelta[1], gridDelta[2]));
  UserDefinedTargetVolume->SetMetaInfo(
      cmtk::META_SPACE, cmtk::AnatomicalOrientation::ORIENTATION_STANDARD);
  UserDefinedTargetVolume->SetMetaInfo(
      cmtk::META_SPACE_ORIGINAL,
      cmtk::AnatomicalOrientation::ORIENTATION_STANDARD);
  UserDefinedTargetVolume->SetMetaInfo(
      cmtk::META_IMAGE_ORIENTATION,
      cmtk::AnatomicalOrientation::ORIENTATION_STANDARD);
  UserDefinedTargetVolume->SetMetaInfo(
      cmtk::META_IMAGE_ORIENTATION_ORIGINAL,
      cmtk::AnatomicalOrientation::ORIENTATION_STANDARD);

  if (numArgs == 9) {
    UserDefinedTargetVolume->SetOffset(
        cmtk::UniformVolume::CoordinateVectorType::FromPointer(gridOffset));
  }
}
...
void CallbackCropImages(const char *arg) {
  ...
  if (CropImages) {
    CropImagesRegion = cmtk::DataGrid::RegionType(
...
void CallbackTargetImageOffset(const char *arg) {
  float x, y, z;
...
void CallbackTargetImageOffsetPixels(const char *arg) {
  float x, y, z;
  if (3 != sscanf(arg, "%15f,%15f,%15f", &x, &y, &z)) {
    x = y = z = 0;
  }
...
void InitializeReformatVolume(cmtk::TypedArray::SmartPtr &reformatData,
                              cmtk::UniformVolume::SmartPtr &targetVolume,
                              cmtk::UniformVolume::SmartPtr &floatingVolume) {
  switch (Mode) {
    default:
    case cmtk::ReformatVolume::REFORMAT_PLAIN: {
      if (!floatingVolume) {
        cmtk::StdErr << "ERROR: floating volume must be defined in plain "
                        "reformat mode; use '--floating' command line option\n";
        throw cmtk::ExitException(1);
      }

      cmtk::ReformatVolume::Plain plain(DataType);
      typename TInterpolator::SmartPtr interpolator(
          new TInterpolator(*floatingVolume));
      if (OutPaddingValueFlag) plain.SetPaddingValue(OutPaddingValue);
      reformatData =
          cmtk::TypedArray::SmartPtr(cmtk::ReformatVolume::ReformatMasked(
              targetVolume, TargetToReference, ReferenceToFloating, plain,
              floatingVolume, interpolator));

      if (MassPreservingReformat) {
        cmtk::ReformatVolume::Jacobian jacobian(cmtk::TYPE_DOUBLE,
                                                false /*correctGlobalScale*/);
        cmtk::XformList emptyXformList;
        cmtk::TypedArray::SmartPtr jacobianData(
            cmtk::ReformatVolume::ReformatMasked(
                targetVolume, emptyXformList, TargetToReference, jacobian, NULL,
                TInterpolator::SmartConstPtr::Null));
    ...
    case cmtk::ReformatVolume::REFORMAT_JACOBIAN: {
      ...
      if (OutPaddingValueFlag) jacobian.SetPaddingValue(OutPaddingValue);
      reformatData =
          cmtk::TypedArray::SmartPtr(cmtk::ReformatVolume::ReformatMasked(
              targetVolume, TargetToReference, ReferenceToFloating, jacobian,
              NULL, TInterpolator::SmartConstPtr::Null));
      break;
    }
  }
}

void ReformatPullback() {
  cmtk::UniformVolume::SmartPtr targetVolume = UserDefinedTargetVolume;
  if (TargetVolumeName) {
    if (TargetMask)
      targetVolume = cmtk::UniformVolume::SmartPtr(
          cmtk::VolumeIO::ReadOriented(TargetVolumeName));
    else
      targetVolume = cmtk::UniformVolume::SmartPtr(
          cmtk::VolumeIO::ReadGridOriented(TargetVolumeName));
  ...
  if (CropImages) {
    ...
    targetVolume =
        cmtk::UniformVolume::SmartPtr(targetVolume->GetCroppedVolume());
  }
  ...
  if (TargetImageOffsetPixels) {
    ...
    cmtk::DebugOutput(1) << "INFO: setting reference image offset to "
                         << TargetImageOffset[0] << "/" << TargetImageOffset[1]
                         << "/" << TargetImageOffset[2] << "\n";
  }
  ...
  if (FloatingVolumeName) {
    floatingVolume = cmtk::UniformVolume::SmartPtr(
        cmtk::VolumeIO::ReadOriented(FloatingVolumeName));
    if (floatingVolume) {
      floatingData = floatingVolume->GetData();
      if (!floatingData) {
        cmtk::StdErr << "ERROR: floating volume " << FloatingVolumeName
                     << " seems to have no data\n";
        throw cmtk::ExitException(1);
      } else {
        if (FltPaddingValueFlag) floatingData->SetPaddingValue(FltPaddingValue);
      }
    } else {
      cmtk::StdErr << "ERROR: floating volume " << FloatingVolumeName
                   << " could not be read\n";
  ...
  switch (Interpolation) {
    ...
    case cmtk::Interpolators::LINEAR: {
      typedef cmtk::UniformVolumeInterpolator<cmtk::Interpolators::Linear>
          TInterpolator;
      InitializeReformatVolume<TInterpolator>(reformatData, targetVolume,
                                              floatingVolume);
      break;
    }
    ...
    case cmtk::Interpolators::NEAREST_NEIGHBOR: {
      typedef cmtk::UniformVolumeInterpolator<
          cmtk::Interpolators::NearestNeighbor>
          TInterpolator;
      InitializeReformatVolume<TInterpolator>(reformatData, targetVolume,
                                              floatingVolume);
      break;
    ...
    case cmtk::Interpolators::COSINE_SINC: {
      switch (InterpolatorWindowRadius) {
        case 2: {
          typedef cmtk::UniformVolumeInterpolator<
              cmtk::Interpolators::CosineSinc<2>>
              TInterpolator;
          InitializeReformatVolume<TInterpolator>(reformatData, targetVolume,
                                                  floatingVolume);
          break;
        ...
        case 4: {
          typedef cmtk::UniformVolumeInterpolator<
              cmtk::Interpolators::CosineSinc<4>>
              TInterpolator;
          InitializeReformatVolume<TInterpolator>(reformatData, targetVolume,
                                                  floatingVolume);
          break;
        ...
        case 5: {
          typedef cmtk::UniformVolumeInterpolator<
              cmtk::Interpolators::CosineSinc<5>>
              TInterpolator;
          InitializeReformatVolume<TInterpolator>(reformatData, targetVolume,
                                                  floatingVolume);
          break;
        ...
        default:
          cmtk::StdErr.printf(
              "ERROR: Sinc window radius %d is not supported.\n",
    ...
    case cmtk::Interpolators::HAMMING_SINC: {
      switch (InterpolatorWindowRadius) {
        case 2: {
          typedef cmtk::UniformVolumeInterpolator<
              cmtk::Interpolators::HammingSinc<2>>
              TInterpolator;
          InitializeReformatVolume<TInterpolator>(reformatData, targetVolume,
                                                  floatingVolume);
          break;
        ...
        case 4: {
          typedef cmtk::UniformVolumeInterpolator<
              cmtk::Interpolators::HammingSinc<4>>
              TInterpolator;
          InitializeReformatVolume<TInterpolator>(reformatData, targetVolume,
                                                  floatingVolume);
          break;
        ...
        case 5: {
          typedef cmtk::UniformVolumeInterpolator<
              cmtk::Interpolators::HammingSinc<5>>
              TInterpolator;
          InitializeReformatVolume<TInterpolator>(reformatData, targetVolume,
                                                  floatingVolume);
          break;
    ...
    if (updateDB) {
      if (TargetVolumeName) {
        try {
          ...
          cmtk::StdErr << "ERROR: updating SQLite database failed - "
                       << ex.what() << "\n";
        }
      }
    }
...
int doMain(const int argc, const char *argv[]) {
  try {
    ...
    cl.SetProgramInfo(cmtk::CommandLine::PRG_DESCR,
                      "Extended volume reformatter tool to compute reformatted "
                      "images and Jacobian maps from arbitrary sequences of "
    ...
    cl.SetProgramInfo(
        ...
        "(If the first transformation in the sequence is inverted, then "
        "'--inverse' must be preceded by '--', i.e., use '-- --inverse "
        "xform.path').");

    typedef cmtk::CommandLine::Key Key;
    cl.BeginGroup("PlainOptions", "Options for Plain Reformatting");
    cmtk::CommandLine::EnumGroup<
        cmtk::Interpolators::InterpolationEnum>::SmartPtr interpolationGroup =
        cl.AddEnum("interpolation", &Interpolation,
                   "Image interpolation method.");
    interpolationGroup->AddSwitch(Key("linear"), cmtk::Interpolators::LINEAR,
                                  "Trilinear interpolation");
    interpolationGroup->AddSwitch(Key("nn"),
                                  cmtk::Interpolators::NEAREST_NEIGHBOR,
                                  "Nearest neighbor interpolation");
    interpolationGroup->AddSwitch(Key("cubic"), cmtk::Interpolators::CUBIC,
                                  "Tricubic interpolation");
    interpolationGroup->AddSwitch(Key("pv"), cmtk::Interpolators::PARTIALVOLUME,
                                  "Partial volume interpolation");
    interpolationGroup->AddSwitch(Key("sinc-cosine"),
                                  cmtk::Interpolators::COSINE_SINC,
                                  "Sinc interpolation with cosine window");
    interpolationGroup->AddSwitch(Key("sinc-hamming"),
                                  cmtk::Interpolators::HAMMING_SINC,
                                  "Sinc interpolation with Hamming window");

    cl.AddOption(Key("sinc-window-radius"), &InterpolatorWindowRadius,
                 "Window radius for Sinc interpolation");

    cl.AddSwitch(
        Key("preserve-mass"), &MassPreservingReformat, true,
        "Mass-preserving reformatting: multiply every reformatted value with "
        "the Jacobian determinant of the applied transformation.");
    cl.EndGroup();

    cl.BeginGroup("JacobianOptions", "Options for Jacobian Map Reformatting");
    cl.AddSwitch(Key("jacobian-correct-global"), &JacobianCorrectGlobal, true,
                 "Correct Jacobian maps for global scale.");
    cl.AddSwitch(Key("no-jacobian-correct-global"), &JacobianCorrectGlobal,
                 false, "Do not correct Jacobian maps for global scale.");
    cl.EndGroup();

    cl.BeginGroup("Input", "Input Options");
    cl.AddCallback(Key("target-grid"), CallbackTargetVolume,
                   "Define target grid for reformating as "
                   "Nx,Ny,Nz:dX,dY,dZ[:Ox,Oy,Oz] (dims:pixel:offset)");
    cl.AddSwitch(Key('m', "mask"), &TargetMask, true,
                 "Use target pixel data as binary mask.");
    cl.AddCallback(Key("crop"), CallbackCropImages,
                   "Crop target image: x0,y0,z0,x1,y1,z2");
    cl.AddCallback(Key('O', "target-offset"), CallbackTargetImageOffset,
                   "Override target image offset and set to x,y,z mm");
    cl.AddCallback(Key("target-offset-pixels"), CallbackTargetImageOffsetPixels,
                   "Override target image offset and set to dx,dy,dz pixels");
    cl.AddOption(Key('F', "floating"), &FloatingVolumeName,
                 "Format and path of floating image.");
    cl.AddOption(Key("pad-floating"), &FltPaddingValue,
                 "Padding value for the floating image", &FltPaddingValueFlag);

    cl.BeginGroup("Output", "Output Options");
    cl.AddOption(Key('o', "outfile"), &OutputImageName,
                 "Format and path of output image.");
    cl.EndGroup();

    cmtk::CommandLine::EnumGroup<cmtk::ScalarDataType>::SmartPtr typeGroup =
        cl.AddEnum("outputtype", &DataType,
                   "Scalar data type for the output image.");
    ...
    cl.AddOption(Key('P', "pad-out"), &OutPaddingValue,
                 "Padding value for output image", &OutPaddingValueFlag);
    cl.EndGroup();
    ...
    if (!UserDefinedTargetVolume) TargetVolumeName = cl.GetNext();

    const char *next = cl.GetNextOptional();
    while (next) {
      if (!strcmp(next, "-j") || !strcmp(next, "--jacobian")) break;
      ...
      switch (Mode) {
        case cmtk::ReformatVolume::REFORMAT_JACOBIAN:
          while (next) {
            ...
            cmtk::Xform::SmartPtr xform(cmtk::XformIO::Read(next));
            if (!xform) {
              cmtk::StdErr << "ERROR: could not read target-to-floating "
                              "transformation from "
                           << next << "\n";
              throw cmtk::ExitException(1);
            }
