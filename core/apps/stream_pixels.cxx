...
int doMain(int argc, const char *argv[]) {
  ...
  try {
    ...
    cl.SetProgramInfo(
        ...
        "standard output in binary form. Optionally, each image can be "
        "reoriented to a specified anatomical orientation "
        "and/or converted to a different data type. This is useful for piping "
        "image data through a pipeline, e.g., the Camino DTI toolkit.");

    typedef cmtk::CommandLine::Key Key;
    cl.AddOption(Key("reorient"), &readOrientation,
                 "Reorient all images to anatomy-based standard orientation, "
                 "e.g., RAS or LPS for axial. Default: no reorientation.");
    cl.AddSwitch(Key("change-endian"), &changeEndian, true,
                 "Change endianness. Default: native endianness.");

    cmtk::CommandLine::EnumGroup<cmtk::ScalarDataType>::SmartPtr typeGroup =
        cl.AddEnum("convert", &convertToType,
                   "Convert data to new scalar type. Default: no conversion.");
    typeGroup->AddSwitch(Key("char"), cmtk::TYPE_CHAR,
                         "8 bits, signed integer");
    typeGroup->AddSwitch(Key("byte"), cmtk::TYPE_BYTE,
                         "8 bits, unsigned integer");
    typeGroup->AddSwitch(Key("short"), cmtk::TYPE_SHORT,
                         "16 bits, signed integer");
    typeGroup->AddSwitch(Key("ushort"), cmtk::TYPE_USHORT,
                         "16 bits, unsigned integer");
    typeGroup->AddSwitch(Key("int"), cmtk::TYPE_INT, "32 bits, signed integer");
    typeGroup->AddSwitch(Key("uint"), cmtk::TYPE_UINT,
    ...
    cl.AddParameterVector(&imagePaths, "ImagePaths",
                          "List of paths to one or more image files. These "
                          "will be read and processed in the given order.");
  ...
  for (size_t idx = 0; idx < imagePaths.size(); ++idx) {
    ...
    if (!dataArray) {
      cmtk::StdErr << "ERROR: image " << imagePaths[idx]
                   << " does not have any valid pixel data\n";
      throw cmtk::ExitException(1);
    }
    ...
    std::cout.write(reinterpret_cast<const char *>(dataArray->GetDataPtr()),
                    dataArray->GetDataSizeBytes());
  }
