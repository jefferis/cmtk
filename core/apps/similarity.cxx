...
bool ParseCommandLine(const int argc, const char *argv[]) {
  try {
    ...
    cl.SetProgramInfo(cmtk::CommandLine::PRG_DESCR,
                      "Compute similarity measures such as intensity "
                      "difference or label overlaps between two images.");

    typedef cmtk::CommandLine::Key Key;
    cl.AddSwitch(Key("outside-bg"), &OutsideBG, true,
                 "Assume voxels outside floating image to be background");

    cl.AddOption(Key("pad0"), &PaddingValue0,
                 "Define padding value for reference image", &Padding0);
    cl.AddOption(Key("pad1"), &PaddingValue1,
                 "Define padding value for floating image", &Padding1);

    cl.AddSwitch(Key('g', "grey"), &LabelMode, false,
                 "Image pixels are intensities");
    cl.AddSwitch(Key('l', "labels"), &LabelMode, true,
                 "Image pixels are labels");

    cl.AddOption(Key('m', "mask"), &MaskFileName,
                 "Mask file for optional region-based analysis");

    cl.AddOption(Key("force-max"), &ForceMaxLabel, "Force maximum label value");

    cl.AddOption(Key("histogram-text-file"), &HistogramTextFileName,
                 "Output file name for histogram (plain text format).");

    cl.AddParameter(&imagePath0, "Image0",
                    "First input image path. When applicable, this provides "
                    "the 'ground truth' image.")
        ->SetProperties(cmtk::CommandLine::PROPS_IMAGE);
    cl.AddParameter(&imagePath1, "Image1",
                    "Second input image path. When applicable, this provides "
                    "the 'test' image.")
        ->SetProperties(cmtk::CommandLine::PROPS_IMAGE);
...
cmtk::JointHistogram<int> *AnalyseStudies(
    unsigned int &voxelCount, double &sumAbs, double &sumSq, double &sumMult,
    unsigned int &countVoxelsUnequal, double &maxDifference,
    double &maxRelDifference, cmtk::VoxelMatchingCrossCorrelation &ccMetric,
  ...
  if (LabelMode) {
    ...
    histogram =
        new cmtk::JointHistogram<int>(1 + static_cast<size_t>(range0.Width()),
                                      1 + static_cast<size_t>(range1.Width()));
  ...
  unsigned int numberOfVoxels =
      std::min(data0->GetDataSize(), data1->GetDataSize());
  ...
  for (size_t r = 0; r < numberOfVoxels; ++r) {
    ...
    if (dataExist0 && dataExist1) {
      ...
      if (insideMask) {
        ...
        maxRelDifference = std::max(maxRelDifference,
                                    d / std::max(fabs(value0), fabs(value1)));
        sumSq += d * d;
      ...
      if (insideMask) {
        ++voxelCountM;
        histogram->Increment(histogram->ValueToBinX(value0),
                             histogram->ValueToBinY(value1));
      ...
      if (dataExist0 || dataExist1) countVoxelsUnequal++;
    }
  }
  ...
  if (numberOfVoxels && (numberOfVoxels != reject)) {
    fprintf(
        ...
        1.0 * recog / numberOfVoxels, 1.0 * subst / numberOfVoxels,
        1.0 * reject / numberOfVoxels, 1.0 * recog / (numberOfVoxels - reject));
  }

  if (voxelCountFG && (voxelCountFG != rejectFG)) {
    fprintf(stdout, "CL-fg\t%.4f\t%.4f\t%.4f\t%.4f\n",
            1.0 * recogFG / voxelCountFG, 1.0 * substFG / voxelCountFG,
            1.0 * rejectFG / voxelCountFG,
            1.0 * recogFG / (voxelCountFG - rejectFG));
  }

  if (mask) {
    if (voxelCountM && (voxelCountM != rejectM)) {
      fprintf(stdout, "CL-mask\t%.4f\t%.4f\t%.4f\t%.4f\n",
              1.0 * recogM / voxelCountM, 1.0 * substM / voxelCountM,
              1.0 * rejectM / voxelCountM,
              1.0 * recogM / (voxelCountM - rejectM));
    }
  }
  fputs("\n", stdout);
...
int doMain(const int argc, const char *argv[]) {
  ...
  cmtk::UniformVolume::SmartPtr volume(
      cmtk::VolumeIO::ReadOriented(imagePath0));
  if (!volume) {
  ...
  if (!MaskFileName.empty()) {
    ...
    if (maskVolume) mask = maskVolume->GetData();
  }

  volume =
      cmtk::UniformVolume::SmartPtr(cmtk::VolumeIO::ReadOriented(imagePath1));
  if (!volume) {
  ...
  cmtk::JointHistogram<int>::SmartPtr histogram(
      AnalyseStudies(voxelCount, sumAbs, sumSq, sumMult, countVoxelsUnequal,
                     maxDifference, maxRelDifference, ccMetric, mask));
  ...
  if (voxelCount) {
    fprintf(stdout,
            "SIM\tNDIFF\tDMAX\trDMAX\tMSD\tMAD\tNCC\tHXY\tMI\tNMI\nSIMval\t%"
            "u\t%.5g\t%.5g\t%.5g\t%.5g\t%.5g\t%.5g\t%.5g\t%.5g\n",
            countVoxelsUnequal, maxDifference, maxRelDifference,
            sumSq / voxelCount, sumAbs / voxelCount, ccMetric.Get(), hXY,
            hX + hY - hXY, (hX + hY) / hXY);
  }
  ...
  if (LabelMode) {
    ...
    for (unsigned int i = 0; i < numberLabelsRef; ++i) {
      ...
      if (totalRef) {
        const unsigned int correct =
            (i < numberLabelsFlt) ? histogram->GetBin(i, i) : 0;
        ...
        const unsigned int totalFlt =
            (i < numberLabelsFlt) ? histogram->ProjectToY(i) : 0;
        const double SI = static_cast<double>(
            2.0 * correct /
            (totalRef + totalFlt));  // Similarity Index / Dice score

        const unsigned int totalRefFlt = totalRef + totalFlt - correct;
        const double J =
            static_cast<double>(1.0 * correct / totalRefFlt);  // Jaccard index

        if (totalRef) {
          fprintf(stdout,
                  "\n%06u\t%u\t%u\t%.5lf\t%u\t%.5lf\t%u\t%.5lf\t%.5lf\t%.5lf",
                  i, totalRef, correct, 1.0 * correct / totalRef, falseNeg,
                  1.0 * falseNeg / totalRef, falsePos,
                  1.0 * falsePos / totalRef, SI, J);
        }

        if (i) {  // assume background is label #0 and exclude this
          ++countLabels;
          sumTotal += totalRef;
          sumCorrect += correct;
          sumSI += SI;
        ...
        if (i <= ForceMaxLabel) fprintf(stdout, "\nLabel #%u\t<n/a>", i);
      }
    }
    ...
    if (sumTotal && countLabels) {
      fprintf(stdout, "Total:\t%u\t%u\t%.2lf\t%.4lf\t%.4lf\n\n", sumTotal,
              sumCorrect, 100.0 * sumCorrect / sumTotal, sumSI / countLabels,
              sumSIweighted / sumTotal);
    }
