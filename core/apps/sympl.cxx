...
bool ParseCommandLine(const int argc, const char *argv[]) {
  try {
    ...
    cl.AddOption(
        Key('s', "sampling"), &Sampling,
        "Resampled image resolution. This is the resolution [in mm] of the "
        "first (finest) resampled image in the multi-scale pyramid, "
    ...
    cl.AddOption(
        ...
        "in successive order of increasing resolution before using the "
        "original images at the final level.");
    cl.EndGroup();

    cl.BeginGroup("Initial", "Initial approximate symmetry plane orientation");
    cmtk::CommandLine::EnumGroup<InitialPlaneEnum>::SmartPtr initialPlane =
        cl.AddEnum(
            "initial-plane", &InitialPlane,
    ...
    cl.BeginGroup("Pre-computed", "Pre-computed symmetry")
        ->SetProperties(cmtk::CommandLine::PROPS_ADVANCED |
                        cmtk::CommandLine::PROPS_NOXML);
    cl.AddOption(
        Key("output-only"), &SymmetryParameters,
        "Give symmetry parameters [Rho Theta Phi] as option, skip search.",
        &OutputOnly);
    cl.AddOption(Key("output-only-file"), &SymmetryParametersFile,
    ...
    cl.BeginGroup("Preprocessing", "Data pre-processing")
        ->SetProperties(cmtk::CommandLine::PROPS_ADVANCED);
    cl.AddOption(Key("min-value"), &MinValue, "Force minumum data value.",
                 &MinValueSet);
    cl.AddOption(Key("max-value"), &MaxValue, "Force maximum data value.",
                 &MaxValueSet);
    cl.EndGroup();

    cl.BeginGroup("OutputImages", "Output of Images");
    cmtk::CommandLine::EnumGroup<
        cmtk::Interpolators::InterpolationEnum>::SmartPtr interpGroup =
        cl.AddEnum("interpolation", &Interpolation,
                   "Interpolation method used for reformatted output data");
    interpGroup->AddSwitch(Key('L', "linear"), cmtk::Interpolators::LINEAR,
                           "Use linear image interpolation for output.");
    interpGroup->AddSwitch(Key('C', "cubic"), cmtk::Interpolators::CUBIC,
                           "Use cubic image interpolation for output.");
    interpGroup->AddSwitch(
        Key('S', "sinc"), cmtk::Interpolators::COSINE_SINC,
        "Use cosine-windowed sinc image interpolation for output.");

    cl.AddOption(Key('P', "pad-out"), &PadOutValue,
                 "Padding value for output images.", &PadOutValueSet)
        ->SetProperties(cmtk::CommandLine::PROPS_ADVANCED);
    cl.AddOption(Key("mark-value"), &MarkPlaneValue,
                 "Data value to mark (draw) symmetry plane.");
    cl.AddOption(Key("write-marked"), &MarkedOutFile,
    ...
    cl.AddOption(Key("write-aligned"), &AlignedOutFile,
                 "File name for symmetry plane-aligned output image.")
        ->SetProperties(cmtk::CommandLine::PROPS_IMAGE |
                        cmtk::CommandLine::PROPS_OUTPUT);
    cl.AddSwitch(Key("mark-aligned"), &MarkPlaneAligned, true,
                 "Mark symmetry plane in aligned output image.");
    cl.AddOption(Key("write-subtract"), &DifferenceOutFile,
                 "File name for mirror subtraction image.")
        ->SetProperties(cmtk::CommandLine::PROPS_IMAGE |
                        cmtk::CommandLine::PROPS_OUTPUT);
    cl.AddOption(Key("write-mirror"), &MirrorOutFile,
                 "File name for image mirrored w.r.t. symmetry plane.")
        ->SetProperties(cmtk::CommandLine::PROPS_IMAGE |
                        cmtk::CommandLine::PROPS_OUTPUT);
    cl.EndGroup();

    cl.BeginGroup("OutputParameters", "Output of Parameters")
        ->SetProperties(cmtk::CommandLine::PROPS_ADVANCED);
    cl.AddOption(Key('o', "outfile"), &SymmetryOutFileName,
                 "File name for symmetry plane parameter output.")
        ->SetProperties(cmtk::CommandLine::PROPS_FILENAME |
                        cmtk::CommandLine::PROPS_OUTPUT);
    cl.AddOption(Key("write-xform"), &WriteXformPath,
    ...
    cl.AddParameter(&InFileName, "InputImage", "Input image path")
        ->SetProperties(cmtk::CommandLine::PROPS_IMAGE);
    ...
    if (!SymmetryParameters.empty()) {
      ...
      if (3 == sscanf(SymmetryParameters.c_str(), "%20lf %20lf %20lf", &rho,
                      &theta, &phi)) {
        Rho = rho;
    ...
    if (!SymmetryParametersFile.empty()) {
      ...
      if (inStream.IsValid()) {
        ...
        cmtk::StdErr.printf(
            "ERROR: Could not open symmetry parameter file %s\n",
...
void WriteDifference(const cmtk::UniformVolume *originalVolume,
                     const cmtk::UniformVolumeInterpolatorBase *interpolator,
                     const cmtk::ParametricPlane &parametricPlane,
  ...
  cmtk::TypedArray::SmartPtr diffData = cmtk::TypedArray::SmartPtr(
      cmtk::TypedArray::Create(GetSignedDataType(originalData->GetType()),
                               originalData->GetDataSize()));
  ...
  for (int z = 0; z < originalVolume->GetDims()[2]; ++z)
    for (int y = 0; y < originalVolume->GetDims()[1]; ++y)
      for (int x = 0; x < originalVolume->GetDims()[0]; ++x, ++offset) {
        ...
        cmtk::UniformVolume::CoordinateVectorType w =
...
void WriteMirror(const cmtk::UniformVolume *originalVolume,
                 const cmtk::UniformVolumeInterpolatorBase *interpolator,
                 const cmtk::ParametricPlane &parametricPlane,
                 const std::string &outFileName) {
  cmtk::TypedArray::SmartPtr mirrorData =
      cmtk::TypedArray::Create(originalVolume->GetData()->GetType(),
  ...
  for (int z = 0; z < originalVolume->GetDims()[2]; ++z) {
    for (int y = 0; y < originalVolume->GetDims()[1]; ++y)
      for (int x = 0; x < originalVolume->GetDims()[0]; ++x, ++offset) {
        cmtk::UniformVolume::CoordinateVectorType v =
            originalVolume->GetGridLocation(x, y, z);
...
void WriteMarkPlane(const cmtk::UniformVolume *originalVolume,
                    const cmtk::ParametricPlane &parametricPlane,
                    const cmtk::Types::DataItem markPlaneValue,
                    const std::string &outFileName) {
  ...
  for (int z = 0; z < originalVolume->GetDims()[2]; ++z) {
    for (int y = 0; y < originalVolume->GetDims()[1]; ++y) {
      ...
      for (int x = 0; x < originalVolume->GetDims()[0]; ++x, ++offset) {
        int newSideOfPlane = parametricPlane.GetWhichSide(
            originalVolume->GetGridLocation(x, y, z));
...
void WriteAligned(const cmtk::UniformVolume *originalVolume,
                  const cmtk::UniformVolumeInterpolatorBase *interpolator,
                  const cmtk::ParametricPlane &parametricPlane,
                  const InitialPlaneEnum initialPlane,
                  const std::string &outFileName) {
  const cmtk::TypedArray *originalData = originalVolume->GetData();

  cmtk::TypedArray::SmartPtr alignData = cmtk::TypedArray::Create(
      originalData->GetType(), originalData->GetDataSize());
  if (PadOutValueSet) {
  ...
  cmtk::AffineXform::SmartPtr alignment(
      parametricPlane.GetAlignmentXform(normalAxis));
  int offset = 0;
  for (int z = 0; z < originalVolume->GetDims()[2]; ++z) {
    for (int y = 0; y < originalVolume->GetDims()[1]; ++y) {
      for (int x = 0; x < originalVolume->GetDims()[0]; ++x, ++offset) {
        const cmtk::UniformVolume::CoordinateVectorType v =
...
int doMain(const int argc, const char *argv[]) {
  ...
  cmtk::UniformVolume::SmartPtr originalVolume(
      cmtk::VolumeIO::ReadOriented(InFileName));
  if (!originalVolume) {
  ...
  if (OutputOnly) {
    ...
    for (int level = 0; level < Levels; ++level) {
      ...
      if (level < Levels - 1) {
        cmtk::Types::Coordinate voxelSize =
            Sampling * pow(2.0, (Levels - level - 2));
        volume = cmtk::UniformVolume::SmartPtr(
            originalVolume->GetResampled(voxelSize));
      ...
      if (MinValueSet || MaxValueSet) {
        ...
        if (MinValueSet) valueRange.m_LowerBound = MinValue;
        if (MaxValueSet) valueRange.m_UpperBound = MaxValue;

        functional = cmtk::SmartPointer<cmtk::SymmetryPlaneFunctional>(
            new cmtk::SymmetryPlaneFunctional(volume, valueRange));
      } else {
        functional = cmtk::SmartPointer<cmtk::SymmetryPlaneFunctional>(
            new cmtk::SymmetryPlaneFunctional(volume));
      }

      optimizer.SetFunctional(
          cmtk::Functional::SmartPtr::DynamicCastFrom(functional));
      optimizer.Optimize(v, pow(2.0, Levels - level - 1),
                         Accuracy * pow(2.0, Levels - level - 1));
  ...
  if (!SymmetryOutFileName.empty()) {
    cmtk::ClassStreamOutput stream(SymmetryOutFileName,
                                   cmtk::ClassStreamOutput::MODE_WRITE);
    stream << parametricPlane;
    stream.Close();
  }

  const cmtk::UniformVolumeInterpolatorBase::SmartPtr interpolator(
      cmtk::ReformatVolume::CreateInterpolator(Interpolation, originalVolume));
  ;

  if (!AlignedOutFile.empty())
    WriteAligned(originalVolume, interpolator, parametricPlane, InitialPlane,
                 AlignedOutFile);

  if (!MarkedOutFile.empty())
    WriteMarkPlane(originalVolume, parametricPlane, MarkPlaneValue,
                   MarkedOutFile);

  if (!DifferenceOutFile.empty())
    WriteDifference(originalVolume, interpolator, parametricPlane,
                    DifferenceOutFile);
