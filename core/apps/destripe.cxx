...
int doMain(const int argc, const char *argv[]) {
  try {
    ...
    cl.SetProgramInfo(
        ...
        "This program corrects stripe artifacts in acquired image stacks which "
        "can result from between-slice intensity scale differences.");

    typedef cmtk::CommandLine::Key Key;

    cl.BeginGroup("SliceOrient", "Slice Orientation");
    cmtk::CommandLine::EnumGroup<int>::SmartPtr sliceGroup =
        cl.AddEnum("slice-axis", &SliceAxis,
                   "Define slice direction axis: this is the through-slice "
                   "direction of the acquisition.");
    sliceGroup->AddSwitch(Key("guess-from-input"), -1,
                          "Guess from input image");
    sliceGroup->AddSwitch(Key('a', "axial"), (int)cmtk::AXIS_Z,
                          "Sliced axial images");
    sliceGroup->AddSwitch(Key('s', "sagittal"), (int)cmtk::AXIS_X,
                          "Sliced sagittal images");
    sliceGroup->AddSwitch(Key('c', "coronal"), (int)cmtk::AXIS_Y,
                          "Sliced coronal images");
    sliceGroup->AddSwitch(Key('x', "slice-x"), (int)cmtk::AXIS_X,
                          "Sliced along x axis");
    sliceGroup->AddSwitch(Key('y', "slice-y"), (int)cmtk::AXIS_Y,
                          "Sliced along y axis");
    sliceGroup->AddSwitch(Key('z', "slice-z"), (int)cmtk::AXIS_Z,
                          "Sliced along z axis");

    cl.AddOption(Key("kernel-fwhm"), &KernelFWHM,
                 "Gaussian kernel full width at half maximum (FWHM) in units "
                 "of slice indexes.");
    cl.AddOption(Key("kernel-radius"), &TruncateKernel,
                 "Truncate kernel radius to this many samples. By default, "
                 "kernel width is determined to limit approximation error.");

    cl.AddSwitch(Key("write-float"), &WriteFloat, true,
                 "Write output image in floating point representation. By "
                 "default, write using input image pixel data type.");
  ...
  // guess slice orientation - if two dimensions are equal, the thir is usually
  // the slice direction
  if (SliceAxis == -1) {
    if (volumeDims[0] == volumeDims[1])
      ...
      cmtk::StdErr << "ERROR: cannot guess slice axis when all three image "
...
#pragma omp parallel for
  ...
  const std::vector<cmtk::Types::DataItem> kernel =
      cmtk::GaussianKernel<cmtk::Types::DataItem>::GetHalfKernel(
          cmtk::Units::GaussianFWHM(KernelFWHM));
  if (!TruncateKernel) TruncateKernel = kernel.size() - 1;
...
#pragma omp parallel for
  for (int slice = 0; slice < nSlices; ++slice) {
    ...
    const cmtk::Types::DataItem correction =
        smoothed[slice] / sliceProjection[slice];

    cmtk::Types::DataItem value;
    for (idx[idxX] = 0; idx[idxX] < volumeDims[idxX]; ++idx[idxX]) {
      for (idx[idxY] = 0; idx[idxY] < volumeDims[idxY]; ++idx[idxY]) {
        if (volume->GetDataAt(value, volume->GetOffsetFromIndex(idx))) {
          volume->SetDataAt(value * correction,
                            volume->GetOffsetFromIndex(idx));
        }
      }
    }
  }

  if (!OutputFilePath.empty()) cmtk::VolumeIO::Write(*volume, OutputFilePath);
