...
int doMain(const int argc, const char *argv[]) {
  ...
  try {
    ...
    cmtk::CommandLine::EnumGroup<int>::SmartPtr interleaveGroup =
        cl.AddEnum("stacking", &axis, "Define slice axis for stacking.");
    ...
    cl.AddOption(
        Key("spacing"), &forceSpacing,
        "If non-zero, force slice spacing in the output image to given value. "
        "This is required when stacking single-slice images.");
    cl.EndGroup();
  ...
  for (std::list<const char *>::const_iterator it = inputFilePaths.begin();
       it != inputFilePaths.end(); ++it) {
      ...
      for (int dim = 0; dim < 3; ++dim) {
        if (dim == axis) {
          if ((volume->m_Dims[dim] != volumes[0]->m_Dims[dim]) &&
              (volume->m_Dims[dim] + 1 != volumes[0]->m_Dims[dim])) {
            cmtk::StdErr
                << "ERROR: interleaving dimension of image " << *it
                << " must be same as, or one smaller than first image's\n";
            throw cmtk::ExitException(1);
          }
          stackDims[dim] += volume->m_Dims[dim];
        } else {
          if (volume->m_Dims[dim] != volumes[0]->m_Dims[dim]) {
            cmtk::StdErr << "ERROR: in-plane dimensions of image " << *it
                         << " do not match first image's\n";
            throw cmtk::ExitException(1);
          }
        }
      }
  ...
  cmtk::DebugOutput(1) << "Stacked image will have dimensions " << stackDims[0]
                       << "x" << stackDims[1] << "x" << stackDims[2] << "\n"
                       << "Stacked image will have pixel size " << stackDelta[0]
                       << "x" << stackDelta[1] << "x" << stackDelta[2] << "\n";

  cmtk::UniformVolume::SmartPtr stacked(new cmtk::UniformVolume(
      stackDims, stackDelta[0], stackDelta[1], stackDelta[2]));
  stacked->CreateDataArray(volumes[0]->GetData()->GetType());

  int toSlice = 0;
  for (int fromSlice = 0; fromSlice < volumes[0]->m_Dims[axis]; ++fromSlice) {
    for (size_t fromVolume = 0; fromVolume < volumes.size();
         ++fromVolume, ++toSlice) {
      cmtk::ScalarImage::SmartPtr slice(
          volumes[fromVolume]->GetOrthoSlice(axis, fromSlice));
      stacked->SetOrthoSlice(axis, toSlice, slice);
    }
  }

  // get origin and orientation from first input image
  cmtk::AffineXform::MatrixType xformMatrix =
      volumes[0]->GetImageToPhysicalMatrix();
  // and copy to output
  stacked->m_IndexToPhysicalMatrix *= xformMatrix;
  for (std::map<int, cmtk::AffineXform::MatrixType>::iterator it =
           stacked->m_AlternativeIndexToPhysicalMatrices.begin();
       it != stacked->m_AlternativeIndexToPhysicalMatrices.end(); ++it) {
