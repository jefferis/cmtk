...
void CallbackAddPercentile(const double arg) {
  percentiles.push_back(static_cast<cmtk::Types::DataItem>(arg));
}

void AnalyzeLabels(const cmtk::UniformVolume &volume,
                   const cmtk::TypedArray *maskData) {
  const cmtk::TypedArray *data = volume.GetData();
  cmtk::Types::DataItemRange range = data->GetRange();

  if (MaskOutputAllUpTo) range.m_UpperBound = MaskOutputAllUpTo;

  const unsigned int numberOfLabels =
      static_cast<unsigned int>(range.m_UpperBound - range.m_LowerBound + 1);
  ...
  std::vector<cmtk::UniformVolume::CoordinateVectorType> centerOfMass(
      numberOfLabels);
  std::fill(centerOfMass.begin(), centerOfMass.end(),
            cmtk::UniformVolume::CoordinateVectorType(0.0));
          ...
          for (int dz = -1; (dz < 2) && !isSurface; ++dz)
            for (int dy = -1; (dy < 2) && !isSurface; ++dy)
              for (int dx = -1; (dx < 2) && !isSurface; ++dx)
                if (dx || dy || dz)
                  if ((dx + x) >= 0 &&
                      (dx + x) < volume.GetDims()[cmtk::AXIS_X] &&
                      (dy + y) >= 0 &&
                      (dy + y) < volume.GetDims()[cmtk::AXIS_Y] &&
                      (dz + z) >= 0 &&
                      (dz + z) < volume.GetDims()[cmtk::AXIS_Z]) {
                    const int offset =
                        (x + dx) +
                        volume.GetDims()[cmtk::AXIS_X] *
                            ((y + dy) +
                             volume.GetDims()[cmtk::AXIS_Y] * (z + dz));
                    if (data->Get(neighbor, offset) && (neighbor != value))
                      isSurface = true;
                  }

          if (isSurface) ++countSurface[labelIdx];
        }
      }
    }
  }
  ...
  const cmtk::Types::Coordinate voxelVolume =
  ...
  for (unsigned int idx = 0; idx < numberOfLabels; ++idx) {
    if (count[idx] || MaskOutputAllUpTo) {
      if (count[idx]) centerOfMass[idx] *= (1.0 / count[idx]);

      if (OutputExpNotation)
        fprintf(stdout, "%03d\t%14d\t%14d\t%.4e\t(%e,%e,%e)\n",
                (int)(idx + range.m_LowerBound), count[idx], countSurface[idx],
                count[idx] * voxelVolume, centerOfMass[idx][0],
                centerOfMass[idx][1], centerOfMass[idx][2]);
      else
  ...
  if (totalCount) {
    for (unsigned int idx = 0; idx < numberOfLabels; ++idx) {
      if (count[idx]) {
        cmtk::Types::DataItem p =
            ((cmtk::Types::DataItem)count[idx]) / totalCount;
        entropy += p * log(p);
      }
    }
  }
...
void AnalyzeGrey(const cmtk::UniformVolume &volume,
                 const cmtk::TypedArray &maskData) {
  const cmtk::TypedArray *data = volume.GetData();
  ...
  if (MaskOutputAllUpTo) maxLabel = MaskOutputAllUpTo;
      ...
      if (!WriteAsColumn) {
        if (OutputExpNotation)
          fprintf(stdout, "%03d\t%.5f\t%.5e\t%.5e\t%.5e\t%d\t%.5e\t%f\n",
                  maskSelect, seq.GetMinimum(), seq.GetMaximum(),
                  seq.GetAverage(), sqrt(seq.GetVariance()), seq.GetNValues(),
                  histogram.GetEntropy(), seq.GetSum());
        else
...
void AnalyzeGrey(const cmtk::UniformVolume &volume) {
    ...
    if (WriteAsColumn) {
      if (OutputExpNotation)
        fprintf(stdout,
                ...
                sqrt(seq.GetVariance()), seq.GetNValues(),
                data->GetEntropy(true /*fractional*/, NumberOfHistogramBins),
                seq.GetSum());
      else
        fprintf(stdout,
                "min\t%.5f\nmax\t%.5f\nmean\t%.5f\nsdev\t%.5f\nn\t%"
  ...
  if (!percentiles.empty()) {
    ...
    for (std::list<cmtk::Types::DataItem>::const_iterator it =
             percentiles.begin();
         it != percentiles.end(); ++it) {
      fprintf(stdout, "%.5f\t%.5f\n", (cmtk::Types::DataItem)(*it),
              (cmtk::Types::DataItem)data->GetPercentile(
                  *it, NumberOfHistogramBins));
    }
  }
}
...
int doMain(const int argc, const char *argv[]) {
  try {
    ...
    cl.SetProgramInfo(cmtk::CommandLine::PRG_DESCR,
                      "Statistical computations on image pixel intensities, "
    ...
    cl.AddSwitch(Key('l', "label"), &Label, true,
                 "Interpret voxel values as labels");
    cl.AddSwitch(Key('g', "gray"), &Label, false,
                 "Interpret voxel values as gray values (default)");

    cl.AddSwitch(Key('L', "log"), &LogData, true,
                 "Apply log() function to data");
    cl.AddSwitch(Key('e', "exp"), &ExpData, true,
                 "Apply exp() function to data");

    cl.AddOption(Key('n', "num-bins"), &NumberOfHistogramBins,
                 "Number of histogram bins.");

    cl.AddSwitch(Key('C', "column"), &WriteAsColumn, true,
                 "Write statistics in column format rather than line format.");
    cl.AddSwitch(Key('E', "e-notation"), &OutputExpNotation, true,
                 "Write floating point numbers in #e# notation (i.e., 1e-3 "
                 "instead of 0.001)");

    cl.AddOption(Key('m', "mask"), &MaskFileName,
                 "Analyze region based on binary mask file", &MaskIsBinary);
    cl.AddOption(
        Key('M', "Mask"), &MaskFileName,
        "Analyze regions separately based on mask file (label field).");
    cl.AddOption(Key("mask-output-all-up-to"), &MaskOutputAllUpTo,
                 "Output results for all mask values up to given value, even "
                 "such that do not actually appear in the mask.");

    cl.AddCallback(Key('p', "percentile"), CallbackAddPercentile,
  ...
  if (MaskFileName) {
    maskVolume = cmtk::UniformVolume::SmartPtr(
    ...
    if (!maskData) {
      cmtk::StdErr << "ERROR: could not read data from mask file "
                   << MaskFileName << "\n";
      throw cmtk::ExitException(1);
    }
  ...
  for (; it != ImageFileNames.end(); ++it) {
    ...
    cmtk::UniformVolume::SmartPtr volume(
        cmtk::VolumeIO::ReadOriented(imageFileName));
    if (!volume) {
    ...
    if (!volumeData) {
      cmtk::StdErr << "ERROR: could not read data from image file "
                   << imageFileName << "\n";
      continue;
    }

    if (maskVolume && !volume->GridMatches(*maskVolume)) {
      cmtk::StdErr << "ERROR: mask grid does not match grid of image "
                   << imageFileName << "\n";
      continue;
    }
    ...
    if (Label) {
      ...
      if (LogData) volumeData->ApplyFunctionDouble(cmtk::Wrappers::Log);
      if (ExpData) volumeData->ApplyFunctionDouble(cmtk::Wrappers::Exp);
      if (maskData)
