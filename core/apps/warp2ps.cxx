...
void SetCropRegion(const char *arg) {
  if (4 == sscanf(arg, "%6d,%6d:%6d,%6d", &CropRegion[0], &CropRegion[1],
...
void DrawLine(const std::vector<double> &outputX,
              const std::vector<double> &outputY) {
  if (outputX.empty()) return;
...
int doMain(const int argc, const char *argv[]) {
  try {
    ...
    cl.SetProgramInfo(cmtk::CommandLine::PRG_DESCR,
                      "Write deformation field as deformed grid in PostScript "
                      "format for visualization and illustration");
    cl.SetProgramInfo(cmtk::CommandLine::PRG_SYNTX,
                      "warp2ps [options] referenceImage xform");

    typedef cmtk::CommandLine::Key Key;
    cl.AddSwitch(Key("invert-xform"), &InvertXform, true,
                 "Invert the main transformation");
    cl.AddOption(Key('m', "minus-xform"), &MinusXformPath,
                 "Subtract this transformation (e.g., to exclude initial "
                 "affine component)");
    cl.AddSwitch(Key("invert-minus-xform"), &InvertMinusXform, true,
                 "Invert the minus transformation");

    cl.AddSwitch(Key('Z', "axis-z"), &Axis, 2,
                 "Slice orthogonal to z-axis (axial, default)");
    cl.AddSwitch(Key('Y', "axis-y"), &Axis, 1,
                 "Slice orthogonal to y-axis (sagittal)");
    cl.AddSwitch(Key('X', "axis-x"), &Axis, 0,
                 "Slice orthogonal to x-axis (coronal)");
    cl.AddOption(Key("slice"), &SliceIndex,
                 "Index of z-slice (default: center )");

    cl.AddOption(Key("line-width"), &LineWidth, "Line width [default: 0]");
    cl.AddOption(Key("sampling"), &SamplingFactor,
                 "Sampling factor (default: 1)");
    cl.AddOption(Key("scale"), &ScaleFactor,
                 "Coordinage scale factor (default: 1.0)");
    cl.AddSwitch(Key('b', "box"), &DrawBox, true,
                 "Draw reference bounding box");
    cl.AddCallback(Key('C', "crop"), SetCropRegion,
  ...
  if (SliceIndex < 0) SliceIndex = volume->GetDims()[Axis] / 2;

  const cmtk::Types::Coordinate pixelSizeX =
      volume->m_Size[axisX] / (volume->GetDims()[axisX] - 1);
  const cmtk::Types::Coordinate pixelSizeY =
      volume->m_Size[axisY] / (volume->GetDims()[axisY] - 1);
  ...
  std::cout << "%%BoundingBox: " << xmin << " " << ymin << " " << xmax << " "
            << ymax << "\n";
  ...
  const int SamplingFactorInLine =
      (SamplingFactor > 1) ? SamplingFactor / 2 : SamplingFactor;

  int idx[3] = {0, 0, 0};
  idx[axisZ] = SliceIndex;

  // vertical lines
  for (idx[axisY] = CropRegion[1]; idx[axisY] < CropRegion[3];
       idx[axisY] += SamplingFactor) {
    std::cout << "n\n";
    ...
    outputX.resize(0);
    outputY.resize(0);
    for (idx[axisX] = CropRegion[0]; idx[axisX] < CropRegion[2];
         idx[axisX] += SamplingFactorInLine) {
      bool success = true;
  ...
  for (idx[axisX] = CropRegion[0]; idx[axisX] < CropRegion[2];
    ...
    outputY.resize(0);
    for (idx[axisY] = CropRegion[1]; idx[axisY] < CropRegion[3];
         idx[axisY] += SamplingFactorInLine) {
      bool success = true;
      v0 = v1 = volume->GetGridLocation(idx[0], idx[1], idx[2]);
      if (xform) {
        if (InvertXform)
  ...
  if (DrawBox) {
    std::cout << LineWidth << " setlinewidth\n";
    std::cout << xmin << " " << ymin << " m " << xmax << " " << ymin << " l "
              << xmax << " " << ymax << " l " << xmin << " " << ymax << " l "
              << xmin << " " << ymin << " l\ns\n";
  }
