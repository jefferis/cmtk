...
int doMain(const int argc, const char *argv[]) {
  try {
    ...
    cl.SetProgramInfo(
        ...
        "MR Image Intensity Bias Field Correction on GPU using CUDA");
    cl.SetProgramInfo(cmtk::CommandLine::PRG_DESCR,
                      "This program corrects intensity inhomogeneity artifacts "
                      "in MR images using a bias field estimated via entropy "
                      "minimization using GPU-accelerated computation.");
  ...
  for (size_t polynomialDegree = polynomialDegreeFrom;
    ...
    functional =
        cmtk::CreateEntropyMinimizationIntensityCorrectionFunctionalDevice(
