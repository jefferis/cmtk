...
int doMain(const int argc, const char *argv[]) {
  try {
    ...
    cl.SetProgramInfo(
        ...
        "transformation to them. The transformation can optionally be "
        "inverted. The transformed coordinates are then written to standard "
        "output.");
    ...
    cl.AddOption(Key('a', "accuracy"), &Accuracy,
                 "Approximation accuracy (maximum error)");
    cl.AddSwitch(Key('x', "no-check"), &NoCheck, true,
                 "Disable accuracy checking and always output something");
    cl.AddSwitch(Key('f', "forward"), &Forward, true,
                 "Apply forward transformation (default: backward)");
    cl.AddOption(Key('F', "fallback-inverse"), &FallbackInversePath,
                 "Fallback inverse transformation (used for initialization and "
                 "in case of failure)");

    // Greg's additions:
    cl.AddOption(Key('i', "input-file"), &InputPath,
                 "Input path [default: STDIN]. ");
    cl.AddOption(Key('o', "output-file"), &OutputPath,
                 "Output path [default: STDOUT]. ");

    cl.AddSwitch(
        Key('n', "affine"), &AffineOnly, true,
        "Apply affine transformation even if warp studylist specified");
    cl.AddSwitch(Key('b', "binary"), &Binary, true,
                 "Read Binary input and produce binary output (floats)");
    cl.AddSwitch(
        Key('j', "jacobian"), &Jacobian, true,
    ...
    cl.AddSwitch(Key('J', "normalised-jacobian"), &NormalisedJacobian, true,
                 "Calculate jacobian determinant normalised by global scaling "
                 "factor (default: false)");
    cl.AddSwitch(
        Key('g', "return-global-scaling"), &ReturnGlobalScaling, true,
  ...
  cmtk::SplineWarpXform::SmartPtr splineWarp =
      cmtk::SplineWarpXform::SmartPtr ::DynamicCastFrom(warpXform);
  ...
  try {
    if (affineXform) inverseAffineXform = affineXform->GetInverse();
  ...
  const cmtk::Types::Coordinate globalScaling =
      (splineWarp) ? splineWarp->GetGlobalScaling()
                   : affineXform->GetGlobalScaling();

  cmtk::Xform::SmartPtr fallbackInverseXform;
  if (FallbackInversePath) {
    fallbackInverseXform =
        cmtk::Xform::SmartPtr(cmtk::XformIO::Read(FallbackInversePath));
  }
  ...
  while (!feof(infile)) {
    ...
    if (numRead == 3) {
      cmtk::FixedVector<3, cmtk::Types::Coordinate> v =
          cmtk::FixedVector<3, cmtk::Types::Coordinate>::FromPointer(xyz);
      ...
      if (splineWarp && !AffineOnly) {
        if (Jacobian || NormalisedJacobian) {
          ...
          if (NormalisedJacobian) j /= globalScaling;
        ...
        if (Forward) {
          ...
          if (fallbackInverseXform) {
            ...
            success = splineWarp->ApplyInverseWithInitial(v, v, initialEstimate,
                                                          Accuracy);
          } else {
        ...
        if (!success) {
          ...
          fprintf(stderr,
                  "ERROR: %f %f %f is not inside target image or inversion "
                  "failed (error = %f)\n",
                  u[0], u[1], u[2], error);
        }
      } else {
        if (Forward) {
          if (affineXform) v = affineXform->Apply(v);
        } else {
          if (inverseAffineXform) v = inverseAffineXform->Apply(v);
        }
      }
      if (success || NoCheck) {
        ...
        if (Binary) {
          const float nan3[3] = {std::numeric_limits<float>::signaling_NaN(),
