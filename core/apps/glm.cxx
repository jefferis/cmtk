...
void CallbackIgnore(const char *argv) { IgnoreSet.insert(atoi(argv)); }

void CallbackSelect(const char *argv) { SelectSet.insert(argv); }
...
void CallbackCropImages(const char *arg) {
  ...
  CropImages =
      (6 == sscanf(arg, "%4d,%4d,%4d,%4d,%4d,%4d", cropFrom, cropFrom + 1,
...
void Import(const char *ctlFileName, const char *imgFilePatt,
            cmtk::UniformVolume::SmartPtr &refVolume,
            std::vector<cmtk::TypedArray::SmartPtr> &imagesData,
            size_t &nParameters, cmtk::Types::DataItem *&parameters,
  ...
  if (!ctlFile.is_open()) {
    std::cerr << "Error opening control/parameter file " << ctlFileName
              << std::endl;
  ...
  while (!ctlFile.eof()) {
    ...
    if (line.length() && line[0] != '#') {
      ...
      if (snprintf(imgPath, sizeof(imgPath), imgFilePatt, imgName.c_str()) >
          PATH_MAX) {
      ...
      cmtk::UniformVolume::SmartPtr volume(
          cmtk::VolumeIO::ReadOriented(imgPath));
      if (!volume) {
      ...
      if (ExponentialModel) imageData->ApplyFunctionDouble(cmtk::Wrappers::Log);
      ...
      if (nParametersTotal) {
        for (size_t p = 0; p < nParametersTotal; ++p) {
          if (lineStream.eof()) {
            cmtk::StdErr
                << "ERROR: insufficient number of model parameters in line '"
                << line << "'\n";
            throw cmtk::ExitException(1);
          }
          lineStream >> param;
        ...
        if (!ExcludeConstant) vParam.push_back(1.0);
      } else {
        while (!lineStream.eof()) {
          lineStream >> param;
          if (IgnoreSet.find(nParametersTotal) == IgnoreSet.end()) {
            ...
            cmtk::DebugOutput(1) << "INFORMATION: Ignoring parameter #"
                                 << nParametersTotal << "\n";
          }
          ++nParametersTotal;
        }
  ...
  parameters =
      cmtk::Memory::ArrayC::Allocate<cmtk::Types::DataItem>(vParam.size());
  for (size_t i = 0; i < vParam.size(); ++i) parameters[i] = vParam[i];
  ...
  if (!ExcludeConstant) ++nParametersTotal;
}
...
int doMain(const int argc, const char *argv[]) {
  try {
    ...
    cl.SetProgramInfo(
        ...
        "Statistical modeling of pixel intensities in multiple images using a "
        "General Linear Model.\n\n"
        "The independent variables of the model are defined in one of more "
        "control files. Each control file is a text file with one "
        "whitespace-separated column per independent variable.\n\n"
        "The first line of the control file defines the variable names, i.e., "
        "the labels that identify each variable. Each following line contains "
        "one value per independent variable.\n\n"
        "Example:\n\n  ID  age  sex\n  01  20   0\n  02  30   1\n\n"
        "Each control file name is followed by a file name pattern. In that "
        "pattern, a single '%s' place holder is replaced by the value found in "
        "the first column of each control file row. The resulting "
        "string is the path of the image read and associated with the model "
        "variables listed on that particular control file line.\n\n"
        "Using the above control file example, the pattern "
        "'images/subject%s.nii' would  expand to the image file names "
        "'images/subject01.nii' and 'images/subject02.nii'.\n\n"
        "Multiple control files can be used, each with a different image file "
        "pattern.");
    cl.SetProgramInfo(
        cmtk::CommandLine::PRG_SYNTX,
        "glm [options] ctlfile imgfile_pattern [ctlfile imgfile_pattern ...]");
    cl.SetProgramInfo(cmtk::CommandLine::PRG_CATEG,
                      "CMTK.Statistics and Modeling");

    typedef cmtk::CommandLine::Key Key;

    cl.BeginGroup("Input", "Input Settings");
    cl.AddCallback(Key('c', "crop"), CallbackCropImages,
                   "To save space/time, crop images: x0,y0,z0,x1,y1,z2");
    cl.EndGroup();

    cl.BeginGroup("Model", "Model Settings");
    cl.AddSwitch(Key('n', "normalize"), &NormalizeParameters, true,
                 "Normalize model parameters w.r.t. data variances.");
    cl.AddSwitch(Key('e', "exp"), &ExponentialModel, true,
                 "Use exponential model rather than linear model.");
    cl.EndGroup();

    cl.BeginGroup("Variables", "Selection of Independent Variables");
    cl.AddSwitch(Key('x', "exclude-constant"), &ExcludeConstant, true,
                 "Exclude automatic constant parameter from model.");
    cl.AddCallback(
        Key('i', "ignore-parameter"), CallbackIgnore,
        "Ignore parameter with given NUMBER (0..n-1). Can be repeated.");
    cl.AddCallback(
        Key('s', "select-parameter"), CallbackSelect,
        "Select parameter with given NAME for model. Can be repeated.");
    cl.EndGroup();

    cl.BeginGroup("Output", "Output Settings");
    cl.AddOption(
        Key('O', "output-pattern"), &OutputFilePatt,
        "Filename pattern for output.\n\n"
        "  %s is replaced with image type ('fstat', 'tstat', or 'param')\n"
        "  %d is replaced with independent variable number (0 for entire "
        "model)\n"
  ...
  if (CtlFileName.empty() || ImgFilePatt.empty()) {
    cmtk::StdErr
        << "ERROR: need both a control file name and an image file pattern\n";
    throw cmtk::ExitException(1);
  }
  ...
  std::list<const char *>::const_iterator itCtlFile = CtlFileName.begin(),
                                          itImgFile = ImgFilePatt.begin();
  for (size_t idx = 0;
       (itCtlFile != CtlFileName.end()) && (itImgFile != ImgFilePatt.end());
       ++itCtlFile, ++itImgFile, ++idx) {
    std::vector<cmtk::TypedArray::SmartPtr> imagesDataNext;
    Import(*itCtlFile, *itImgFile, refVolume, imagesDataNext, nParameters[idx],
           parameters[idx], nParametersTotal[idx]);
    ImagesData.push_back(imagesDataNext);

    if (nParametersTotal[idx] != nParametersTotal[0]) {
      cmtk::StdErr << "Total number of parameters for control file #" << idx
                   << "does not match that for file #0\n";
      throw cmtk::ExitException(1);
    }
    if (nParameters[idx] != nParameters[0]) {
      cmtk::StdErr << "Number of active parameters for control file #" << idx
                   << "does not match that for file #0\n";
      throw cmtk::ExitException(1);
    }
    ...
    std::copy(ImagesData[idx].begin(), ImagesData[idx].end(),
              &allImages[totalNumberOfImages]);
  }

  double *allParameters =
      cmtk::Memory::ArrayC::Allocate<double>(nParameters[0] * allImages.size());
  size_t idx = 0;
  for (size_t ctl = 0; ctl < parameters.size(); ++ctl) {
    for (size_t param = 0; param < ImagesData[ctl].size() * nParameters[0];
         ++param, ++idx) {
  ...
  if (OutputFilePatt) {
    ...
    if (snprintf(outFileName, PATH_MAX, OutputFilePatt, "fstat", 0, "model") >
        PATH_MAX) {
      cmtk::StdErr << "ERROR: output path exceeds maximum path length\n";
    } else {
    ...
    for (size_t pp = 0; pp < nParametersTotal[0]; ++pp) {
      ...
      if (snprintf(outFileName, sizeof(outFileName), OutputFilePatt, "param",
                   pp, FieldNames[pp].c_str()) > PATH_MAX) {
        cmtk::StdErr << "ERROR: output path exceeds maximum path length\n";
      } else {
      ...
      if (snprintf(outFileName, PATH_MAX, OutputFilePatt, "tstat", pp,
                   FieldNames[pp].c_str()) > PATH_MAX) {
        cmtk::StdErr << "ERROR: output path exceeds maximum path length\n";
      } else {
