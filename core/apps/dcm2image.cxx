...
void AddPatternToMap(std::map<DcmTagKey, std::string> &map,
                     const char *pattern) {
  const char *equals = strchr(pattern, '=');
  if (equals) {
    const std::string tagName(pattern, static_cast<size_t>(equals - pattern));

    const DcmDictEntry *dictEntry =
        dcmDataDict.rdlock().findEntry(tagName.c_str());
    dcmDataDict.unlock();

    if (dictEntry) {
      map.insert(
          std::pair<DcmTagKey, std::string>(dictEntry->getKey(), equals + 1));
    } else {
      cmtk::StdErr
          << "WARNING: DCMTK data dictionary does not have a tag entry named '"
          << tagName << "' - ignoring this\n";
    }
  }
}
...
class VolumeList : public std::vector<ImageStackDICOM::SmartPtr> {
 public:
   ...
   *\return Return code - 0, no problems; 3, at least one volume was not written
   *correctly (e.g., due to non-uniform slice spacing).
   */
  int WriteVolumes();
...
std::string PutNumberAndSanitize(const std::string &path,
                                 const std::string &numberString) {
  if (numberString.empty())
    return cmtk::StrReplace(cmtk::StrReplace(path, "%n", ""), "%N", "");
  else
    return cmtk::StrReplace(cmtk::StrReplace(path, "%n", numberString), "%N",
                            "-" + numberString);
}
...
int VolumeList::WriteVolumes() {
  ...
  for (const_iterator it = this->begin(); it != this->end(); ++it) {
    if (((*it)->size() > 1) || (*(*it)->begin())->m_IsMultislice ||
      ...
      path = cmtk::StrReplace(path, "%D",
                              cmtk::StrMakeLegalInPath((*it)[0][0]->GetTagValue(
                                  DCM_SeriesDescription)));
      path = cmtk::StrReplace(path, "%R",
                              cmtk::StrMakeLegalInPath((*it)[0][0]->GetTagValue(
                                  DCM_RepetitionTime)));
      path = cmtk::StrReplace(
          path, "%E",
          cmtk::StrMakeLegalInPath((*it)[0][0]->GetTagValue(DCM_EchoTime)));
      ...
      char replaceThis[3] = {
          '%', '0', 0};  // this will be modified to be the current replacement
      std::string directory =
          (*it)[0][0]->m_FileDir;  // start with full directory
      for (size_t dirIdx = 1; (dirIdx < 10) && !directory.empty(); ++dirIdx) {
        ...
        if (lastSlash == std::string::npos) {
          ...
          path = cmtk::StrReplace(path, replaceThis,
                                  directory.substr(lastSlash + 1));
  ...
  if (cntSingleImages) {
    cmtk::DebugOutput(1)
        << "\n====================================================\n";
    cmtk::DebugOutput(1)
        << "WARNING: " << cntSingleImages
        << " single image(s) could not be assigned to multi-image stacks:\n\n";
  ...
  for (std::map<std::string, Self>::const_iterator it = pathToVolumeMap.begin();
       it != pathToVolumeMap.end(); ++it) {
    ...
    if (nVolumes == 1) {
      ...
      cmtk::UniformVolume::SmartConstPtr volume =
          it->second[0]->WriteImage(uniquePath.c_str(), EmbedInfo);
      ...
      for (size_t i = 0; i < nVolumes; ++i) {
        ...
        numberString.width(1 +
                           static_cast<int>(log((double)nVolumes) / M_LN10));
        numberString.fill('0');
        numberString << std::right << 1 + i;

        std::string uniquePath =
            PutNumberAndSanitize(it->first, numberString.str());

        cmtk::UniformVolume::SmartConstPtr volume =
            it->second[i]->WriteImage(uniquePath.c_str(), EmbedInfo);
...
void VolumeList::AddImageFile(ImageFileDICOM::SmartConstPtr &newImage) {
  if (!empty()) {
    ...
    while (it != this->end()) {
      ...
      if (study->Match(*newImage, this->m_Tolerance, DisableOrientationCheck,
  ...
  ImageStackDICOM::SmartPtr newImageStack(
      new ImageStackDICOM(this->m_Tolerance));
  newImageStack->AddImageFile(newImage);
  push_back(newImageStack);
}

void VolumeList::AppendFromDirectory(const std::string &path,
                                     const char *wildcard) {
  ...
  do {
    ...
    if (fData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
      ...
      try {
        ...
        if (newFile->MatchAllPatterns(IncludePatterns) &&
            !newFile->MatchAnyPattern(ExcludePatterns)) {
  ...
  if (dir_pointer != NULL) {
    ...
    while ((entry_pointer = readdir(dir_pointer))) {
      ...
      if (!stat(fullname.c_str(), &entry_status)) {
        if (S_ISDIR(entry_status.st_mode) && Recursive &&
  ...
  switch (SortFiles) {
    ...
    case SORT_INSTANCE_NUMBER:
      std::sort(fileList.begin(), fileList.end(),
                ImageFileDICOM::lessInstanceNumber);
      break;
  }

  for (std::vector<ImageFileDICOM::SmartConstPtr>::iterator it =
           fileList.begin();
       it != fileList.end(); ++it) {
    try {
...
int doMain(const int argc, const char *argv[]) {
...
#endif
    {
      cmtk::StdErr << "Data dictionary not avaliable. Please set DCMDICTPATH "
                      "variable as path to dicom.dic file.\n";
  ...
  try {
    ...
    cl.AddSwitch(Key("no-progress"), &Progress, false,
                 "Disable progress reporting.");

    cl.BeginGroup("Input", "Input Options");
    cl.AddSwitch(Key('r', "recurse"), &Recursive, true,
                 "Recurse into directories");
    cl.EndGroup();

    cl.BeginGroup("Output", "Output Options");
    cl.AddOption(Key('O', "out-pattern"), &OutPathPattern,
                 "Output image path pattern. Use the following substitutions: "
                 "printf-style %d variante (image number); "
                 "%n (image number with automatic number of digits); "
                 "%N (like %n, but with a hyphen '-' before number if there is "
                 "more than one image); "
    ...
    cl.AddSwitch(Key('x', "xml"), &WriteXML, true,
                 "Write XML sidecar file for each created image.");
    cl.AddSwitch(
        Key("include-identifiers"), &IncludeIdentifiers, true,
        "Include potentially protected identifying information (e.g., UIDs, "
        "device serial numbers, dates) in the created XML sidecar files.");

    cmtk::CommandLine::EnumGroup<cmtk::ImageStackDICOM::EmbedInfoEnum>::SmartPtr
        embedGroup =
            cl.AddEnum("embed", &EmbedInfo,
                       "Embed DICOM information into output images as "
                       "'description' (if supported by output file format).");
    embedGroup->AddSwitch(Key("StudyID_StudyDate"),
                          cmtk::ImageStackDICOM::EMBED_STUDYID_STUDYDATE,
                          "StudyID, tag (0020,0010), then underscore, followed "
                          "by StudyDate, tag (0008,0020). "
                          "Date is appended because StudyID is four digits "
                          "only and will repeat sooner or later.");
    embedGroup->AddSwitch(Key("PatientName"),
                          cmtk::ImageStackDICOM::EMBED_PATIENTNAME,
                          "Patient name, tag (0010,0010)");
    embedGroup->AddSwitch(Key("SeriesDescription"),
                          cmtk::ImageStackDICOM::EMBED_SERIESDESCR,
                          "Series description, tag (0008,103e)");
    embedGroup->AddSwitch(
        Key("None"), cmtk::ImageStackDICOM::EMBED_NONE,
        "Embed no information - leave 'description' field empty.");
    cl.EndGroup();

    cl.BeginGroup("Filtering", "Filtering Options")
        ->SetProperties(cmtk::CommandLine::PROPS_ADVANCED);
    cl.AddCallback(
        Key("filter"), &CallbackAddInclude,
        "Filter DICOM files and include only those matching the given pattern "
        "of the form 'TagName=text', such that the value of the DICOM tag with "
        "the given name contains the given "
        "text. If multiple filter patterns are provided via repeated use of "
        "this option, only files that match ALL patterns are included.");
    cl.AddCallback(Key("exclude"), &CallbackAddExclude,
                   "Exclude all DICOM files matching the given pattern of the "
                   "form 'TagName=text', such that the value of the DICOM tag "
                   "with the given name contains the given text. "
                   "If multiple exclusion patterns are provided, all files are "
                   "excluded that match ANY of the patterns.");
    cl.EndGroup();

    cl.BeginGroup("Sorting", "Sorting Options")
        ->SetProperties(cmtk::CommandLine::PROPS_ADVANCED);
    cl.AddSwitch(Key("no-sort"), &SortFiles, SORT_NONE,
                 "Do NOT sort files by file name (sorting determines image "
                 "stack order when resolving spatial collisions)");
    cl.AddSwitch(Key("sort-by-name"), &SortFiles, SORT_FILENAME,
                 "Sort files lexicographically by file name. Use this when "
                 "instance numbers are non-unique.");
    cl.AddSwitch(Key("sort-by-instance"), &SortFiles, SORT_INSTANCE_NUMBER,
                 "Sort files by image instance number. Use this when file "
                 "names are different lengths, etc.");
    cl.EndGroup();

    cl.BeginGroup("Stacking", "Stacking Options")
        ->SetProperties(cmtk::CommandLine::PROPS_ADVANCED);
    cl.AddSwitch(
        Key("write-single-slices"), &WriteSingleSlices, true,
        "Also write output images for single-slice DICOM files that could not "
        "be assigned to any 3D stacks. By default, these are skipped.");
    cl.AddSwitch(Key("ignore-acq-number"), &IgnoreAcquisitionNumber, true,
                 "Ignore 'AcquisitionNumber' tag for image grouping, i.e., do "
                 "not separate stacks based on this tag.");
    cl.AddSwitch(
        Key("no-orientation-check"), &DisableOrientationCheck, true,
        "Disable checking of image orientations (to avoid rounding issues)");
    cl.AddOption(Key("tolerance"), &Tolerance,
                 "Tolerance for floating-point comparisons (must be >= 0; 0 = "
                 "exact matches only; default: 1e-5). "
                 "If one or more volumes cannot be stacked because of "
                 "non-uniform slice spacing that exceeds this threshold, "
                 "dcm2image will return an exit code of 3.");
    cl.EndGroup();

    cl.AddParameterVector(
        &SearchRootDirVector, "SearchDirList",
        "List of directories to search for DICOM files. Subdirectories are "
        "also search if '--recurse' option is used.");
  ...
  if (!dcmDataDict.rdlock().findEntry("RawDataType_ImageType")) {
    dcmDataDict.unlock();
    dcmDataDict.wrlock().addEntry(new DcmDictEntry(0x0043, 0x102f, EVR_SS,
                                                   "RawDataType_ImageType", 1,
                                                   1, NULL, OFFalse, "GE"));
    dcmDataDict.unlock();
  }
  ...
  for (std::vector<std::string>::const_iterator it =
           SearchRootDirVector.begin();
       it != SearchRootDirVector.end(); ++it) {
