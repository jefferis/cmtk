...
int doMain(const int argc, const char *argv[]) {
  ...
  try {
    ...
    cl.SetProgramInfo(cmtk::CommandLine::PRG_DESCR,
                      "Compute initial affine transformation by aligning "
                      "centers of mass or principal axes");
    ...
    cmtk::CommandLine::EnumGroup<int>::SmartPtr modeGroup =
        cl.AddEnum("mode", &mode, "Mode selection for initialization");
    modeGroup->AddSwitch(Key("direction-vectors"), 0,
                         "Alignment based on image direction vectors");
    modeGroup->AddSwitch(
        Key("centers-of-mass"), 1,
        "Alignment based on centers of mass (translation only)");
    modeGroup->AddSwitch(Key("principal-axes"), 2,
                         "Alignment based on principal axes");
    modeGroup->AddSwitch(Key("identity"), -1,
                         "Create only an identity transformation");

    cl.AddSwitch(Key('C', "center-xform"), &centerXform, true,
                 "Set transformation center (for rotation, scale) to center of "
                 "reference image.");
    cl.AddSwitch(Key("native-space"), &writeXformNativeSpaces, true,
                 "Write transformation between the native image spaces, rather "
                 "than in CMTK standard RAS space. This is the default when "
                 "running this tool as a Slicer plugin.")
    ...
    cl.AddOption(
        ...
        "newly transformation from reference to floating image.");
    cl.EndGroup();
#endif

    cl.AddParameter(&referenceImagePath, "ReferenceImage",
    ...
    cl.AddParameter(&floatingImagePath, "FloatingImage",
        ...
        ->SetProperties(cmtk::CommandLine::PROPS_IMAGE);
    cl.AddParameter(&outputXformPath, "OutputXform",
  ...
  cmtk::UniformVolume::SmartPtr floatingImage(
      cmtk::VolumeIO::ReadOriented(floatingImagePath));
  if (!floatingImage) {
  ...
  switch (mode) {
    ...
    case 0: {
      try {
        xform = cmtk::AffineXform::SmartPtr(
            cmtk::MakeInitialAffineTransformation::AlignDirectionVectors(
                *referenceImage, *floatingImage, centerXform));
      } catch (const cmtk::AffineXform::MatrixType::SingularMatrixException &) {
        cmtk::StdErr << "ERROR: singular matrix encountered in call to "
                        "cmtk::MakeInitialAffineTransformation::"
                        "AlignDirectionVectors()\n";
        throw cmtk::ExitException(1);
      }
    } break;
    case 1:
      xform = cmtk::AffineXform::SmartPtr(
          cmtk::MakeInitialAffineTransformation::AlignCentersOfMass(
              *referenceImage, *floatingImage));
      break;
    case 2: {
      try {
        xform = cmtk::AffineXform::SmartPtr(
            cmtk::MakeInitialAffineTransformation::AlignPrincipalAxes(
                *referenceImage, *floatingImage));
      } catch (const cmtk::AffineXform::MatrixType::SingularMatrixException &) {
        cmtk::StdErr
            << "ERROR: singular matrix encountered in call to "
               "cmtk::MakeInitialAffineTransformation::AlignPrincipalAxes()\n";
        throw cmtk::ExitException(1);
      }
    } break;
  ...
  if (xform) {
    if (writeXformNativeSpaces) {
      cmtk::TransformChangeToSpaceAffine toNative(*xform, *referenceImage,
                                                  *floatingImage);
    ...
    if (!updateDB.empty()) {
      try {
        ...
        db.AddImagePairXform(outputXformPath, true /*always affine*/,
                             referenceImagePath, floatingImagePath);
    ...
    cmtk::StdErr << "ERROR: at least one of the two images does not have a "
                    "grid-to-physical space coordinate transformation.\n";
    throw cmtk::ExitException(1);
  }
