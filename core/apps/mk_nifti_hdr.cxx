...
int doMain(const int argc, const char *argv[]) {
  try {
    ...
    cl.SetProgramInfo(
        ...
        "Make header file according to NIFTI file format based on "
        "user-supplied parameters for geometry, data type, orientation, etc.");
    cl.SetProgramInfo(cmtk::CommandLine::PRG_SYNTX,
                      "mk_nifti_hdr [options] [output.nii]");

    typedef cmtk::CommandLine::Key Key;
    cl.AddCallback(Key('D', "dims"), SetDims,
                   "Set dimensions in voxels. Provided as 'DimsX,DimsY,DimsZ'");
    cl.AddCallback(Key('V', "voxel"), SetDeltas,
    ...
    cl.AddSwitch(Key("reset-coordinates"), &ResetCoordinates, true,
                 "In an imported header, reset the image-to-physical "
                 "coordinate mapping to an identity matrix.");
    cl.AddSwitch(
        Key("attached"), &DetachedHeader, false,
        "Create attached header for use in single-file images. Image data must "
        "be concatenated directly onto the created header file.");
  ...
  if (ImportHdrFile) {
    ...
    if (sizeof(header) != hdrStream.Read(&header, 1, sizeof(header))) {
      cmtk::StdErr.printf(
          "ERROR: could not read %d bytes from header file %s\n",
  ...
  if (!ImportHdrFile || ResetCoordinates) {
    ...
    cmtk::AffineXform::MatrixType m4 =
  ...
  if (Description) {
    ...
    memcpy(header.descrip, Description,
  ...
  if (DetachedHeader) {
    ...
    if (hdrFile) {
      ...
      cmtk::StdErr << "ERROR: file '" << HdrFileName
