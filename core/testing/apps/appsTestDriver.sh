...
case ${RUNTEST} in
	...
	run congeal -v --force-background 0 -O ${tmpdir} --template spgr_brain_1.hdr --dofs 6 --dofs 9 -e 2 -a 0.25 --downsample-from 2 --downsample-to 1 --sampling-density 1.0 --template-with-data spgr_brain_1.hdr spgr_brain_2.hdr spgr_brain_3.hdr
	check_results congeal.xforms average_congeal.img
        unset CMTK_NUM_THREADS
	;;
    CongealZeroSumSmooth)
        export CMTK_NUM_THREADS=1
	run congeal -v --smooth 0.5 --downsample-to 0 -O ${tmpdir} --template spgr_brain_1.hdr --dofs 6 --dofs 9 -e 2 -a 0.25 --downsample-from 2 --sampling-density 1.0 --zero-sum spgr_brain_1.hdr spgr_brain_2.hdr spgr_brain_3.hdr
	check_results congeal.xforms average_congeal.img
        unset CMTK_NUM_THREADS
	;;
    CongealWarpFromInit)
	run congeal_warp -v --force-background 0 -O ${tmpdir} --grid-spacing 200 --refine-grid 1 -e 2 -a 1 --downsample-from 2 --downsample-to 1 groupwise_init_brain123.xforms
	check_results congeal_warp.xforms average_congeal_warp.img
	;;
    CongealWarpFitFromInit)
	run congeal_warp -v --force-background 0 -O ${tmpdir} --grid-spacing 200 --grid-spacing-fit --refine-grid 1 -e 4 -a 1 --downsample-from 2 --downsample-to 1 groupwise_init_brain123.xforms
	check_results congeal_warp.xforms average_congeal_warp.img
	;;
    CongealWarpUseTemplate)
	run congeal_warp -v --force-background 0 -O ${tmpdir} --grid-spacing 200 --refine-grid 1 -e 2 -a 1 --downsample-from 2 --downsample-to 2 --template-with-data spgr_brain_1.hdr groupwise_init_brain123.xforms
	check_results congeal_warp.xforms average_congeal_warp.img
	;;
    CongealWarpFromInitZeroSum)
	run congeal_warp -v --zero-sum --force-background 0 -O ${tmpdir} --grid-spacing 200 --refine-grid 1 -e 2 -a 1 --downsample-from 2 --downsample-to 1 groupwise_init_brain123.xforms
	check_results congeal_warp.xforms average_congeal_warp.img
	;;
    ConvertAutoCrop)
	...
	run convert --downsample 8,4,1 phantom_ax.nhdr ${tmpdir}/downsample.nhdr
	check_results downsample.nhdr downsample.raw
	;;
    ConvertErodeDilateErode)
	run convert --erode 1 --dilate 2 --erode 1 parc1_bin.hdr ${tmpdir}/parc1_ede.img
	check_results parc1_ede.img
	;;
    ConvertDilateErodeDilate)
	run convert --dilate 1 --erode 2 --dilate 1 parc1_bin.hdr ${tmpdir}/parc1_ded.img
	...
	;;
    Dcm2Image)
	run dcm2image -O ${tmpdir}/%03d.hdr ${PWD}/dcm/
	check_results 001.hdr 001.img 002.hdr 002.img 003.hdr 003.img
	;;
    Dcm2ImageZ)
	run dcm2image -O ${tmpdir}/%03d.nii ${PWD}/dcmz/
	check_results 001.nii 002.nii 003.nii
	;;
    Dcm2ImageNrrd)
	run dcm2image -O ${tmpdir}/%03d.nhdr ${PWD}/dcm/
	...
	run groupwise_rmi --force-background 0 -O ${tmpdir} --dofs 6,9 -e 4 --delta-f-threshold 0.1 -a 0.25 --downsample-from 2 --downsample-to 1 --sampling-density 1.0 --zero-sum groupwise_init_brain123.xforms
	check_results groupwise_rmi.xforms average_groupwise_rmi.img
	;;
    GroupwiseRMIFromInitSampling)
	run groupwise_rmi --force-background 0 -O ${tmpdir} --dofs 6 --dofs 9 -e 4 -a 0.25 --downsample-to 1 --sampling-density 0.5 --zero-sum groupwise_init_brain123.xforms
	## no baseline; this mode uses probabilistic sampling
	;;
    GroupwiseRMIBackground)
	run groupwise_rmi --force-background 0 -O ${tmpdir} --template spgr_brain_1.hdr --dofs 6 --dofs 9 -e 2 -a 0.25 --downsample-from 2 --downsample-to 1 --sampling-density 1.0 --zero-sum spgr_brain_1.hdr spgr_brain_2.hdr spgr_brain_3.hdr
	check_results groupwise_rmi.xforms average_groupwise_rmi.img
	;;
	...
	;;
    MkPhantomBox)
	run mk_phantom_3d -o ${tmpdir}/phantom.nii --dims 10,10,10 --voxel 1,1,1 box 2,2,2 5,5,5 10
	check_results phantom.nii
	;;
    MkPhantomSphere)
	run mk_phantom_3d -o ${tmpdir}/phantom.nii --dims 10,10,10 --voxel 1,1,1 sphere 7,7,7 5 20
	check_results phantom.nii
	;;
    MkPhantomBoxSphere)
